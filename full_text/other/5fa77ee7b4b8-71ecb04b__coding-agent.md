...
metadata:
  {
    "clawbot-plus": { "emoji": "🧩", "requires": { "anyBins": ["claude", "codex", "opencode", "pi"] } },
  }
---
...
**⚠️ CRITICAL: Never review PRs in ClawBot-Plus's own project folder!**
...
8. **NEVER start Codex in ~/.clawbot-plus/** - it'll read your soul docs and get weird ideas about the org chart!
9. **NEVER checkout branches in ~/Projects/clawbot-plus/** - that's the LIVE ClawBot-Plus instance!
...
For long-running background tasks, append a wake trigger to your prompt so ClawBot-Plus gets notified immediately when the agent finishes (instead of waiting for the next heartbeat):

```
... your task here.

When completely finished, run this command to notify me:
clawbot-plus system event --text "Done: [brief summary of what was built]" --mode now
```
...
When completely finished, run: clawbot-plus system event --text \"Done: Built todos REST API with CRUD endpoints\" --mode now'"
```
