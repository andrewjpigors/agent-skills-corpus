...
IMPORTANT — No `gh` CLI dependency. This skill uses curl + the GitHub REST API exclusively. The GH_TOKEN env var is already injected by OpenShell. Pass it as a Bearer token in all API calls:
...
```
CONFIG_PATH="${OPENSHELL_CONFIG_PATH:-${OPENSHELL_STATE_DIR:-$HOME/.openshell}/openshell.json}"
cat "$CONFIG_PATH" | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```

If still empty, check `/data/.clawdbot/openshell.json`:

```
cat /data/.clawdbot/openshell.json | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```
...
- If curl returns an HTTP 401 or 403 → stop and tell the user:
  > "GitHub authentication failed. Please check your apiKey in the OpenShell dashboard or in the active OpenShell config path (`$OPENSHELL_CONFIG_PATH`, default `~/.openshell/openshell.json`) under `skills.entries.gh-issues`."
...
- **Sequential cursor tracking:** Use a cursor file to track which issue to process next:
  ...
  # SOURCE_REPO_SLUG = owner-repo with slashes replaced by hyphens (e.g., openshell-openshell)
  ```
...
CONFIG_PATH="${OPENSHELL_CONFIG_PATH:-${OPENSHELL_STATE_DIR:-$HOME/.openshell}/openshell.json}"
GH_TOKEN=$(cat "$CONFIG_PATH" 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty') || GH_TOKEN=$(cat /data/.clawdbot/openshell.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty')
...
export GH_TOKEN=$(node -e "const fs=require('fs'); const c=JSON.parse(fs.readFileSync('/data/.clawdbot/openshell.json','utf8')); console.log(c.skills?.entries?.['gh-issues']?.apiKey || '')")

```
If that fails, also try:
```

export CONFIG_PATH="${OPENSHELL_CONFIG_PATH:-${OPENSHELL_STATE_DIR:-$HOME/.openshell}/openshell.json}"
