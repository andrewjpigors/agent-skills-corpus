...
1. Load `config/local/codex-for-legal/litigation-legal/CLAUDE.md` → role, work-product header, decision posture, document storage.
...
**Conflicts gate — unbypassable.** Before building a claim chart, check `config/local/codex-for-legal/litigation-legal/matters/_log.yaml` for the matter slug. If the matter is not in `_log.yaml`, refuse and route:
...
Prepend the work-product header from `config/local/codex-for-legal/litigation-legal/CLAUDE.md` `## Outputs`.
...
If matter workspaces enabled and a matter is active: `config/local/codex-for-legal/litigation-legal/matters/<matter-slug>/claim-charts/`. Otherwise: `config/local/codex-for-legal/litigation-legal/claim-charts/`. Surface the path. Append a one-line entry to the matter's `history.md`.
