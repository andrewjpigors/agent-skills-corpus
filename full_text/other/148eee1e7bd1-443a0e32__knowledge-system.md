...
ref: <orgSlug>/<slug>:<version>   # canonical resource reference (e.g. "utilities-invoicing/vendor:1.0.0")
...
> **Note on a recent platform fix:** the OpenAPI generator used to emit `{type: "object"}` for `options`/`extendedOptions` (they're `json.RawMessage` server-side), which made `kdx apply` reject bare arrays even though the server accepted them. That was loosened in `kodexa-ai/platform#246` (merged into develop 2026-05-11) — RawMessage fields now emit no `type` constraint, so the CLI accepts whatever shape the UI wants. If you hit `Error at "/options": value must be an object`, you're on an older CLI build that pre-dates the fix.
...
#   - vendor-acme01-recategorize-charges.yml  → properties = {mappings: [...]}
#   - vendor-acme01-capture-actual-demand.yml → properties = {mode: column, ...}
```
