...
- User wants a coding agent that can run on a non-Anthropic / non-OpenAI provider (DeepSeek, Qwen, Ollama, vLLM, NasTech Portal, etc.) — sibling skills `claude-code` and `codex` are tied to one vendor.
