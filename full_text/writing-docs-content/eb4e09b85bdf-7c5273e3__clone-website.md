...
You are about to reverse-engineer and rebuild **$ARGUMENTS** as pixel-perfect clones into a Laravel 13 + Inertia + Vue 3 codebase.

When multiple URLs are provided, process them independently and in parallel where possible, while keeping each site's extraction artifacts isolated in dedicated folders (for example, `docs/research/<hostname>/`). Each URL becomes its own Inertia page (`resources/js/Pages/<Name>.vue`) with a matching route in `routes/web.php`.
...
3. Verify the base project builds: `pnpm build` (Vite) and `pnpm typecheck` (`vue-tsc --noEmit`). The Laravel + Inertia + Vue 3 + Tailwind v4 scaffold should already be in place. If the project is freshly cloned, run `composer install`, `pnpm install`, `cp .env.example .env`, `php artisan key:generate`, and `touch database/database.sqlite` first.
...
Extract the actual text, images, videos, and SVGs from the live site. This is a clone, not a mockup. Use `element.textContent`, download every `<img>` and `<video>`, extract inline `<svg>` elements as Vue components. The only time you generate content is when something is clearly server-generated and unique per session.

**Layered assets matter.** A section that looks like one image is often multiple layers — a background watercolor/gradient, a foreground UI mockup PNG, an overlay icon. Inspect each container's full DOM tree and enumerate ALL `<img>` elements and background images within it, including absolutely-positioned overlays. Missing an overlay image makes the clone look empty even if the background is correct.

### 4. Foundation First

Nothing can be built until the foundation exists: global CSS with the target site's design tokens (colors, fonts, spacing) under Tailwind v4's `@theme` block, TypeScript types for the content structures, and global assets (fonts, favicons). This is sequential and non-negotiable. Everything after this can be parallel.
...
Every builder agent must verify `pnpm exec vue-tsc --noEmit` passes before finishing. After merging worktrees, you verify `pnpm build` (Vite) AND `pnpm typecheck` pass. A broken build is never acceptable, even temporarily.
...
**Fonts** — Inspect `<link>` tags for Google Fonts or self-hosted fonts. Check computed `font-family` on key elements (headings, body, code, labels). Document every family, weight, and style actually used. Configure them by adding `<link rel="preconnect" ...>` and `<link href="https://fonts.googleapis.com/...">` tags to `resources/views/app.blade.php` (or `@font-face` rules in `resources/css/app.css` for self-hosted fonts), then declare matching `--font-*` custom properties under the `@theme` block in `app.css` so Tailwind generates `font-sans`, `font-display`, etc., utilities automatically.

**Colors & design tokens** — Extract the site's color palette and spacing/radius/shadow scales from computed styles across the page. Update the `@theme { ... }` block in `resources/css/app.css` with the target's actual values. Tailwind v4 auto-generates utilities from any `@theme` custom property: declare `--color-primary: #0070f3;` and you get `bg-primary`, `text-primary`, `border-primary` for free; declare `--spacing-section: 6rem` and you get `py-section`, `px-section`, etc. For dark-mode variants, override the same `--*` properties under a `.dark` selector or a `@variant dark { ... }` rule. **Do not create a `tailwind.config.js`** — Tailwind v4 has no JS config; everything lives in CSS.

**Favicons & Meta** — Download favicons, apple-touch-icons, OG images, webmanifest to `public/seo/`. Reference them via `<link>` and `<meta>` tags in the `<head>` of `resources/views/app.blade.php`.

**Global UI patterns** — Identify any site-wide CSS or JS: custom scrollbar hiding, scroll-snap on the page container, global keyframe animations, backdrop filters, gradients used as overlays, **smooth scroll libraries** (Lenis, Locomotive Scroll — check for `.lenis`, `.locomotive-scroll`, or custom scroll container classes). Add CSS to `app.css` (under `@layer base` or as keyframes) and note any libraries that need to be installed via `pnpm install`.
...
1. **Update fonts** — add the target's `<link>` tags to the `<head>` of `resources/views/app.blade.php` (or `@font-face` declarations in `resources/css/app.css`). Declare matching `--font-sans`, `--font-display`, etc. under `@theme` in `app.css`.
2. **Update `resources/css/app.css`** with the target's color tokens, spacing values, keyframe animations, utility classes, and any **global scroll behaviors** (Lenis, smooth scroll CSS, scroll-snap on body). All design tokens go inside the `@theme { ... }` block as custom properties so Tailwind auto-generates utilities.
3. **Create TypeScript interfaces** in `resources/js/types/` for the content structures you've observed.
4. **Extract SVG icons** — find all inline `<svg>` elements on the page, deduplicate them, and save as named Vue components in `resources/js/Components/icons.ts`. Name them by visual function (e.g., `SearchIcon`, `ArrowRightIcon`, `LogoIcon`). Each icon is a tiny `defineComponent` using `h()` to construct the SVG tree — see the worked example below.

   **Worked example for `icons.ts` (copy this pattern):**
   ```ts
       ...
       setup() {
           return () =>
               h(
                   ...
                   [h('path', { d: 'M5 12h14m-6-6 6 6-6 6' })],
               );
       },
   });
   ```
   Use kebab-case for SVG attributes that contain dashes (`'stroke-width'`, `'stroke-linecap'`). Use `currentColor` for `stroke`/`fill` so the icon inherits text color via Tailwind classes.

5. **Download global assets** — write and run a Node.js script (`scripts/download-assets.mjs`) that downloads all images, videos, and other binary assets from the page to `public/`. Preserve meaningful directory structure (`public/images/`, `public/videos/`, `public/seo/`).
6. Verify: `pnpm build` and `pnpm typecheck` pass.
...
5. **Identify assets** this section uses — which downloaded images/videos from `public/`, which icon components from `icons.ts`. Check for **layered images** (multiple `<img>` or background-images stacked in the same container).
...
## Overview
- **Target file:** `resources/js/Components/<ComponentName>.vue`
...
- **Implementation approach:** <CSS transition + scroll listener via composable | IntersectionObserver | CSS animation-timeline | etc.>
...
- Icons used: `SearchIcon`, `ArrowRightIcon` from `@/Components/icons`
...
- Which shared assets to import (icons from `@/Components/icons`, layouts from `@/Layouts/...`, composables from `@/composables/...`)
- The target file path (e.g., `resources/js/Components/HeroSection.vue`)
- Instruction to use `<script setup lang="ts">` + `defineProps<{ ... }>()` for the component shell
- Instruction to verify with `pnpm exec vue-tsc --noEmit` before finishing
...
- After each merge, verify the build still passes: `pnpm build` and `pnpm typecheck`
...
After all sections are built and merged, wire everything together in `resources/js/Pages/Home.vue` (and any additional `Pages/<Name>.vue` files for multi-URL clones):
...
- Implement page-level behaviors: scroll snap, scroll-driven animations, dark-to-light transitions, intersection observers, smooth scroll (Lenis etc.) — typically wired via Vue composables in `resources/js/composables/`
- For multi-URL clones, ensure `routes/web.php` has a `Route::get(...)` entry for each page that calls `Inertia::render('<PageName>')`
- Verify: `pnpm build` and `pnpm typecheck` pass clean

## Phase 5: Visual QA Diff

After assembly, do NOT declare the clone complete. Take side-by-side comparison screenshots:

1. Open the original site and your clone side-by-side (or take screenshots at the same viewport widths). Run `composer dev` to start the local server (Laravel + Vite) and visit `http://localhost:8000`.
...
- **Don't create a `tailwind.config.js`.** Tailwind v4 has no JS config; design tokens live in the `@theme { ... }` block of `resources/css/app.css`. Adding a JS config will silently override Vite's Tailwind plugin and break utilities.
- **Don't write Options-API Vue components.** Use `<script setup lang="ts">` only. Mixing styles makes the codebase inconsistent and harder for builder agents to follow.

## Completion

When done, report:
- Total sections built
- Total components created (under `resources/js/Components/`)
- Total spec files written (should match components)
- Total assets downloaded (images, videos, SVGs, fonts)
- Inertia pages created (under `resources/js/Pages/`) and corresponding routes added to `routes/web.php`
- Build status (`pnpm build` and `pnpm typecheck` results)
