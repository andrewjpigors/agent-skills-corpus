...
description: Build, test, and debug Monika RL environments for Atropos training. Covers the HermesAgentBaseEnv interface, reward functions, agent loop integration, evaluation with tools, wandb logging, and the three CLI modes (serve/process/evaluate). Use when creating, reviewing, or fixing RL environments in the monika-agent repo.
version: 1.1.0
author: Monika
license: MIT
metadata:
...
# Monika Atropos Environments

Guide for building RL environments in the monika-agent repo that integrate with the Atropos training framework.
...
Monika environments are special because they run a **multi-turn agent loop with tool calling** — not just single-turn completions. The base env handles the loop; you implement the task and scoring.
...
| `environments/tool_call_parsers.py` | Phase 2 tool call parsers (monika, mistral, etc.) |
...
The whole point of monika-agent environments is agentic evaluation:
...
2. **evaluate() must use HermesAgentLoop, not chat_completion** — Single-turn chat_completion has no tools. The whole point of monika-agent benchmarks is agentic evaluation with tool use.
