...
description: "Use when the user wants stable, phased LaTeX writing for a graduation thesis or design report under a school template, including outline planning, page-count planning, controlled chapter drafting, conservative rollback, TOC formatting, and constrained figure/table support. Best for requests such as '先搭三级标题', '先定页数', '只写第一章', '按已确认目录继续写', '正文控制在 80 页', or '只改目录格式'. This skill is thesis-first: figure and table support is available but constrained, and must serve the argument rather than dominate it."
---
