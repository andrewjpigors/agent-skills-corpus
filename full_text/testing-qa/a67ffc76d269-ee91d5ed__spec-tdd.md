...
when_to_use: Invoke manually when building or modifying pure software projects across desktop, web, mobile, backend, CLI, or libraries, especially when the user wants gated architecture -> patterns -> SPEC -> test-first validation -> implementation until tests pass.
argument-hint: "<feature/request or architecture-doc paths> [--skip-research] [--small|--slice]"
  ...
  PreToolUse:
    - matcher: "Edit|Write|MultiEdit|Bash"
      hooks:
        - type: command
          command: node
          args: ["{HOME}/.claude/skills/spec-tdd/scripts/guard-frozen-tests.mjs"]
          timeout: 10
        - type: command
          command: node
          args: ["{HOME}/.claude/skills/spec-tdd/scripts/guard-review-gates.mjs"]
          timeout: 10
  PostToolUse:
    - matcher: "Bash"
      hooks:
        - type: command
          command: node
          args: ["{HOME}/.claude/skills/spec-tdd/scripts/post-bash-integrity.mjs"]
          timeout: 10
  Stop:
    - hooks:
        - type: command
          command: node
          args: ["{HOME}/.claude/skills/spec-tdd/scripts/verify-on-stop.mjs"]
          timeout: 600
---
