...
- What annual items do you manage? (Eleição de administradores, ratificação de auditor independente, aprovação de planos de outorga de ações/opções, remuneração global dos administradores — Art. 152 LSA — annual board self-assessment — whatever applies to you. If you're SEC-registered rather than CVM-registered, this may also include a say-on-pay vote.)
...
Ask which regime applies first — this changes every question below: **B3 (CVM-regulated, Brazilian issuer)**, **NYSE/Nasdaq (SEC-regulated, US issuer or ADR)**, or both (dual listing). A Brazilian company on B3 is not "SEC reporting" — it's CVM reporting, and the two regimes don't map field-for-field `[flag if the user's answer suggests they're conflating the two — verify]`.

**If B3 / CVM:**
- Segmento de listagem — Novo Mercado, Nível 2, Nível 1, tradicional?
- Fiscal year end?
- Categoria de registro na CVM — Categoria A ou B (Resolução CVM 80)?

**Disclosure committee (Comitê de Divulgação):**
- Do you have a formal disclosure committee? Who's on it — CFO, controladoria, RI, jurídico, others?
...
- When are your trading windows open relative to earnings (período de vedação de negociação)?
- Who is covered by pre-clearance requirements — all administradores and controladores, or a broader list?
...
**If NYSE / Nasdaq (SEC-regulated — US issuer or ADR program):**
...
- §16 reporting: who tracks §16 filer transactions, and what's your internal target for Form 4 (SEC requires 2 business days)?
- Same disclosure-committee, insider-trading-policy, and earnings-call questions as above, applied to the SEC regime.

Write to `## Public Company` in the config, tagging each field with which regime (B3/CVM or SEC) it belongs to if both apply.
...
- What entity type is each one — Ltda. (Código Civil arts. 1.052-1.087) or S.A. (Lei 6.404/1976)? For non-Brazilian entities in the group: what's the local equivalent (Delaware C-corp/LLC, UK Ltd, etc.)?
- What are the key jurisdictions — just Brazil (and which estados, for Junta Comercial purposes), or a meaningful multi-jurisdiction footprint?
- Who handles registro/manutenção — the advogado(a)/contador(a) responsible for the Junta Comercial filing and CNPJ upkeep for Brazilian entities, in-house, or varies by jurisdiction? (Brazilian entities don't have a "registered agent" in the US sense — see the Nota Brasil in the Entity Management config template.)
