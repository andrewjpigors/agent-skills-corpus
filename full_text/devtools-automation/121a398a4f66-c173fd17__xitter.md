...
version: 1.0.0
author: Siddharth Balyan + Monika
license: MIT
...
This skill intentionally does not vendor a separate CLI implementation into Monika. Install and use upstream `x-cli` instead.
...
If you already keep your X credentials in `~/.monika/.env`, the cleanest setup is:

```bash
mkdir -p ~/.config/x-cli
ln -sf ~/.monika/.env ~/.config/x-cli/.env
```
...
x-cli tweet search "from:thisismynewfmail-ui" --max 3
...
- **Credential drift**: if you rotate tokens in `~/.monika/.env`, make sure `~/.config/x-cli/.env` still points at the current file.
