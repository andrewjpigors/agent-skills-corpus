...
**Formato de horarios:** Los cron jobs se almacenan en UTC internamente, pero al presentar schedules al usuario convertir SIEMPRE a hora de Madrid (CET/CEST). No presentar nunca horarios en UTC cuando el usuario está en Madrid.
...
**Edge TTS voices:** Default is `en-US-AriaNeural` (female, US English). To change, edit `tts.edge.voice` in `config.yaml` (e.g. `es-ES-AlvaroNeural` for male Spanish from Spain). See `references/tts-voice-troubleshooting.md` for full Spanish voice list and troubleshooting. Voice changes require `/reset` to take effect.
...
Beyond spawning individual Hermes processes, there are established patterns for orchestrating multiple agents:

#### Pattern 1: Simple (tasks < 5 steps)
```
Tarea → Agente único → Resultado
```

#### Pattern 2: Complex (tasks 5+ steps) — Subagent roles
```
Tarea → Orquestador → [Explorer → Planner → Implementer → Reviewer → Critic → Synthesizer]
```

#### Pattern 3: Parallel with Orca (stablyai/orca ⭐3534)
Orca is the IDE for working with fleets of parallel agents. Each agent runs in its own git worktree — full isolation but tracked in one place. Supports 20+ agents: Claude Code, Codex, Grok, Gemini, Antigravity, Pi, Hermes Agent, OpenCode, Goose, Amp, Auggie, Autohand Code, Charm, Cline, Codebuff, Command Code, Continue, Cursor, and more.
- **URL:** https://onOrca.dev
- **Key pattern:** Drag files to create worktrees, each agent gets isolated context and filesystem
- **Platform:** macOS, Windows, Linux

#### Pattern 4: Persistent Memory (NtizarBrainMasterMind)
A multi-agent orchestration framework built on OpenCode + Obsidian with:
- **Memory persistence** with Ebbinghaus decay curve
- **11 specialized agents** with defined roles
- **Multi-model routing** (each agent uses its optimal model)
- **40-60% token savings** vs traditional prompting via smart loading
- **Mandatory review + adversarial critic** for quality

#### Pattern 5: Visual Board (Kanvas ⭐169)
Visual project board in Obsidian Canvas where humans and agents collaborate. Board is shared space — humans add tasks, agents propose and execute. Git-friendly JSON files, agent-agnostic.

### When to use which
- **Simple tasks:** Direct agent (default Mastermind behavior)
- **Complex tasks (5+ steps):** Use `delegate_task` with subagent roles
- **Large projects:** Consider Orca with isolated worktrees
- **Long-running autonomous work:** Use `cronjob` or `terminal(background=True)`
- **Knowledge accumulation:** Use NtizarBrainMasterMind pattern with archivist + Ebbinghaus decay
...
5. **`gh auth login --with-token` fails with "GITHUB_TOKEN environment variable is being used for Authentication"** when `GITHUB_TOKEN` is set in the environment. Fix: clear it first:
   ```bash
   GITHUB_TOKEN="" echo "$token" | gh auth login --with-token
   ```
   Or: `unset GITHUB_TOKEN && echo "$token" | gh auth login --with-token`
...
To use your NaN API key in VS Code via the **Continue** extension:
...
**Finding your NaN API key:** check environment variables — it's typically in `NAN_API` or `NTIZAR_API`. In config.yaml, the `model.base_url` is `https://api.nan.builders/v1` and `model.api_key` may be empty (key stored in env var instead).

**Alternative endpoints:** Hermes also supports `hermes acp` (ACP server mode) and `hermes mcp serve` (MCP server mode) for IDE integration without third-party extensions.
