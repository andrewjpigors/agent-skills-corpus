...
**Tier 1 — repo-local (automatic):** Scan the repo's local skills directory (or whatever directory the project's CLAUDE.md / AGENTS.md declares as its skill root) for suspicious patterns. In this workspace the canonical skill root is `_system/skills/`, not `.claude/skills/`:

```bash
ls -la .claude/skills/ 2>/dev/null
ls -la _system/skills/ 2>/dev/null   # AI_Projects' canonical skill root (per CLAUDE.md/AGENTS.md)
```

Use Grep to search all local skill SKILL.md (and wrench) files for suspicious patterns:
...
**FP rules:** First-party skills from this trusted stack are not flagged (check if the skill path resolves to a known repo). Skills that use `curl` for legitimate purposes (downloading tools, health checks) need context — only flag when the target URL is suspicious or when the command includes credential variables.
...
22. Skill files that are part of this trusted first-party skill stack
