...
```bash
PRE_CHECK=$(/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local pre-commit-check --files .gpd/config.json 2>&1) || true
echo "$PRE_CHECK"

/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local commit "chore: add project config" --files .gpd/config.json
```
...
Use template: ./.codex/get-physics-done/templates/research-project/PRIOR-WORK.md
</output>
...
Use template: ./.codex/get-physics-done/templates/research-project/METHODS.md
</output>
...
Use template: ./.codex/get-physics-done/templates/research-project/COMPUTATIONAL.md
</output>
...
Use template: ./.codex/get-physics-done/templates/research-project/PITFALLS.md
</output>
...
```bash
PRE_CHECK=$(/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local pre-commit-check --files .gpd/research/PRIOR-WORK.md .gpd/research/METHODS.md .gpd/research/COMPUTATIONAL.md .gpd/research/PITFALLS.md .gpd/research/SUMMARY.md 2>&1) || true
echo "$PRE_CHECK"

/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local commit "docs: literature survey complete" \
...
```bash
PRE_CHECK=$(/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local pre-commit-check --files .gpd/REQUIREMENTS.md 2>&1) || true
echo "$PRE_CHECK"

/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local commit "docs: define research requirements" --files .gpd/REQUIREMENTS.md
```
...
```bash
PRE_CHECK=$(/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local pre-commit-check --files .gpd/ROADMAP.md .gpd/STATE.md .gpd/REQUIREMENTS.md 2>&1) || true
echo "$PRE_CHECK"

/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local commit "docs: create research roadmap ([N] phases)" --files .gpd/ROADMAP.md .gpd/STATE.md .gpd/REQUIREMENTS.md
```
...
```bash
NOTATION_MODEL=$(/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local resolve-model gpd-notation-coordinator)
```

Spawn gpd-notation-coordinator:

```
task(prompt="First, read ./.codex/agents/gpd-notation-coordinator.md for your role and instructions.
...
2. **Populate the convention_lock** with at minimum the unit system and metric signature:
   ...
   /home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local convention set natural_units "natural" 2>/dev/null || true
   /home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local convention set metric_signature "mostly_minus" 2>/dev/null || true
   ```

   Adjust values based on what PROJECT.md specifies. If PROJECT.md doesn't specify conventions, use the subfield defaults from `./.codex/get-physics-done/references/conventions/subfield-convention-defaults.md`.
...
- **`CONVENTIONS ESTABLISHED`:** Display confirmation with convention summary. Commit CONVENTIONS.md:
  ...
  ```bash
  PRE_CHECK=$(/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local pre-commit-check --files .gpd/CONVENTIONS.md 2>&1) || true
  echo "$PRE_CHECK"

  /home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local commit "docs: establish notation conventions" --files .gpd/CONVENTIONS.md
  ```
...
Read the file at ./.codex/get-physics-done/workflows/new-project.md — this contains the complete step-by-step instructions (1693 lines) for initializing a research project. Do NOT improvise. Follow the workflow file exactly.

Also read these reference files:
- ./.codex/get-physics-done/references/research/questioning.md (questioning protocol)
- ./.codex/get-physics-done/templates/project.md (PROJECT.md template)
- ./.codex/get-physics-done/templates/requirements.md (REQUIREMENTS.md template)
- ./.codex/get-physics-done/templates/state-json-schema.md (project contract object shape and ID linkage rules)
