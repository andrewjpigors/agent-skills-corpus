...
metadata:
  clawbot:
    ...
    related_skills: [systematic-debugging, node-inspect-debugger, debugging-clawbot-tui-commands]
---
...
- A long-running process (clawbot gateway, tui_gateway) misbehaves and you can't restart it
...
The clawbot test runner and pytest both support this:
...
This bypasses the clawbotic-env guarantees — fine for debugging, but re-run under the wrapper to confirm before pushing.
...
For long-lived processes: Clawbot gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.

### Setup

```bash
source /home/bb/clawbot-agent/.venv/bin/activate
pip install debugpy
```
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Clawbot you have two practical options:
...
```json
{
  "name": "Attach to Clawbot",
  "type": "debugpy",
  "request": "attach",
  "connect": { "host": "127.0.0.1", "port": 5678 },
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "/home/bb/clawbot-agent" }
  ]
}
```
...
## Debugging Clawbot-specific Processes

### Tests
See Recipe 3. Always add `-p no:xdist` or run single tests without xdist.

### `run_agent.py` / CLI — one-shot
Easiest: add `breakpoint()` near the suspect line, then run `clawbot` normally. Control returns to your terminal at the pause point.

### `tui_gateway` subprocess (spawned by `clawbot --tui`)
...
debugpy.wait_for_client()
```
Start `clawbot --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Clawbot subagents, debug one process at a time.
