...
> **DISABLED — nocnoc security policy §2.0:** This step sends source code to OpenAI, which is not permitted. All code must remain within Anthropic infrastructure.

Skip this step. Continue to Step 4.
...
## Step 6.9: Human Push Confirmation (nocnoc security policy §2.4)

Before pushing anything to the remote, use AskUserQuestion:

```
/ship is ready to push. Review and approve:

Branch:  <branch-name>  →  <base>
Commits: N commits
Tests:   all passed
Review:  <summary of pre-landing findings or "No issues">

A) Approve — push and create PR
B) Abort — I need to review something first
```

Only proceed to Step 7 if the user explicitly chooses A.
