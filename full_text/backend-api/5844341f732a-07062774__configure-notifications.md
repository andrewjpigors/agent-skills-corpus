...
This is an interactive, natural-language configuration skill. Walk the user through setup by asking questions with AskUserQuestion. Write the result to `~/.grok/.omg-config.json`.

### Step 1: Detect Existing Configuration

```bash
CONFIG_FILE="$HOME/.grok/.omg-config.json"
...
Config saved to: ~/.grok/.omg-config.json

You can also set these via environment variables:
  OMC_TELEGRAM_BOT_TOKEN=123456789:ABCdefGHI...
  OMC_TELEGRAM_CHAT_ID=123456789

To reconfigure: /oh-my-grok:configure-notifications telegram
To configure Discord: /oh-my-grok:configure-notifications discord
To configure Slack: /oh-my-grok:configure-notifications slack
```
...
2. **Grok Build** - Display as "Grok Build"
...
To reconfigure: /oh-my-grok:configure-notifications discord
To configure Telegram: /oh-my-grok:configure-notifications telegram
To configure Slack: /oh-my-grok:configure-notifications slack
```
...
To reconfigure: /oh-my-grok:configure-notifications slack
To configure Discord: /oh-my-grok:configure-notifications discord
To configure Telegram: /oh-my-grok:configure-notifications telegram
```
...
- `omg --telegram` — Activates Telegram notifications (sets `OMC_TELEGRAM=1`)
- `omg --discord` — Activates Discord notifications (sets `OMC_DISCORD=1`)
- `omg --slack` — Activates Slack notifications (sets `OMC_SLACK=1`)
- `omg --webhook` — Activates webhook notifications (sets `OMC_WEBHOOK=1`)
- `omg --openclaw` — Activates OpenClaw gateway integration (sets `OMC_OPENCLAW=1`)
...
Check if `~/.grok/omc_config.hook.json` exists. If it does, show the current configuration. If not, explain what it does.
...
Config file: ~/.grok/omc_config.hook.json
```
...
Read or create `~/.grok/omc_config.hook.json` and merge the new settings:
...
- `/oh-my-grok:configure-openclaw` — Configure OpenClaw gateway integration
...
If `~/.grok/omc_config.openclaw.json` exists, detect and offer migration:

**Step 1: Detect Legacy Config**
```bash
LEGACY_CONFIG="$HOME/.grok/omc_config.openclaw.json"
