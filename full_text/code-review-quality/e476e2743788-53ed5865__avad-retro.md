...
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `~/.avadbot/retros/` (not `.context/avad-retros/`). Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".
