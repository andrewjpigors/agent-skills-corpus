...
### Open questions and decisions for the user

Any point where the doc needs the user to pick between options, whether that's a dedicated "Open questions" section or a decision buried inside a normal section, must not be phrased as a prose bullet the user can skim past. Render each one as its own callout, using an accent that clearly stands out from the rest of the doc (the template ships `.callout`, `.callout.green`, and `.callout.red`; pick whichever one isn't already doing double duty elsewhere in this doc so questions don't blend into warnings or success notes), and frame it as multiple choice: state the question, list the concrete options as clickable/markable choices, and let the user pick rather than write free text.

Where the tooling supports it, wire the callout so picking an option sends the answer back to you over the annotate channel the same way a click annotation does. As of this writing, `annotate/public/sdk.js` doesn't have a dedicated answerable-question widget, it captures clicks, text selections, and chat, but not a structured choice that posts a specific option back as its own payload. That's a real gap worth building into the annotate SDK; flag it to the user if you're counting on it. Until it exists, the callout still works without the widget: the user answers by clicking the option text (captured as a normal text-selection annotation) or by naming their pick in the side panel chat, and you treat it as an ordinary Phase 1.5 annotation. The authoring rule applies regardless of whether the widget exists: never leave a decision for the user sitting in plain prose. Make it a callout, make it multiple choice, make it hard to miss.
...
6. **Serve it the moment it's ready, without being asked.** The instant the checks above are green, open the doc in the user's browser. Use the annotate loop's `open` command (see Phase 1.5) so the review tooling is live from the start, or `xdg-open "$DOC"` for a plain view if you haven't started the annotate server yet. Do not ask "want me to open it?" and do not wait for the user to request it. The user should land on the rendered doc without having to ask for it.
...
Once the doc passes the pre-flight checks, do NOT jump straight to asking for approval, and do NOT wait for the user to ask to see it. Open the doc in the bundled annotate loop right away so the user can review it visually, point at specific elements, and tell you what to change. This is an interactive review pass that runs against the same HTML file you just wrote.
...
If Phase 1 was delegated to a sub-agent (for example, spawned to author the doc against a codebase the main thread hasn't loaded in full), that same sub-agent owns Phase 1.5, not the main thread. It's the one holding full codebase and doc context, so it should be the one applying edits and keeping the doc in sync with the intended implementation. The main thread should not apply edits blindly without that context. In that setup, the main thread's job is narrower: serve the doc, then relay each piece of user feedback (annotations, chat messages, whatever comes in) to the authoring agent, typically over SendMessage, and pass its replies back to the user. Only run the loop directly, as described below, when the main thread wrote the doc itself.

### The loop

1. **Open it, immediately.** The moment the doc passes pre-flight, spawn the server and launch the browser, don't wait to be asked:

   ```bash
   node "$SKILL_DIR/annotate/cli.js" open "$DOC"
   ```

   It prints the local URL (loopback, default port 4388) and `xdg-open`s it. Tell the user: click any element or select text in the doc to attach an annotation, or type in the side panel chat, or just talk to you in normal chat instead (see "Chat feedback is a first-class option" below). Their notes queue until you poll.
...
### Chat feedback is a first-class option

The annotate panel is good for pointing at one element, but batching everything behind a poll can feel slow and clunky for a user who'd rather just talk through changes. Offer both up front and let the user pick, don't force the panel. If the user prefers plain chat, keep the doc served (the file watcher live-reloads their browser on every save no matter which path they choose), take their feedback as normal chat messages instead of requiring a click-to-annotate, and treat each note the same way you'd treat an annotation: apply the edit (or relay it to the authoring agent, per "Who runs this phase" above), save, and the reload shows them the result. Confirm back in chat rather than through `--agent-reply` if that's the channel the user is using.
