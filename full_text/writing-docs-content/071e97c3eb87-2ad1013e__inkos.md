...
`inkos studio` launches a local web UI (default port 4577) that provides a visual interface for all InkOS operations:
...
```bash
inkos studio              # Start on default port 4577
...
- **No elevated privileges**: InkOS requires no sudo, no global state mutation, no network listening port (Studio binds `localhost:4577` only).
