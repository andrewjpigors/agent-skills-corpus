---
name: deepscientist-analysis-campaign
...
- Codex-native execution boundary: use Codex-native file/shell/Git/test/build tools for routine inspection, setup checks, and non-evidence smoke tests. Use `ds_bash_exec` for formal campaign slice commands or validations whose logs must become DeepScientist quest evidence.
- Prefer `ds_bash_exec` for campaign slice commands so each run has a durable session id, quest-local log folder, and later `read/list/kill` control.
- Keep ordinary subtask completions concise. When an analysis campaign or a stage-significant campaign checkpoint is complete, upgrade to a richer `ds_artifact_record payload={'kind': 'milestone'}, reply_mode='threaded', ...)` report.
...
- If the active communication surface is external messaging and external messaging milestone media is enabled in config, prefer at most one aggregated campaign summary PNG on a meaningful campaign milestone.
- That attachment should summarize the campaign as a whole; do not auto-send one image per slice.
- Treat summary campaign PNGs as report charts, not draft paper figures.
...
8. Before returning to `write`, outline selection, or paper bundle submission, use the Codex-native `ds_get_analysis_campaign` tool if available and verify there is no active campaign with pending required slices.
...
If ids, active refs, or current quest state are unclear after restart, call `artifact.get_quest_state(detail='summary')` and `ds_artifact_record evidence path resolution(...)` before launching or recording slices.
...
- `ds_bash_exec` session ids and managed shell logs for campaign runs
...
After the charter and launch decision are durably recorded, send one threaded `ds_artifact_record payload={'kind': 'milestone'}, ...)` update naming:
...
- if the active surface is external messaging, keep that campaign-launch milestone text-first unless a single summary image is already genuinely useful
...
If ids or refs are unclear, recover them first with `ds_artifact_record evidence path resolution(...)`, `artifact.get_analysis_campaign(...)`, or `artifact.list_paper_outlines(...)` instead of guessing.
Treat `campaign_id` as system-owned, and treat `slice_id` / `todo_id` as agent-authored semantic ids.
Do not replace the normal campaign flow with repeated manual `artifact.submit_idea(...)` calls.
After each slice finishes, call `ds_artifact_record_analysis_slice(...)` immediately so the result is mirrored back to the parent branch and the next slice can be activated.
If a slice fails or becomes infeasible, still call `ds_artifact_record_analysis_slice(...)` with an honest non-success status plus the real blocker and next recommendation; do not leave the campaign state ambiguous.
...
Each `ds_artifact_record_analysis_slice(...)` call should also include an `evaluation_summary` with exactly these six fields:
...
- once the smoke test passes, launch the real slice with `ds_bash_exec mode='detach', ...)` and normally leave `timeout_seconds` unset for that long run
...
- if you need a middle section that was omitted from that default preview, use `ds_bash_exec mode='read', id=..., start=..., tail=...)`
- monitor them with `ds_bash_exec mode='list')` and `ds_bash_exec mode='read', id=..., tail_limit=..., order='desc')`
- after the first read, prefer `ds_bash_exec mode='read', id=..., after_seq=last_seen_seq, tail_limit=..., order='asc')` for incremental monitoring
- if ids become unclear, recover them through `ds_bash_exec mode='history')`
- launch long slices with a structured `comment` such as `{stage, goal, action, expected_signal, next_check}`
- use `silent_seconds`, `progress_age_seconds`, `signal_age_seconds`, and `watchdog_overdue` from `ds_bash_exec mode='list'|'read', ...)` as the default stall checks
...
- after the first meaningful signal and then at real checkpoints (e.g., completion, blocker, recovery, or a materially changed evidence frontier), send `ds_artifact_record payload={'kind': 'progress'}, ...)` so the user sees the newest real state
- after each completed sleep / await monitoring cycle for an active slice, inspect state first; only send another `ds_artifact_record payload={'kind': 'progress'}, ...)` update if the user-visible state materially changed
- include the estimated next reply time or next check time in those monitoring updates
- stop them with `ds_bash_exec mode='kill', id=..., wait=true, timeout_seconds=...)` if the slice is invalid, wedged, or superseded; add `force=true` when immediate termination is required
...
Before a long slice starts, emit a `progress` artifact or `ds_artifact_record payload={'kind': 'progress'}, ...)` update so the quest shows that the slice is active.
...
Send a threaded `ds_artifact_record payload={'kind': 'milestone'}, ...)` update at that point with the new boundary or risk.
...
When the aggregated campaign report is complete, send a richer threaded `ds_artifact_record payload={'kind': 'milestone'}, ...)` update.
Lead that milestone with a concise `1-2` sentence campaign outcome summary before expanding into slice-level detail.

If external messaging milestone media is enabled and the aggregated report materially changes the claim boundary, you may attach one campaign summary PNG to that closing milestone update.
...
- begin every analysis campaign pass with `ds_memory_search(scope='quest', limit=5)`
- then run at least one analysis-relevant `ds_memory_search(...)` before launching or resuming slices
...
- if the campaign produced a durable cross-slice lesson, failure pattern, or comparability caveat, write at least one `ds_memory_write(...)` before leaving the stage
