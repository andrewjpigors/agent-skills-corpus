---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for ClawBot-Plus deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, ClawBot-Plus cron scheduling for periodic checks, or version status checks on a machine running ClawBot-Plus (laptop, workstation, Pi, VPS).
---

# ClawBot-Plus Host Hardening

## Overview

Assess and harden the host running ClawBot-Plus, then align it to a user-defined risk tolerance without breaking access. Use ClawBot-Plus security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim ClawBot-Plus changes the host firewall, SSH, or OS updates; it does not.
...
5. ClawBot-Plus gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if ClawBot-Plus can access sensitive data.
...
### 2) Run ClawBot-Plus security audits (read-only)

As part of the default read-only checks, run `clawbot-plus security audit --deep`. Only offer alternatives if the user requests them:

1. `clawbot-plus security audit` (faster, non-probing)
2. `clawbot-plus security audit --json` (structured output)

Offer to apply ClawBot-Plus safe defaults (numbered):

1. `clawbot-plus security audit --fix`

Be explicit that `--fix` only tightens ClawBot-Plus defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check ClawBot-Plus version/update status (read-only)

As part of the default read-only checks, run `clawbot-plus update status`.
...
- Credential hygiene notes (location of ClawBot-Plus creds, prefer disk encryption)
...
- ClawBot-Plus security audit (re-run)
...
After ClawBot-Plus install or first hardening pass, run at least one baseline audit and version check:

- `clawbot-plus security audit`
- `clawbot-plus security audit --deep`
- `clawbot-plus update status`

Ongoing monitoring is recommended. Use the ClawBot-Plus cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `clawbot-plus cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `clawbot-plus update status`
...
Before creating, `clawbot-plus cron list` and match on exact `name`. If found, `clawbot-plus cron edit <id> ...`.
If not found, `clawbot-plus cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `clawbot-plus update status` (preferred for source checkouts and channels)
2. `npm view clawbot-plus version` (published npm version)

## ClawBot-Plus command accuracy

Use only supported commands and flags:

- `clawbot-plus security audit [--deep] [--fix] [--json]`
- `clawbot-plus status` / `clawbot-plus status --deep`
- `clawbot-plus health --json`
- `clawbot-plus update status`
- `clawbot-plus cron add|list|runs|run`

Do not invent CLI flags or imply ClawBot-Plus enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by ClawBot-Plus compaction:
