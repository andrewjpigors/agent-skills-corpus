...
if [ ! -f ./land-deploy-confirmed ]; then
  ...
  SAVED_HASH=$(cat ./land-deploy-confirmed 2>/dev/null)
...
```bash
# Check for persisted deploy config in copilot-instructions.md
DEPLOY_CONFIG=$(grep -A 20 "## Deploy Configuration" copilot-instructions.md 2>/dev/null || echo "NO_CONFIG")
echo "$DEPLOY_CONFIG"
...
If `PERSISTED_PLATFORM` and `PERSISTED_URL` were found in copilot-instructions.md, use them directly
...
and any persisted config from copilot-instructions.md.
...
1. **copilot-instructions.md persisted config:** Check for a staging URL in the Deploy Configuration section:
```bash
grep -i "staging" copilot-instructions.md 2>/dev/null | head -3
```
...
**If C:** **STOP.** "Running `/setup-deploy` will walk through your deploy platform, production URL, and health checks in detail. It saves everything to copilot-instructions.md so I'll know exactly what to do next time. Run `/land-and-deploy` again when that's done."
...
Read copilot-instructions.md to find the project's test command. If not specified, use `bun test`.
...
```bash
git diff --name-only $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main)...HEAD -- README.md CHANGELOG.md ARCHITECTURE.md CONTRIBUTING.md copilot-instructions.md VERSION
```
...
```bash
eval $(.github/skills/bin/gstack-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
...
If staging was detected in Step 1.5c (or from copilot-instructions.md deploy config), and the changes
...
If a deploy status command was configured in copilot-instructions.md (e.g., `fly status --app myapp`), use it instead of or in addition to GitHub Actions polling.
...
If copilot-instructions.md has a custom deploy status command in the "Custom deploy hooks" section, run that command and check its exit code.
