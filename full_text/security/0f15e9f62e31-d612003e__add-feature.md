...
**Integration-first lane.** If the feature touches HTTP/webhook dispatch, queues, jobs, cron, IPC, MCP tool calls, file writes, env-var injection, spawned processes, external APIs, email/SMS/push, imports/exports, or cache invalidation, plan the runtime contract and observation point in Phase 3. Dispatch the **`runtime-contract-tracer`** subagent via `task({ subagent_type: "runtime-contract-tracer", description: "Trace runtime contract for integration", prompt: "Trace the trigger → dispatch → receive → observe flow for the integration <name>. Return file:line refs and flag silent-failure sites." })` with the integration name to get the trigger → dispatch → receive → observe trace with file:line refs and silent-failure sites flagged — this becomes the input to Phase 3's planning. After Phase 6, invoke `skill({ name: "add-observability" })` unless the project already has equivalent structured evidence and you can name where it lives.

**Mode safety override.** If `mode=fast` is requested for work that touches auth, permissions, payments, secrets, schema migrations, destructive deletes, background jobs, queues, cron, external APIs, email/SMS/push, imports/exports, file writes, spawned processes, IPC, caching/invalidation, feature flags, analytics/business reporting, concurrency-sensitive mutations, or external webhooks, pause and surface the conflict via `question` tool: *"Detected high-risk signals. You requested fast mode — that skips clarify, plan, reviews, and tests. Confirm fast anyway, or upgrade to production?"* The `/ship` orchestrator enforces this upstream, but direct manual callers may not — don't silently honor a dangerous override.
...
6. **CRUD/input surfaces** — enumerate every place a user creates, edits, configures, imports, or duplicates the affected artifact (create dialog AND edit dialog, settings page, bulk import, CSV upload, API client). Dispatch the **`crud-surface-mapper`** subagent via `task({ subagent_type: "crud-surface-mapper", description: "Map CRUD surfaces for artifact", prompt: "Map every create, edit, configure, import, and duplicate surface for the artifact '<name>'. Return a complete inventory with file:line refs." })` with the artifact name to get a complete surface inventory with file:line refs — keeps the search out of the parent's context. For each surface, decide explicitly whether the new field/behavior applies — silently shipping to only one is the most common "incomplete" failure. This is the inward complement to #5 (which looks outward at unrelated features that might break).
...
**Always use the `question` tool** (multi-question, structured choices) to ask clarifying questions — never present them as a static numbered list the user has to type answers to. Fall back to free-form prose only for a single question whose option space is genuinely open-ended.
...
Prefer `codegraph_explore` or `codegraph_search` for codebase exploration. If `.codegraph/` is not initialized, mention the user can run `codegraph init -i` for faster exploration, then fall back to `grep` + `read` + `glob`.
...
**UI surface parity.** If the feature will create a new instance of a recurring UI surface (modal, dialog, drawer, sheet, popover, form, confirm prompt, command palette, toolbar button, list-row action), dispatch the **`ui-pattern-inspector`** subagent via `task({ subagent_type: "ui-pattern-inspector", description: "Inspect UI pattern siblings", prompt: "Read 2–3 sibling instances of the surface '<name>'. Return a structured inventory: submit/cancel hotkeys, <Kbd> hint placement, autofocus target, loading/disabled states, footer chrome, error display, click-outside behavior. Also return 'consistent across siblings' / 'varies' verdict per property." })` with the surface name. The subagent reads 2–3 sibling instances and returns a structured inventory (submit/cancel hotkeys, `<Kbd>` hint placement, autofocus target, loading/disabled states, footer chrome, error display, click-outside behavior) plus a "consistent across siblings" / "varies" verdict. Record the inventory in the plan; matching consistent conventions is the default, diverging requires a stated reason. The `skill({ name: "propagate-ui-pattern" })` skill is the operational tool when 3+ siblings exist.

**Pre-write utility check — prevent duplication at creation.** For each non-trivial helper/utility/component the plan calls for, dispatch the **`utility-finder`** subagent via `task({ subagent_type: "utility-finder", description: "Find existing utility", prompt: "Search the repo for existing equivalents of a utility that does: '<description>'. Check naming variants. Return verdict: reuse / extend / write-new, with file:line refs for any existing match." })` with the function description. It searches the repo for existing equivalents (across naming variants) and returns a verdict — reuse / extend / write-new. The result short-circuits Phase 7b duplication-scan findings and keeps duplication out of the diff in the first place.

**Subagent fan-out — use when scope is wide.** Spawn parallel `task({ subagent_type: "explore", ... })` agents in a single message when:
...
**Realignment boundary:** If exploration shows the request is actually a domain-model rename or lifecycle/state/value realignment with persisted values (enum migration, status rename, vocabulary change, adding/removing a lifecycle state), stop and invoke `skill({ name: "realign" })`. Do not squeeze model realignment through a feature-add plan.
...
Present the plan to the user. **Do not write feature code before approval.** Pause. Present the plan. Ask for approval via `question` tool with options: (a)pprove / (r)evise / (q)uit.
...
**Subagent fan-out — use when work is genuinely independent.** Spawn parallel implementation agents via `task` in one message only when:
...
**If implementation reveals the plan was wrong** (not just incomplete — wrong assumption, wrong shape, wrong integration point): stop, `todowrite` this phase as `cancelled`, return to Phase 3, re-design, and re-approve. Do not silently improvise around a broken plan.
...
Use `todowrite` to mark this phase as `in_progress`. Mark sub-phases `completed` as they finish.
...
Subagent fan-out is encouraged here: dispatch the applicable reviews in parallel via `task` as separate agents, then consolidate findings.
...
Also dispatch the **`reviewer-security-regression`** subagent via `task({ subagent_type: "reviewer-security-regression", description: "Review security regression", prompt: "Review the diff for security regressions. Focus on auth, payments, file upload, webhook signing, secrets/env, external APIs, unsafe redirects, and user-rendered HTML. Return severity-ranked findings with auto-fixable flags." })` when the backend change touches auth, payments, file upload, webhook signing, secrets/env, external APIs, unsafe redirects, or user-rendered HTML. The subagent runs read-only and returns a severity-ranked findings report; apply the `auto-fixable: true` items mechanically and surface the rest to the user. If the security concern is specifically authorization/ownership across server entry points, dispatch the **`reviewer-authz`** subagent via `task({ subagent_type: "reviewer-authz", description: "Review authorization/ownership", prompt: "Review the diff for authorization and ownership issues across server entry points. Return severity-ranked findings (critical/high/medium/low) with concrete fix snippets using the project's existing auth helpers." })` — it returns severity-ranked findings (critical/high/medium/low) with concrete fix snippets using the project's existing auth helpers. The `skill({ name: "audit-authz" })` skill remains available as a manual entry point.
...
Dispatch the **`reviewer-perf`** subagent via `task({ subagent_type: "reviewer-perf", description: "Review performance impact", prompt: "Review the diff for performance issues. Focus on new query patterns, list pages, aggregation, upload/image-heavy routes, and user-scale loops. Return severity-ranked findings." })` when the gate is non-trivial (new query pattern, list page, aggregation, upload/image-heavy route, or user-scale loop) so the diff gets the dedicated perf pass instead of only checklist reasoning. The subagent runs read-only and returns a severity-ranked findings report; surface the findings to the user — perf fixes are tradeoffs and the user owns the apply decision. The `skill({ name: "audit-perf" })` skill remains available as a manual entry point.
...
Dispatch the **`reviewer-contracts`** subagent via `task({ subagent_type: "reviewer-contracts", description: "Review contract compatibility", prompt: "Review the diff for contract breaks or mismatches across client/server, route/schema, IPC, OpenAPI, tRPC, server-function, DTO, and generated-client boundaries. Return severity-ranked findings with producer and consumer file:line refs. Flag auto-fixable mechanical renames." })` when the diff crosses a client/server, route/schema, IPC, OpenAPI, tRPC, server-function, DTO, or generated-client boundary. The subagent runs read-only and returns a severity-ranked findings report with both producer and consumer file:line refs; apply the `auto-fixable: true` mechanical renames and surface structural mismatches to the user. Skip for comment/format-only changes, doc-only edits, and changes with no contract surface.

### 7f. Concurrency Review — GATED
Dispatch the **`reviewer-concurrency`** subagent via `task({ subagent_type: "reviewer-concurrency", description: "Review concurrency safety", prompt: "Review the diff for concurrency issues. Focus on mutations, background jobs, webhook handlers, queue workers, async UI writes, transactions, retries, idempotency, and read-modify-write flows. Return severity-ranked findings with auto-fixable flags (e.g., AbortController injection)." })` when the diff adds or changes mutations, background jobs, webhook handlers, queue workers, async UI writes, transactions, retries, idempotency, or read-modify-write flows. The subagent runs read-only and returns a severity-ranked findings report; apply the `auto-fixable: true` items (e.g., AbortController injection) and surface the rest to the user. Skip for read-only, UI-only, and doc/comment-only changes.

### 7g. Observability Coverage — GATED
Dispatch the **`reviewer-observability-coverage`** subagent via `task({ subagent_type: "reviewer-observability-coverage", description: "Review observability coverage", prompt: "After critical-path changes, review the diff for observability gaps. Focus on new integration boundaries, async work, error paths, jobs, webhooks, and external API calls. Return gaps report (no auto-instrumentation)." })` after critical-path changes, especially new integration boundaries, async work, error paths, jobs, webhooks, or external API calls. The subagent runs read-only and reports gaps (no auto-instrumentation). If it reports missing evidence and the change is still in scope, invoke `skill({ name: "add-observability" })` to instrument the boundary before declaring done.

### 7h. Data Integrity Review — GATED
Dispatch the **`reviewer-data-integrity`** subagent via `task({ subagent_type: "reviewer-data-integrity", description: "Review data integrity", prompt: "Review the diff for data integrity issues. Focus on persistence shape changes, migrations, status/enum values, denormalized data, deletes, backfills, import/export paths, and data-access invariants. Return severity-ranked findings with change classification (additive / mutating / destructive). Flag auto-fixable seed-fixture renames." })` when the diff changes persistence shape, migrations, status/enum values, denormalized data, deletes, backfills, import/export paths, or data-access invariants. The subagent runs read-only and returns a severity-ranked findings report with a change classification (additive / mutating / destructive); apply `auto-fixable: true` seed-fixture renames and surface schema/orphan/uniqueness findings to the user. Skip for changes with no persisted-data impact.

If the feature requires a schema migration, invoke `skill({ name: "add-migration" })` before this review; then run data-integrity on the migration + code diff.

### 7i. Failure UX — GATED
Dispatch the **`reviewer-error-boundaries`** subagent via `task({ subagent_type: "reviewer-error-boundaries", description: "Review error boundaries", prompt: "Review the diff for failure UX gaps. Focus on user-facing async flows, route loaders, form submits, server actions surfaced in UI, background failures visible to users, and paths where failure could blank the screen or silently corrupt state. Return findings with auto-fixable flags (button disable, errorComponent stub when convention exists)." })` when the diff adds or changes a user-facing async flow, route loader, form submit, server action surfaced in UI, background failure visible to users, or any path where failure could blank the screen or silently corrupt state. Apply auto-fixable items (button disable, errorComponent stub when convention exists); surface the rest. Skip for backend-only changes with no user-facing flow, docs, and comments.

### 7j. Async UI — GATED
Dispatch the **`reviewer-loading-states`** subagent via `task({ subagent_type: "reviewer-loading-states", description: "Review loading states", prompt: "Review the diff for async UI gaps. Focus on route data, useQuery/useSuspenseQuery, useMutation, optimistic updates, submit buttons, polling, and client-side fetches. Return findings with auto-fixable flags (disabled={isPending}, aria-busy)." })` when the diff adds or changes async UI: route data, `useQuery`/`useSuspenseQuery`, `useMutation`, optimistic updates, submit buttons, polling, or client-side fetches. Apply auto-fixable items (`disabled={isPending}`, `aria-busy`); surface the rest. If the change wires data fetching that can return zero/null or fail, run `skill({ name: "add-empty-error-states" })`.

### 7k. UI Regression — GATED
Dispatch the **`reviewer-accessibility-regression`** subagent via `task({ subagent_type: "reviewer-accessibility-regression", description: "Review accessibility regression", prompt: "Review the diff for accessibility regressions after interactive UI mutation. Focus on buttons, forms, dialogs, menus, custom click targets, focus changes, and error messages. Return findings with auto-fixable flags (icon-button labels, htmlFor on labels, decorative alt=\"\")." })` after interactive UI mutation (buttons, forms, dialogs, menus, custom click targets, focus changes, error messages). Apply auto-fixable items (icon-button labels, htmlFor on labels, decorative alt=""); surface the rest. Run `skill({ name: "audit-responsive" })` when the diff changes layout containers, grids, tables, nav/sidebar structure, modals, or mobile-sensitive sizing.

### 7l. Client Bundle — GATED
Dispatch the **`reviewer-client-bundle`** subagent via `task({ subagent_type: "reviewer-client-bundle", description: "Review client bundle impact", prompt: "Review the diff for client bundle regressions. Focus on client-side code changes in routes/components/hooks, new dependencies imported from UI code, server-only modules that might cross into the browser bundle, and large editor/chart/image/media dependencies. Flag auto-fixable items (e.g., lodash → lodash-es swap)." })` when client-side code changes in routes/components/hooks, when a new dependency is imported from UI code, when server-only modules might cross into the browser bundle, or when large editor/chart/image/media dependencies are added. Apply the lodash → lodash-es swap if flagged auto-fixable; surface the rest.
...
After implementation lands and tests pass, run `skill({ name: "simplify" })` against the diff to sweep for duplication, missed reuse of existing utilities, oversized components, magic numbers, and tight coupling — refactored in place without changing behavior.
