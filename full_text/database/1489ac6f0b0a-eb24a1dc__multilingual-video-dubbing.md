...
- Write down a short protocol checklist before acting: the exact required deliverable, required output paths, allowed tool/action interface, required wrapper schema, whether one action per turn plus waiting is required, any required progress-update shape, and the exact completion token.
- Before every tool/action message, compare the outgoing message against that checklist and use only the permitted schema. Do **not** introduce undeclared tool names, alternate action syntaxes, XML-style tags, markdown pseudo-calls, raw tool-call markup, or free-form narration outside the required wrapper.
- If the runtime requires one specific action wrapper or one-action-per-turn waiting behavior, use exactly that shape on every step and wait exactly when required.
- If the runtime requires structured progress or todo updates, emit syntactically complete updates only; never send truncated JSON, partial keys or values, or stale status payloads.
...
- Treat non-executable shell prose as a protocol failure. Every shell action must be directly runnable as written, including the concrete program, flags, quoted paths, redirects, and output locations.
- Before sending any shell action, check that it contains no ellipses, TODO wording, empty fragments, stray separators, or descriptive placeholders like `check files`, `create outputs`, `measure loudness`, or `normalize audio`.
...
- Immediately before the final message, perform a protocol-only check: re-read the runtime instructions, confirm the exact required final wrapper and exact completion token, and emit only that required terminator in the required wrapper with no extra prose when exact termination is required.
...
2aa. Prefer the smallest diagnostic ladder before any heavy script: verify import, then model load or CLI voice listing, then a 1-line target-language utterance written to disk and probed as readable.
2ab. Treat import-only checks as preflight, not validation. Before any full generation, prove model load or voice selection plus one real bounded synthesis call completes and produces a non-empty readable audio file.
...
2f. Do not treat partial `--help`, package file listings, cached model files, source-code inspection, or nearby similarly named assets as proof that a target-language voice is runnable. Before full synthesis, obtain either a concrete voice list or one successful short offline utterance using the exact intended voice and language settings.
2g. Before committing to a backend, check which local TTS or media tools are actually installed and runnable, then prefer the simplest already-working local CLI or API path that can satisfy the task.
2h. If the first candidate engine is unavailable, broken, or incomplete locally, pivot to an actually installed offline alternative that can produce a short target-language sample now instead of continuing speculative setup on the original engine.
...
3a. When both source text and target-language reference text are provided, treat `/root/reference_target_text.srt` as the canonical dubbing script and synthesize from it directly rather than performing a new translation step.
3b. Build the dubbing plan from the provided subtitle artifacts: map each segment window from `/root/segments.srt` to the exact target-language text that should be spoken, and use `/root/source_text.srt` only for grounding or ambiguity checks, not to replace the provided target wording.
...
4a.i. For the first expensive synthesis run, redirect stdout and stderr to a log file under `/outputs`, print progress markers before model load and before synthesis, and use an explicit short timeout so a hang produces evidence instead of a silent stall.
4a.ii. Do not let the first real generation attempt be a full multi-segment script or end-to-end batch run.
...
4b.i. If an inline probe fails to parse or raises a syntax or indentation error, stop treating that engine as tested. Rewrite the exact probe as a tiny saved script under `/outputs`, read the file back, and rerun that same minimal synthesis before writing any larger pipeline code.
...
8c. If a fallback path has already produced readable target-language audio and no visible task requirement rules it out, defer further engine exploration until after `/outputs/tts_segments/seg_0.wav`, `/outputs/dubbed.mp4`, and `/outputs/report.json` are fully delivered and verified.
...
10d1. If a run crashes with a traceback, exits quickly with stderr, or times out with little diagnostic output, capture that first concrete failure to a log file and inspect the actual exception, failing line, or likely blocking stage before switching engines or rewriting broader orchestration.
10d2. A timeout plus a 0-byte, missing, or unreadable output file means the current invocation is still unproven. Do not rerun the same command shape unchanged; first isolate the failing stage with a smaller observable check such as import-only, model-load-only, voice-listing, or a one-line synthesis probe with explicit logging.
10d3. Execute that recovery loop immediately; do not stop at a narrative statement such as 'I will check the file and rerun if needed.' The next action must be the actual probe, log inspection, or mitigation command.
10d4. Do not justify the next retry with an unverified story such as `initialization likely finished`. Name the observed evidence first, then choose a retry or pivot that directly follows from that evidence.
...
10e1. If the rerun still shows the same engine name, traceback location, or failing API call, do **not** continue with broader retries. Treat that as proof the fallback did not take effect; stop, inspect the executed script or command path, and fix the control flow before any new full run.
10e2. Prefer a branch proof before rerunning the full pipeline: print the selected engine or path, run one tiny direct synthesis through the fallback only, and confirm the resulting logs no longer mention the disabled primary engine.
...
10h1. Do not treat extra logging, progress prints, or wrapping the same heavy generation call in a new script as a materially different strategy if the underlying engine, settings, and full-generation workload are unchanged.
10h2. After one timeout on a full script, the next step should usually be a bounded direct probe of the same engine, not another full multi-segment run.
...
10k. If timing math, report generation, or metadata refresh needs only a few numeric fields, prefer direct `ffprobe` or JSON parsing commands, or one short saved script, over multi-line inline Python. After one repeated syntax or indentation failure, abandon the inline pattern for that task and validate the replacement before continuing.
10l. If a short synthesis probe times out or produces no readable WAV, do not proceed to orchestration, segment assembly, or final-pipeline scripting. First isolate the blocker with a smaller observable check such as import-only, model-load-only, voice-listing, or a 1-line synthesis with progress logging.
10m. If any full pipeline script exits nonzero, stops before its own explicit success message, or its captured log ends mid-step, treat the entire run as unproven. Read the full stderr/stdout or traceback from the saved log, inspect the actual script contents, and make one evidence-based fix before rerunning.
10n. If `/outputs/tts_segments/seg_0.wav` is missing, empty, or unreadable after the smoke test, treat the TTS path as unproven and block all downstream dubbing steps until that exact file is generated and probed successfully.
...
14c. During debugging, keep one canonical helper script path per approach and say, edit, read back, and execute that exact same path. If you switch to a different script name, explicitly retire the old one and verify the command now points to the new file.
14d. After writing or editing a helper script, read back the saved file contents before execution and confirm it is real executable code, not a tool summary, placeholder description, or stale prior version.
...
16a. Use a measure-first timing loop: synthesize the segment, probe its actual duration, compare it to `window_end_sec - window_start_sec`, then choose the least invasive allowed duration control from that measured result.
16b. If a synthesized segment is shorter than its subtitle window, prefer `pad_silence`; use `rate_adjust` only when needed to reduce a modest overrun without cutting words; keep `trim` limited to removable leading or trailing silence.
16c. Re-measure the adjusted segment and use that measured post-adjustment duration as the source of truth for downstream placement and report fields; do not derive final timing from nominal TTS settings or planned ratios alone.
16d. Finalize all timing-affecting edits before normalization. Do not run loudness normalization on a segment or assembled dub that you still expect to stretch, pad, trim, or otherwise retime.
...
24a. After any TTS recovery or engine switch, lock the working artifact format before downstream assembly: probe codec or container, sample rate, channels, and duration, then either convert immediately to the pipeline's expected 48 kHz mono WAV or explicitly update the pipeline to use the actual recovered format.
24b. Do not continue from a recovered artifact that merely exists. Require a successful probe showing the exact media properties you will rely on in later ffmpeg filters, timing calculations, and report generation.
...
33a. Treat visibly truncated target text in logs, scripts, JSON, or generated prompts as a blocking defect even if media files were produced. If a segment expected a full subtitle line but the TTS input or log shows only a fragment, stop and fix text extraction or quoting before downstream verification.
...
36. Do not write or run full subtitle-parsing, normalization, and mux orchestration until the chosen TTS path has already produced one tiny target-language sample on disk within the runtime limits. A timed-out, unreadable, or syntax-failed probe means the dependency is still unproven.
37. Once one smoke-tested path can meet the stated quality bar, lock the plan to that path and finish the minimal end-to-end pipeline instead of continuing broad environment exploration or creating more diagnostic scripts.
38. Treat completion as blocked until one same-run verification pass confirms all of the following on the canonical saved outputs: target-language text from `/root/reference_target_text.srt` was actually synthesized, `/outputs/tts_segments/seg_0.wav` is readable 48000 Hz mono, timing values were recomputed from final measured artifacts, final loudness is numeric from a successful measurement, and `/outputs/report.json` parses with those measured values.
...
- Treat `/root` inputs as read-only unless the task explicitly authorizes writing there. Never place helper scripts, rewritten assets, temp files, or convenience outputs under `/root` just because the source files live there; use `/outputs` or another explicitly allowed workspace.
...
- If a generated script contains placeholder prose, schema text, TODOs, or descriptions like `script to ...` instead of literal executable code, treat it as invalid and rewrite it before any execution attempt.
- Prefer this concrete sequence for helper scripts: write under `/outputs` -> read back the saved file with `sed -n '1,120p'` or `cat` -> run `python -m py_compile` or `bash -n` when applicable -> only then execute that exact path.
...
- Keep script identity exact during debugging: choose one canonical filename per purpose, print or read back that saved path after writing, and execute that same path verbatim on the next step.
- If the observed executed filename differs from the file you just edited, stop and reconcile the mismatch before continuing; do not debug one script while running another.
...
- Keep normalization downstream of timing edits: first finish `rate_adjust`, `pad_silence`, `trim`, resampling, and final assembly, then run loudness normalization and final measurement on that timing-final artifact.
...
- A blank or non-numeric loudness summary forbids any compliance claim and forbids writing fallback values such as `-23.0` into `/outputs/report.json` or the final response.
...
- Treat rolling `ebur128` status lines or interim integrated values as non-final diagnostics only. They are not acceptable evidence for `measured_lufs`, compliance claims, or task completion.
- If using `ebur128`, capture and parse the completed final summary; if you cannot obtain a final numeric integrated loudness result from the delivered artifact, switch to another concrete measurement method and do not claim compliance yet.
...
- Treat timing compliance as a release gate: if measured start alignment or end drift is still out of bounds on the saved final artifact, do not finalize. Apply the least invasive corrective step available, typically trailing silence for short speech or `rate_adjust` for slight overruns, then re-probe and recompute report values.
...
- Preserve this order when reporting: probe final adjusted segment durations and placement, verify final muxed media properties, measure final loudness, then write `/outputs/report.json` from those verified outputs.
...
- If `/outputs/dubbed.mp4` or any delivered segment WAV is regenerated after the report was written or partially updated, discard the old report state and rebuild every derived field from the current artifacts in one pass. Do not keep earlier `new_duration_sec`, timing fields, or loudness values because one field still 'looks close'.
...
- Never create `/outputs/report.json` from a natural-language placeholder or schema description. Build the full object with measured values, serialize it, then immediately parse the saved file successfully before using it in any completion claim.
- After writing `/outputs/report.json`, inspect the parsed structure or pretty-printed content once; file existence or directory listing alone is not evidence that the report is valid.
...
- If one verifier fails because a helper is missing or output is truncated, do not infer success from partial evidence. Switch immediately to another available concrete check such as `python`, `ffprobe`, `python -m json.tool`, redirected logs, or a saved parsing script, then rerun validation on the same canonical path.
- Do not let successful spot checks outweigh one concrete failed validation. One nonzero final-check command means the task is still incomplete until that exact failure is repaired and re-verified.
- Do not claim any artifact is complete, compliant, or verified while any earlier check for that artifact is still failing or unresolved. Zero-byte files, JSON decode errors, unreadable media, missing numeric loudness output, and known quality shortfalls are release blockers until repaired and revalidated.
...
- Treat a placeholder, prose-only, or schema-description `report.json` as a blocking failure even if the file exists at the correct path; rewrite the file from measured artifact data and re-parse it before completion.
- A truncated `cat`, partial terminal preview, or clipped UI rendering does not count as report verification. Use a structural validator such as `python -m json.tool /outputs/report.json` and, when content matters, inspect the full saved file with a non-truncating method before finishing.
- Do complete final verification on all canonical deliverables, not spot checks alone: parse the full `/outputs/report.json`, probe `/outputs/tts_segments/seg_0.wav`, and probe `/outputs/dubbed.mp4` on their final saved paths before the completion token.
...
- Use an evidence-only completion gate before the final response: every claimed requirement must have a successful command result on the canonical saved path from this run. Do not rely on intentions, prior runs, partial banners, or placeholder text.
- Add protocol compliance to the release gate: confirm every prior tool or action used the required schema and that the final response will be the exact required terminator with no extra prose.
- Add evidence compliance to the release gate: if loudness evidence is only partial analyzer progress, truncated logs, or missing a final numeric integrated result, the task is not ready to finalize.
...
- If you switch from a failing primary TTS engine to a fallback, prove the switch on a tiny sample before any full rerun. For a compact checklist, read [references/fallback-branch-verification.md](references/fallback-branch-verification.md).
