...
2. `.github/skills/.feature-prompted-model-overlay` →
   ...
   --model gpt-5.5`). Default is claude."
...
After `$B screenshot`, `$B snapshot -a -o`, or `$B responsive`, always use the view tool on the output PNG(s) so the user can see them. Without this, screenshots are invisible.
