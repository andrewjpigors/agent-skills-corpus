...
Detailed prompt templates in `${COPILOT_PLUGIN_ROOT}/skills/visual-explainer/`. In GitHub Copilot CLI, these are slash commands (`/diff-review`). In Claude Code, namespaced (`/visual-explainer:diff-review`).
...
**Usage:**
```bash
bash ${COPILOT_PLUGIN_ROOT}/skills/visual-explainer/scripts/share.sh <html-file>
```

**Example:**
```bash
bash ${COPILOT_PLUGIN_ROOT}/skills/visual-explainer/scripts/share.sh ~/.agent/diagrams/my-diagram.html
...
See `.github/commands/visual-explainer/share.md` for the `/share` command template.
