...
`sl-ideate` precedes `sl-brainstorm`.

- `sl-ideate` answers: "What are the strongest ideas worth exploring?"
- `sl-brainstorm` answers: "What exactly should one chosen idea mean?"
- `sl-plan` answers: "How should it be built?"
...
Use `AskUserQuestion` (call `ToolSearch` with `select:AskUserQuestion` first if its schema isn't loaded). Fall back to numbered options in chat only if it is unavailable or the call errors. Never silently skip the question.
...
- a path such as `plugins/super-looper/skills/`
...
3. **Route action into brainstorming** - Ideation identifies promising directions; `sl-brainstorm` defines the selected one precisely enough for planning. Do not skip to planning from ideation output.

## Model Tiers

Sub-agent dispatch is tiered by task shape, never hardcoded to a model name:

- **Extraction tier** — evidence scouts and other retrieval/quoting work. The cheapest capable model (`model: "haiku"`). "Capable" is part of the spec — escalate to the generation tier when the repo is large or the stack obscure.
- **Generation tier** — evidence-driven ideation frames and basis verification. The mid-tier model (`model: "sonnet"`).
...
Unlike `sl-plan` and `sl-brainstorm` (which default to `md`), sl-ideate defaults to **`html`** — ideation artifacts are read mainly by humans weighing candidate directions, and a rich self-contained HTML file (with illustrative diagrams for the top candidates) makes the ideas easier to approach.

**Read config.** The repo root is pre-resolved at skill load:
!`git rev-parse --show-toplevel 2>/dev/null || true`

If the line above is an absolute path, use it as `<repo-root>`. If it is empty or still shows a backtick command string (a non-Claude harness that did not run the pre-resolution), resolve `<repo-root>` at runtime by running `git rev-parse --show-toplevel` with the shell tool. Then read `<repo-root>/.super-looper/config.local.yaml` with the native file-read tool. If the root cannot be resolved (not a git repo) or the file does not exist, fall through to the defaults below.
...
The `output:` preference does NOT auto-propagate to `sl-brainstorm` on handoff (Phase 5) — sl-brainstorm re-resolves its own `brainstorm_output` config independently. Asymmetric output (`ideation.html` + `requirements.md`) is acceptable; users who want HTML for both set both keys in `.super-looper/config.local.yaml`.
...
- Never ask about solution direction, constraints, audience, tone, success criteria, or anything that characterizes the subject — those belong to `sl-brainstorm`.
...
- Stop as soon as the subject is identifiable or the user has delegated to "Surprise me." More than 3 total questions across 0.2 and 0.4 is a smell that ideation is not the right workflow — consider suggesting `sl-brainstorm`.
...
2. **Learnings search** — dispatch `sl-learnings-researcher` with a brief summary of the ideation focus.

3. **Web research** (always-on; see "Web research" subsection below for skip-phrase and V15 cache handling).

4. **Issue intelligence** (conditional) — if issue-tracker intent was detected in Phase 0.3, dispatch `sl-issue-intelligence-analyst` with the focus hint. Run in parallel with the other agents.
...
2. **Learnings search** *(elsewhere-software only; skipped by default in elsewhere-non-software)* — dispatch `sl-learnings-researcher` with the topic summary in case relevant institutional knowledge exists (skill-design patterns, prior solutions in similar shape). Skip for elsewhere-non-software: the CWD's `docs/solutions/` is unlikely to be topically relevant for non-digital topics, and running it risks polluting generation with unrelated engineering patterns.
...
Always-on for both modes. Skip when the user said "no external research", "skip web research", or equivalent in their prompt or earlier answers; in that case, omit `sl-web-researcher` from dispatch and note the skip in the consolidated grounding summary.

Reuse prior web research within a session via a sidecar cache — see `references/web-research-cache.md` for the cache file shape, reuse check, append behavior, and platform-degradation rules. Read it the first time `sl-web-researcher` would be dispatched in this run (and on every subsequent dispatch where the cache might apply).

When dispatching `sl-web-researcher`, pass: the focus hint, a brief planning context summary (one or two sentences), and the mode. Do not pass codebase content — the agent operates externally.
...
**Enrichment, not substitution.** A supplied research artifact does not replace the `sl-web-researcher` dispatch — these artifacts typically cover source classes (social platforms, niche communities, prediction markets, short-video) that web research does not reach, and vice versa. Dispatch web research as normal.
