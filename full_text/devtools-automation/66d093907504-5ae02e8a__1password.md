...
version: 1.0.0
author: arceus77-7, enhanced by Monika
license: MIT
metadata:
  monika:
...
### Service Account (recommended for Monika)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.monika/.env` (the skill will prompt for this on first load).
...
Monika terminal commands are non-interactive by default and can lose auth context between calls.
