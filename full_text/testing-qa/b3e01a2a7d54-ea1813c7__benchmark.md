...
description: |
  ...
  "bundle size", "load time". (cyberdart)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"benchmark","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.cyberdart/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/cyberdart/bin/cyberdart-timeline-log '{"skill":"benchmark","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
mkdir -p .cyberdart/benchmark-reports
mkdir -p .cyberdart/benchmark-reports/baselines
```
...
Write to `.cyberdart/benchmark-reports/baselines/baseline.json`.
...
Write to `.cyberdart/benchmark-reports/{date}-benchmark.md` and `.cyberdart/benchmark-reports/{date}-benchmark.json`.
