...
```bash
source <(.github/skills/bin/gstack-diff-scope <base> 2>/dev/null) || true
...
For each selected specialist, launch an independent subagent via the task tool.
...
```bash
.github/skills/bin/gstack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"COMMIT"}'
```
