...
- Before emitting any tool call, copy the required interaction pattern literally from the runtime instructions and match it field-for-field: exact wrapper/prefix, exact JSON keys, exact tool/interface name, one-action-per-turn rule if any, allowed paths, and exact completion token. Keep that syntax unchanged for the rest of the run.
- On the first executable turn, make exactly one valid action using the required runtime wrapper to inspect the workspace or open the task files; do not spend the first turn on planning, todo updates, approval-seeking, or clarification questions when the environment can answer them.
- If the runtime requires a literal pattern such as `Thought:` followed by `Action:` plus a JSON object, mirror that exact capitalization, labels, and payload shape on every action; do not substitute XML/tool tags, markdown fences, pseudo-function calls, renamed tools, alternate capitalization, or prose in place of that structure.
...
- Every action argument must be directly executable or directly applicable as written. Use literal commands, paths, offsets, limits, JSON fields, and exact old/new text or full file contents; do not send placeholders such as `...`, pseudo-calls, or natural-language intent like `inspect inputs`, `run the solver`, or `replace previous block`.
- Before creating scripts, virtual environments, logs, or outputs, identify the explicitly allowed or workspace-confirmed directories and anchor every file operation to those paths; do not assume habitual locations are permitted.
- Reuse the same confirmed path consistently for any environment or artifact you create; do not drift between similar locations later in the run.
- If the runtime exposes a special sub-workflow such as plan mode, approval prompts, or question/answer mode, enter it only when explicitly required by the interface and exit it cleanly with the exact required call before resuming normal actions. If such a mode-switch returns an unexpected prompt, malformed-response error, or ambiguous intermediate state, correct that interaction immediately instead of continuing in free-form prose.
...
- Before finalizing, run a last protocol gate: confirm the required artifact was actually generated and validated from the latest completed run, then compare your closing message against the runtime's exact completion token character-for-character.
- If the runtime requires an exclusive completion string such as `ACTION: TASK_COMPLETE`, reserve it for the final turn and output exactly that string alone with no added prose, markdown, prefix, suffix, or punctuation.
...
- If a traceback points to a specific file and line, read that exact on-disk code region before editing and base the patch on the observed failing expression rather than a guessed root cause.
...
- Do not chain `solve && view-report` and then treat truncated combined output as evidence of success; first confirm the solve command finished, then inspect the saved report in a separate explicit validation step.
- If solver construction or execution fails repeatedly, capture the full exception from that exact failing call before changing variable types, formulation, or solver options.
...
- Use the first invalid metric to drive the next check: negative losses or generation below load -> audit balance/sign/unit conversions; extreme branch overloads -> audit branch admittance, thermal-limit mapping, and flow formulas; voltage-limit violations with odd angles -> audit voltage-variable bounds and indexing before changing solver options.
...
- Treat names like `x`, `g`, `f`, `Vm`, `Va`, `Pg`, and `Qg` as reserved once they represent symbolic decision variables or expressions; use distinct loop names such as `br_x`, `tap`, `rateA`, or `bus_i` for numeric fields so a later reassignment cannot overwrite the model.
...
- If using CasADi or another symbolic library, explicitly confirm the decision vector is still symbolic immediately before solver creation; if a debug print shows a plain float or array where a symbolic object is expected, stop and fix the overwrite before changing solver settings.
...
- If you temporarily use a reduced diagnostic model to debug environment or formulation issues, label it explicitly as a debugging aid only, keep it separate from the main deliverable, and return to the full `math-model.md` formulation before claiming completion.
- Do not replace the primary solver/report path with a simplified fallback script and then hand off that fallback as if it satisfied the requested AC-OPF model.
...
- Before executing a script, verify the command names the same path you most recently created or edited; do not switch to a similarly named file unless you have explicitly inspected it and confirmed it is the intended artifact.
- If you created a scratch or repro script during debugging, keep it separate from the production run and make sure the final solve/report command uses the intended main script.
...
- If the solver completed but report creation, serialization, or post-processing failed, treat that first as a reporting/integration bug. Patch the output-generation path with minimal edits, then rerun the solve -> report pipeline instead of changing the mathematical model without evidence.
...
- Triage failures in order: (1) runtime/protocol violation, (2) Python/type/format/path/reporting exception from the latest traceback, (3) only then solver feasibility or formulation issues. Do not describe a run as infeasible or retune the model when the latest visible failure is a concrete runtime, typing, formatting, path, or reporting exception.
- If a file read returns truncated content and the environment offers offsets, ranges, pagination, or follow-up reads, immediately continue reading until the specific needed sections are visible; do not start implementing AC equations, report logic, or schema mapping from a partial file view.
- Before implementing the model, inspect `network.json` structurally: confirm the dimensions of `bus`, `gen`, `branch`, and `gencost`, identify bus-type codes and the slack/reference bus, and verify the column layout you will use for limits, setpoints, costs, and branch ratings. If the file is array-based MATPOWER data, map columns explicitly before writing equations.
- If you need a concrete pattern for inspecting MATPOWER-style arrays and column mapping before modeling, read [references/network-input-inspection.md](references/network-input-inspection.md).
- Probe the environment before committing to a stack: verify which interpreter names exist, check whether key libraries are importable, and detect managed-package restrictions before attempting installation. Treat environment recovery as incomplete until an observed proof step succeeds by printing the chosen Python executable/version and importing the exact packages needed for this run.
- After writing or overwriting any solver/report script, reopen the saved file and inspect enough content to confirm it contains concrete executable code, not a prose summary or placeholder description, before running it.
- Do not delete solver scripts, helper scripts, scratch diagnostics, generated reports, environments, or other artifacts during the task just because a run appears complete; preserve them until the final artifact has been re-opened, validated, and the exact required completion signal has been emitted.
...
- For large or truncated report reads, use the targeted parser-based verification pattern in [references/report-verification.md](references/report-verification.md) before making final claims.
...
- If you suspect a schema mismatch, first locate the exact required schema in the task instructions or referenced files and compare the saved report against that requirement before editing code or rewriting the report.
- Do not patch report structure or `summary.solver_status` based on a truncated preview, missing tail section, or a desire to make the output look successful. Change report fields only when explicit solver termination evidence or explicit post-solve validation from the latest run shows the mapped status or structure should differ.
...
- Apply a hard-stop physical consistency check before any success claim: if `solver_status` is not explicitly successful, `total_generation_MW < total_load_MW` beyond tolerance, implied losses are materially negative, any voltage/branch/generator violation is positive, or your own feasibility metrics show unresolved nonzero violations, do not finalize as solved.
- Treat solver statuses such as `Infeasible_Problem_Detected`, `infeasible`, failed convergence, or physically contradictory totals as hard blocks on completion. Do not emit the final completion signal, and do not describe the case as solved, until you have either fixed the formulation/run and regenerated a feasible report or explicitly reported failure to obtain an AC-feasible solution.
- If a report is schema-correct but fails any physical consistency check, shows contradictory evidence such as `optimal` plus negative losses or explicit overloads, or comes from a run that never reached final termination evidence, treat it as debugging evidence only; continue remediation or clearly report failure to obtain an AC-feasible point.
- When the saved report is too large for a single view or any read shows truncation, use the parser-based pattern in [references/report-verification.md](references/report-verification.md) to inspect `summary`, full `feasibility_check`, and samples from `generators`, `buses`, and `most_loaded_branches` from the regenerated file.
...
- Treat obvious red-flag numerics as formulation or reporting bugs until disproven: zero or near-zero generation with substantial load, negative losses beyond tolerance, branch loading in the hundreds or thousands of percent, or any result that is clearly unrealistic or nonphysical.
- If any such red flag appears, do not describe the case as feasible, solved, or constraint-satisfying; debug the model or report calculations first and rerun before making success claims.
...
- Prefer a two-step validation pattern near the end: (1) run the solver and confirm the process completed, then (2) separately parse and print the saved report fields you will cite. Do not rely on one chained command when output may truncate.
- Never stop at a waiting state after starting an async or background solve; fetch its completion, verify outputs, and then perform the exact required completion step.
...
- Immediately after writing or patching a `.py` file, read back the saved contents or run a lightweight syntax/import smoke check before treating it as the script you intended to execute.
- If a write tool accepts free-form content, assume semantic write failure is possible: inspect the saved file itself before execution rather than trusting the write acknowledgement.
...
- Treat finalization as its own last checkpoint: verify the required artifact exists and was validated from disk, verify the latest run status supports the claim, then send exactly the mandated completion token if the runtime requires one.
- Do not place any explanatory prose after the required final token; if exclusivity is required, emit only that token.
...
- For long-running or chained commands, do not infer that later stages ran just because early output appeared or an old report file exists. First confirm the command completed, then verify the target artifact was regenerated by that completed run, then inspect the saved report.
- Keep edits concrete and reviewable: write exact code or exact replacements, then reopen the saved file and inspect the changed lines before rerunning. Preserve create -> inspect -> run traceability: if you created or patched a solver/report script, validate by running that same file explicitly unless you first inspect and justify a different entrypoint.
...
- Make critical edits auditable. For any fix to equations, limits, indexing, or report logic, inspect the saved file and verify the exact changed lines or behavior are present; do not rely on vague edit descriptions, placeholder replacements, or file-exists evidence.
...
- For newly written solver/report scripts, do not trust the write action alone. Read back the file immediately and confirm it contains real code structure such as imports, function/class definitions, or executable Python statements, not a sentence describing intended behavior.
- If the read-back shows placeholder prose or a summary instead of executable code, repair the file contents before any execution or completion claim.
...
- Ground every final success claim in the latest completed rerun of the exact file version now on disk. If you rewrote or restored a solver script, rerun that same script to completion and inspect the regenerated artifact before concluding.
- Do not report feasibility metrics, overload counts, limiting branches, mismatch values, or solver-success claims unless you directly re-read those exact fields from the regenerated artifact or explicit verification output of the latest completed run.
- Do not treat solver startup banners, iteration headers, or launch messages as solve completion; wait for explicit final termination evidence, process completion, or verified regenerated report contents from the latest run before claiming success.
...
- For large JSON reports, prefer a parser-based check that prints the required fields and selected counts/entries from the saved file, then base final claims only on that observed output. If a file read reports truncation or shows only a preview, treat verification as incomplete and continue reading with offsets or use the targeted parsing pattern in [references/report-verification.md](references/report-verification.md) before claiming the report is complete or valid.
...
- Treat solver startup text, model dimensions, banners, dependency-install logs, and lines ending at `Solving...` as nonterminal. Do not claim convergence, optimality, or completion until you observe explicit final termination text, process-completion evidence, or a regenerated validated artifact from that run.
- Before the final response, perform one last gate in order: verify the latest completed run regenerated the target report, re-read every report section you intend to cite from the saved artifact (using targeted parsing if large or truncated), then emit the exact required completion token and stop acting unless cleanup was explicitly requested.
- Do not state that a solve succeeded, that a report is complete, or that specific totals, statuses, feasibility findings, branch results, or bus/generator details were obtained unless those exact facts were directly read from the latest completed run output or from the regenerated saved report.
