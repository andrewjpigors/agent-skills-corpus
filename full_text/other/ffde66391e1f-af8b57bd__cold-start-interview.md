...
> Who'll be using this plugin day to day? (This feeds the work-product header on every clearance memo, C&D draft, and portfolio memo — and for Agentes da Propriedade Industrial, drives the narrower "limited professional secrecy" header on INPI matters only.)
>
> 1. **Lawyer or legal professional (inscrito na OAB)** — advogado(a), paralegal, legal ops, IP specialist working under attorney oversight.
> 2. **Agente da Propriedade Industrial registrado no INPI** — you're registered to represent applicants before INPI (Lei 9.279/1996 art. 216) but are not a licensed attorney (não inscrito na OAB). Your representation authority covers deposit, examination, opposition, and administrative appeal before INPI; it does not carry attorney-client privilege under the Estatuto da OAB (Lei 8.906/1994), and it does not reach trademark, copyright, OSS, or general-contract matters outside INPI proceedings.
...
> If you need to find a licensed attorney or Agente da Propriedade Industrial in your jurisdiction: your professional regulator's referral service is the fastest starting point (OAB — seccional local — in Brazil, state bar in the US, SRA/Bar Standards Board in England & Wales, Law Society in Scotland/NI/Ireland/Canada/Australia, or your jurisdiction's equivalent). Many offer free or low-cost initial consultations. For IP specifically, the OAB's comissões de propriedade intelectual (Brazil), the ABA IP section and state IP law associations (US), CIPA/ITMA (UK), and equivalent bodies elsewhere have referral lists. For small businesses, university IP clinics (núcleos de propriedade intelectual — NITs) can be a resource for clearance and policy work.

If the answer is 2 (Agente da Propriedade Industrial), say this in addition to the Role-2/3 framing above:

> A note on how I'll handle sigilo for your work. On matters of representation before INPI (Lei 9.279/1996 art. 216 — depósito, exame, oposição, recurso administrativo), I'll mark those outputs `SIGILO PROFISSIONAL LIMITADO — ATUAÇÃO PERANTE O INPI — NÃO EQUIVALE A SIGILO ADVOGADO-CLIENTE`. Brazilian law has no doctrine of attorney-client privilege for a non-lawyer agent — the Estatuto da OAB (Lei 8.906/1994) reserves that protection for advogados inscritos. On anything outside INPI practice (trademark strategy outside deposit/prosecution, copyright, OSS, trade secret, contracts, general advice), I'll mark those outputs `NOTAS DE PESQUISA — SEM SIGILO PROFISSIONAL` and flag them to bring to a supervising advogado before relying on them. This isn't a cautious default; it's the actual scope of your representation authority. If you're doing substantive non-INPI IP work, you're also running an unauthorized-practice risk — keep that work tightly scoped to research notes for an attorney, not client advice.
...
> This plugin can work with: IP management systems (Anaqua, CPA Global, PatSnap, Clarivate), patent research (Solve Intelligence), legal research (JusBrasil, Escavador, PJe), document storage (Google Drive, SharePoint, Box), and Slack. Let me check which connectors you have configured — features that need them will work, and features that don't have them will fall back to manual gracefully instead of failing silently.
...
>
> - **Marks registered in:** Brazil (INPI)? US (USPTO)? EU (EUIPO)? UK (UKIPO)? Madrid member states — which (Brazil joined the Madrid Protocol in 2019, Decreto 10.033/2019)? National filings elsewhere? Uso de fato only (no registration)?
> - **Patents granted in:** Brazil (INPI)? US? EPO? PCT national phase countries? Any specific jurisdictions that matter (Germany, Japan, China)?
> - **Where you enforce:** Brazil (which comarcas/varas empresariais or specialized IP courts, if relevant)? US federal / state? Outside US or Brazil? Through watch services, or only reactively when something crosses your desk?
