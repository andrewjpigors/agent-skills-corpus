...
When asking a material planning question, use the platform's blocking question tool when available, such as `AskUserQuestion` in Claude Code. Fall back to structured numbered choices in chat only when no blocking tool exists or the call errors. Never silently skip a material question.
