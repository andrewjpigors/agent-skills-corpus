...
metadata:
  solanaos:
    emoji: "🧠"
    registryKey: "openrouter.text"
    kind: "inference"
    requires:
      bins: ["node"]
      env: ["OPENROUTER_API_KEY"]
    injectAt: "agent-birth"
---
...
```typescript
import { fromClawdMessages, toClawdMessage } from '@openrouter/agent';

// Clawd messages → OpenRouter format
const result = client.callModel({
  model: 'anthropic/claude-3-opus',
  input: fromClawdMessages(clawdMessages)
});

// Response → Clawd message format
const response = await result.getResponse();
const clawdMsg = toClawdMessage(response);
```
