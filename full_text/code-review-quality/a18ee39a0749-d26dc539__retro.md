...
**If prior retros exist:** Load the most recent one using the view tool. Calculate deltas for key metrics and include a **Trends vs Last Retro** section:
```
                    Last        Now         Delta
...
Use the create tool to save the JSON file with this schema:
```json
{
  "date": "2026-03-08",
  "window": "7d",
...
```bash
DISCOVER_BIN=""
[ -x .github/skills/bin/gstack-global-discover ] && DISCOVER_BIN=.github/skills/bin/gstack-global-discover
...
If a matching prior retro exists, load it with the view tool. Show a **Trends vs Last Global Retro** table with deltas for key metrics: total commits, LOC, sessions, streak, context switches/day.
...
Use the create tool to save JSON to `~/.gstack/retros/global-${today}-${next}.json`:
...
- Do not read copilot-instructions.md or other docs — this skill is self-contained
