...
- **Auto memory** (Codex only) — does the injected auto-memory block in your system prompt contain entries in the same problem domain? Scan that block directly. If the block is absent, skip this dimension. A memory note describing a different approach than what the learning recommends is a supplementary drift signal.
...
- Add context to the evidence report ("(auto memory [Codex]) notes suggest approach X may have changed since this learning was written")
...
>
> Also scan the "user's auto-memory" block injected into your system prompt (Codex only). Check for notes related to the learning's problem domain. Report any memory-sourced drift signals separately from codebase-sourced evidence, tagged with "(auto memory [Codex])" in the evidence section. If the block is not present in your context, skip this check.
...
- Prefer the platform's native content-search tool (e.g., Grep in Codex) over shell. Drop to shell when materially better for the case.
...
- What evidence was found -- tag any memory-sourced findings with "(auto memory [Codex])" to distinguish them from codebase-sourced evidence
