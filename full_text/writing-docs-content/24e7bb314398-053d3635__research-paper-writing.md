...
license: MIT
python_dependencies: [semanticscholar, arxiv, habanero, requests, scipy, numpy, matplotlib, SciencePlots]
...
| **Low** (major unknowns) | Ask the user 1-2 targeted questions, then draft |
...
Create a structured project plan (a `TODO.md` in the workspace, or your agent's task-tracking tool):
...
**Use the `arxiv` skill** for structured paper discovery (in the same `research/` skills folder). It provides arXiv REST API search, Semantic Scholar citation graphs, author profiles, and BibTeX generation — all via plain `curl`.

Use a web search tool (WebSearch) for broad discovery and `curl`/WebFetch for fetching specific papers:

```
# Broad discovery (WebSearch tool, or a search-API curl call):
"[main technique] + [application domain] site:arxiv.org"
"[baseline method] comparison ICML NeurIPS 2024"

# Fetch a specific paper (curl, or the WebFetch tool):
curl -sL "https://arxiv.org/abs/2303.17651"
```
...
**For agent-based workflows**: If your agent can spawn sub-agents (e.g. parallel `claude -p` invocations), run each round's queries in parallel. Collect results, deduplicate, then generate the next round's queries from the combined learnings. Otherwise run them sequentially.
...
This pipeline runs with ordinary agent tooling: a shell (bash), file read/write/edit, git, `curl`, a web search tool, and the `claude` CLI for spawning helper sub-agents. Map the patterns below onto whatever concrete tools your agent exposes.

### Companion Skills

Compose this skill with other skills for specific phases:

| Skill | When to Use |
|-------|-------------|
| **arxiv** | Phase 1 (Literature Review): searching arXiv, generating BibTeX, finding related papers via Semantic Scholar (curl-based, in the same `research/` folder) |
| **llm-wiki** | Phase 1 (Literature): build a persistent interlinked markdown knowledge base of the sources you ingest |

### Capability Reference

| Capability | Usage in This Pipeline |
|------------|----------------------|
| **Shell (bash)** | LaTeX compilation (`latexmk -pdf`), git operations, launching experiments (`nohup python run.py &`), process checks (`ps aux`), `tail`/`ls` for monitoring |
| **Background processes** | Long experiments via `nohup ... &` (record the PID with `echo $!`); poll with `ps`, read logs with `tail`, stop with `kill` |
| **Python (`python3`)** | Citation verification, statistical analysis, data aggregation. Run inline or as scripts. |
| **File read / write / edit** | Paper editing, experiment scripts, result files. Use targeted edits (not full rewrites) for large `.tex` files. |
| **Web search** | Literature discovery: search `"transformer attention mechanism 2024"` etc. (WebSearch tool or a search-API curl call) |
| **`curl` / WebFetch** | Fetch paper content, verify citations: `curl -sL "https://arxiv.org/abs/2303.17651"` |
| **Sub-agents (`claude -p`)** | Parallel section drafting — spawn an isolated `claude -p` per section. Also for concurrent citation verification. Optional: fall back to sequential if unavailable. |
| **TODO file / task tracker** | Primary state tracker across sessions. Update after every phase transition. |
| **Notes / memory file** | Persist key decisions across sessions in a `NOTES.md`: contribution framing, venue choice, reviewer feedback. |
| **Scheduler (cron / `mpcron`)** | Schedule experiment monitoring, deadline countdowns, automated arXiv checks. |
...
tail -30 <logfile>
ls results/
python3 analyze_results.py        # compute metrics from results JSON
git add -A && git commit -m "<descriptive message>" && git push
# then notify the user with a one-line summary
```

**Parallel section drafting** (sub-agents). Spawn one `claude -p` per section, each with a self-contained prompt (no shared context — include everything it needs):
```bash
claude -p "Draft the Methods section based on these experiment scripts and configs.
  Include: pseudocode, all hyperparameters, architectural details sufficient for
  reproduction. Write in LaTeX using the neurips2025 template conventions." > paper/methods.tex

claude -p "Draft the Related Work section. Use web search + curl to find papers.
  Verify every citation via Semantic Scholar. Group by methodology." > paper/related.tex

claude -p "Draft the Experiments section. Read all result files in results/.
  State which claim each experiment supports. Include error bars and significance." > paper/experiments.tex
```
Collect outputs and integrate. If your agent can't spawn sub-agents, draft sections sequentially in the main session.
...
**Notes file** — persist key decisions in a `NOTES.md` in the workspace:

```
Paper: autoreason. Venue: NeurIPS 2025 (9 pages).
...
Update after major decisions or phase transitions so a future session can resume.

**TODO file** — track granular progress in a `TODO.md` (checkbox list), marking items in-progress / done as you go.

**Session startup protocol:**
```bash
cat TODO.md                       # Check current task list
cat NOTES.md                      # Recall key decisions
git log --oneline -10             # Check recent commits
ps aux | grep python              # Check running experiments
ls results/ | tail -20            # Check for new results
# Then report status to the user and ask for direction
```

### Scheduled Monitoring

Use a scheduler (system `cron`, or mindpalace's `mpcron`) to run periodic experiment checks. The scheduled job should invoke the agent with a prompt like:
...
**[SILENT] protocol**: When nothing has changed since the last check, respond with exactly `[SILENT]` so the run produces no notification. Only report genuine changes.

**Deadline tracking**: a daily scheduled job that computes days remaining to the submission deadline, checks the TODO list, and warns the user when <7 days remain with tasks outstanding.
