...
IMPORTANT — No `gh` CLI dependency. This skill uses curl + the GitHub REST API exclusively. The GH_TOKEN env var is already injected by Velaclaw. Pass it as a Bearer token in all API calls:
...
```
CONFIG_PATH="${VELACLAW_CONFIG_PATH:-${VELACLAW_STATE_DIR:-$HOME/.velaclaw}/velaclaw.json}"
cat "$CONFIG_PATH" | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```

If still empty, check `/data/.clawdbot/velaclaw.json`:

```
cat /data/.clawdbot/velaclaw.json | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```
...
- If curl returns an HTTP 401 or 403 → stop and tell the user:
  > "GitHub authentication failed. Please check your apiKey in the Velaclaw dashboard or in the active Velaclaw config path (`$VELACLAW_CONFIG_PATH`, default `~/.Zavianx/velaclaw-dev.json`) under `skills.entries.gh-issues`."
...
- **Sequential cursor tracking:** Use a cursor file to track which issue to process next:
  ...
  # SOURCE_REPO_SLUG = owner-repo with slashes replaced by hyphens (e.g., velaclaw-velaclaw)
  ```
...
CONFIG_PATH="${VELACLAW_CONFIG_PATH:-${VELACLAW_STATE_DIR:-$HOME/.velaclaw}/velaclaw.json}"
GH_TOKEN=$(cat "$CONFIG_PATH" 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty') || GH_TOKEN=$(cat /data/.clawdbot/velaclaw.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty')
...
export GH_TOKEN=$(node -e "const fs=require('fs'); const c=JSON.parse(fs.readFileSync('/data/.clawdbot/velaclaw.json','utf8')); console.log(c.skills?.entries?.['gh-issues']?.apiKey || '')")

```
If that fails, also try:
```

export CONFIG_PATH="${VELACLAW_CONFIG_PATH:-${VELACLAW_STATE_DIR:-$HOME/.velaclaw}/velaclaw.json}"
