...
- **导出安全护栏**：Step 4 的 HTML / Review prompt 必须额外注入导出安全与兼容参考，约束高风险 CSS、内容安全区和 Review 修复顺序，避免浏览器效果与最终 PPTX 交付脱节。
...
P5.03  [WAIT_AGENT] FINALIZE
P5.04  回收校验 speech-script.json → 派生 speech-script.md
P5.05  生成 preview.html
P5.06  PNG 导出 → presentation-png.pptx（写入 speaker notes）
P5.07  SVG 导出 → presentation-svg.pptx（写入 speaker notes）
P5.08  写入 delivery-manifest.json
```
...
| 导出安全真源 | `references/design-runtime/pptx-export-safety.md` + `references/design-runtime/pptx-html-compat.md` | Step 4 HTML/Review 注入 |
...
| P5 | 演讲稿 + 导出交付 | speech-script.json / speech-script.md / preview.html / 双 pptx / delivery-manifest.json | `contract_validator speech-script` + `delivery-manifest` | 只回退导出 |
...
Step 5 内部新增 **SpeechSynth** 子流程，但**不改变**顶层主链，仍然属于 P5。

执行顺序：
1. 先由 SpeechSynth subagent 生成 `speech-script.json` 真源
2. 再由脚本派生 `speech-script.md`
3. 然后并行执行 PNG / SVG 导出，并把 `speech-script.json` 写入两份 PPTX 的 speaker notes

导出失败只回退导出，不回退内容生产。命令见 cheatsheet Step 5。
