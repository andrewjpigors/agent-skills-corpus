...
Important: in `tfpolicy test`, **all three levels mark the resource as `fail` on a failing condition** — the only difference is the severity tag on the diagnostic. `expect_failure = true` catches all three. The "still passes overall" claim some docs make is about HCP run *outcome*, not the test framework.
