...
1. Check `config/local/codex-for-legal/product-legal/CLAUDE.md` state.
...
5. Migration: if a populated CLAUDE.md (no `[PLACEHOLDER]` markers) exists at `config/local/cache/codex-for-legal/product-legal/*/CLAUDE.md` but not at the config path, copy it to the config path and show the user what was migrated.
6. Write `config/local/codex-for-legal/product-legal/CLAUDE.md` (create parent directories as needed). Show calibration table for confirmation.

## `--check-integrations`

Re-runs the integration availability check (launch tracker, document storage, Slack (connector optional; not enabled in v1)) and updates `## Available integrations` in `config/local/codex-for-legal/product-legal/CLAUDE.md`. Does not re-interview. Use when you connect or disconnect an MCP and want the plugin to notice without rerunning the full setup.
...
If a CLAUDE.md exists at the old cache path `config/local/cache/codex-for-legal/product-legal/*/CLAUDE.md` but not at the config path, copy it forward.
...
>
> Quick or full? (Upgrade any time with `$product-legal:cold-start-interview --full`.)
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `$product-legal:cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `config/local/codex-for-legal/product-legal/CLAUDE.md` with a `<!-- SETUP PAUSED AT: [section name] — run $product-legal:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
> This plugin can work with: launch tracker (Jira, Linear, Asana), document storage (Google Drive (connector optional; not enabled in v1), SharePoint), and Slack (connector optional; not enabled in v1). Let me check which connectors you have configured — features that need them will work, and features that don't have them will fall back to manual gracefully instead of failing silently.
...
For connectors that show as not connected, tell the user how to connect. Example phrasing: "Jira isn't connected. In Codex: add the Jira MCP to your config or via `/mcp`. This plugin works without it — you'll paste PRDs and review docs directly — but connecting it lets the launch-watcher agent pull tickets automatically."
...
**Risk appetite:** *(This feeds `$product-legal:launch-review` and `$product-legal:is-this-a-problem` — sets what counts as a P0 blocker at your company vs. an FYI.)*
...
**What keeps you up at night?** *(This feeds `$product-legal:launch-review` — the questions the GC always asks become mandatory checks on every launch memo.)*
...
**Escalation — who signs off above you?** *(This feeds every skill's routing — `$product-legal:launch-review`, `$product-legal:is-this-a-problem`, and `$product-legal:marketing-claims-review` all know when to say "you can handle this" vs. "loop in [X]".)*
...
**What's your framework?** *(This feeds `$product-legal:launch-review` — the categories you check here become the section headings of every launch memo.)*
- Do you have categories you check every launch against? (Contractual, privacy, IP, regulatory, etc.)
- Formal sign-off, or advisory?
- What's the output — a memo, a ticket comment, a Slack (connector optional; not enabled in v1) thread?
...
*(This feeds `$product-legal:marketing-claims-review` — substantiation standard and comparative-claims posture drive how the skill flags marketing copy.)*
...
| Slack (connector optional; not enabled in v1) | [✓ / ✗] | Triage replies delivered inline instead of posted |
...
> - **Fast triage on a Slack (connector optional; not enabled in v1) question** — e.g., "'Hey legal, quick question' gets a same-minute fine / needs a real look / stop." Try: `$product-legal:is-this-a-problem`
...
>
> **My suggestion for your first one:** Run `$product-legal:is-this-a-problem` on one PM question you already answered — see if the answer matches how you calibrated it. Or tell me what's on your plate and I'll pick.
...
2. **Research connector prompt.** Say:
   ...
   > "Before your first launch review: connect a research tool. Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Cowork: Settings → Connectors. In Codex: authorize when a skill prompts you."
...
5. **Close with the changeability note.** Say:
   ...
   > "Done. Your configuration is at `config/local/codex-for-legal/product-legal/CLAUDE.md` — a plain-text file you can read and edit directly. Anything you answered can be changed:
   >
...
> - Run `$product-legal:cold-start-interview --redo <section>` to re-interview one part, or edit the config file directly.
>
