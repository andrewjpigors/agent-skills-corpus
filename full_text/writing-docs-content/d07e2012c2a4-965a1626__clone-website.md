...
- **Fidelity level:** Pixel-perfect — exact match in spacing, typography, layout, animations. **Colors are the exception: this project has a LOCKED brand theme (see AGENTS.md → Theming). Re-skin the target's colors onto the existing `globals.css` tokens by role; do NOT copy the target's palette or overwrite the theme.**
...
Nothing can be built until the foundation exists: global CSS design tokens (fonts, spacing), TypeScript types for the content structures, and global assets (fonts, favicons). This is sequential and non-negotiable. Everything after this can be parallel. **Note: the color foundation is already done — `globals.css` ships a locked brand `@theme` palette. Do NOT overwrite it with the cloned site's colors; map new sections onto the existing tokens (AGENTS.md → Theming).**
...
**Colors** — This project already ships a **locked brand palette** in `src/app/globals.css` (the `@theme` block, plus `:root`/`.dark`). **Do NOT overwrite it with the target's colors.** Read the target's palette only to learn each color's *role*, then map it onto the existing tokens: target primary/brand → `--color-brand`, darkest text → `--color-ink`, card background → `--color-surface`, borders → `--color-line`, muted labels → `--color-ink-muted`, success/error/promo → `--color-green*`/`--color-pink*`/`--color-orange*`. Add a new `--color-*` token only for a genuinely new role. See AGENTS.md → Theming for the full token vocabulary and rules.
...
2. **Update globals.css** with spacing values, keyframe animations, utility classes, and any **global scroll behaviors** (Lenis, smooth scroll CSS, scroll-snap on body). **Leave the brand `@theme` color palette LOCKED — do not overwrite it with the target's colors; map every section's colors onto the existing tokens (AGENTS.md → Theming).**
