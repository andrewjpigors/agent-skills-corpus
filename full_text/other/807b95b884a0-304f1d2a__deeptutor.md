---
name: deeptutor
description: Use when evaluating or comparing a professor as a potential graduate advisor, including requests such as "调查导师", "选导师", "导师怎么样", "这个导师 push 吗", "investigate this advisor", "should I join this lab", or a professor name plus institution with evaluation intent. DeepTutor investigates publications, co-author networks, student trajectories, funding, lab culture, and exploitation/toxicity risk; applies region-specific searches for mainland China and international institutions; and renders a cited standalone HTML report in Chinese or English. Student outcomes are the highest-priority signal.
license: MIT
metadata:
  author: jiadizhunine
  version: "1.4.1"
...
- **Low ceiling**: Multiple verified weak outcomes or frequent attrition; do not infer this from missing public profiles alone
- **Information gap**: No public student outcomes may mean a new PI, privacy choices, or sparse records. Mark it unresolved and verify directly rather than treating absence as a negative fact.
...
**Universal red flags:**
- Multiple documented poor student outcomes or attrition cases; missing public records alone are an information gap, not a red flag
...
- An official misconduct finding, or a retraction whose verified cause creates a current supervision or lab-safety risk
...
**检查项：**
- 官方公开的任期、退休或招生安排；不要根据照片、姓名、性别或其他代理信息猜测年龄
- 当前国家/地区法律与学校政策（必须查当期官方来源，不使用固定年龄的跨地区假设）
- Tenure status（国际）：pre-tenure PI有被deny tenure导致实验室关闭的风险
- 经费连续性：当前经费何时到期？是否有续期迹象？
- 是否有实验室搬迁/跳槽迹象？（关注近期的职位变动、多个affiliation）
- 活跃度与招生承诺：近期是否持续招生、指导学生、申请经费和发表成果；不要据此推断健康状况
...
1. Use public, professionally relevant information only. Do not seek or publish home addresses, private phone numbers, personal accounts, family details, or data behind access controls.
   `search_social.py` sends query terms to Bing and may fall back to Sogou, so never include private or sensitive information in a query.
...
- `scripts/robust_fetch.py` — Public-network-only stdlib fetch with DNS/IP pinning; optional curl/browser interfaces fail closed (adapted from [Web-Rooter](https://github.com/baojiachen0214/web-rooter), MIT; see `THIRD_PARTY_NOTICES.md`)
