...
echo '{"skill":"ship","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.mzstack/analytics/skill-usage.jsonl 2>/dev/null || true
...
- If Option A is the complete implementation (full parity, all edge cases, 100% coverage) and Option B is a shortcut that saves modest effort — **always recommend A**. The delta between 80 lines and 150 lines is meaningless with CC+mzstack. "Good enough" is the wrong instinct when "complete" costs minutes more.
- **Lake vs. ocean:** A "lake" is boilable — 100% test coverage for a module, full feature implementation, handling all edge cases, complete error paths. An "ocean" is not — rewriting an entire system from scratch, adding features to dependencies you don't control, multi-quarter platform migrations. Recommend boiling lakes. Flag oceans as out of scope.
- **When estimating effort**, always show both scales: human team time and CC+mzstack time. The compression ratio varies by task type — use this reference:

| Task type | Human team | CC+mzstack | Compression |
|-----------|-----------|-----------|-------------|
...
Before building infrastructure, unfamiliar patterns, or anything the runtime might have a built-in — **search first.** Read `~/.claude/skills/mzstack/ETHOS.md` for the full philosophy.
...
Log eureka moments:
```bash
jq -n --arg ts "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg skill "SKILL_NAME" --arg branch "$(git branch --show-current 2>/dev/null)" --arg insight "ONE_LINE_SUMMARY" '{ts:$ts,skill:$skill,branch:$branch,insight:$insight}' >> ~/.mzstack/analytics/eureka.jsonl 2>/dev/null || true
```
...
**Review tiers:**
- **Eng Review (required by default):** The only review that gates shipping. Covers architecture, code quality, tests, performance. Can be disabled globally with \`mzstack-config set skip_eng_review true\` (the "don't bother me" setting).
...
1. **Check for a prior override on this branch:**
   ```bash
   eval "$(~/.claude/skills/mzstack/bin/mzstack-slug 2>/dev/null)"
   grep '"skill":"ship-review-override"' ~/.mzstack/projects/$SLUG/$BRANCH-reviews.jsonl 2>/dev/null || echo "NO_OVERRIDE"
   ```
...
2. **If no override exists,** use AskUserQuestion:
   ...
   - For Design Review: run `source <(~/.claude/skills/mzstack/bin/mzstack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 3.5, but consider running /design-review for a full visual audit post-implementation." Still never block.

3. **If the user chooses A or C,** persist the decision so future `/ship` runs on this branch skip the gate:
   ```bash
   eval "$(~/.claude/skills/mzstack/bin/mzstack-slug 2>/dev/null)"
   echo '{"skill":"ship-review-override","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","decision":"USER_CHOICE"}' >> ~/.mzstack/projects/$SLUG/$BRANCH-reviews.jsonl
   ```
...
[ -f .mzstack/no-test-bootstrap ] && echo "BOOTSTRAP_DECLINED"
```
...
If user picks H → write `.mzstack/no-test-bootstrap` and continue without tests.
...
If user picks C → write `.mzstack/no-test-bootstrap`. Tell user: "If you change your mind later, delete `.mzstack/no-test-bootstrap` and re-run." Continue without tests.
...
- Create a GitHub issue assigned to that person:
  ...
  gh issue create \
    ...
    --body "Found failing on branch <current-branch>. Failure is pre-existing.\n\n**Error:**\n```\n<first 10 lines>\n```\n\n**Last modified by:** <author>\n**Noticed by:** mzstack /ship on <date>" \
...
- **>= minimum, < target:** Use AskUserQuestion — offer to generate more tests, ship anyway, or mark as intentionally uncovered.
- **< minimum:** Use AskUserQuestion — coverage critically low. Options: generate tests or override.

**Coverage percentage undetermined:** Skip with: "Coverage gate: could not determine percentage — skipping." Do not default to 0% or block.

**Test-only diffs:** Skip the gate.
...
```bash
eval "$(~/.claude/skills/mzstack/bin/mzstack-slug 2>/dev/null)" && mkdir -p ~/.mzstack/projects/$SLUG
USER=$(whoami)
DATETIME=$(date +%Y%m%d-%H%M%S)
```

Write to `~/.mzstack/projects/{slug}/{user}-{branch}-ship-test-plan-{datetime}.md`:
...
Check if the diff touches frontend files using `mzstack-diff-scope`:

```bash
source <(~/.claude/skills/mzstack/bin/mzstack-diff-scope <base> 2>/dev/null)
```
...
```bash
~/.claude/skills/mzstack/bin/mzstack-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
which codex 2>/dev/null && echo "CODEX_AVAILABLE" || echo "CODEX_NOT_AVAILABLE"
# Respect old opt-out
OLD_CFG=$(~/.claude/skills/mzstack/bin/mzstack-config get codex_reviews 2>/dev/null || true)
...
```bash
~/.claude/skills/mzstack/bin/mzstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"medium","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
~/.claude/skills/mzstack/bin/mzstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"large","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
Append to `~/.mzstack/projects/$SLUG/$BRANCH-reviews.jsonl`:

```bash
echo '{"skill":"ship","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","coverage_pct":COVERAGE_PCT,"plan_items_total":PLAN_TOTAL,"plan_items_done":PLAN_DONE,"verification_result":"VERIFY_RESULT","version":"VERSION","branch":"BRANCH"}' >> ~/.mzstack/projects/$SLUG/$BRANCH-reviews.jsonl
```
