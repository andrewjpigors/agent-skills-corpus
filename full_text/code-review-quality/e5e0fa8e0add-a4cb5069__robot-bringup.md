...
name: robot-bringup
description: >
  Production bringup patterns for ROS2 robots on onboard computers. Use when configuring robot
  startup on boot, writing systemd units for ROS2 launch, composing full-stack launch files,
  ordering startup with health checks, configuring udev device names, adding watchdogs or
  restarts, managing logs, debugging boot-time races, setting up multi-machine networking, or
  handling graceful shutdown for long-running robot services on Ubuntu with ROS2 Humble, Iron,
  or Jazzy.
---
