...
**输出**：完整的 NPC Design Bible Markdown 文档 + 最终展示图 + 可交互角色卡牌 + 展示页 HTML
...
**展示页 HTML 生成规则（内部脚本调用）**：
> 在 Bible 文档和最终展示图完成后，自动调用本 Skill 内置的 `scripts/generate_display.py` 脚本生成 Kami 设计风格的展示页 HTML。
>
> **质量守门规则**：
> - 调用前必须确认 `scripts/generate_display.py` 和 `templates/bible-display.html` 存在于本 Skill 目录。
> - 若脚本或模板缺失，报告"展示页资源缺失"，停止生成。
> - 生成的 HTML 为 standalone 单文件，无需外部依赖。
>
> **触发条件**：
> - Step 8 的 Bible 文档已生成并保存到文件 `npc-design-bible-*.md`
> - 同目录下存在 NPC 概念图（`[NPC名称].png/jpg/webp`）或用户已上传图片
>
> **调用流程**：
> 1. **读取 Bible 文件**：脚本自动解析 `npc-design-bible-*.md` 内容
> 2. **图片检测**：自动查找同目录下以 NPC 名称命名的图片文件
> 3. **生成 HTML**：执行 `python scripts/generate_display.py --input bible.md --output display.html`
>    - 生成结果：单文件 HTML，包含角色信息、肖像框、功能声明、设计校准等
>    - 肖像框位于页面右上方，类似人物档案风格
>    - 图片自动转为 base64 内嵌，无需外部文件
> 4. **告知用户**：
>    > "这是你的 NPC 展示页，可以直接在浏览器中打开或打印为 PDF。图片已自动嵌入，无需额外文件。"
>
> **不触发的情况**：
> - 用户明确拒绝生成展示页
> - 脚本或模板资源缺失
...
>
> **与主 Skill 的衔接**：
> - 展示页是 Step 8 的附加产出，不阻塞后续步骤
> - 如果用户已有 `npc-card-generator` 生成的卡牌，展示页作为补充文档使用
> - 无图时显示占位框，保持排版完整
