...
- **No blocking prompts.** Never use `AskUserQuestion` or other blocking question tools. Infer intent, plan, and scope from explicit tokens, git state, PR metadata, and conversation. Note uncertainty in Coverage or the verdict — do not stop to ask.
...
| **`mode:agent`** | One JSON object (see ### JSON output format below) + the same `/tmp/.../sl-code-review/<run-id>/` artifacts |

`mode:agent` is **report-only**: it skips the Stage 5c apply (the caller applies) and serializes findings as JSON instead of markdown. It does not change reviewer selection, merge logic, or scope rules — the JSON is the deterministic contract for programmatic callers. The default markdown is the human view; keep it ASCII-safe (pipe tables, `->` not middot `·`, no box-drawing) so it degrades gracefully across terminals.
...
**Always-on (every review):** `sl-correctness-reviewer`, `sl-testing-reviewer`, `sl-maintainability-reviewer`, `sl-project-standards-reviewer`, plus CE agents `sl-agent-native-reviewer` and `sl-learnings-researcher`.

**Cross-cutting conditional (per diff):**

- `sl-security-reviewer` — auth, public endpoints, user input, permissions
- `sl-performance-reviewer` — DB queries, data transforms, caching, async
- `sl-api-contract-reviewer` — routes, serializers, type signatures, versioning
- `sl-data-migration-reviewer` — migration files / schema dumps / backfills (see spawn gate in Stage 3)
- `sl-reliability-reviewer` — error handling, retries, timeouts, background jobs
- `sl-adversarial-reviewer` — >=50 changed code lines, or auth / payments / data mutations / external APIs
- `sl-previous-comments-reviewer` — PR with existing review comments (PR-only, comment-gated)

**Stack-specific conditional (per diff):** `sl-julik-frontend-races-reviewer` (Stimulus/Turbo, DOM events, async UI) and `sl-swift-ios-reviewer` (Swift/SwiftUI/UIKit, entitlements, Core Data, `.pbxproj`).

**CE conditional (migration-specific):** `sl-deployment-verification-agent` — deployment checklist + rollback when the migration gate applies and the change is risky.
...
The following paths are super-looper pipeline artifacts and must never be flagged for deletion, removal, or gitignore by any reviewer:

- `docs/brainstorms/*` -- requirements documents created by sl-brainstorm
- `docs/plans/*.md` -- plan files created by sl-plan (decision artifacts; execution progress is derived from git, not stored in plan bodies)
...
- sl-agent-native-reviewer (always)
- sl-learnings-researcher (always)
...
- sl-deployment-verification-agent -- destructive migration with backfill
```
...
2. Filter to those whose directory is an ancestor of at least one changed file. A standards file governs all files below it (e.g., `plugins/super-looper/AGENTS.md` applies to everything under `plugins/super-looper/`).
...
Three reviewers inherit the session model with no override: `sl-correctness-reviewer`, `sl-security-reviewer`, and `sl-adversarial-reviewer`. These perform the highest-stakes analysis — logic bugs, security vulnerabilities, adversarial failure scenarios — and should run at whatever capability level the user has configured. If the user is on Opus, these get Opus.
...
mkdir -p "/tmp/super-looper/sl-code-review/$RUN_ID"
```

Pass `{run_id}` to every persona sub-agent so they can write their full analysis to `/tmp/super-looper/sl-code-review/{run_id}/{reviewer_name}.json`.
...
**Model override at dispatch time.** Pass the mid-tier model on every dispatch except `sl-correctness-reviewer`, `sl-security-reviewer`, and `sl-adversarial-reviewer`, which inherit the session model (per the Model tiering subsection above). Add `model: "sonnet"` to the `Agent` tool call. Check this on every `Agent` call in the parallel dispatch; omitting it on Opus sessions silently 3-4x's the cost of a review.
...
Each persona sub-agent writes full JSON (all schema fields) to `/tmp/super-looper/sl-code-review/{run_id}/{reviewer_name}.json` and returns compact JSON with merge-tier fields only:
...
3. **Spawn validators with bounded parallelism.** One sub-agent per finding, dispatched independently using the validator template and the same bounded scheduler from Stage 4. Each validator receives:
   ...
   - `why_it_matters` when available — loaded from the per-agent artifact file at `/tmp/super-looper/sl-code-review/{run_id}/{reviewer_name}.json`; omit when the file is absent or the artifact write failed. The validator proceeds without it, using the diff and cited code directly.
   - The full diff
...
Emit **one raw JSON object** as the primary response — a single bare JSON value, **no markdown code fence**. A leading ```` ```json ```` fence makes the response start with backticks and breaks naive `JSON.parse` consumers, so never wrap it. Also write `review.json` under `/tmp/super-looper/sl-code-review/<run-id>/` with the same payload.
...
```json
{
  ...
  "testing_gaps": [],
  "coverage": {},
  "artifact_path": "/tmp/super-looper/sl-code-review/<run-id>/",
  "run_id": "<run-id>"
}
```
...
Stack-specific reviewers fire only when the diff touches runtime behavior they specialize in (async UI races, iOS/Swift lifecycle) — never mechanically from file extensions alone; the trigger is meaningful changed behavior in that stack's runtime domain. Structural quality (complexity deletion, 1k-line regressions, type-boundary leaks) lives in the always-on `sl-maintainability-reviewer`; do not spawn extra reviewers for language conventions, philosophy, or "strict bar" passes.

## After Review

After Stage 6, stop. Never push, open PRs, or file tickets from this skill. In default (interactive) mode, Stage 5c has already applied and (on a clean pre-review tree) committed the safe fixes; in `mode:agent` the review mutates nothing — the caller (for example `sl-work`) and the user apply fixes, file tickets, or accept residual risk using the report and artifact.
...
- Include the run-artifact path when one was written: `/tmp/super-looper/sl-code-review/<run-id>/`
...
Always write run artifacts under `/tmp/super-looper/sl-code-review/<run-id>/`:
