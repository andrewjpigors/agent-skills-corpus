---
name: clawbot-agent
description: "Configure, extend, or contribute to Clawbot Agent."
version: 2.1.0
...
**Docs:** https://clawbot-agent.aayushsoam.com/docs/
...
clawbot skills install ID    Install a skill (ID can be a hub identifier OR a direct https://…/SKILL.md URL; pass --name to override when frontmatter has no name)
...
Platform docs: https://clawbot-agent.aayushsoam.com/docs/user-guide/messaging/
...
authoritative list or see the [live slash commands reference](https://clawbot-agent.aayushsoam.com/docs/reference/slash-commands).
The registry of record is `clawbot_cli/commands.py` — every consumer
...
/snapshot [sub]      Create or restore state snapshots of Clawbot config/state (CLI)
/background <prompt> Run prompt in background
/queue <prompt>      Queue for next turn
/steer <prompt>      Inject a message after the next tool call without interrupting
/agents (/tasks)     Show active agents and running tasks
/resume [name]       Resume a named session
/goal [text|sub]     Set a standing goal Clawbot works on across turns until achieved
...
/busy [sub]          Control what Enter does while Clawbot is working (CLI)
...
/reload-skills       Re-scan ~/.clawbot/skills/ for added/removed skills
...
Full config reference: https://clawbot-agent.aayushsoam.com/docs/user-guide/configuration
...
Full provider docs: https://clawbot-agent.aayushsoam.com/docs/integrations/providers
...
Full enumeration lives in `toolsets.py` as the `TOOLSETS` dict; `_CLAWBOT_CORE_TOOLS` is the default bundle most platforms inherit from.
...
Common "why is Clawbot doing X to my output / tool calls / commands?" toggles — and the exact commands to change them. Most of these need a fresh session (`/reset` in chat, or start a new `clawbot` invocation) because they're read once at startup.

### Secret redaction in tool output

Secret redaction is **off by default** — tool output (terminal stdout, `read_file`, web content, subagent summaries, etc.) passes through unmodified. If the user wants Clawbot to auto-mask strings that look like API keys, tokens, and secrets before they enter the conversation context and logs:

```bash
clawbot config set security.redact_secrets true       # enable globally
```

**Restart required.** `security.redact_secrets` is snapshotted at import time — toggling it mid-session (e.g. via `export CLAWBOT_REDACT_SECRETS=true` from a tool call) will NOT take effect for the running process. Tell the user to run `clawbot config set security.redact_secrets true` in a terminal, then start a new session. This is deliberate — it prevents an LLM from flipping the toggle on itself mid-task.

Disable again with:
```bash
clawbot config set security.redact_secrets false
```
...
```bash
clawbot config set privacy.redact_pii true    # enable
clawbot config set privacy.redact_pii false   # disable (default)
```

### Command approval prompts

By default (`approvals.mode: manual`), Clawbot prompts the user before running shell commands flagged as destructive (`rm -rf`, `git reset --hard`, etc.). The modes are:
...
```bash
clawbot config set approvals.mode smart       # recommended middle ground
clawbot config set approvals.mode off         # bypass everything (not recommended)
```

Per-invocation bypass without changing config:
- `clawbot --yolo …`
- `export CLAWBOT_YOLO_MODE=1`

Note: YOLO / `approvals.mode: off` does NOT turn off secret redaction. They are independent.

### Shell hooks allowlist

Some shell-hook integrations require explicit allowlisting before they fire. Managed via `~/.clawbot/shell-hooks-allowlist.json` — prompted interactively the first time a hook wants to run.

### Disabling the web/browser/image-gen tools

To keep the model away from network or media tools entirely, open `clawbot tools` and toggle per-platform. Takes effect on next session (`/reset`). See the Tools & Skills section above.
...
the `cronjob` tool, the `clawbot cron` CLI (`list`, `add`, `edit`,
...
User docs: https://clawbot-agent.aayushsoam.com/docs/user-guide/features/cron
...
- **CLI:** `clawbot curator <verb>` — `status`, `run`, `pause`, `resume`,
...
- **Telemetry:** sidecar at `~/.clawbot/skills/.usage.json` holds
  per-skill `use_count`, `view_count`, `patch_count`,
  `last_activity_at`, `state`, `pinned`.

Config: `curator.*` (`enabled`, `interval_hours`, `min_idle_hours`,
`stale_after_days`, `archive_after_days`, `backup.*`).
User docs: https://clawbot-agent.aayushsoam.com/docs/user-guide/features/curator

### Kanban (multi-agent work queue)

Durable SQLite board for multi-profile / multi-worker collaboration.
Users drive it via `clawbot kanban <verb>`; dispatcher-spawned workers
see a focused `kanban_*` toolset gated by `CLAWBOT_KANBAN_TASK` so the
...
- **Isolation:** board is the hard boundary (workers get
  `CLAWBOT_KANBAN_BOARD` pinned in env); tenant is a soft namespace
  within a board for workspace-path + memory-key isolation.

User docs: https://clawbot-agent.aayushsoam.com/docs/user-guide/features/kanban

---

## Windows-Specific Quirks

Clawbot runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
without BOM. `clawbot config edit` writes without BOM; manual edits in
...
Clawbot's own env scrubber dropping `SYSTEMROOT` / `WINDIR` / `COMSPEC`
...
POSIX venv layouts (`.venv/bin/activate`). The Clawbot-installed venv at
...
every Clawbot tool and most Windows APIs. Prefer forward slashes in code
...
|----------------|----------|
| Config options | `clawbot config edit` or [Configuration docs](https://clawbot-agent.aayushsoam.com/docs/user-guide/configuration) |
| Available tools | `clawbot tools list` or [Tools reference](https://clawbot-agent.aayushsoam.com/docs/reference/tools-reference) |
| Slash commands | `/help` in session or [Slash commands reference](https://clawbot-agent.aayushsoam.com/docs/reference/slash-commands) |
| Skills catalog | `clawbot skills browse` or [Skills catalog](https://clawbot-agent.aayushsoam.com/docs/reference/skills-catalog) |
| Provider setup | `clawbot model` or [Providers guide](https://clawbot-agent.aayushsoam.com/docs/integrations/providers) |
| Platform setup | `clawbot gateway setup` or [Messaging docs](https://clawbot-agent.aayushsoam.com/docs/user-guide/messaging/) |
| MCP servers | `clawbot mcp list` or [MCP guide](https://clawbot-agent.aayushsoam.com/docs/user-guide/features/mcp) |
| Profiles | `clawbot profile list` or [Profiles docs](https://clawbot-agent.aayushsoam.com/docs/user-guide/profiles) |
| Cron jobs | `clawbot cron list` or [Cron docs](https://clawbot-agent.aayushsoam.com/docs/user-guide/features/cron) |
| Memory | `clawbot memory status` or [Memory docs](https://clawbot-agent.aayushsoam.com/docs/user-guide/features/memory) |
| Env variables | `clawbot config env-path` or [Env vars reference](https://clawbot-agent.aayushsoam.com/docs/reference/environment-variables) |
| CLI commands | `clawbot --help` or [CLI reference](https://clawbot-agent.aayushsoam.com/docs/reference/cli-commands) |
...
For occasional contributors and PR authors. Full developer docs: https://clawbot-agent.aayushsoam.com/docs/developer-guide/
...
**Windows contributors:** `scripts/run_tests.sh` currently looks for POSIX venvs (`.venv/bin/activate` / `venv/bin/activate`) and will error out on Windows where the layout is `venv/Scripts/activate` + `python.exe`. The Clawbot-installed venv at `venv/Scripts/` also has no `pip` or `pytest` — it's stripped for end-user install size. Workaround: install pytest + pytest-xdist + pyyaml into a system Python 3.11 user site (`/c/Program Files/Python311/python -m pip install --user pytest pytest-xdist pyyaml`), then run tests directly:
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/clawbot_cli/test_auth_toctou_file_modes.py`)
- `signal.SIGALRM` → Unix-only (see `tests/conftest.py::_enforce_test_timeout`)
