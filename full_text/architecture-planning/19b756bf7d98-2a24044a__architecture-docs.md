...
##### References file (`docs/11-references.md`) propagation

`docs/11-references.md` is a cross-cutting file that aggregates links from multiple sections. It MUST be included in the downstream scan when **any** of these change:
...
After ADR file creation (or skip), **always update `docs/11-references.md`**:
...
## Workflow 11: Layout Migration Gate (Legacy → Current)
...
- `docs/10-references.md` exists, AND
- `docs/10-risks-and-technical-debt.md` does NOT exist

If both true → legacy layout, run the steps below. Otherwise → current layout, exit with `ℹ️ Layout already current — nothing to migrate.`

**Steps**:

1. **Confirm with user** before any rename:
   ```
...
2. **Rename References file**: `docs/10-references.md` → `docs/11-references.md`.
   - Under git: `git mv docs/10-references.md docs/11-references.md`
   - Otherwise: `mv docs/10-references.md docs/11-references.md`

3. **Create Risks & Technical Debt file**: write `docs/10-risks-and-technical-debt.md` from the Section 13 template in `ARCHITECTURE_DOCUMENTATION_GUIDE.md` (empty 13.1 Risks and 13.2 Technical Debt tables, plus the conventions block).

4. **Rewrite internal cross-references**: locate every link to `docs/10-references.md` across `docs/`, `docs/components/`, `adr/`, and `ARCHITECTURE.md`, then rewrite to `docs/11-references.md`.
   ```bash
   grep -rl "docs/10-references" docs/ adr/ ARCHITECTURE.md 2>/dev/null \
     | xargs -r sed -i 's|docs/10-references|docs/11-references|g'
   ```

5. **Update the navigation table in `ARCHITECTURE.md`**: locate the Documentation table (rows of the form `| NN | <Section Name> | [docs/...](docs/...) | ... |`) and:
   - Update the References row to point at `docs/11-references.md`
   - Insert a new row for `| 13 | Risks and Technical Debt | [docs/10-risks-and-technical-debt.md](docs/10-risks-and-technical-debt.md) | ... |` if S13 is not yet listed
...
7. **Resume the original workflow** (the one whose preflight detected the legacy layout). The user does NOT re-issue the original command — Workflow 11 hands control back automatically.
