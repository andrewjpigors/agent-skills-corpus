...
# Honcho Memory for Clawbot

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Clawbot profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/clawbot#running-honcho-locally-with-clawbot

### Verify

```bash
clawbot honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Clawbot profile's AI peer
...
Honcho models conversations as interactions between **peers**. Clawbot creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Clawbot instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Clawbot run |
| `global` | Single session across all directories |

Manual override: `clawbot honcho map my-project-name`
...
Each Clawbot profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
clawbot profile create coder --clone
# creates host block clawbot.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `clawbot.coder` host block in `honcho.json`
...
```bash
clawbot honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Clawbot when Honcho memory is active.
...
Config file: `$CLAWBOT_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `clawbot honcho setup`. Ensure `memory.provider: honcho` is in `~/.clawbot/config.yaml`.

### Memory not persisting across sessions
Check `clawbot honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `clawbot profile create <name> --clone`. For existing profiles: `clawbot honcho sync`.
...
| Command | Description |
|---------|-------------|
| `clawbot honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `clawbot honcho status` | Show resolved config, connection test, peer info for active profile |
| `clawbot honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `clawbot honcho disable` | Disable Honcho for the active profile |
| `clawbot honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `clawbot honcho peers` | Show peer identities across all profiles |
| `clawbot honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `clawbot honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `clawbot honcho sessions` | List known directory-to-session-name mappings |
| `clawbot honcho map <name>` | Map current working directory to a Honcho session name |
| `clawbot honcho identity` | Seed AI peer identity or show both peer representations |
| `clawbot honcho sync` | Create host blocks for all Clawbot profiles that don't have one yet |
| `clawbot honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Clawbot + Honcho |
