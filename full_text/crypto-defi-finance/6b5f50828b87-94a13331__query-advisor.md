---
name: query-advisor
description: Analyze a natural-language question against the scraped Discourse corpus and recommend which tool to run (`stats` vs `query`), which retrieval mode (`local`/`global`/`hybrid`/`mix`/`naive`), and tuned knobs (`TOP_K`, `CHUNK_TOP_K`, env vars). Use when the user asks a question they want to answer from the forum data and you want the right tool + config before spending tokens. Invocation shape: **the question is the load-bearing argument; the data dir is optional** (resolved from `DISCOURSE_DATA_DIR` or a discovered `./data/*/` candidate if omitted). Triggers: "how should I ask", "which mode for", "advise on this question", "help me ask", "plan a query", "best way to query", "stats or graph for". Also invoke when the user proposes a natural-language question and you can tell it'd hedge badly on the wrong tool (e.g. "top-10 pain points") — route them to the better combination.
---
...
2. **Resolve the data dir** → CLI arg ≻ `DISCOURSE_DATA_DIR` ≻ `AskUserQuestion`.
...
8. **Ask via `AskUserQuestion`** whether to run, split ambiguous concepts, + where to persist. This MUST happen in the main conversation (subagents can't prompt).
9. **Execute** — for any non-trivial chain, delegate to a Sonnet subagent with a self-contained prompt (see "Two-tier delegation"). Skip delegation for pure-stats or show-only runs.
...
3. **Resolve data dir when not given:**
   ...
   - If bootstrap fails, scan `ls -d ./data/*/` and present via `AskUserQuestion` with "Other" escape.
...
If `<data-dir>/QUERY-GUIDE.md` does not exist and the question is *not* a pure-`stats` question, stop before routing. Via `AskUserQuestion` offer:
...
Many questions look one way at first but have two+ defensible interpretations. When the tool or mode choice genuinely depends on user intent, stop and ask via `AskUserQuestion` rather than guess. One unnecessary clarifying question is cheap; running the wrong tool and hedging for 30 seconds is not.
...
Present 2–3 interpretations, each with its own routed command + one-line rationale. Let the user pick via `AskUserQuestion`. Example:
...
Reasoning and disambiguation in this skill (matching §7 bad-shapes, judging monolith vs split, catching coverage gaps, spotting citation-number hallucinations) is Opus-tier work. Running the commands, parsing their output, enriching references via SQL, formatting the answer, and persisting are structured enough that **Sonnet 4.6 in a subagent handles them reliably for significantly less output cost** — and keeps the massive query/log output out of the main conversation's context. Split the work:

**Main conversation (Opus) — routing + disambiguation**
...
4. Use `AskUserQuestion` to resolve sub-concept splits, time scopes, persistence destination. **`AskUserQuestion` does NOT work in subagents** — if you delegate before disambiguating, the subagent has to guess, which defeats the skill's purpose.
5. Delegate execution to a Sonnet subagent via the `Agent` tool (`subagent_type: "general-purpose"`, `model: "sonnet"`).

**Subagent (Sonnet) — execution + format + persist**
...
Fit the skeleton to the chain's actual shape: for a single-query run, the schema/table collapse to whatever structure the answer has; for `stats + query`, include the stats stage rows. **The skeleton is a template, not a literal prompt** — always fill in the resolved commands and user choices before calling `Agent`.
...
5. **Offer to run + pick answer destination** via `AskUserQuestion`. Also use this to resolve any still-ambiguous interpretations (sub-concept split, time scope) — the decisions gated here unblock the next step.
6. **Execute.** For non-trivial chains, delegate to a Sonnet subagent per "Two-tier delegation". The subagent persists and returns a condensed echo. Pass it through to the user, then add the footer (path + line count).
...
1. Run the query chain (directly, or via delegated Sonnet subagent per "Two-tier delegation").
...
**After presenting the recommendation, always ask via `AskUserQuestion`** with these defaults:
...
- **`topic_tags` column is `tag_name`**, not `tag`. Using `WHERE tag = '...'` raises a binder error.
- **Some corpora store punctuation in tags as Unicode lookalikes rather than ASCII.** Example: a forum whose admin created tags named with a `U+2024` one-dot leader (`2024․06`) instead of ASCII `.` will silently return 0 rows for `WHERE tag_name = '2024.06'`. Fix: check `stats tags` output for the real characters, copy them verbatim, or use a `LIKE 'prefix%suffix'` pattern. Not universal — only bites on corpora where the tag admin used non-ASCII punctuation. Always verify tag names first.
...
Recommended:
    ...
    # Then count:
    uv run discourse-explorer stats --path <data-dir> sql \
      "SELECT COUNT(*) FROM topic_tags WHERE tag_name = '<exact tag from step 1>'"

Rationale: graph can't count; DuckDB can. Two gotchas: the column is `tag_name`
(not `tag`); and on some corpora the tag admin uses Unicode lookalike punctuation
(e.g. U+2024 one-dot-leader `․` instead of ASCII `.`), so copy the tag verbatim
from the `stats tags` output. See "SQL invariants".
```
