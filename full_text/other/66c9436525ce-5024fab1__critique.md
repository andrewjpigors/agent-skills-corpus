---
name: critique
description: Use to evaluate the design quality of an interface. Produces a scored UX critique covering Nielsen's 10 heuristics, cognitive load, AI-slop anti-patterns, persona red flags (power-user, first-timer, accessibility-dependent, stress-tester, mobile), priority issues with P0–P3 severity, and concrete next-step recommendations.
---

# Critique

Source: adapted from [pbakaus/impeccable](https://github.com/pbakaus/impeccable) — Apache 2.0.

### Purpose

Resolve one stable target, run two independent assessments, synthesize a design critique, and ask the user what to improve next. The chat response is the primary deliverable.
...
1. **Resolve the target** to a concrete file path or URL. Prefer a source path over a dev-server URL when both identify the same surface; ports drift, paths do not.
   - "the homepage" → `index.html` or the route's source file
...
Evaluate:
- **AI slop**: Would someone believe "AI made this" immediately? Check the anti-pattern list in the *Anti-Patterns Verdict* section below.
...
Run any available CLI detector / linter and capture browser-rendered evidence (screenshots, console errors, computed styles). Assessment B is mandatory and must remain isolated from Assessment A until both are complete.

If the project has no detector, fall back to manual visual review at multiple viewports plus accessibility-tree inspection in DevTools.

Return: deterministic findings, browser console findings if applicable, false positives, and skipped/failed browser steps with concrete reasons.

### Generate Combined Critique Report

Synthesize both assessments into a single report. Do NOT simply concatenate. Weave the findings together, noting where the LLM review and detector agree, where the detector caught issues the LLM missed, and where detector findings are false positives.

The chat response is the primary user-facing deliverable. Present the full structured critique below in chat; do not replace it with a summary.
...
Common AI-slop tells to scan for:
- **Side-stripe borders** (`border-left`/`border-right` > 1px as a colored accent on cards/alerts)
...
- **Numbered section markers as default** (01 / 02 / 03 above every section)
- **Cream/sand/beige body backgrounds** (warm-neutral AI default)
- **Cyan/purple gradients**, **neon-on-dark**, **scroll-fade-rise on every section**
...
- **Suggested skill**: Which related skill could address this (e.g. `polish`, `bolder`, `quieter`, `distill`, `delight`, or whatever the project uses for layout / typography / accessibility / motion / copy fixes)

#### Persona Red Flags
> *Consult the [Personas reference](#persona-based-design-testing) below.*

Auto-select 2-3 personas most relevant to this interface type (use the selection table in the reference). If `CLAUDE.md` contains a `## Design Context` section, also generate 1-2 project-specific personas from the audience/brand info.
...
**After presenting findings**, use targeted questions based on what was actually found. STOP and ask to clarify. These answers will shape the action plan.
...
List recommended skills/steps in priority order, based on the user's answers:

1. **`skill-name`**: Brief description of what to fix (specific context from critique findings)
2. **`skill-name`**: Brief description (specific context)

**Rules for recommendations**:
- Order by the user's stated priorities first, then by impact
- Each item's description should carry enough context that the executor knows what to focus on
- Map each Priority Issue to the appropriate skill (e.g. visual loudness → `quieter`, blandness → `bolder`, clutter → `distill`, micro-interaction gap → `delight`)
...
If `CLAUDE.md` contains a `## Design Context` section, derive 1–2 additional personas from the audience and brand information:
