...
echo '{"skill":"design-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.mzstack/analytics/skill-usage.jsonl 2>/dev/null || true
...
```bash
REPORT_DIR=".mzstack/design-reports"
...
**Local:** `.mzstack/design-reports/design-audit-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:**
```bash
eval "$(~/.claude/skills/mzstack/bin/mzstack-slug 2>/dev/null)" && mkdir -p ~/.mzstack/projects/$SLUG
```
Write to: `~/.mzstack/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md`
...
Source: [OpenAI "Designing Delightful Frontends with GPT-5.4"](https://developers.openai.com/blog/designing-delightful-frontends-with-gpt-5-4) (Mar 2026) + mzstack design methodology.
...
**Log the result:**
```bash
~/.claude/skills/mzstack/bin/mzstack-review-log '{"skill":"design-outside-voices","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
