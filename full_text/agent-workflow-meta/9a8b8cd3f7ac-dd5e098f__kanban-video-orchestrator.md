...
metadata:
  nastech:
    ...
    credits: |
      ...
      https://github.com/NasTech-AI/kanban-video-pipeline.
---
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `nastech kanban create` call to a specialist profile. The
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `~/.nastech/.env` or the user's secret store.
