...
You have access to the `generate_image` tool. Do NOT instruct the user to generate images or paste external links. Use the `generate_image` tool to create each section layout image sequentially, and save them with descriptive lower_snake_case filenames (e.g., `landing_hero_section.png`).
