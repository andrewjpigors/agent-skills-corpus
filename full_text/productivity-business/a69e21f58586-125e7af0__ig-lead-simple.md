...
name: ig-lead-simple
description: >
  IG Lead Magnet Simple — v1 simplified loop. Asset creation → publish → comment-keyword trigger → DM Google Drive URL directly.
  Differs from v2: no Intercom, no email capture, no email sequence, no nurture; DM hands the link straight to the user. Carousel images use VPick `gpt-image-2`.
  Designed for "fast single-shot run, single-product PRD-driven" scenarios.
  Triggers on phrases like "ig-lead-simple", "simple IG lead magnet", "no email", "DM the drive link", "fast one-shot run", "follow my product PRD".
metadata:
  methodology: "Alex Hormozi — $100M Leads, Lead Magnets + Content (simple track)"
  integrations: ["boring", "vpick"]
  sister-skills: ["ig-leadmagnet-machine", "lead-magnet-pdf-creator", "copywriting-bible/*"]
  required_plugins:
    - "100m-leads"
    - "copywriting-bible"
    - "marketing-agent"   # provides lead-magnet-pdf-creator
  required_mcp:
    - name: "boring"
      usage: "IG publish + comment auto-reply + DM"
      note: "Self-hosted Boring instance. URL goes in your local config."
    - name: "vpick"
      tools: ["run_image_generator"]
      usage: "Carousel image generation (model: gpt-image-2)"
      note: "Self-hosted VPick instance. URL goes in your local config."
    - name: "obsidian-mcp"
      tools: ["list_files", "read_file", "create_file", "update_file", "search_files"]
      usage: "Read past campaign records + write this run's record (cross-machine portable, no local path dependency)"
      note: "Use any Obsidian MCP server (e.g. obsidian-rest, mcp-obsidian). See README."
  not_required:
    - "intercom"      # this version does not capture email
    - "nano-banana"   # gpt-image-2 replaces it
---
