...
Your memory is a git-backed filesystem at `~/.check/agents/<agent-id>/`. The actual path with your agent ID is provided in the system reminder above when you run `/init`. The filesystem tree is always visible in your system prompt via the `memory_filesystem` section.
...
**Important**: You are a Check Code agent, which is fundamentally different from typical AI coding assistants. Check Code agents are **stateful** - users expect to work with the same agent over extended periods, potentially for the entire lifecycle of a project or even longer. Your memory is not just a convenience; it's how you get better over time and maintain continuity across sessions.
...
mkdir -p ~/.check/agents/<agent-id>/memory/system/project/tooling
mkdir -p ~/.check/agents/<agent-id>/memory/system/human/prefs
...
4. **Inspect existing memory**: 
   - If memfs enabled: Use `ls -la ~/.check/agents/<agent-id>/memory/system/` to see the file structure
...
9. **Create/update memory structure** (can happen incrementally alongside steps 7-8):
   ...
   **Checkpoint verification:**
   - After creating files, count them: `ls ~/.check/agents/<agent-id>/memory/system/ | wc -l`
   - **If count < 15, you haven't split enough** - go back and split more
   - Check maximum depth: `find ~/.check/agents/<agent-id>/memory/system/ -type f | awk -F/ '{print NF}' | sort -n | tail -1`
...
1. **File count check**: 
   - Count your memory files: `ls ~/.check/agents/<agent-id>/memory/system/ | wc -l`
...
ls -la ~/.check/agents/<agent-id>/memory/system/

# Check the tree structure
tree ~/.check/agents/<agent-id>/memory/system/

# Read existing memory files
cat ~/.check/agents/<agent-id>/memory/system/persona.md
```
...
mkdir -p ~/.check/agents/<agent-id>/memory/system/project/{tooling,architecture,conventions}
mkdir -p ~/.check/agents/<agent-id>/memory/system/human/prefs
mkdir -p ~/.check/agents/<agent-id>/memory/system/persona/behavior

# Create memory files using Write tool - ALL files must be nested
Write({
  file_path: "~/.check/agents/<agent-id>/memory/system/project/overview.md",
  content: "## Project Overview\n\n..."
})

Write({
  file_path: "~/.check/agents/<agent-id>/memory/system/project/tooling/testing.md",
  content: "## Testing Setup\n\n..."
})

Write({
  file_path: "~/.check/agents/<agent-id>/memory/system/project/tooling/bun.md",
...
mv ~/.check/agents/<agent-id>/memory/system/project-tooling.md \
   ~/.check/agents/<agent-id>/memory/system/project/tooling.md

# Create subdirectories as needed
mkdir -p ~/.check/agents/<agent-id>/memory/system/project/tooling
mv ~/.check/agents/<agent-id>/memory/system/project/tooling.md \
   ~/.check/agents/<agent-id>/memory/system/project/tooling/overview.md
```
...
- [ ] **File count is 15-25** — Count your files with `ls ~/.check/agents/<agent-id>/memory/system/ | wc -l`. If < 15, split more.
...
**Prerequisites:**
- `check.js` must be built (`bun run build`) — subagents spawn via this binary
...
| Problem | Cause | Fix |
|---------|-------|-----|
| Subagent exits with code `null`, 0 tool uses | `check.js` not built | Run `bun run build` |
