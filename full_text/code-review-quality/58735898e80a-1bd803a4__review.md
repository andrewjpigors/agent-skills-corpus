...
- **PR number or same-repo URL** (e.g., `123` or a URL whose owner/repo matches the current repo — cross-repo URLs are handled by the lightweight mode above):
  - **Create an ephemeral worktree** to avoid modifying the user's working tree. This eliminates all stash/checkout/restore complexity:
    1. **Clean up stale worktree** from a previously interrupted review (if any): if `.airiscode/tmp/review-pr-<number>` exists, remove it with `git worktree remove .airiscode/tmp/review-pr-<number> --force` and delete the stale ref `git branch -D qwen-review/pr-<number> 2>/dev/null || true`. This ensures a fresh start.
    ...
    3. **Incremental review check** (run BEFORE creating worktree to avoid wasting time): If `.airiscode/review-cache/pr-<number>.json` exists, read the cached `lastCommitSha` and `lastModelId`. Get the fetched HEAD SHA via `git rev-parse qwen-review/pr-<number>` and the current model ID (`{{model}}`). Then:
    ...
    5. Create a temporary worktree: `git worktree add .airiscode/tmp/review-pr-<number> qwen-review/pr-<number>`. If this fails, inform the user and stop.
...
1. `.airiscode/review-rules.md` (AIRIS Code native)
...
**Before posting**, check for existing AIRIS Code review comments: `gh api repos/{owner}/{repo}/pulls/{pr_number}/comments --jq '.[] | select(.body | test("via AIRIS Code /review")) | .id'`. If found, inform the user and let them decide whether to proceed.
  ...
  "comments": [
    {
      "path": "src/file.ts",
      "line": 42,
      "body": "**[Critical]** issue description\n\n```suggestion\nfix code\n```\n\n_— YOUR_MODEL_ID via AIRIS Code /review_"
    }
  ]
}
````
...
gh api repos/{owner}/{repo}/pulls/{pr_number}/reviews \
  ...
  -f event="APPROVE" \
  -f body="No issues found. LGTM! ✅ _— YOUR_MODEL_ID via AIRIS Code /review_"
```
...
- Local changes review → `.airiscode/reviews/<YYYY-MM-DD>-<HHMMSS>-local.md`
- PR review → `.airiscode/reviews/<YYYY-MM-DD>-<HHMMSS>-pr-<number>.md`
- File review → `.airiscode/reviews/<YYYY-MM-DD>-<HHMMSS>-<filename>.md`

Include hours/minutes/seconds in the filename to avoid overwriting on same-day re-reviews.

Create the `.airiscode/reviews/` directory if it doesn't exist. **For PR worktree mode, use absolute paths to the main project directory** (not the worktree) — e.g., `mkdir -p /absolute/path/to/project/.airiscode/reviews/`. Relative paths would land inside the worktree and be deleted in Step 11.
...
1. Create `.airiscode/review-cache/` directory if it doesn't exist
2. Write `.airiscode/review-cache/pr-<number>.json` with:
...
3. Ensure `.airiscode/reviews/` and `.airiscode/review-cache/` are ignored by `.gitignore` — a broader rule like `.airiscode/*` also satisfies this. Only warn the user if those paths are not ignored at all.
...
1. `git worktree remove .airiscode/tmp/review-pr-<number> --force`
