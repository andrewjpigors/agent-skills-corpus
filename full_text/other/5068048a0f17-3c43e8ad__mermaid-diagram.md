...
description: 在使用者要用 Mermaid 視覺化流程、時間關係或資訊結構時使用。常見觸發像「畫流程圖」「做時序圖」「把架構整理成 Mermaid」。輸出合適圖型與可貼上的 Mermaid；不直接取代投影片或 UI 設計。
version: 2026.3.31
homepage: https://github.com/AllanYiin/skills/tree/main/skills/mermaid-diagram
license: MIT
metadata: {"author":"Allan Yiin","language":"zh-TW","category":"visualization","short-description":"Mermaid 圖表規劃、選型、中文相容與產出指引","openclaw":{"emoji":"📊"}}
---
...
這個 skill 用來把流程、時間關係、互動時序、資訊結構或系統架構，轉成可貼上即用的 Mermaid 圖表。核心工作不是背語法大全，而是先選對圖表類型，再處理中文、特殊字元、版本相容與平台限制。

## Scope

### In scope
- flowchart、sequenceDiagram、stateDiagram、classDiagram、erDiagram、gantt、mindmap、timeline、quadrantChart 等 Mermaid 圖表規劃與撰寫。
- 中文標籤相容、保留字避雷、逸出字元、換行、平台版本差異與 render troubleshooting。
- 協助使用者從需求判斷應該用哪種 Mermaid 圖表，而不是只機械轉成 flowchart。

### Out of scope
- 不直接取代投影片規劃、UI 視覺設計或一般圖片設計。
- 不保證所有平台都支援最新 Mermaid 類型；遇到平台限制時要明講。
- 若任務更適合用其他產物，例如投影片、前端 UI、正式架構圖工具，不應硬用 Mermaid。

## Primary use cases (2-3)

1) **把流程或架構需求轉成 Mermaid**
- Trigger examples: 「畫流程圖」「做時序圖」「把架構整理成 Mermaid」
- Expected result: 選對圖型，並輸出可直接渲染的 Mermaid 程式碼。

2) **處理中文與特殊字元相容**
- Trigger examples: 「Mermaid 中文會壞掉怎麼辦」「有括號/冒號/保留字怎麼寫」
- Expected result: 提供正確的 quoting、escaping、ID/label 分離與替代寫法。

3) **診斷平台版本或 render 問題**
- Trigger examples: 「為什麼在 Obsidian 不顯示」「GitHub 可以但 VS Code 不行」
- Expected result: 說清楚是語法問題、平台版本限制，還是圖型本身不被支援。

## Workflow overview

1. 先判斷需求類型與最適合的圖表。
2. 檢查目標平台的 Mermaid 版本與支援範圍。
3. 撰寫圖表時優先處理 ID、label、中文、保留字與特殊符號。
4. 若 render 失敗，先縮成最小可用版本再逐步加回內容。
5. 最後交付可貼上的 Mermaid 與必要說明。

## Routing boundaries

- Neighboring skills / workflows:
  - `slide-content-planner` / `pptx-maker`：產物是投影片而不是 Mermaid。
  - `frontend-design`：需求是實際 UI，而不是抽象圖表。
  - `problem-decomposer`：先拆解問題，再決定是否需要視覺化。
- Negative triggers:
  - 「幫我做一份投影片」
  - 「幫我設計網站介面」
  - 「幫我畫一般宣傳海報」
- Handoff rule: 若重點不在 Mermaid 語法與圖表選型，就應轉交更貼近主產物的 skill。

## Language coverage

- Primary language(s): 繁體中文、英文。
- Mixed-language trigger phrases: flowchart、sequence diagram、state machine、ERD、Gantt、mindmap、timeline、render error。
- Locale-specific wording risks: 使用者說「流程圖」有時其實要的是投影片圖示或正式架構圖，必須先分清目標產物。

## Success criteria

### Quantitative (targets)
- 交付內容必須包含正確圖型與可執行 Mermaid 程式碼。
- 含中文或特殊字元時，需明確使用 quoting/escaping 規則。
- 若遇到平台限制，必須標出版本或支援性風險，不得假裝通用。

### Qualitative
- 圖型選擇合理，不濫用 flowchart。
- 說明精準，能幫使用者避開 parser 地雷。
- 交付內容兼顧語法正確性與實務可渲染性。

## Instructions

### Step 0: Select the diagram type first
- 先用需求判斷是流程、時序、狀態、資料模型、時間軸、矩陣還是架構圖。
- 如果需求其實不是 Mermaid 最佳解，直接說明替代方向。

### Step 1: Confirm platform compatibility
- 先確認使用者的目標平台，例如 GitHub、Obsidian、VS Code、Claude Artifacts。
- 若圖表類型需要 v10+ / v11+，要先標明支援條件。

### Step 2: Author the Mermaid safely
- ID 優先用英文或數字，中文放在 label。
- 有特殊字元、括號、冒號、保留字時，優先用雙引號或實體逸出。
- 若 render 失敗，先縮成最小可用圖，再逐步恢復複雜內容。

### Step 3: Deliver with troubleshooting notes
- 最終輸出包含 Mermaid 程式碼、圖型說明與必要的相容性提示。
- 如果有已知平台雷點，直接在結果中標出。
...
### Functional tests
- 交付前要確認圖型是否真的對應需求。
- 中文、括號、冒號、保留字案例要能提供安全寫法。
- 若平台不支援，需直接提示，而不是假設在所有環境可用。
