...
The latency-finder uses a 7-day rolling median p95 as its primary baseline, not just yesterday's data. This prevents false positives when yesterday was an outlier.

Rules:
- **Primary baseline**: 7-day rolling median p95 for each metric
- **Fallback**: yesterday-only when <3 days of history exist
- **Prefer REFUTE** if today's p95 fits within the 7-day historical range, even if it's >2× yesterday's value
- Example: yesterday 12.6ms (outlier), 4-day range 18-28ms, today 23.8ms → REFUTE (today fits the range)

When reporting a regression candidate, always show: today's value, yesterday's value, AND the 7-day range if available.
...
CRITICAL — duration_ms lives on 'tool_result' rows, NOT 'tool_use' rows. The producer (assistant/sdk_session.py _process_message) emits the 'tool_use' event BEFORE the tool runs, so duration_ms is always NULL there; duration_ms, tool_name, and is_error are populated only on the matching 'tool_result' event. ALL duration-based aggregation queries below therefore filter event_type = 'tool_result'. (Query 6, the inter-tool-gap query, intentionally still uses 'tool_use' because it measures spacing between tool INVOCATIONS and is NULL-safe.)
...
- **Silent observer group chats**: Group chat sessions that run in silent observer mode have no `message.sent` events by design — they monitor without responding. Do NOT flag these as "unanswered messages" or latency issues. Identify silent observer sessions by checking for a CLAUDE.md containing "NEVER CALL ANY MESSAGING CLI" or "CRITICAL RULE: NEVER" in the transcript directory.
