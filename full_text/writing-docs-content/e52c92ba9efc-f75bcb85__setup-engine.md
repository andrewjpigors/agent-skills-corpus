...
> Codex port note: This skill was adapted from the original Claude Code Game Studios workflow. In Codex, use it through natural language or by naming the skill; ask before writing project files when the workflow requires user choice.
...
## 4. Update Codex.md Technology Stack
...
Record the choice. It determines the Codex.md template, naming conventions, specialist routing, and which agent is spawned for code files throughout the project.

---

Read `Codex.md` and show the user the proposed Technology Stack changes.
Ask: "May I write these engine settings to `Codex.md`?"
...
After updating Codex.md, create or update `.Codex/docs/technical-preferences.md` with
...
## 8. Update Codex.md Import

Ask: "May I update the `@` import in `Codex.md` to point to the new engine reference?"
...
### A1. Codex.md Technology Stack Templates
