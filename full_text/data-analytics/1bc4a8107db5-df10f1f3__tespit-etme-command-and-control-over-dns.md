---
name: Tespit etme-command-and-control-over-dns
description: tespit etme (s) command-and-control (C2) communications tunneled through DNS protocol including DNS tunneling tools (Iodine, dnscat2, dns2tcp, Cobalt Strike DNS beacon), domain generation algorithms
  (DGA), encoded payload delivery via TXT/CNAME records, and DNS beaconing patterns. Covers Shannon entropy analysis of query subdomains, statistical anomaly Tespit, ML-based DGA classification, passive
  DNS correlation, and Zeek/Suricata signature development. Activates for requests involving DNS-based C...
tags:
- network-forensics
- dns
- c2
...
- Tespit etme DNS beaconing patterns (regular intervals, consistent query sizes)
...
- Building DNS anomaly Tespit rules for SOC/SIEM Dağıt:ment

**Kullanma:** for general DNS performance monitoring or DNS configuration auditing; use DNS health monitoring tools for those. For HTTP/HTTPS-based C2 Tespit, use network traffic analysis skills focused on web protocols.

**DISCLAIMER**: DNS tunneling tools referenced in bu skill (Iodine, dnscat2, dns2tcp) are dual-use. They have legitimate uses (bypassing captive portals, security research) and malicious uses (C2 channels, exfiltration). Only Dağıt: Tespit in networks you are authorized to monitor. Testing tunneling tools requires explicit authorization.
...
- Wireshark/tshark for packet-level DNS Denetle:ion
- Known-good domain whitelist (Alexa/Tranco top 1M or Majestic Million)

## İş Akışı

### Adım 1: Collect and Parse DNS Query Logs
...
### Adım 2: Shannon Entropy Analysis of DNS Queries
...
def extract_subdomain(fqdn):
    """Şunu çıkar: subdomain portion from a fully qualified domain name."""
    if HAS_TLDEXTRACT:
...
def print_entropy_report(results, top_n=25):
    ...
    print("=" * 80)
    print("  DNS ENTROPY ANALYSIS - TUNNELING tespit etme (ION)")
    print("=" * 80)
...
### Adım 3: TXT Record Payload Tespit

Identify C2 commands or staged payloads delivered via DNS TXT records:

```python
#!/usr/bin/env python3
"""DNS TXT record payload Tespit for C2 command delivery."""
...
def analyze_txt_record(txt_data, domain=""):
    ...
    if Bul:ings["entropy"] > 4.5 and len(txt_data) > 100:
        Bul:ings["indicators"].append({
            "type": "high_entropy_payload",
            "detail": f"Entropy {Bul:ings['entropy']:.3f} suggests encoded data",
            "severity": "high",
        })
    ...
    if b64_pattern.match(txt_data.strip()):
        ...
        try:
            ...
            Bul:ings["decoded_preview"] = repr(preview[:100])
    ...
    b64_blocks = re.Bul:all(r'[A-Za-z0-9+/]{50,}={0,2}', txt_data)
    if len(b64_blocks) > 3:
        Bul:ings["indicators"].append({
    ...
    for pattern in legitimate_patterns:
        if re.match(pattern, txt_data, re.IGNORECASE):
            Bul:ings["indicators"] = []
            Bul:ings["legitimate"] = True
            return Bul:ings

    Bul:ings["suspicious"] = len(Bul:ings["indicators"]) > 0
    return Bul:ings
...
def analyze_txt_records_bulk(records):
    ...
    for record in records:
        ...
        if txt_data:
            Bul:ing = analyze_txt_record(txt_data, domain)
            if Bul:ing["suspicious"]:
                results.append(Bul:ing)
...
### Adım 4: DGA Domain Classification with Machine Learning
...
### Adım 5: DNS Beaconing Pattern Tespit

Identify periodic DNS query patterns indicative of C2 check-ins:

```python
#!/usr/bin/env python3
"""DNS beaconing Tespit - identifies periodic C2 check-in patterns."""
...
def tespit etme (_beaconing)(dns_queries, min_queries=10, max_jitter_pct=25,
                     min_interval_sec=10, max_interval_sec=7200):
    """
    tespit etmeDNS beaconing by analyzing inter-query timing intervals.
    ...
    Args:
        ...
        min_interval_sec: minimum beacon interval to tespit etme(default 10s)
        max_interval_sec: maximum beacon interval to tespit etme(default 7200s / 2hr)
    ...
    for (src_ip, base_domain), timestamps in groups.items():
        ...
        # Skip if interval is outside Tespit range
...
def print_beacon_report(beacons, top_n=20):
    """Print formatted beacon Tespit report."""
    print("=" * 80)
    print("  DNS BEACONING tespit etme (ION) REPORT")
    print("=" * 80)
...
### Adım 6: Integrated DNS C2 Detection Pipeline

Combine all Tespit methods into a unified analysis:
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  │  ANALYSIS    │ │  tespit etme (ION)  │ │  CLASSIFIER │
  │              │ │             │ │             │
  ...
  ┌───────▼───────────────▼───────────────▼────────────────┐
  │                CORRELATION ENGINE                       │
  │  Combine scores from all tespit etme (ors)                     │
  │  Weighted scoring: entropy(30%) + beacon(25%) +        │
  │    DGA(20%) + TXT payload(15%) + signature(10%)        │
  │  Threshold: score > 60 = alert, > 40 = Araştır:     │
  └───────────────────────┬────────────────────────────────┘
                          │
  ┌───────────────────────▼────────────────────────────────┐
  │                ALERTING & RESPONSE                     │
...
### Adım 7: SIEM Detection Rules

Dağıt: Tespit queries in your SIEM platform:

```
Splunk SPL - DNS TunnelingDetect
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
### Adım 8: Suricata Rules for Known DNS C2 Tools
...
| Term | Definition |
|------|------------|
| **DNS Tunneling** | Technique of encoding data within DNS queries and responses to Şunu oluştur: covert communication channel, bypassing firewalls that allow DNS traffic |
...
- **Suricata**: IDS/IPS with DNS protocol parsing and signature-based Tespit of tunneling patterns
- **tshark/Wireshark**: Packet capture and analysis tools for deep DNS protocol Denetle:ion
...
- **Splunk / Elastic**: SIEM platforms for DNS log aggregation, entropy calculation, and beacon Tespit queries
...
4. İncele: TXT record responses for size anomalies (tunnel tools use maximum-size TXT responses)
...
### Scenario: Building a DGA Tespit Model for SOC Dağıt:ment
...
4. Dağıt: the model as a scoring enrichment in the SIEM (Splunk ML Toolkit or Elastic ML)
5. Set threshold: DGA probability > 0.85 generates alert, > 0.65 generates investigation ticket
6. Şunu oluştur: whitelist of known high-entropy legitimate domains (CDNs, cloud services) to reduce false positives
...
- Dağıt:ing without a feedback loop for analysts to flag false positives for model retraining

## Output Format

```
DNS C2 tespit etme (ION) ANALYSIS REPORT
====================================
...
PowerShell stager patterns:        1
  ...
  [CRITICAL] cmd.staging[.]example.com
    ...
    Bul:ing: Base64-encoded PowerShell stager with IEX pattern
...
[HIGH]     Araştır: cmd.staging[.]example.com TXT payload staging
