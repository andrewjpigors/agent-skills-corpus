...
if (-not (Test-Path "~/.claude/skills/graphify/scripts/detect.py")) {
    Write-Error "FATAL: skill scripts missing — reinstall graphify skill"
    exit 1
}
```
...
```powershell
python ~/.claude/skills/graphify/scripts/detect.py "INPUT_PATH" > .graphify_detect.json
```
...
python ~/.claude/skills/graphify/scripts/build.py "INPUT_PATH"
```
...
```powershell
python ~/.claude/skills/graphify/scripts/export_html.py
```
...
```powershell
# BFS (default):
python ~/.claude/skills/graphify/scripts/query.py "QUESTION" --budget BUDGET
# DFS:
python ~/.claude/skills/graphify/scripts/query.py "QUESTION" --dfs --budget BUDGET
```

Replace `QUESTION` with the user's actual question and `BUDGET` with the token budget (default `2000`, or whatever `--budget N` specifies). Then answer based on the script output above.
...
```powershell
python ~/.claude/skills/graphify/scripts/path_find.py "NODE_A" "NODE_B"
```
...
```powershell
python ~/.claude/skills/graphify/scripts/explain.py "NODE_NAME"
```
