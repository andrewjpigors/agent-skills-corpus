...
Two complementary features that give your team persistent analytical memory:
- **DR** answers "How did we decide to measure/analyze this?"
- **Wiki** answers "What have we learned from measuring it?"

---

### Analysis Decision Records — `/analysis-dr`

Append-only log of methodology decisions. Prevents AI from re-proposing alternatives your team already evaluated and rejected.
...
| Methodology | Statistical method choices | A/B 테스트는 베이지안, 신뢰도 95% |
...
|---------|-------------|
| `/analysis-dr new` | Write a new DR — AI proposes draft from current context, or enter manually |
| `/analysis-dr scan` | Scan existing analyses → find DR candidates ranked by frequency |
| `/analysis-dr list` | Show all recorded decisions (reads `analyses/decisions/INDEX.md`) |
| `/analysis-dr show 003` | Show DR-003 in full detail |

**ALIVE loop integration:**
- **ASK**: AI checks for Metric/Scope DRs before framing the question
- **INVESTIGATE**: Stats-agent and causal-agent check Methodology DRs before suggesting tests
- **EVOLVE**: AI asks "Should this decision be recorded as a DR?"

**Append-only principle:** Never edit or delete an Accepted DR. If reversed, mark old DR as `Superseded by DR-NNN` and write a new DR. The full audit trail shows how team thinking evolved.

Framework files: `core/decisions/GUIDE.md`, `core/decisions/DR-001.md` (example)
Your team's DRs: `analyses/decisions/DR-NNN.md`

---

### Analysis Wiki — `/analysis-wiki`

AI-maintained knowledge base compiled from all your archived analyses. Based on Andrej Karpathy's LLM Wiki concept: structured entity pages that compound as analyses accumulate.

**Wiki page types:**
- `analyses/wiki/metrics/{metric}.md` — Everything known about a metric: findings, validated/rejected hypotheses, watch points
- `analyses/wiki/patterns/{pattern}.md` — Recurring analytical patterns (Simpson's paradox, day-of-week effects, etc.)
...
| Command | What it does |
|---------|-------------|
| `/analysis-wiki update` | Scan all archives → compile/update wiki pages |
| `/analysis-wiki check` | Health check: contradictions, stale pages, orphan claims, missing pages |
...
| Feature | Use when |
|---------|---------|
| `/analysis-search` | Find a specific past analysis by keyword or tag |
| `/analysis-retro` | Summarize what happened in a time period |
| Dashboard | Explore analysis relationships visually |
| `/analysis-wiki` | Know everything learned about a metric or pattern |

**Getting started:** Archive 3-5 analyses first, then run `/analysis-wiki update` to build the initial wiki.

Framework: `core/wiki/GUIDE.md`, `core/wiki/templates/`
