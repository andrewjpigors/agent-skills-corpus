...
- `gws_bridge.py` refreshes the Monika token and injects it into `gws` via `GOOGLE_WORKSPACE_CLI_TOKEN`
...
```bash
MONIKA_HOME="${MONIKA_HOME:-${HERMES_HOME:-$HOME/.monika}}"
GWORKSPACE_SKILL_DIR="$MONIKA_HOME/skills/productivity/google-workspace"
PYTHON_BIN="${MONIKA_PYTHON:-${HERMES_PYTHON:-python3}}"
if [ -x "$MONIKA_HOME/monika-agent/venv/bin/python" ]; then
  PYTHON_BIN="$MONIKA_HOME/monika-agent/venv/bin/python"
fi
