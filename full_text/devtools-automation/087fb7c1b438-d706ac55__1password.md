...
- `tmux` available for stable authenticated sessions during Clawbot terminal calls (desktop app flow only)
...
### Service Account (recommended for Clawbot)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.clawbot/.env` (the skill will prompt for this on first load).
...
## Clawbot Execution Pattern (desktop app flow)

Clawbot terminal commands are non-interactive by default and can lose auth context between calls.
