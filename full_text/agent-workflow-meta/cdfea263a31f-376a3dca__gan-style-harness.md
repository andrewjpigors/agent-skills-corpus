...
```bash
# Step 1: Plan
copilot-agent --model "Claude Opus 4" "You are a Product Planner. Read PLANNER_PROMPT.md. Expand this brief into a full product spec: 'Build a Kanban board app'. Write spec to spec.md"

# Step 2: Generate (iteration 1)
copilot-agent --model "Claude Opus 4" "You are a Generator. Read spec.md. Implement Sprint 1. Start the dev server on port 3000."

# Step 3: Evaluate (iteration 1)
copilot-agent --model "Claude Opus 4" 
            $tools = ---
name: gan-style-harness
description: "GAN-inspired Generator-Evaluator agent harness for building high-quality applications autonomously. Based on Anthropic's March 2026 harness design paper."
origin: ECC-community
...
# Step 3: Evaluate (iteration 1)
copilot-agent --model "Claude Opus 4" --allowedTools "Read,Bash,mcp__playwright__*" "You are an Evaluator. Read EVALUATOR_PROMPT.md. Test the live app at http://localhost:3000. Score against the rubric. Write feedback to feedback-001.md"

# Step 4: Generate (iteration 2 — reads feedback)
copilot-agent --model "Claude Opus 4" "You are a Generator. Read spec.md and feedback-001.md. Address all issues. Improve the scores."
...
.Groups[1].Value
            $tools = $tools -replace '\bRead\b', 'read'
            $tools = $tools -replace '\bWrite\b', 'edit'
            $tools = $tools -replace '\bEdit\b', 'edit'
            $tools = $tools -replace '\bGrep\b', 'search'
            $tools = $tools -replace '\bGlob\b', 'search'
            $tools = $tools -replace '\bBash\b', 'execute'
            $tools = $tools -replace '\bShell\b', 'execute'
            $tools = $tools -replace '\bWebFetch\b', 'web'
            $tools = $tools -replace '\bTodoWrite\b', 'todo'
            $tools = $tools -replace '\bSubagent\b', 'agent'
            # Deduplicate (e.g., Grep,Glob both become search)
            $toolList = ($tools -split ',') | ForEach-Object { ---
name: gan-style-harness
description: "GAN-inspired Generator-Evaluator agent harness for building high-quality applications autonomously. Based on Anthropic's March 2026 harness design paper."
origin: ECC-community
tools: Read, Write, Edit, Bash, Grep, Glob, Task
...
.Trim() } | Select-Object -Unique
            "--allowedTools `"$($toolList -join ',')`""
         "You are an Evaluator. Read EVALUATOR_PROMPT.md. Test the live app at http://localhost:3000. Score against the rubric. Write feedback to feedback-001.md"
