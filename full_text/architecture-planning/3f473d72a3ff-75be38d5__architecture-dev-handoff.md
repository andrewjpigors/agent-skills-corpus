...
prompt:
  ...
  caller: orchestrator-dev-handoff
```

That's the complete prompt — no `task_type`, no `config_path`, no `agent_version`, no `extra_terms`, no `force`. The `caller` field is a contract anchor (see the explorer's Input Parameters): it does not change the explorer's output, but it gives the haiku a structured signal that a string-scanning orchestrator is reading the result, reducing the chance of prose drift. The explorer (a) reads the component's `<!-- EXPLORER_HEADER -->` block and copies `related_adrs:` into `focus_component.related_adrs`, and (b) runs parallel Grep + targeted Read across the canonical doc surface (`ARCHITECTURE.md`, `docs/**/*.md`, `adr/**/*.md`) for the query terms, emitting line-level matches with headings and 3–5-line excerpts.
...
**Malformed-output detection + retry-once** (v3.14.8+): for each explorer reply, check whether the literal token `EXPLORE_FINDINGS:` appears at the start of any line. If absent (the agent returned synthesis prose), spawn **one** retry of the same component's explorer call with the same `component_file` + `query`, prepending this corrective line to the prompt:
...
The retry is a fresh Task() invocation (no edit-in-flight), capped at 1 per component. If the retry still lacks `EXPLORE_FINDINGS:`, treat that component's call as `FAILED` for the purposes of `explorer_mode` derivation and the rules below.

If any explorer call returns `FAILED`, has empty `files[]`, or fails the malformed-output check above (including after the single retry), log a warning. The context-builder will run in v3.13.0 legacy in-builder grep slicing for those slugs.
