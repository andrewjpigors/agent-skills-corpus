...
app = func.FunctionApp(httpauthlevel=func.AuthLevel.FUNCTION)
...
@app.timertrigger(schedule="0 /5 *", argname="myTimer")
def timer_trigger(myTimer: func.TimerRequest) -> None:
 logging.info("Timer trigger executed")

@app.blobtrigger(argname="myblob", path="samples-workitems/{name}",
 connection="AzureWebJobsStorage")
def blob_trigger(myblob: func.InputStream):
 logging.info(f"Blob trigger: {myblob.name}, Size: {myblob.length} bytes")

@app.queuetrigger(argname="msg", queue_name="myqueue",
 connection="AzureWebJobsStorage")
...
public class ParallelProcessing
{
 ...
 // Python equivalent
 // @app.orchestrationtrigger(contextname="context")
 // def processimagesorchestrator(context: df.DurableOrchestrationContext):
