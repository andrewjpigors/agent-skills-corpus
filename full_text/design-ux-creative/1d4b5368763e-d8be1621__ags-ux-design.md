...
- **Game concept**: Read `design/gdd/game-concept.md` — if missing, warn:
  > "No game concept found. Run `/ags-brainstorm` first to establish the game's
...
Check for `design/art/ags-art-bible.html`. If found, read the visual direction
...
Read `.ags/rules/technical-preferences.md` and extract the `## Input & Platform`
...
>
> (Run `/ags-setup-engine` to save this permanently so you won't be asked again.)
...
Write at least 5 specific, testable criteria that a QA tester can verify without reading any other design document. These become the pass/fail conditions for `/ags-story-done`.
...
`/ux-review` on all completed specs before running `/ags-gate-check pre-production`."
...
1. Read `.ags/project/state.md` — it records the current screen
...
- Run `/ags-gate-check pre-production` once all key screens have approved UX specs
