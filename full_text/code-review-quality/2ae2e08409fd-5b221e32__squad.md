...
`micro` is review-only in V1 (`/squad:implement --micro` degrades to `quick`).
...
9. **Advisory dispatches MUST be parallel.** When you have ≥ 2 advisory agents to dispatch in Phase 5, they MUST be issued as multiple `Task` tool calls **in a single assistant message** so the host (Claude Code, Cursor, etc.) runs them concurrently. Spreading dispatches across multiple turns (one Task per turn, awaiting each) is a hard violation: it linearises a parallelisable workflow and multiplies wall time by N. Wait for all parallel results before proceeding to Phase 6 / Phase 10. Sequential is permitted ONLY for the strict ordering of: Phase 2 planner → Phase 5 advisory → Phase 10 consolidator (each phase blocks on the previous), never within a phase. **N/A when `mode === "micro"`** — micro has no dispatch.
10. **Mode resolution is binding.** `compose_squad_workflow` returns a `mode` field (`micro` / `quick` / `normal` / `deep`) — either the user's flag or the auto-detected value — and a typed `execution_plan` that encodes which phases run or skip for that mode + `run_mode`. **Honour the plan** (see "Execution plan is the source of truth"): run each step whose `run` is true, skip those whose `run` is false. The one mode behaviour the plan does NOT carry: the reject-loop cap (Phase 11) is 3 instead of 2 when `mode === "deep"`. `--deep` overrides auto-detect even for Low-risk diffs (the user explicitly opted in). `--quick` on a high-risk diff (auth / money / migration / High risk) keeps the cap at 2 but force-includes `security` and emits `mode_warning`. `--micro` on a high-risk diff DEGRADES to `quick` with `mode_warning.code === "forced_micro_on_high_risk"` (micro has no dispatch and is unsafe on security-relevant changes). Never silently honour `--quick` or `--micro` on a security-relevant change without those overrides.
...
- `prune_learnings` — lifecycle for `.squad/learnings.jsonl` — archive aged entries and promote recurring ones to team policy
...
- `init_local_config` — scaffold a local `.squad.yaml` config in the workspace
- `read_squad_config` — read the resolved `.squad.yaml` config for the workspace
- `record_run` — append a RunRecord to `.squad/runs.jsonl` (terminal telemetry; single-writer)
- `list_runs` — read aggregated run history from `.squad/runs.jsonl`
...
**Implement-mode execution order.** Phase numbers are stable across files, but `/squad:implement` runs Phase 5 (advisory squad) AFTER Phase 8 (implementation). Advisors score the diff that actually landed, not a plan draft (eliminates plan-version drift). See `modes/implement.md` for the inverted order. Review mode is linear (no implementation step exists).
...
### Mode resolution (`micro` / `quick` / `normal` / `deep`) — both modes

`compose_squad_workflow` returns a `mode` field. Resolution order:

1. **Explicit user flag wins.** `/squad:implement --quick <task>`, `/squad:review --micro [target]`, or `--deep` set `mode` directly. `compose_squad_workflow` accepts the value and emits `mode_source: "user"`.
2. **Auto-detect** when no flag is present (`mode` omitted from the call):
   - `mode = "deep"` if `risk.level == High` OR `work_type == "Security"` OR any of `touches_auth` / `touches_money` / `touches_migration` is true.
   - `mode = "micro"` (v1.7+) if `risk.level == Low` AND `files_count <= MICRO_AUTO_MAX_FILES` (default 2) AND `loc_changed < MICRO_AUTO_MAX_LOC` (default 30) AND none of the high-risk signals fire AND `work_type ∈ {Feature, Bug Fix, Refactor, Performance}`. Security and Business Rule never auto-trip micro — they need multi-agent eyes (security agent / PO). Requires upstream `detect_changed_files` to have produced `total_loc_changed` (auto-enabled when caller omitted `mode`); when stat is unavailable, micro auto-detect is silently skipped and the resolution falls through to quick/normal.
   - `mode = "quick"` if `risk.level == Low` AND `files_count <= QUICK_AUTO_MAX_FILES` (default 8) AND none of the high-risk signals fire AND `work_type != "Security"` AND micro didn't trip.
   - `mode = "normal"` otherwise. Implicit default.
   - Returned as `mode_source: "auto"`.
3. **Safety override on forced `--quick` over high-risk diff.** The cap-to-2 stays, but `security` is force-included as one of the two agents, and `mode_warning.code === "forced_quick_on_high_risk"` is set. Never silently honour `--quick` on a security-relevant change without that warning.
4. **Safety override on forced `--micro` over high-risk diff (v1.7+).** Micro mode skips advisor dispatch entirely; it is unsafe on auth/money/migration/Security. The orchestrator DEGRADES the request to `quick` (so security still gets force-included) and sets `mode_warning.code === "forced_micro_on_high_risk"`. Surface this loudly — the user asked for the fastest path and got a slower-but-safer one.

**Which phases run vs skip is driven by `execution_plan`** (see "Execution plan is the source of truth" below) — do not re-derive the per-mode skip matrix in your head; consult the plan. One behaviour the plan does NOT encode (it carries no Phase 11 step), so it stays here:

- **Phase 11 reject-loop cap — raised from 2 to 3 when `mode === "deep"`. N/A in `micro` (review-only in V1; no implementation step to re-run).**
...
### Execution plan is the source of truth (phase run/skip)

`compose_squad_workflow` returns a typed `execution_plan: ExecutionStep[]` — a deterministic projection of the resolved `mode` + `run_mode` + shaped agent list. It encodes, as data, which mode-dependent phases run or skip this invocation. **Drive phase execution from it; do not re-derive the skip matrix in prose.**

Each step is `{ phase, run, skip_reason?, agents?, model_source, dispatch, gate? }`:

- **Run the phase** when `run: true`. **Skip it** when `run: false` (the `skip_reason` — `"quick"` / `"micro"` / `"review-mode"` — tells you why; surface it if relevant).
- `dispatch` says how Phase 5 runs: `"parallel"` (fan out the `agents` list as concurrent `Task` calls per Inviolable Rule 9), `"inline"` (micro single-pass — no `Task` dispatch), or `"none"` (sequential persona / no advisor fan-out).
- `agents` is the shaped advisory list for Phase 5 (cap-2 in quick, +architect/security in deep, `[]` in micro).
- `gate: "user"` means the phase blocks on user approval (Gate 1, Phase 4, implement mode only).
- `model_source: "frontmatter"` always — each agent's own `model` pin wins in every mode; the skill never overrides a dispatch's model.

The covered mode-dependent phases are 2 (planner), 4 (Gate 1), 5 (advisory), 8 (implementation), 10 (consolidator). Phases not in the plan (detect/classify/score, Gate 2, escalation, Phase 11 reject-loop, wrap) are not mode-gated by the plan and run per their own spine rules. The per-phase bodies below still carry the operational detail (the micro inline investigation steps, dispatch mechanics, gate semantics, telemetry) — the plan tells you WHICH phases fire; those sections tell you HOW each one behaves.
...
Resolve target first (v1.7+ — explicit working_tree support):

- **Empty argument** → run `git status --porcelain` first.
  - If output is non-empty (dirty working tree, staged or unstaged): pass `working_tree: true` to `compose_advisory_bundle`. This emits `git diff HEAD` and covers BOTH staged and unstaged uncommitted changes. **This is the v1.7 default for "review what I'm working on right now."**
  - If output is empty (clean tree): fall back to `base_ref: "HEAD~1"` (review the most recent commit). If even that has no diff (initial commit or no commits at all), halt and ask the user what to review.
- **`--working-tree` / `--dirty` flag** → force `working_tree: true` regardless of git status (no shell call needed). Useful when the user wants to be explicit.
- **`--staged` flag** → pass `staged_only: true` (review only staged changes, ignore unstaged).
- **Branch name** → review `<branch>..HEAD` or `main..<branch>` per user intent (`base_ref: "<branch>"`).
- **PR URL or `#N`** → run the clone-and-fetch recipe in `modes/review.md` "Phase 0 — Resolve PR target" BEFORE calling any MCP tool. NEVER fetch the PR diff via REST API, WebFetch, `curl` on `/diff`, or any HTTP path — every one of those has produced 10+ minute hangs against private repos (auth-walled, Cloudflare-rate-limited). Local git only. On auth failure, halt and surface the setup steps; do NOT fall back to slow paths.
- **File path** → review the working-tree changes under that path (still pass `working_tree: true`; the file list will be filtered by the path).

`working_tree`, `staged_only`, and `base_ref` are mutually exclusive. The MCP tool schema rejects combinations.

Run `compose_advisory_bundle` with `workspace_root`, the resolved diff selector (`working_tree`, `staged_only`, or `base_ref`), `user_prompt = "review the changes in this diff"` (or richer if user gave context), and `plan = ""` (empty — no plan to validate in review).

Surface to the user: file count, work type, risk level, selected agents, and **which diff mode was used** (working_tree vs staged vs branch range). When auto-selected from `git status --porcelain`, also surface: "review of uncommitted changes (X files dirty)" so the user knows you didn't review a committed range.
...
> **In implement mode, this phase runs AFTER Phase 8 implementation, not before Phase 4.** Phase 5 advisors review the diff that the implementer just landed — not a plan draft. In review mode it runs in its declared spine position.
...
### Phase 5 — `mode === "micro"` short-circuit (v1.7+)

When `workflow.mode === "micro"`, **do not call `Task()` at all**. The bundle from `compose_advisory_bundle` already carries `micro_prompt` — a complete, single-pass review prompt with the full diff, criteria, mandatory contextual-investigation steps (v1.7.7+), scorecard template, and verdict rule. Skip the dispatch path and emit the review inline:

1. Read `bundle.micro_prompt`.
2. Treat the string as instructions FOR THE ORCHESTRATOR (you). Execute the prompt's **"Investigação contextual obrigatória"** section first — Read the full files in the diff, Grep the new symbols for consumers, weigh PR title/branch context. **This is not optional.** Without it the review degrades to "syntax inspection on a hunk", which has been the failure mode for micro since v1.7.0 (file:line citations off-by-N, dead-code blind spots, severity miscalibration because consumers were invisible). 2-4 tool calls total, 30-90s extra wall-clock.
3. After the investigation, analyse the diff. Fill in the scorecard with citations to **real file lines** (visible via Read), not hunk offsets. Declare the verdict.
4. Surface the result to the user as your own response, with file:line citations for any findings + a brief note on what the investigation revealed (e.g., "consumer found at handlers/cards.cs:88" or "no consumers — field looks staged for a future PR").
5. Then proceed directly to Phase 10 (which is itself short-circuited when `mode === "micro"` — see below).

Do NOT dispatch `tech-lead-consolidator`. Do NOT call `apply_consolidation_rules`. Do NOT call `read_learnings` (v1 of micro mode is learnings-free; backlog: `--with-learnings` flag in v2). The verdict you declared in your inline response IS the verdict — record it in Phase 10's finalise call and stop.

Inviolable Rule 9 (parallel dispatch) does not apply here; there is no dispatch.

### Model strategy by mode (binding, v1.5+)

This is what `execution_plan`'s `model_source: "frontmatter"` means in practice: each agent declares its preferred model in its own frontmatter (`agents/<name>.md`), and **the frontmatter pin wins in EVERY mode** — micro, quick, normal, and deep alike. **OMIT the `model` parameter on every `Task()` dispatch**, in every mode; the skill never overrides a dispatch's model based on the resolved mode. (Frontmatter pins today: sonnet for everyone except code-explorer (haiku) and implementer (opus); the consolidator, developer, planner, architect, dba, security, qa, product-owner all pin sonnet.)

What `--deep` actually changes (semantic only, never the model — agent frontmatter pin governs):

- Force-includes `architect` + `security` in the advisory squad regardless of file signals.
- Raises the Phase 11 reject-loop cap from 2 to 3.
- Suggests Codex as an optional extra review (Phase 3 / Phase 9), still gated on explicit `--codex` consent.
...
3. Then in **one** assistant message, emit N `Task(subagent_type="<agent-name>", description="<Role> review", prompt=<advisory prompt with learnings injected>)` blocks — one per selected agent. Omit the `model` parameter (per "Model strategy by mode" above / the plan's `model_source: "frontmatter"`).
...
- When `hunks_by_agent` is absent (caller passed `include_hunks: false`, or extraction failed silently), fall back to passing `file_contents` from `slices_by_agent` inline.
...
## Plan / Context
{In implement mode: approved plan AND a one-paragraph summary of what the implementer just landed (Implementation Report Section 2). Advisors score the actual diff, not the plan draft.
 In review mode: "Review of existing changes" + any user-supplied context.}
...
Each agent emits at most:

- **0 Blockers by default.** Emit 1 ONLY when you can paste, INSIDE the finding body itself, a ≤2-line failing test OR attack script that triggers the failure on the current diff. Not a sketch ("you could write a test that..."), not a hypothesis ("if X is also true, then Y"), not a chain that depends on code outside this diff that you have not read — an actual runnable artifact a reader can paste into the repo and watch fail. If you cannot paste the artifact, the finding is Major at best, Suggestion when more honest. "If `AsNoTracking` is not used elsewhere, then mutation persists" is a hypothetical — it depends on code outside the diff and asks the reader to verify the precondition; that is Suggestion, NOT Blocker.
- **2 Major** — each with cited file:line evidence AND either a concrete repro path you walked yourself or a documented incident class. A Major may still depend on context outside the diff (e.g. "this changes a public API contract"), but the impact statement must be specific, not "depending on how this is used, it could break callers".
- **2 Minor + Suggestion combined** — keep these tight; they are advisory only.

If you find more candidates than the cap permits, fold the weakest into the most-related neighbor or drop them entirely. Prioritization is the deliverable; enumeration is not. Padding (findings without concrete evidence, repro path, or known incident class) is treated as low signal, not thoroughness, and degrades the rubric.

### Realism gate (binding, inline — do NOT rely on the role-file copy alone)

Before classifying any finding as Blocker or Major, run it through this gate:

1. **Concrete evidence.** Can you point to a `file:line` in the diff that, by itself, exhibits the problem? If the problem only exists when combined with code you have NOT been shown, downgrade.
2. **Reproducible artifact for Blocker.** Can you paste a ≤2-line failing test or attack script in the finding body that triggers the failure on the diff as-is? If you have to write "assuming the repo also does X" before the artifact runs, it is not a Blocker.
3. **Concrete impact for Major.** Can you describe a specific user-facing failure, data-corruption path, or contract break — not "depending on usage, could lead to issues"? If the impact is conditional on configuration you cannot inspect, it is Minor or Suggestion.
4. **Honesty test.** If a calibrated single-pass review (one expert reading the same diff for 15 minutes) would NOT raise this at the severity you chose, downgrade. Squad-style parallel agents amplify each other's caution; the gate exists to counter that drift.

A finding that fails the gate is not "still useful at the original severity, we'll let the consolidator decide". It is downgraded BY YOU before it leaves your report, or dropped. The consolidator's job is to arbitrate cross-agent conflicts, not to clean up severity inflation upstream.
...
## Phase 10 — TechLead-Consolidator (both modes; run/skip per `execution_plan`)

### Phase 10 — `mode === "micro"` finalise mapping (v1.7+)

When the `execution_plan` marks Phase 10 `run: false` (skip_reason `"micro"`), the orchestrator already emitted the review + scorecard + verdict inline in Phase 5 — there is no consolidation work to do. Skip the consolidation steps below and go straight to the finalise call (the `record_run` block at the end of Phase 10), mapping the inline result onto the record like so:

- `verdict` = the value you declared inline in Phase 5 (`APPROVED` / `CHANGES_REQUIRED` / `REJECTED`).
- `weighted_score` = the Nota Geral (0-10) you assigned in the scorecard, **multiplied by 10** to fit the 0-100 schema field. Round to one decimal.
- `agents` = `[]` (micro has no advisor dispatch).
- All other fields as documented below.

Then continue to Phase 12 (wrap).

### Phase 10 — non-micro path
...
- `arbitration_needed` — deterministic boolean: `true` iff there is something for the consolidator persona to arbitrate/narrate (any blocker, any unjustified major, or any cross-agent forwarded finding); `false` only when none hold.

**Fast-path (skip consolidator persona when nothing to arbitrate).** SKIP the `tech-lead-consolidator` subagent dispatch when:

- `arbitration_needed === false`

This GENERALIZES the previous clean-APPROVED-only fast-path: the persona is now skipped whenever there is nothing to arbitrate, **regardless of whether the verdict is APPROVED or CHANGES_REQUIRED** (e.g. a CHANGES_REQUIRED driven only by Minors/Suggestions with no blocker, no unjustified major, and no forwarded item). ALWAYS run the persona when `arbitration_needed === true` (any blocker → REJECTED, any unjustified major, or any cross-agent forward). When in doubt, the tool errs toward `true` and the persona runs.

When the persona is skipped, surface the tool output (verdict + `rubric.scorecard_text` + summary) directly to the user. The rubric + `scorecard_text` are ALWAYS computed by `apply_consolidation_rules` and MUST be surfaced verbatim to the user even when the persona is skipped. Rationale: when there is nothing to arbitrate, the rubric speaks for itself; this saves one consolidator (sonnet) dispatch (~30s). A user who wants the consolidator's narration + rollback plan on a skipped diff can re-run `/squad:review` (without `--quick`) — that path goes through the persona whenever `arbitration_needed` is true, and re-running on the merged commit forces it.
...
Before dispatching the consolidator (normal / deep only; only when `arbitration_needed === true`, i.e. not on the fast-path), call `read_learnings` once with `workspace_root` and `changed_files: <full diff file list>` (no agent filter — the consolidator needs the full picture across agents). Capture `rendered`.

Then dispatch `tech-lead-consolidator` subagent via `Task(subagent_type="tech-lead-consolidator", description="Consolidate verdict", prompt=<all reports + apply_consolidation_rules output INCLUDING the rubric.scorecard_text + learnings.rendered>)`. The consolidator surfaces the verdict + scorecard + rollback plan / mitigation guidance. The `model` parameter is OMITTED; the consolidator's frontmatter already pins sonnet.

Keep the persona prompt trimmed: pass it the EXISTING reports + the `apply_consolidation_rules` output (verdict + `rubric.scorecard_text`) and have it reference those — it must NOT re-derive the scorecard or re-score dimensions. Its job is arbitration + narration on top of the deterministic rubric, not recomputation.
...
record_run({
  ...
  record: {
    ...
    agents: [
      {
        ...
        started_at: <ISO 8601 timestamp captured the moment THIS agent's Task() was dispatched>,
    ...
    // Capture each agent's dispatch instant (ISO 8601) at the moment you call its
    // `Task()` and carry it into `started_at` above. Together with `batch_duration_ms`
    // it gives each agent a `[started_at, started_at + batch_duration_ms]` window; the
    // aggregator overlaps those windows to infer `dispatch_shape` (parallel vs
    // sequential). Linearized dispatch — non-overlapping windows — surfaces in
    // `/squad:stats` as a Rule 9 (parallel dispatch) violation. Omit / 0 only in the
    // Phase 1 in_flight row, where dispatch timing is not yet known.
