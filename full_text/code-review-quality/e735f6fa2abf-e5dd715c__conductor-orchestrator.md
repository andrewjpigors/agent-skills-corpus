...
### New in v4
12. **Superpowers as PRIMARY dispatch** — `superpowers:writing-plans`, `superpowers:executing-plans`, `superpowers:systematic-debugging` are the default path for new tracks
13. **Legacy agents as FALLBACK** — `loop-planner`, `loop-executor`, `loop-fixer` remain available as fallback if superpowers fail or are unavailable
14. **Unified step naming** — `EXECUTE` is the canonical step name (parallelism is an implementation detail inside EXECUTE, not a separate step)
15. **Atomic metadata writes** — Write to metadata.json.tmp then rename to metadata.json for crash safety
...
**Atomic writes**: Write to metadata.json.tmp then rename to metadata.json for crash safety.
...
```typescript
async function stepExecute(trackId: string, metadata: dict) {
...
## Board of Directors Integration (v4)
...
1. **Update metadata.json** (Write to metadata.json.tmp then rename to metadata.json for crash safety.)
```json
{
  "status": "complete",
...
- `conductor/tracks/[track]/metadata.json` — State updates (Write to metadata.json.tmp then rename to metadata.json for crash safety.)
