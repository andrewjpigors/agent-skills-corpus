...
6. **Spawn continuation agent (NOT resume)** using `./.codex/get-physics-done/templates/continuation-prompt.md` template:
...
```bash
PRE_CHECK=$(/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local pre-commit-check --files "${RECOVERY_FILE}" .gpd/STATE.md 2>&1) || true
echo "$PRE_CHECK"
...
Follow the verification workflow. Read `./.codex/get-physics-done/workflows/verify-phase.md` using the read_file tool.
...
```bash
DEBUGGER_MODEL=$(/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local resolve-model gpd-debugger)
```
...
task(
  ...
  read_file: {VERIFICATION_FILE}, {GAP_CLOSURE_SUMMARY}, {ORIGINAL_SUMMARY}
...
```bash
VERIFIER_MODEL=$(/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local resolve-model gpd-verifier)
```
...
task(
  ...
  readonly=false,
  prompt="First, read ./.codex/agents/gpd-verifier.md for your role and instructions.
...
```bash
CONSISTENCY_MODEL=$(/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local resolve-model gpd-consistency-checker)
```

Spawn the consistency checker in rapid mode:

task(prompt="First, read ./.codex/agents/gpd-consistency-checker.md for your role and instructions.
...
read_file: .gpd/STATE.md, .gpd/state.json
read_file: All SUMMARY.md files from phase {PHASE_NUMBER}
...
task(
  ...
  readonly=false,
  prompt="First, read ./.codex/agents/gpd-notation-coordinator.md for your role and instructions.
...
<project_context>
read_file: .gpd/STATE.md, .gpd/state.json, .gpd/CONVENTIONS.md
read_file: All SUMMARY.md files from phase {PHASE_NUMBER}
...
Follow the full transition protocol. Read `./.codex/get-physics-done/workflows/transition.md` using the read_file tool for PROJECT.md evolution, DECISIONS.md updates, and parallel phase detection.

```bash
PRE_CHECK=$(/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local pre-commit-check --files .gpd/ROADMAP.md .gpd/STATE.md "${phase_dir}"/*-VERIFICATION.md .gpd/REQUIREMENTS.md 2>&1) || true
echo "$PRE_CHECK"

/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local commit "docs(phase-${phase_number}): complete phase execution" --files .gpd/ROADMAP.md .gpd/STATE.md "${phase_dir}"/*-VERIFICATION.md .gpd/REQUIREMENTS.md
```
...
Read the file at ./.codex/get-physics-done/workflows/execute-phase.md — this contains the complete step-by-step instructions. Do NOT improvise. Follow the workflow file exactly.
