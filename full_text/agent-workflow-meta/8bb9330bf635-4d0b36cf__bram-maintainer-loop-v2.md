---
name: bram-maintainer-loop-v2
description: "Cross-repository maintainer orchestration: Codex app workers, work recovery, dependencies, vision, releases."
---

# Bram Maintainer Loop v2
...
- Maintain this canonical `bram-maintainer-loop-v2` skill in the current root orchestrator session, never in a project thread or collaboration subagent. Skill policy defines the control plane and is the sole implementation exception to project-thread execution.
...
- Scan repositories owned by `BramVR`. Add repositories under other owners only when Bram explicitly names them.
- Read `references/portfolio-exclusions.md` during portfolio discovery. Treat listed repositories as outside routine orchestration responsibility: do not use them for queue refills, dependency sweeps, release monitoring, or unsolicited maintenance. Explicit Bram requests override an exclusion for the requested work only.
...
- macOS apps: use repository release docs and any matching available skill;
