...
`node ${CLAUDE_PLUGIN_ROOT}/nacl-core/scripts/claim-task.mjs release --task <id> --dev "$NACL_DEVELOPER_ID"` (run the
...
`tr | sed | cut` pipeline; equivalence pinned by `${CLAUDE_PLUGIN_ROOT}/nacl-core/scripts/branch.test.sh`):
```bash
slug=$(bash ${CLAUDE_PLUGIN_ROOT}/nacl-core/scripts/branch.sh slug "$message")
...
authority in `${CLAUDE_PLUGIN_ROOT}/nacl-core/scripts/branch.sh` (equivalence pinned by `${CLAUDE_PLUGIN_ROOT}/nacl-core/scripts/branch.test.sh`):
...
bash ${CLAUDE_PLUGIN_ROOT}/nacl-core/scripts/branch.sh guard "$(git rev-parse --abbrev-ref HEAD)" "$base_branch" "$strategy"
```
