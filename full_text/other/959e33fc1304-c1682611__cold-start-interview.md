...
> If you need to find a lawyer: in Brazil, the OAB (Ordem dos Advogados do Brasil) seccional offers a lawyer referral service and, in many states, a Comissão de Assistência Judiciária. Outside Brazil: your jurisdiction's equivalent (SRA/Bar Standards Board in England & Wales; Law Society in Scotland/NI/Ireland/Canada/Australia; state bar in the US). For individuals without means, Defensoria Pública covers many practice areas.
...
The Diário Oficial da União (DOU) is a free public source and is always available — it does not require an MCP connector, though there's no single structured API equivalent to the US Federal Register's; DOU seções I/II/III are published daily and typically require RSS/scraping or the in.gov.br search interface rather than one JSON endpoint.

For connectors that show as not connected, tell the user how to connect. Example phrasing: "[Feed provider] isn't connected. In Claude Cowork: Settings → Connectors → Add → [provider] → sign in. In Claude Code: add the provider's MCP to your config or via `/mcp`. This plugin works without it — DOU + manual paste covers federal coverage — but connecting it adds enrichment and alert import."
...
You don't need all of these. Core features work with free feeds (DOU + the relevant agência reguladora's own portal, e.g. gov.br/anpd, gov.br/anvisa) and file access alone. Paid feeds add enrichment; manual paste-in always works. If you set something up later, re-run `/regulatory-legal:cold-start-interview --check-integrations`.
...
- Which regulators? Name them. (ANPD, BCB/CMN, ANVISA, ANATEL, ANEEL, ANS, CADE, sector-specific, or non-Brazilian if you have cross-border exposure — e.g. EU DPAs, US agencies?)
  *Coverage note: this plugin treats the DOU (Diário Oficial da União) as the general-purpose federal source, plus the specific agência's own gov.br portal for sector rules (ANPD, ANVISA, ANATEL, ANEEL, ANS, CADE, BCB/CMN). None of these publish a single structured feed API the way the US Federal Register does — coverage here is via user-provided RSS URLs, the agency's own publication list, or manual entry. Non-Brazilian regulators require manual entry or user-provided feeds.*
- Why each one? ("We're a fintech, BCB/CMN is obvious" vs. "ANPD because of the open TAC")
...
- A proposed rule with an open consulta pública (public consultation — note: unlike the US notice-and-comment/APA model, each Brazilian agency runs its own consulta pública procedure with its own format and timeline, not a uniform federal process)
...
The Diário Oficial da União (in.gov.br) is the default primary source for federal publications — but unlike the US Federal Register, it has no single structured JSON/API endpoint returning document type, effective date, and comment deadlines in one call; it's organized by seção (I: atos normativos, II: atos de pessoal, III: contratos/licitações) and typically requires RSS or a search-portal scrape. Treat DOU as the baseline for any federal regulator in the watchlist, but expect to supplement with the specific agência's own portal for structured detail (e.g. gov.br/anpd/pt-br/assuntos/noticias, gov.br/anvisa, gov.br/anatel).

For every other named regulator (ANPD, BCB/CMN, ANVISA, ANATEL, ANEEL, ANS, CADE, sector-specific regulators, non-Brazilian regulators), ask the user for their preferred feed URL, or direct them to the agency's website to find the current feed. Feed URLs change; don't rely on a cached list.
...
**Step 4: Consulta pública tracking** *(This feeds `/regulatory-legal:comments` — the consultation calendar logs deadlines and surfaces decisions when a consultation window opens. Note the deadline format and even the existence of a consultation period is agency-specific — ANPD, BCB, and ANATEL each run their own process; there's no uniform APA-style rulemaking clock to key off of.)*

> When I see a proposed rule with an open consulta pública from your watchlist, I'll automatically log the deadline. Do you want me to flag these so you can decide whether to submit a contribution?
...
|---|---|---|
| [name] | DOU / agência gov.br portal / RSS / manual | [endpoint or "manual entry"] |

**Paid feeds (if configured):**
| Service | Subscription | Alerts |
|---|---|---|
| [Paid feed provider] | [yes/no] | [alert names] |
| JusBrasil | [yes/no] | [tracker names] |
...
> - **Track open consultas públicas** — e.g., "What's open, deadlines, and a decision log on whether to submit a contribution." Try: `/regulatory-legal:comments`
>
