---
name: aurelia2
description: Create new and work with existing Aurelia 2 applications. Use this skill when scaffolding a new Aurelia 2 project, creating components/routes, writing Aurelia 2 code, debugging, adding SSR/prerendering, migrating an Aurelia 1 app to Aurelia 2, refactoring, or reviewing existing Aurelia 2 codebases. Covers project setup with Vite, Tailwind CSS v4, Vitest, Playwright, Storybook, SSR, the first-party plugins (HTTP/fetch-client, validation, dialog, state, i18n), Aurelia 1-to-2 migration, and best practices for templates, routing, DI, observation, and testing.
---
...
`-s` takes any **subset** of features (comma-separated, no spaces); unspecified groups fall back to defaults (a Vite + TypeScript app). For the full list of feature flags, the choice groups, conditional rules (e.g. `vitest` needs `vite`), and more example commands, read `references/cli.md`.
...
export class ProductDetail implements IRouteViewModel {
  ...
  loaded(params: Params, next: RouteNode, current: RouteNode | null) {
...
**Full hook signatures:** every route hook receives a trailing `options: INavigationOptions` argument after the parameters shown above (`canLoad(params, next, current, options)`, `loading(params, next, current, options)`, `loaded(params, next, current, options)`, `canUnload(next, current, options)`, `unloading(next, current, options)`). Declare only the parameters you actually use — TypeScript allows implementing a hook with fewer parameters.
...
## Additional Packages (HTTP, Validation, Dialogs, State, i18n)

Aurelia ships first-party plugins for common app needs. Each has a focused reference file — read
the relevant one before writing code against that package, and only add a package when the app
actually needs it. Register configurations in `main.ts` (see Application Bootstrap).

- **HTTP requests** → `references/fetch-client.md`. `@aurelia/fetch-client` (`IHttpClient`) is the
  standard HTTP client and ships **inside the `aurelia` meta-package** (no extra install).
  Covers DI, `configure()`, interceptors (auth/error), retry, caching, and cancellation.
- **Form/model validation** → `references/validation.md`. `@aurelia/validation` +
  `@aurelia/validation-html`: the fluent rule API, `IValidationController`
  (`newInstanceForScope`), the `& validate` binding behavior, and error display.
- **Modals/dialogs** → `references/dialog.md`. `@aurelia/dialog`: `IDialogService.open()`,
  `DialogConfigurationStandard`, the dialog component contract (`activate`, `IDialogController`,
  `ok`/`cancel`/`error`), and reading the close result.
- **Shared state (Redux-style)** → `references/state.md`. `@aurelia/state`:
  `StateDefaultConfiguration.init`, action handlers, the `.state`/`.dispatch` binding commands and
  `& state` behavior, `IStore`, and `@fromState`.
- **Internationalization** → `references/i18n.md`. `@aurelia/i18n` (wraps i18next, a peer
  dependency): the `t` attribute/value converter, `nf`/`df`/`rt` formatting, the `I18N` service,
...
Components are tested with **Vitest** (jsdom) + `@aurelia/testing`'s `createFixture`. A minimal
test — use `appHost` (not `host`) for DOM queries, and `await .started` before asserting:
...
Mock services through the 4th `createFixture` argument
(`[Registration.instance(IApiService, mockApi)]`), and reach a component's view-model with
`CustomElement.for(el).viewModel`.

For the full setup (`vitest.config.ts`, `test/setup.ts` platform bootstrap), the complete
`createFixture` fluent API and assertion/event helpers, testing value converters / custom attributes
/ async / routing, mocks and spies, common pitfalls, and **Storybook** setup, read
`references/testing.md`.

## Troubleshooting & Forms

- **Debugging** → `references/troubleshooting.md`. Silent-failure triage (unregistered resources,
  unobservable array mutations, hook-name shadowing, early `ref` access, SVG without `SVGAnalyzer`),
  reading `AURxxxx` errors (use a `useDev: true` build to expand them), and debugging techniques.
- **Forms** → `references/forms.md`. Inputs, checkboxes/radios/selects with `model.bind` and
  `matcher.bind`, file inputs, and form submission — note `submit.trigger` does **not** preventDefault
  by default; use `submit.trigger:prevent`.
...
16. **Do NOT expect `submit.trigger` to preventDefault**  -  Aurelia 2 does not prevent a form's default submit. Use `submit.trigger:prevent="..."` (or call `$event.preventDefault()`), otherwise the page reloads. See `references/forms.md`.
...
- `<compose>` -> `<au-compose>`
- `<require>` -> `<import>`, `<router-view>` -> `<au-viewport>`, `view-model.ref` -> `component.ref`
- `PLATFORM.moduleName()` -> direct `import()` calls
- `@computedFrom` getters -> plain getters (v2 tracks dependencies automatically)
- `experimentalDecorators: true` in tsconfig -> remove it
- A lingering `compatRegistration` (`@aurelia/compat-v1`) registration -> the migration is unfinished; it is a temporary shim, not a destination.

For a full Aurelia 1 -> 2 migration playbook — the `@aurelia/compat-v1` escape hatch, bootstrap/template/component/router/plugin changes, and a phased migration strategy with checklist — read `references/migration.md`. Reach for it when porting a v1 app or reviewing a partially-migrated codebase.
