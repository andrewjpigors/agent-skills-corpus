...
3. Read the design context — `DESIGN.md` and `brand-spec.md` if they exist in the repo
   ...
   spacing / typography constraints.
...
3. Read the design context tokens. `DESIGN.md` overrides extracted system-level
   properties (font family, spacing scale, layout conventions). `brand-spec.md`
...
- CSS custom properties for design tokens from the design context / Step 1 extraction
...
If no design context exists in the repo root, offer to create it from the generated HTML:
...
- Shadow values
- Any obvious frozen brand cues already visible in the approved mockup or finalized HTML
  (logo treatment, product imagery rules, palette constraints)

Use AskUserQuestion:
> No design context found. I can extract the tokens from the HTML we just built and
> create `DESIGN.md` for your project. If this screen belongs to a named brand or
> product, I can also freeze the brand-specific rules into `brand-spec.md`.
> A) Create `DESIGN.md` only
> B) Create `DESIGN.md` + `brand-spec.md`
> C) Skip — I'll handle the design context later
