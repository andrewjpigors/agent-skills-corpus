...
HTTP request processing so can focus on app logic.
...
app = App(
 ...
 signingsecret=os.environ["SLACKSIGNING_SECRET"]
)
...
@app.command("/ticket")
def handleticketcommand(ack, body, client):
 ...
 {
 "type": "input",
 "blockid": "priorityblock",
 "element": {
 "type": "static_select",
 "actionid": "priorityselect",
 "options": [
...
def handleticketsubmission(ack, body, client, view):
 """Handle ticket modal submission."""
 ack()

 @ Extract values from the view
 values = view["state"]["values"]
 title = values["titleblock"]["titleinput"]["value"]
 desc = values["descblock"]["descinput"]["value"]
 priority = values["priorityblock"]["priorityselect"]["selected_option"]["value"]
 user_id = body["user"]["id"]

 @ Create ticket in your system
 ticketid = createticket(title, desc, priority, user_id)
...
@ Listen for apphomeopened events
@app.event("apphomeopened")
...
if name == "main":
 handler = SocketModeHandler(app, os.environ["SLACKAPPTOKEN"])
 handler.start()
...
def buildnotificationblocks(incident: dict) -> list:
 """Build Block Kit blocks for incident notification."""
 severity_emoji = {
 "critical": ":red_circle:",
 "high": ":largeorangecircle:",
 "medium": ":largeyellowcircle:",
 "low": ":white_circle:"
 }
...
"text": f"Reported:\n<!date^{incident['timestamp']}^{date_short} {time}; {incident['timestamp']}>"
 ...
 {
 "type": "actions",
 "blockid": f"incidentactions_{incident['id']}",
 "elements": [
 {
 "type": "button",
...
def sendincidentnotification(channel: str, incident: dict):
 """Send incident notification with Block Kit."""
 blocks = buildnotificationblocks(incident)
...
def handle_acknowledge(ack, body, client):
 ...
 @ Update your system
 acknowledgeincident(incidentid, user)
 ...
 "type": "mrkdwn",
 "text": f":whitecheckmark: Acknowledged by <@{user}>"
 })

 @ Remove acknowledge button (prevent double-click)
 actionblock = next(b for b in originalblocks if b.get("blockid", "").startswith("incidentactions"))
 actionblock["elements"] = [e for e in actionblock["elements"] if e["actionid"]!= "acknowledgeincident"]
...
from slack_bolt import App
from slackbolt.oauth.oauthsettings import OAuthSettings
from slacksdk.oauth.installationstore import FileInstallationStore
from slacksdk.oauth.statestore import FileOAuthStateStore
import os
...
class DatabaseInstallationStore:
 ...
 "bottoken": encrypt(installation.bottoken),
 ...
 async def findinstallation(self, *, enterpriseid, teamid, userid=None, isenterpriseinstall=False):
 ...
 if record:
 return Installation(
 bottoken=decrypt(record["bottoken"]),
 @... other fields
)
 return None
...
app = App(
 ...
 clientid=os.environ["SLACKCLIENT_ID"],
 clientsecret=os.environ["SLACKCLIENT_SECRET"],
 scopes=[
 "channels:history",
 "channels:read",
 "chat:write",
 ...
 statestore=FileOAuthStateStore(expirationseconds=600)
)
)
...
@flaskapp.route("/slack/oauthredirect", methods=["GET"])
def oauth_redirect():
 return handler.handle(request)
...
def handleoauthsuccess(args):
 ...
 return "Installation successful! can close this window."

@app.oauth_failure
def handleoauthfailure(args):
...
def requestadditionalscopes(teamid: str, newscopes: list):
 """
...
if name == "main":
 ...
 app,
 os.environ["SLACKAPPTOKEN"] # xapp-... token
)
...
from slackbolt.adapter.socketmode.async_handler import AsyncSocketModeHandler
import asyncio

asyncapp = AsyncApp(token=os.environ["SLACKBOT_TOKEN"])

@async_app.message("hello")
async def handlehelloasync(message, say):
 await say(f"Hey <@{message['user']}>!")

async def main():
 handler = AsyncSocketModeHandler(asyncapp, os.environ["SLACKAPP_TOKEN"])
...
def save(ack, view, update):
 ...
 inputs = {
 "ticket_type": {
 "value": values["tickettype"]["typeselect"]["selected_option"]["value"]
 },
 "title": {
 "value": values["title_input"]["title"]["value"]
 },
 "assignee": {
 "value": values["assigneeinput"]["assignee"]["selecteduser"]
 }
 }
...
createticketstep = WorkflowStep(
 callbackid="createticket_step",
 edit=edit,
 save=save,
 execute=execute
)

app.step(createticketstep)
...
4. must verify state matches what you stored
...
@ If must use Socket Mode in production
...
Message: Hardcoded signing secret. Use os.environ['SLACKSIGNINGSECRET'].
