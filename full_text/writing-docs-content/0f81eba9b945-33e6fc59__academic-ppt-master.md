---
name: academic-ppt-master
description: >
  Turn papers, PDFs, documents, URLs, or Markdown into editable academic PowerPoint
  decks through a structured SVG-to-PPTX workflow. Use when the user asks to make
  an academic PPT, paper-reading deck, journal-club deck, thesis defense deck,
  research presentation, "生成PPT", "做PPT", "制作演示文稿", or mentions
  "academic-ppt-master".
---

# Academic PPT Master Skill

> Converts academic source material into high-quality SVG pages, verifies the visual output, and exports the result as a natively editable PPTX deck.
...
>
> - `academic-ppt-master` is a repository-specific workflow, not a general application scaffold
...
| `${SKILL_DIR}/scripts/formula_audit.py` | Formula safety audit for SVG output before PPTX export |
...
>
> **Do NOT convert EMF/WMF to PNG.** The Academic PPT Master pipeline preserves them as external
...
| Anything else — bare template names ("用 academic_defense"), style descriptions ("极简学术风"), brand mentions ("某高校风格"), vague intent ("想用个模板"), or silence | Skip Step 3, free design |
...
> Bare names ("academic_defense", "academic-blue") do NOT trigger Step 3 even if a matching directory exists in the library. The user must give a path. AI must not "helpfully" resolve a name to a path.
...
Field-level micro-adjustment (e.g. "use academic-blue but primary changed to #FF0000") is **not** part of Step 3 fusion — it flows into Strategist Eight Confirmations e–g as a normal user request.

#### Same-kind multiple paths — conflict resolution

When the user gives two paths of the **same kind** (e.g. `brands/academic-blue` + `brands/academic-graphite`), Step 3 surfaces a conflict prompt before fusing — like resolving a git merge conflict:

```
AI: 你给了两个 brand，检测到段级冲突：
    - Color Scheme（academic-blue 深蓝 vs academic-graphite 石墨灰）
    - Typography（academic default vs custom template font）
    - Logo（no logo vs lab logo）
    - Voice & Tone（paper-report concise vs defense formal）
    - Icon Style（stroke vs filled）

    要 (a) 全部按 academic-blue / (b) 全部按 academic-graphite / (c) 逐段挑？
```
...
```markdown
> **Fused from:**
> - deck: `templates/decks/academic_paper/` （base）
> - brand: `templates/brands/academic-graphite/` （identity override）
> - layout: `templates/layouts/academic_defense/` （structure override）
> - conflicts resolved: Color Scheme from academic-graphite（user picked b）
```
...
For academic, scientific, engineering, and mathematical decks, recommend
`render-all` whenever the user asks for formulas or symbols to render correctly.
PDF/DOCX extraction often corrupts formulas into Markdown emphasis fragments
such as `*QK**T* *√**d**k*`; treat those fragments as signals to reconstruct the
faithful LaTeX from the source, not as text to copy into a slide.
...
Formula-worthy expressions include fractions, radicals, sums, products,
matrices, Greek-letter optimizer settings, and compound super/subscript
expressions such as `QK^T / \sqrt{d_k}`, `d_model`, `W_i^Q`, `PE_(pos,2i)`,
or `lrate = d_model^{-0.5} ...`. Under `render-all`, even short symbols with
subscripts/superscripts must become formula PNGs unless the user explicitly
chose `text-only`.
...
If the deck is an academic paper report and the user wants richer visuals, read
`references/academic-visual-enrichment.md` before finalizing `design_spec.md §VIII`.
Add only generated visuals that clarify research content, and keep exact
formulas/data in SVG or formula PNG layers.
...
```
Then run the formula safety audit:
```bash
python3 ${SKILL_DIR}/scripts/formula_audit.py <project_path>
```
...
- Any formula audit finding MUST be fixed before proceeding: add or repair the
  formula manifest, rerender with `latex_render.py`, and replace the offending
  SVG `<text>` with `<image href="../images/<formula>.png" preserveAspectRatio="xMidYMid meet"/>`.
...
Canonical export pipeline (mirrors `references/shared-standards.md` §5, with formula audit added before post-processing):
...
**Step 7.1b** — Formula audit before post-processing:
```bash
python3 ${SKILL_DIR}/scripts/formula_audit.py <project_path>
```
...
| Academic generated visual enrichment | `references/academic-visual-enrichment.md` |
