...
3. If no systems index exists, fail with:
   > "Usage: `/ags-design-system <system-name>` — e.g., `/ags-design-system movement`
...
- **Game concept**: Read `design/gdd/game-concept.md` — fail if missing:
  > "No game concept found. Run `/ags-brainstorm` first."
- **Systems index**: Read `design/gdd/systems-index.md` — fail if missing:
  > "No systems index found. Run `/ags-map-systems` first to map your systems."
...
- Read `.ags/docs/engine-reference/[engine]/breaking-changes.md` for domain-relevant entries
...
> `/ags-setup-engine` before moving to architecture if you haven't already."
...
in `/ags-architecture-decision`, not the GDD. The GDD describes behavior; the ADR
...
> "📌 **Asset Spec** — Visual/Audio requirements are defined. After the art bible is approved, run `/ags-asset-spec system:[system-name]` to produce per-asset visual descriptions, dimensions, and generation prompts from this section."
...
> `/ags-design-review design/gdd/[system-name].md`
>
> **Never run `/ags-design-review` in the same session as `/ags-design-system`.** The reviewing
...
**NEVER offer to run `/ags-design-review` inline.** Always direct the user to a fresh window.
...
- "What's next?"
  - Options:
    - "Run `/ags-consistency-check` — verify this GDD's values don't conflict with existing GDDs (recommended before designing the next system)"
    - "Design next system ([next-in-order])" — if undesigned systems remain
    - "Fix review findings" — if design-review flagged issues
    - "Stop here for this session"
    - "Run `/ags-gate-check`" — if enough MVP systems are designed
...
| Economy, loot, crafting | `systems-designer` | `systems-designer` (curves), `game-designer` (loops) |
| Progression, XP, skills | `game-designer` | `systems-designer` (curves), `systems-designer` (sinks) |
...
| Audio systems | `game-designer` | `audio-director` (direction), `audio-director` (specs) |
| AI, pathfinding, behavior | `game-designer` | `ai-programmer` (implementation), `systems-designer` (scoring) |
| Level/world systems | `game-designer` | `game-designer` (spatial), `narrative-director` (lore) |
...
> **Context approaching limit (≥70%).** Progress saved — approved sections written to `design/gdd/[system-name].md`. Open fresh session, run `/ags-design-system [system-name]` — it detects complete sections and resumes from next.

---

## Recommended Next Steps

- Run `/ags-design-review design/gdd/[system-name].md` in a **fresh session** to validate the completed GDD independently
- Run `/ags-consistency-check` to verify this GDD's values don't conflict with other GDDs
- Run `/ags-map-systems next` to move to the next highest-priority undesigned system
- Run `/ags-gate-check pre-production` when all MVP GDDs are authored and reviewed
