...
1. Check `config/local/codex-for-legal/litigation-legal/CLAUDE.md`. If already populated and no `--redo`, ask before overwriting.
...
5. Migration: if a populated CLAUDE.md (no `[PLACEHOLDER]` markers) exists at `config/local/cache/codex-for-legal/litigation-legal/*/CLAUDE.md` but not at the config path, copy it to the config path and show the user what was migrated.
6. Write `config/local/codex-for-legal/litigation-legal/CLAUDE.md`. Date the footer.
7. Confirm with the user before finalizing: "Here's what I captured — anything wrong?"

## Flags

- `--redo` — re-run the full interview and overwrite `config/local/codex-for-legal/litigation-legal/CLAUDE.md`.
- `--check-integrations` — re-scan available MCP connectors and refresh the `## Available integrations` table in `config/local/codex-for-legal/litigation-legal/CLAUDE.md` without re-running the full interview. Use after setting up a new connector (DMS, document storage, Gmail, scheduled-tasks, CLM).
...
Read `config/local/codex-for-legal/litigation-legal/CLAUDE.md`:
- **Does not exist** → start the interview.
- **Contains `<!-- SETUP PAUSED AT: -->`** → greet the user and offer to resume from that section.
- **Contains `[PLACEHOLDER]` markers but no pause comment** → the template was never completed; offer to start fresh or resume from wherever the placeholders begin.
- **Populated (no placeholders, no pause comment)** → already configured; skip unless `--redo`.

The template structure lives at `${CLAUDE_PLUGIN_ROOT}/CLAUDE.md` — use it as the section scaffold. Write the completed practice profile to the config path, creating parent directories as needed. If a CLAUDE.md exists at the old cache path `config/local/cache/codex-for-legal/litigation-legal/*/CLAUDE.md` but not here, copy it forward.
...
>
> Quick or full? (Upgrade any time with `$litigation-legal:cold-start-interview --full`.)
...
> **How do you work litigation?** (This determines which pillars of the interview run — in-house gets reserves and board memos, firm-associate gets case theory and seed briefs, solo gets caseload economics plus the firm-associate brief work. It also sets defaults for $litigation-legal:matter-intake, $litigation-legal:portfolio-status, $litigation-legal:oc-status, and every other skill's vocabulary.)
>
...
> **Which side do you mostly represent?** (This feeds $litigation-legal:demand-draft, $litigation-legal:demand-received, $litigation-legal:subpoena-triage, $litigation-legal:chronology, and $litigation-legal:claim-chart — plaintiff framing treats demand letters as assertions and discovery as offensive, defense framing treats them as received and responsive.)
>
...
> This plugin can work with: DMS (iManage (connector optional; not enabled in v1)), document storage (Google Drive (connector optional; not enabled in v1), SharePoint, Box), Gmail, scheduled-tasks, CLM (Ironclad (connector optional; not enabled in v1)), eDiscovery (Everlaw (connector optional; not enabled in v1), Relativity, DISCO, Aurora), legal research (CourtListener (connector optional; not enabled in v1), Descrybe (connector optional; not enabled in v1), Trellis), outside-counsel recommendations (TopCounsel). Let me check which connectors you have configured — features that need them will work, and features that don't will fall back gracefully instead of failing silently.
...
**Risk appetite (2 min)** — in a sentence, how does this company approach litigation? (This feeds $litigation-legal:matter-briefing and $litigation-legal:portfolio-status — sets how conservative or aggressive every matter briefing is when calling a matter's risk tier.)
...
- Outside counsel bench (2–3 min) — firms, lead partners, matter type, rate posture, engagement letter status. *Seed doc:* outside counsel guidelines. (This feeds $litigation-legal:oc-status — the skill later drafts weekly status requests to these firms.)
...
- Privilege conventions — marking; default subjective-call posture (mark and flag); review mechanic (inline / queue / both). (This feeds $litigation-legal:privilege-log-review — the skill applies your marking rules and review mechanic on every priv-log pass.)
- Legal hold — template, issuance protocol, refresh cadence. *Seed doc:* hold template. (This feeds $litigation-legal:legal-hold — the skill issues, refreshes, and releases holds using your house template.)
...
- **Document storage** — Google Drive (connector optional; not enabled in v1), Dropbox, OneDrive, local filesystem, the case-management tool's storage. Where do matter documents actually live?
...
>
> 2. **A prior brief in house style.** Not from this case — any case. The best one you've got. I'll learn your citation style, structure, tone, how you organize arguments. (This feeds $litigation-legal:brief-section-drafter — every future brief section gets drafted in your extracted citation format, heading structure, and tone, not a generic template.)
...
If not:
- eDiscovery platform (Everlaw (connector optional; not enabled in v1), Relativity, DISCO, Aurora)
...
>
> **My suggestion for your first one:** Run `$litigation-legal:portfolio-status` — it shows you at a glance where the portfolio sits, and it's zero-input to try. Or tell me what's on your plate and I'll pick.
...
> "Your practice profile is at `config/local/codex-for-legal/litigation-legal/CLAUDE.md` — a plain text file you can read and edit directly. Anything you answered can be changed:
>
...
**Connect a research tool.** Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Cowork: Settings → Connectors. In Codex: authorize when a skill prompts you.
...
> - Run `$litigation-legal:cold-start-interview --redo <section>` to re-interview one part, or edit the config file directly.
>
