...
description: >
  ...
  student who runs $legal-clinic:ramp reads from the same clinic context. Use on fresh
...
1. Check `config/local/codex-for-legal/legal-clinic/CLAUDE.md`. If populated and no `--redo`, confirm before overwriting.
...
5. Migration: if a populated CLAUDE.md (no `[PLACEHOLDER]` markers) exists at `config/local/cache/codex-for-legal/legal-clinic/*/CLAUDE.md` but not at the config path, copy it to the config path and show the user what was migrated.
6. Write `config/local/codex-for-legal/legal-clinic/CLAUDE.md` including `## Who's using this` and `## Available integrations`. Show supervision choice and practice-area templates for confirmation.
...
This interview sets up the clinic context once, so every student who onboards via `$legal-clinic:ramp` and every skill that runs afterward is working from the same understanding of how *this* clinic operates.

**Audience: the supervising professor.** Students don't run this — they run `$legal-clinic:ramp`.

## Cold-start check

Read `config/local/codex-for-legal/legal-clinic/CLAUDE.md`:
...
>
> **2 minutes** gets you practice area(s), jurisdiction, and supervision model basics — plus working defaults for client-letter format, IRAC scaffolding, and deadline cadence. **15 minutes** adds your ethical-preconditions record, supervision flag triggers, per-practice-area document templates from your filings, handbook content feeding `$legal-clinic:ramp`, local court rules feeding `$legal-clinic:draft`, and semester dates.
>
> Quick or full? (Upgrade any time with `$legal-clinic:cold-start-interview --full`.)
...
**Why this matters.** Every `$legal-clinic:ramp` onboarding, every `$legal-clinic:client-intake`, every `$legal-clinic:draft`, every `$legal-clinic:client-letter`, every `$legal-clinic:status` reads from the configuration this interview writes. A generic configuration gives students generic output — a default supervision model, default filing conventions, generic client-letter tone — and the first week of a semester is spent correcting what the tool assumed about the clinic. Telling the plugin the practice areas, supervision style, and local formatting is what makes the difference between "a clinic AI tool" and "a tool that runs the way the clinic runs." The more specific the answers, the less a new student has to unlearn.
...
- **Pause and resume.** Tell the supervising attorney up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `$legal-clinic:cold-start-interview` again later and I'll pick up where you left off." When the attorney pauses, write a partial configuration to `config/local/codex-for-legal/legal-clinic/CLAUDE.md` with a `<!-- SETUP PAUSED AT: [section name] — run $legal-clinic:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the attorney: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
> This plugin can work with a case management system (Clio) and document storage (Google Drive (connector optional; not enabled in v1), SharePoint, Box). Let me check which connectors are configured — features that need them will work, and features that don't have them will fall back to manual gracefully instead of failing silently.
...
Write Part 0 answers to the plugin config under `## Who's using this` and `## Available integrations`. If a populated CLAUDE.md exists at the old cache path `config/local/cache/codex-for-legal/legal-clinic/*/CLAUDE.md` but not here, copy it forward first.

### Opening

> This is the one-time setup for your clinic. Ten to fifteen minutes. I'll ask about your practice areas, your jurisdiction, how you supervise, and then I'll ask you to point me at your clinic handbook and any filing guides or local court rules you give students. Everything I learn here feeds the `$legal-clinic:ramp` onboarding your students will run at the start of each semester, and every other command in this plugin.
>
...
>
> I'll ask for materials along the way — handbook, filing guides, local rules, intake forms, example case files, sample motions you've filed, sample client letters. Ten to twenty documents across the interview is the target. More is better. If you share fewer than ten, I'll flag the practice profile as LIMITED DATA — the plugin still works, but `$legal-clinic:ramp` is thinner (commands but not your clinic's specific procedures), `$legal-clinic:draft` falls back to state defaults instead of your local formatting, and `$legal-clinic:client-letter` uses generic templates instead of matching your voice. Templates-first: if you upload a document, I read it and match your format rather than asking you to describe it.

### Part 1: The clinic (2-3 min)

**What kind of clinic?** (Practice area feeds $legal-clinic:client-intake and $legal-clinic:draft — each area has its own intake template and document templates, so this is the key that switches between an immigration-clinic workflow and a housing-clinic workflow.)
...
(This feeds $legal-clinic:draft, $legal-clinic:research-start, $legal-clinic:memo, and $legal-clinic:deadlines — jurisdiction determines filing formats, research scope, and default deadline calculations.)
...
> Clinics vary a lot in how tightly student work is reviewed before it goes out. Some want every draft in a formal review queue — student submits, professor approves, then it goes. Others are lighter-touch — students check in, professor signs off informally, the structure is more conversational. What's your model? (This feeds $legal-clinic:supervisor-review-queue and the flag-triggering logic across $legal-clinic:draft, $legal-clinic:client-letter, and $legal-clinic:status — formal queue turns the supervisor-review-queue skill on; configurable flags only surface triggers; lighter-touch suppresses the queue entirely.)
...
> Three things, as many as you have. (The handbook feeds $legal-clinic:ramp onboarding; filing guides feed $legal-clinic:draft formatting; the intake form becomes the backbone of /client-intake.)
>
> 1. **Your clinic handbook or procedures doc.** Whatever you give students on day one. I'll use it to build the `$legal-clinic:ramp` onboarding so students get a guided walkthrough instead of a PDF they skim.
>
> 2. **Filing guides and local court rules.** Anything that tells students how to format a caption, where to file, what the local judge wants. These feed `$legal-clinic:draft` so first drafts are jurisdictionally correct from the start.
>
> 3. **Your intake form, and if you have one, a scrubbed example case file.** The intake form becomes the backbone of `$legal-clinic:client-intake`. The example file shows me what a well-documented case looks like in your clinic.

**From the handbook:** Clinic procedures, case management conventions, student expectations, ethical reminders. This is what `$legal-clinic:ramp` will teach.

**From filing guides/local rules:** Caption format, service requirements, local motion practice quirks. This is what `$legal-clinic:draft` will apply.
...
For each practice area the clinic handles: what are the 3-5 documents students draft most often? (This feeds $legal-clinic:draft — each listed document becomes a template the skill can start from, and anything not listed falls back to a generic first pass.)
...
These become the template set for `$legal-clinic:draft`. If the professor has existing templates, ingest them. If not, note which ones to build.
...
- **Semester** — when do students turn over (so `$legal-clinic:ramp` knows when it'll be needed, and `$legal-clinic:semester-handoff` knows when it'll be triggered)
- **Handbook path** — where the ingested handbook lives, for `$legal-clinic:ramp` to read

**LIMITED DATA flag:** if fewer than 10 materials were shared across the interview, add a `> LIMITED DATA` note at the top of CLAUDE.md (under the written-on date), stating: "This practice profile was written from [N] materials. Downstream skills will operate but outputs will be thinner — `$legal-clinic:ramp` covers commands but not clinic-specific procedures, `$legal-clinic:draft` uses state defaults instead of local formatting, `$legal-clinic:client-letter` uses generic templates. Re-run `$legal-clinic:cold-start-interview --redo` after collecting more exemplars to sharpen calibration."
...
- **Ethical reminders calibrated to task:** e.g., $legal-clinic:draft outputs remind about ABA Formal Op. 512 supervision requirements
...
>
> **My suggestion for your first one:** Run `$legal-clinic:ramp` yourself first so you see what your students will see at the start of the semester. Or tell me what's on your plate and I'll pick.
...
2. **Show the practice-area templates table.** "These are the documents `$legal-clinic:draft` will know how to start. Missing anything?"

3. **Offer a `$legal-clinic:ramp` preview.** "Want to see what a student's onboarding will look like? I can walk you through it as if you were a new student."

4. **Note what wasn't provided.** If no handbook: "`$legal-clinic:ramp` will be thin until you upload a handbook — it'll cover the commands but not your clinic's specific procedures." If no local rules: "`$legal-clinic:draft` will use state defaults for formatting — upload local rules when you have them."

5. **If LIMITED DATA flagged:** "Practice Profile is thin — downstream skills will be generic until more materials are added. Biggest gap: [specific — e.g., no handbook means $legal-clinic:ramp covers commands only]. Biggest easy win: [specific — e.g., upload two or three recent motions you've filed, and $legal-clinic:draft gets dramatically sharper on your formatting conventions]."

6. **Before your first case review, connect a research tool.** Say: "Before your first case review or memo: connect a research tool. Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Cowork: Settings → Connectors. In Codex: authorize when a skill prompts you."
...
> Done. Your clinic's configuration is at `config/local/codex-for-legal/legal-clinic/CLAUDE.md` — a plain text file you can read and edit directly. Anything you answered can be changed:
>
...
>
> The things clinics most commonly tweak later: practice areas (when the clinic takes on a new one), supervision style (formal review queue vs. configurable flags vs. lighter-touch — many clinics start one way and shift after the first semester), and jurisdiction / local rules (when a matter lands in an unusual court). Your configuration will improve as students use the plugin — when `$legal-clinic:ramp` misses something or `$legal-clinic:draft` uses the wrong caption format, the fix is usually here.
...
> - Run `$legal-clinic:cold-start-interview --redo <section>` to re-interview one part, or edit the config file directly.
>
...
- **Onboard students.** That's `$legal-clinic:ramp`. This is the professor's one-time setup.
