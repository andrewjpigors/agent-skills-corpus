---
name: node-connect
description: Diagnose ClawBot-Plus node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify ClawBot-Plus is advertising that route, then fix pairing/auth.
...
- whether `clawbot-plus devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `clawbot-plus qr --json`. It uses the same setup-code payload Android scans.

```bash
clawbot-plus config get gateway.mode
clawbot-plus config get gateway.bind
clawbot-plus config get gateway.tailscale.mode
clawbot-plus config get gateway.remote.url
clawbot-plus config get gateway.auth.mode
clawbot-plus config get gateway.auth.allowTailscale
clawbot-plus config get plugins.entries.device-pair.config.publicUrl
clawbot-plus qr --json
clawbot-plus devices list
clawbot-plus nodes status
```

If this ClawBot-Plus instance is pointed at a remote gateway, also run:

```bash
clawbot-plus qr --remote --json
```
...
`clawbot-plus qr --json` success means:
...
If `clawbot-plus qr --json` says `Gateway is only bound to loopback`:
...
```bash
clawbot-plus devices list
clawbot-plus devices approve --latest
```
...
- `clawbot-plus devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run clawbot-plus qr again, rescan, then approve the pending device pairing.`
