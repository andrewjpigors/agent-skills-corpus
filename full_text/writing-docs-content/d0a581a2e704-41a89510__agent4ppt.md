...
version: "1.0"
description: >
  Parse PPTX templates into markdown, generate PPTX from markdown, and
  interactively revise slides. Markdown is the canonical document — the PPTX
  is always a derived artifact. Compatible with Claude Code, OpenClaw, Codex CLI.
license: MIT
author: Jinwang Mok
...
optional_dependencies:
  - libreoffice   # PNG slide previews in revise-ppt
skills:
  - name: parse-ppt-template
    trigger: /parse-ppt-template
    entry: skills/parse-ppt-template/parse_template.py
    description: Parse a PPTX template → annotated markdown template
  - name: generate-ppt
    trigger: /generate-ppt
    entry: skills/generate-ppt/generate_ppt.py
    description: Generate PPTX from a markdown file
  - name: revise-ppt
    trigger: /revise-ppt
    entry: skills/revise-ppt/revise_ppt.py
