...
**Motion capture sweep (mandatory):** Static screenshots cannot represent motion. Without this pass, the clone will look right and feel dead. Do all four steps:

1. **Detect the motion stack.** Run this in the browser MCP console to identify what's actually driving the animations. Record which libraries are present — builders need this to install matching deps:
   ```javascript
   JSON.stringify({
     framerMotion: !!window.__FRAMER_MOTION__ || [...document.querySelectorAll('*')].some(el => el.style.transform && el.dataset.framerComponent !== undefined),
     gsap: !!window.gsap || !!window.GSAP || !!window.ScrollTrigger,
     lenis: !!window.lenis || !!document.querySelector('.lenis, [data-lenis]') || !!document.documentElement.classList.contains('lenis'),
     locomotive: !!window.LocomotiveScroll || !!document.querySelector('[data-scroll-container]'),
     lottie: !!window.lottie || !!document.querySelector('lottie-player, [data-lottie]'),
     motion: !!window.motion,
     scripts: [...document.scripts].map(s => s.src).filter(s => /framer|gsap|lenis|locomotive|lottie|motion|scrollmagic/i.test(s))
   }, null, 2)
   ```

2. **Extract @keyframes and global animation CSS.** Pull every `@keyframes` rule and every `animation:` shorthand from the page's stylesheets — these are the named motion primitives that builders must reproduce in `globals.css`:
   ```javascript
   const keyframes = [];
   ...
   for (const sheet of document.styleSheets) {
     try {
       for (const rule of sheet.cssRules || []) {
         ...
         if (rule.style?.animation) animations.add(rule.style.animation);
       }
     } catch (e) {}
   }
   JSON.stringify({ keyframes, animations: [...animations] }, null, 2)
   ```

3. **Capture computed motion properties on every interactive/animated element.** For each element flagged in the scroll/click/hover sweeps above, extract these properties via `getComputedStyle()`: `transition`, `transition-duration`, `transition-timing-function`, `transition-delay`, `animation`, `animation-timing-function`, `animation-duration`, `animation-iteration-count`, `transform`, `transform-origin`, `will-change`, `view-timeline`, `animation-timeline`. These are the exact values builders must reproduce — a "fade up" with `0.6s cubic-bezier(0.22, 1, 0.36, 1)` looks fundamentally different from `0.3s ease-out`.

4. **Record short screen videos** of every non-trivial interaction (header scroll-shrink, hover effects on cards, menu open/close, scroll-driven reveals, parallax, page transitions). Save as `.mp4` or `.webm` to `docs/design-references/motion/` with descriptive names (`header-scroll.mp4`, `card-hover.mp4`, `hero-parallax.mp4`). Builder agents will be given the relevant clip alongside the spec — easing curves and durations are nearly impossible to extract from a static description, but trivial to mimic from a 3-second clip.

Save all findings — including the library detection JSON, the keyframes/animations dump, per-element motion CSS, and paths to the recorded clips — to `docs/research/BEHAVIORS.md`. This is your behavior bible — reference it when writing every component spec.
...
- [ ] Animation/transition computed CSS recorded (`transition`, `animation`, easing, duration, delay) for every animated element
- [ ] Any required motion library (Framer Motion, GSAP, Lenis, Lottie, etc.) is identified and listed in the spec's dependencies
- [ ] Screen-recorded clip(s) of every non-trivial motion saved under `docs/design-references/motion/` and linked in the spec
