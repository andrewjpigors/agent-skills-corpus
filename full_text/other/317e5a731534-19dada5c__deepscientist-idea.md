...
- Keep ordinary subtask completions concise. When the idea stage actually finishes a meaningful deliverable such as a selected idea package, a rejected-ideas summary, or a route-shaping ideation checkpoint, upgrade to a richer `ds_artifact_record payload={'kind': 'milestone'}, reply_mode='threaded', ...)` report.
...
- Before opening a broad new search, check quest and global memory with `ds_memory_search(...)` and reuse existing paper notes, idea notes, and knowledge cards.
...
- quest and global memory cards returned by `ds_memory_search(...)` and `ds_memory_search(...)`
...
- `ds_memory_search(...)` and recent memory reads
...
When the stage reaches a route-shaping outcome, notify the user through `ds_artifact_record ...)` deliberately:
...
- begin every idea pass with `ds_memory_search(scope='quest', limit=5)`
- then run at least one idea-relevant `ds_memory_search(...)` before broad new ideation or literature expansion
...
When calling `ds_memory_write(...)`, pass `tags` as an array like `["stage:idea", "type:selection-rationale", "topic:<mechanism>"]`, not as one comma-joined string.

Recommended read timing:

- before any new paper search:
  - run `ds_memory_search(...)` over the baseline, task, dataset, mechanism, and current idea labels
...
- if ideation produced a durable survey conclusion, selected-idea rationale, rejected-idea lesson, or novelty caveat, write at least one `ds_memory_write(...)` before leaving the stage
...
When the native runtime surface supports it, pass the final Markdown draft through `draft_markdown` so the branch records both `idea.md` and `draft.md`.
...
Do not prefer `artifact.submit_idea(...)` for the normal idea-selection path.
