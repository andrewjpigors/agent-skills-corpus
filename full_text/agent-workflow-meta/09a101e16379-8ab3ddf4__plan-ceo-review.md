...
## Persist Findings to plan.md (Speckit Integration)

After the completion summary, **automatically persist key review findings to the plan file**.

**Step 1: Locate the plan file.** Look for:
- `.specify/specs/*/plan.md` matching the current feature branch
- `plan.md` in the current directory
- Any file the user explicitly referenced as their plan

**Step 2: Append a `## Plan Review (CEO)` section** to the end of the plan file using the Edit tool. Include:

```markdown
## Plan Review (CEO)

**Reviewed**: [DATE] | **Mode**: [EXPANSION / SELECTIVE / HOLD SCOPE / REDUCTION]

### Premise Challenge
[Summary of Step 0A — is this the right problem?]

### Dream State Delta
[Current state → this plan → 12-month ideal]

### Scope Decision
[One paragraph summarizing the mode selection and rationale]

### NOT in Scope
[Bulleted list of deferred items with rationale]

### What Already Exists
[Bulleted list of existing code that was identified]

### Key Decisions Made
[Numbered list of issues raised and the option the user chose, e.g.:]
1. [Issue description] → Chose [LETTER]: [one-line summary]

### Error & Rescue Registry
[Table from Section 2, or "No error paths identified"]

### Failure Modes
[Table from failure modes registry, or "No critical gaps identified"]

### Diagrams
[Any ASCII diagrams produced during the review]

### Delight Opportunities (EXPANSION mode only)
[Bulleted list of accepted delight items, or "N/A — not in EXPANSION mode"]

### Unresolved Decisions
[List any unanswered questions, or "None"]
```

**Step 3:** Confirm to the user that findings were saved and show the file path.

**Rules:**
- If a `## Plan Review (CEO)` section already exists in the file, replace it (the review is being re-run).
- Do NOT modify any other section of plan.md — only append/replace the review section.
- If the plan file cannot be found, ask the user for the path.
