...
/* UI panel position and size — adjust to fit your lesson's controls */
#define PANEL_X  20.0f
#define PANEL_Y  60.0f
#define PANEL_W  280.0f
#define PANEL_H  200.0f
...
typedef struct app_state {
    ...
    int sphere_idx_count, box_idx_count;
    ...
    /* UI */
    ForgeUiWindowState ui_window;  /* draggable info panel state */
} app_state;
```
...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char **argv)
{
    ...
    *appstate = state;
    ...
    /* Initialize UI window (MANDATORY — zero-size window won't render) */
    state->ui_window = forge_ui_window_state_default(
        PANEL_X, PANEL_Y, PANEL_W, PANEL_H);

    /* Generate and upload shapes (see upload_shape_vb pattern above) ... */
...
forge_target_assets(NN-topic-name)
...
**Create `forge-assets.toml`** next to CMakeLists.txt:
...
All assets — fonts, textures, models — come from the pipeline's processed
output via the manifest. **NEVER write bespoke CMake copy commands** for
fonts or other assets. See
[`pipeline/README.md`](../../../pipeline/README.md#build-integration)
...
- **All assets must go through the pipeline and be declared via
  `forge-assets.toml` manifests.** NEVER write bespoke CMake copy commands
  for fonts, textures, or models. Use `forge_target_assets()` in
  CMakeLists.txt and list dependencies in the manifest.
- Lesson-local `assets/` directories hold only screenshots and diagrams
  for the README — never runtime assets.
