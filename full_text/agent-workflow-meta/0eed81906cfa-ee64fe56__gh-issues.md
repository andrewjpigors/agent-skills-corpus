---
name: gh-issues
description: "USE WHEN you want to fetch GitHub issues, spawn parallel sub-agents to implement fixes and open PRs, then monitor and address PR review comments. Usage: /gh-issues [owner/repo] [--label bug] [--limit 5] [--milestone v1.0] [--assignee @me] [--fork user/repo] [--watch] [--interval 5] [--reviews-only] [--cron] [--dry-run] [--model <model>] [--notify-channel <channel-id>]"
user-invocable: true
metadata:
...
IMPORTANT — No `gh` CLI dependency. This skill uses curl + the GitHub REST API exclusively. It expects a `GH_TOKEN` (a GitHub PAT or app token with repo scope) in the environment; if your runtime injects one, it is used directly, otherwise Phase 2 resolves it from a local config fallback. Pass it as a Bearer token in all API calls:
...
| --notify-channel | _(none)_ | Optional chat/channel ID to send the final PR summary to (via the host's `message` tool). Only the final result with PR links is sent, not status updates. |
