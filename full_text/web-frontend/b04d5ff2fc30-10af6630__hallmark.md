...
- **Step 0 · Pre-flight scan** — same. Read existing tokens, fonts, framework, microinteraction stance. A button on a DM Sans-bodied Tailwind project must adopt those tokens, not invent new ones.
...
1. **Font stack** — `package.json` for `next/font`, `@fontsource/*`, `expo-google-fonts`, `dm-sans`; any `<link rel="stylesheet" href="...fonts.googleapis.com/...">` in HTML / layout files; `tailwind.config.{js,ts}` `theme.extend.fontFamily`; `@import url("fonts.googleapis.com/...")` in any stylesheet.
...
```
Pre-flight findings:
· Font stack: DM Sans + IBM Plex Mono (next/font, package.json L23)
...
- **Conflicting signals** (e.g. `framer-motion` installed but no `motion.div` usage anywhere; or `DM Sans` import in `package.json` but `font-family: Inter` hard-coded in CSS) → flag the conflict explicitly: _"Conflict: DM Sans imported via next/font but a hard-coded `font-family: Inter` in app/globals.css L4. I'll preserve next/font DM Sans; please confirm or remove the Inter declaration."_
