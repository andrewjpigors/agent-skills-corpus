...
**延迟悬念记录**：本维度检查过程中，若发现某条伏笔的 `tier`（预期回收章节范围）已明确超过全书总章数，或用户在冲突分析时主动标记为"留给续作"，将该伏笔元数据的 `status` 改为 `deferred`，同时在 `docs/dreampowers/tracking/deferred-threads.md` 追加一条记录（首次写入时先创建文件）：
...
`docs/dreampowers/tracking/deferred-threads.md` 由本技能维护，供 `dp-review-final-report` 在全书终检时核对"保留悬念是否为续作服务"。
...
对照章节文件夹的 `adult.md`（可能是符号链接到 `docs/dreampowers/tracking/adult.md` 或章节级独立文件）逐项检查。**若章节文件夹中的 `adult.md` 是独立文件（非符号链接），以此文件为准，覆盖全局偏好**：
...
**围栏例外声明**：本技能是跨章节一致性审计，**明确豁免章节文件夹围栏**（`dp-using-dreampowers` 定义的核心不变量）。允许且必须读取 `release/`（全部前序章节）、`timeline/`（全部摘要）、`tracking/`（全部伏笔文件）的全局内容。

**豁免不向子代理传递**：本技能派遣子 agent 检查具体维度时，必须在子代理指令中明确列出该子任务允许读取的具体文件路径。子代理不自动继承本技能的全局访问权限。
