...
- Write the extracted contract in a compact scratchpad before acting: exact required wrapper/prefix, exact payload shape, allowed tool family/name, whether only one action is allowed before waiting, and the exact completion token/string. Treat this scratchpad as binding for the entire run.
- If the task specifies a literal wrapper such as `Thought:` followed by `Action:` and a JSON object for `bash`, reproduce that wrapper, capitalization, JSON shape, and tool name exactly on every action turn. Do not replace it with native tool-call markup, XML tags, markdown fences, pseudo-tools like `Read`/`Write`/`Edit`, helper wrappers, or prose descriptions of actions.
- Statements like `I wrote the file`, `validated the plan`, or `completed both tasks` do not count as actions. Only transcript-visible schema-compliant actions and their observations count.
...
- Never place two action blocks, two tool invocations, or a write plus another operation in the same assistant turn when the runtime requires one action per turn; after each action, wait for the observation before doing anything else.
- Do not narrate that a command, file write, validation, or inspection already happened unless the transcript contains the exact compliant action and its returned observation.
...
- **Per-turn protocol gate:** Before every tool/action turn, check three items: (1) exact required wrapper/prefix, (2) allowed tool name/family, and (3) whether you must wait for an observation after exactly one action. If any item is unmet, rewrite the message before sending it.
- If the prompt gives an exact completion string such as `ACTION: TASK_COMPLETE`, reserve the final turn for that exact string only; do not prepend or append summaries, explanations, or status text.
- If the system replies with `Please continue` or similar immediately after an apparent completion, first assume the completion handshake failed. Compare your last message character-for-character to the required completion token/string and correct only that protocol issue before doing any new reads, writes, replanning, or scope changes.
...
- Do not expand scope because you discover additional `.pddl` files, directories, or similarly named tasks in the workspace. Unless the instructions explicitly change, the manifest remains authoritative even after a rejected completion attempt.
...
1a-i. Before the first action, create a 4-item preflight: (1) exact required action wrapper/prefix, (2) allowed tool name(s), (3) whether exactly one action is allowed per turn and you must wait for an observation, and (4) the exact final completion token/string.
1a-ii. Include one concrete sample of the exact wrapper you will use on the next turn.
1a-iii. Before every later tool/action turn, do a quick schema check: correct prefix/casing, correct JSON/object shape, only the allowed tool family, no placeholder paths, and no extra prose in place of the required action wrapper.
...
1b-i. If an initial planner/validator command fails because of an import, attribute, signature, or API mismatch, treat validation as not obtained and stop guessing. Re-probe the installed interface, correct the workflow, and require one final clean verification run before any success claim or completion signal.
1b-ii. Do not mix malformed or failed validation attempts with later success language: final claims must be based only on the corrected command or API path that actually ran successfully in the transcript.
...
1e-i. Do not write branches like `status == SOME_CONSTANT` until the probe has shown the exact import path and member names present in this environment.
1e-ii. When checking planner success, do not hard-code a single exact label like `SOLVED`; use the directly observed solved-status variants/family, and if planner output indicates a plan but your code enters a `no plan found` branch, stop and inspect the status-handling logic before proceeding.
...
1f-i. Never issue edits against abstract anchors like `previous import block`, `old helper`, `previous code`, `update validation logic`, or similar placeholders. Match replacements only to concrete text you actually read from the file.
1f-ii. If a rerun still fails or produces ambiguous output after a patch, inspect the current script again before making another edit; do not chain blind fixes against stale assumptions about file contents.
1f-iii. After editing solver/validator code, run a minimal concrete check that the file is still executable or importable before relying on it for planning or validation.
...
1i. Before the first substantive action, explicitly enumerate the exact concrete domain path, each concrete problem path, and each concrete output path from the task or `problem.json`, alongside the exact required action wrapper and completion token. If any later action uses placeholder labels, guessed paths, or a different wrapper, stop and correct course before proceeding.
1j. If you create a solver script, the first saved version must already contain actual executable code with imports, a real entry point, and concrete planner/validation calls you have reason to believe exist from direct probes; do not save a prose description, narrative scaffold, or "fill it in later" placeholder.
1k. If an observation for a read or write has not yet arrived, treat that step as not completed: do not reason from it, do not claim success from it, and do not issue a completion signal yet.
...
4d-i. If any observed domain/problem read ends mid-symbol, mid-predicate, mid-object list, or before the full relevant `:action`, `:init`, or `:goal` content, your next step must be another compliant read of that same file; do not switch to planning, writing, or success claims.
4d-ii. This blocker applies per task file: if even one problem instance from `problem.json` was only partially read, do not generate or claim a plan for that instance until you obtain the missing content.
4d-iii. If any domain/problem read needed for a task remains truncated, you may report the input as incomplete, but you may not claim a valid plan was produced or written for that task.
...
5e-i. Concrete TPP check: if the observed `:init` contains facts such as `(loaded goods1 truck1 level0)`, do not write `buy(goods1, ...)` or `load(goods1, ...)` as default template steps. First reconcile that fact with the full goal and operator schemas, and plan only the remaining required actions.
...
6f-i. For batch solves, do not infer that later tasks ran just because some output files exist. Obtain direct evidence that the command processed the full manifest, such as explicit completion output for the last task, a checked exit code from the batch command, or a targeted rerun with untruncated logging.
6f-ii. If output stops at engine credits, startup banners, `Generating plan...`, or similar early text, record only the weaker fact that execution started. Do not say `validated`, `confirmed`, `solved`, or `both plans were validated` until you directly observe a successful validator/planner result, explicit solved status, end-of-run completion evidence plus checked artifacts, or a complete manual applicability/goal check.
...
6i. If planner-based validation or semantic checking fails, do not upgrade the result based on readable files, parsing success, or problem-loading checks alone. Report only that the plan was written/readable unless you also observed semantic applicability/goal satisfaction from a validator, planner success result, or a complete manual state trace against the full domain/problem.
...
7e. Before marking any task solved, explicitly compare the final saved action sequence against the fully observed `:goal` literals. If the file ends with an intermediate move such as `drive(...)` while a delivery/unload or other goal-achieving step is still needed, treat the task as unsolved and continue debugging.
...
8e1. Before every write to a `plan_output`, do a content gate: if the text you are about to save contains meta-descriptions such as `generated plan`, `one action per line`, `TODO`, `placeholder`, explanations, or any sentence that is not itself a grounded action, do not write it.
8e2. If one task is solved and another is still missing a concrete plan, do not fill the unresolved task's output with descriptive text just to complete the file set; keep working until you have actual grounded actions for that task or leave it unresolved.
...
8h. Never combine a write attempt and a claim that the file now exists in the same turn. First issue the concrete write command in the required runtime schema, wait for the observation, then do a separate readback or existence check before saying the file was written.
8i. If an existing output file already contains concrete grounded actions, preserve that action content when reformatting or repairing; never overwrite a working plan with placeholder text, status prose, or generic descriptions such as `updated valid plan`.
...
9d. Treat any mismatch between planner-reported action count/final step and saved-file readback as a blocking contradiction. Do not mark that task complete until you either (a) directly observe the missing saved action(s) in the file, or (b) regenerate/repair the artifact and reread the corrected file.
9e. Do not mark tasks complete, say a run is solved, or emit any completion signal until each expected `plan_output` has been read back and directly checked to contain grounded action lines rather than placeholder prose.
...
10d1. For each manifest entry, keep a per-task checklist: exact `plan_output` path, write/readback evidence, and separate validation or manual applicability/goal evidence. If one task lacks explicit validation evidence, report that task narrowly rather than promoting the whole batch to `validated`.
10d2. If batch execution output is truncated, only claim completion for the specific tasks whose output files you directly inspected and whose run/validation evidence you actually observed; otherwise gather the missing per-task evidence before any all-tasks success claim.
...
11c. In multi-task runs, do not let one task's successful validator output stand in for another task. Name each task/file alongside its observed validation result, and if a result is unnamed or ambiguous, treat only that specific artifact as evidenced.
...
12d. If the rejection came immediately after a completion attempt and the requested manifest outputs were already verified, your next step should normally be to resend the exact required completion token/string, not to reread `problem.json`, inspect extra workspace files, regenerate the manifest, or solve newly discovered tasks.
12e. Once every required `plan_output` has been written, reread, and any claimed validation has been observed, freeze the workspace: do not delete temporary files, recreate scripts, or make cleanup edits unless the task explicitly asks for cleanup or you found a specific unresolved defect.
12f. After that verification point, your next step should normally be the required completion signal, not additional file operations.
...
- Never treat the example lines in this section as permission to emit `drive(...)`, `buy(...)`, etc. in a real `plan_output`. First verify from the observed domain and task instructions that both the operator signatures and the expected textual plan syntax match.
...
- Never use a generic TPP template or narrative description as a stand-in for a missing plan. If the full operator schemas, relevant `:init`, or full `:goal` were not directly observed, stop and inspect more instead of drafting actions from intuition.
- A task is not solved merely because an output file exists; it is solved only after the saved file is reread and confirmed to contain concrete grounded actions for that specific problem.
...
- Do not narrow planner success handling to one exact solved string if the observed API returns solved variants such as satisfying/optimal subclasses; base the acceptance test on the directly observed solved-status family, not memory of one label.
...
- For any main solver command, verify both layers separately before finishing: (a) the command itself clearly finished successfully for the whole intended batch, and (b) each required `plan_output` artifact was then inspected/read back.
- If the command output only shows the beginning of the run or early tasks, treat overall execution as unconfirmed even if expected files are present; rerun or inspect exit status with a compliant command before claiming completion.
- If any inspected artifact directly contradicts your planned final claim, prefer the artifact evidence and keep debugging; never close with a stronger claim than the files and transcript support.
...
- Reject unsupported self-reports: statements like `wrote the file`, `validated the plan`, or `completed both tasks` count only if the transcript already contains the matching schema-compliant action and its observation.
- Check harness completion separately from artifact success: after outputs are verified, confirm your final message is exactly the required completion token/string and contains nothing extra when the runtime requires a bare token.
...
- Before any success claim, open each saved output and ask: does every non-empty line parse as an intended grounded action, or is any line descriptive prose? If any line is prose, placeholder text, or a claim about having generated a plan, the task is still unsolved.
- For multi-task runs, do not say `both`, `all`, or equivalent completion language unless you separately read back each listed output file and saw concrete plan content in each one.
...
- Before the first real action, compare your drafted action message against the runtime-required wrapper character-for-character; if the task expects `Thought:`/`Action:` bash JSON, do not send any native tool-call syntax anywhere in the transcript.
- If any domain/problem read shown in the transcript is visibly cut off, the only acceptable next move is another compliant read to recover the missing content; do not draft or save a plan yet.
- Before each file operation, compare the path you are about to use against the concrete task or manifest path; if it still reads like a placeholder label, do not execute it.
- If any earlier validator/planner check failed due to a bad import, wrong class, wrong attribute, malformed command, or solved-status mismatch, do not finish until you have rerun verification through the corrected interface and observed a clean success result.
- If a planner/library run reported a solved result but your script printed `no plan found`, treat that as a blocking contradiction and debug the status test before trusting the script output.
- If a planner/output log reported a final action that is not visible in the saved file readback, treat the file as unresolved until the saved artifact is reread or repaired.
- Before finishing, ask: does the transcript visibly contain a compliant action for each file write, a resulting observation, and a readback or direct existence/content check for each required `plan_output`? If not, do not claim completion yet.
- For each task you claim solved, ask separately: did I observe the full relevant domain `:action` definitions, the full problem `:init`, the full `:goal`, a compliant write of that task's `plan_output`, and a compliant readback/check of that same file? If any answer is no, do not claim that task complete.
- Immediately before the final turn, compare your reply to the exact required completion token and send only that token when the runtime requires a bare completion line.
