---
name: macos-ui-pro
description: Practical macOS UI and Human Interface Guidelines for SwiftUI/AppKit apps. Use when building or reviewing Mac UI, status menu utilities, menu bars, popovers, toolbars, window management, keyboard shortcuts, monitoring interfaces, or Mac Catalyst experiences.
license: MIT
metadata:
...
### Rule 1.7 — Status Menu Utilities Need Persistent, Explicit Lifecycle

For utilities that live in the system status area, closing the main window may leave the app running, but the UI must make that behavior obvious. Provide a visible menu bar extra, an Open/Show command, an optional Hide Window command, manual Refresh for stale status, and an explicit Quit item. Do not rely on users force-quitting a background process they can no longer see.

```swift
MenuBarExtra("Status", systemImage: "display") {
...
### Rule 10.4 — Monitoring Popovers Must Be Scannable

Status and monitoring popovers should be dense without becoming cryptic. Use one row per metric family with a stable icon column, a short title, a primary value, and one concise detail line. Do not combine unrelated high-entropy metrics like RAM, disk, CPU, and GPU in one row; they truncate and become impossible to scan.

Put the strongest signal first, for example `GPU 38%` before renderer/tiler details. If text truncates, reduce secondary detail rather than widening the popover indefinitely.

### Rule 10.5 — Verify Symbol Availability

SF Symbols vary by OS release. A symbol that compiles can still render as blank on an older target. For status utilities, prefer widely available symbols and fallbacks: `display`, `cpu.fill`, `memorychip.fill`, `internaldrive.fill`, `battery.100`, `thermometer.medium`, `arrow.clockwise`, and `power`.
