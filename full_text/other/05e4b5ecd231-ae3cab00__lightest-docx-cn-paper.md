---
name: lightest-docx-cn-paper
skill_id: lightest_docx_cn_paper_skill
description: 当用户要中国大陆高校课程论文/毕业论文/数模报告的 Word（.docx）排版——三线表、公式编号、上标引用、GB/T 字体页边距、.md 转国标 docx——时使用。**触发**：课程论文、毕业论文、数模、三线表、中文论文 Word、.md 转 docx、国标排版。**加载**：`skills_fetch(skill_ids=["lightest_docx_cn_paper_skill"])`。**勿用**：通用英文 docx → `lightest_docx_skill`；live Word → `lightest_word_live_skill`；仅 PDF/纯 Excel/无国标要求的英文信 → 其它 skill。
license: MIT. See LICENSE.txt
---

# LighTest 中文课程/学位论文 Word（国标 .docx）

**官方内置**：连接项目时同步到 `.lightest/skills/lightest-docx-cn-paper/`；扫描器读 `skill_id: lightest_docx_cn_paper_skill`。

## 技能分工

| 技能 | 何时 |
|------|------|
| **本技能** | 大陆高校课程论文/毕业论文/数模 **国标 Word**：三线表、公式编号、上标引用、GB/T 7714、页边距与宋体/Times |
| `lightest_docx_skill` | 通用英文 docx、OOXML 精细编辑、非国标模板 |
| `lightest_word_live_skill` | Word/WPS **已打开** 的实时改稿 |
| `lightest_writing_skill` | 先要正文结构与论证，再灌 Word |
| `lightest_latex_skill` | 用户明确要 LaTeX 学位论文而非 Word |

## 工作流（强制）

| 阶段 | 动作 | 工具 / 产出 |
|------|------|-------------|
| 0. 路由 | 只要读文字 → `read`；要国标版式 → 本技能；Word 已开 → word_live | 下表「任务路由」 |
| 1. 定形态 | 新建空白国标稿 vs 改已有 vs `.md` 转 docx | `new_doc.js` / unpack 编辑 / `convert_paper.js` |
| 2a. **新建** | 编辑 `scripts/new_doc.js` 的 CONTENT → `node scripts/new_doc.js` | 预置 GB/T 字体、页边距、标题级 |
| 2b. **.md 转稿** | 用户提供 Markdown 论文稿 | `node scripts/convert_paper.js`（见脚本注释） |
| 2c. **改已有** | `python scripts/office/unpack.py` → 改 XML → repack | 三线表：`table.py`；公式：`formula.py` |
| 3. 校验 | `python scripts/office/validate.py` 输出 docx | 失败则 unpack 修 XML |
| 4. 交付 | 交还 `.docx`；图内数字与引用须与正文一致 | **禁止**用 python-docx 整文档重建 |

**纪律**：国标表必须 **三线表**（粗上下线、表头下细线、无竖线）；表题在表上方「表 章-序号」；公式用 `formula.py` 锚定编号，勿手写乱序。

## 示例 A：课程论文从零生成

用户：「按学校格式写一份课程论文 Word，含一张结果三线表。」

1. `skills_fetch(skill_ids=["lightest_docx_cn_paper_skill"])`
2. 复制并编辑 `scripts/new_doc.js` 正文区（标题、摘要、章节）
3. `node scripts/new_doc.js` → 得到初稿 `paper.docx`
4. `python scripts/office/unpack.py paper.docx unpacked/`
5. `python scripts/table.py unpacked/ "1-1" "实验结果" --headers … --rows …`
6. repack + `validate.py` → 交付

## 示例 B：已有 docx 插入公式与参考文献

用户：「在这份毕业论文里给式 (2-3) 编号，参考文献按 GB/T 7714。」

1. unpack → 在锚定段落用 `formula.py` 插入 LaTeX 块公式
2. 参考文献段用 `new_doc.js` 中 GB/T 模板句式手工填入（见下文 **References (GB/T 7714-2015)**）
3. validate；**勿**整篇用 `read` 当版式来源后重建
...
|--------|------|
| 新建国标空白论文 | `new_doc.js` |
| `.md` → 国标 docx | `convert_paper.js` |
| 插入三线表 / 改表 | unpack + `table.py` |
| 编号公式 | unpack + `formula.py` |
| 仅读论文章节文字 | `read`（不保留版式） |
| 英文商务信 / 无国标 | `lightest_docx_skill` |

## 验收（交付前勾选）

- [ ] 正文：宋体小四、1.5 倍行距、首行缩进 2 字符（与下文规范一致）
- [ ] 表：三线表样式；表题在表上、编号「表 x-y」
- [ ] 式：全文编号连续；引用写「式 (x-y)」
- [ ] 参考文献：GB/T 7714 体例；上标引用与条目一一对应
- [ ] 已跑 `validate.py` 或等价 unpack 修复循环

---

**以下**为 OOXML 技术参考（英文主体，含 Quick Reference、Creating/Editing、三线表与 GB/T 细则；未删减）。
