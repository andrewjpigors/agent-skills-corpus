...
| Codex multi-agent | `grant_writer`（由 `scripts/codex/register-agents.sh` 注册到用户级 `~/.codex/config.toml`；仓库内 `.codex/config.toml` 是 reference configuration） |
...
如果注册脚本不存在、执行失败，或当前运行环境仍无法确认上述 worker 已注册，**不得回退到 general-purpose / 通用 agent**；生成 blocked 版 `unit_result.yaml`，并提示用户在插件根目录手动运行 `bash scripts/codex/register-agents.sh` 后重启 Codex 桌面端或开启新会话。
