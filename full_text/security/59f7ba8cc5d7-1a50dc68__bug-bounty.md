...
## Character and Style Rules (applies to ALL TUIs, always)

These rules apply every time you write a report or PoC -- whether triggered by /report, /autopilot, or a plain request like "write me a report for this bug". No exceptions.

ASCII only in prose AND code comments:
- Use -> not → | use => not ⇒ | use <- not ←
- Use -- not — (em-dash) | use ... not … (ellipsis)
- Use ---- not ──── or ═══ or any box-drawing characters
- No emojis anywhere: not in prose, not in titles, not in code comments
- No unicode a developer would not type directly from a standard keyboard

Report structure:
- No horizontal rules (--- or ***) as visual decorators
- No AI filler: "it's worth noting", "in conclusion", "to summarize", "I hope this helps"
- Summary and impact are prose, not bullet lists
- One header level max for single-bug reports

PoC requirement:
- Every report includes a working PoC -- no exceptions
- For automated flows (autopilot, hunt), derive from captured request/response data
- No author comment inside PoC code blocks
...
[ ] PoC included and reproducible from fresh state
[ ] No unicode arrows, em-dashes, box chars, or emojis anywhere
```
...
# INSTALLATION (Helio Framework)

This skill is included with Helio. Install with:
...
The installer copies this skill to your TUI's skills directory automatically:
- **Claude Code**: `~/.claude/commands/`
- **OpenCode**: `~/.config/opencode/commands/`
- **Qwen Code**: `~/.qwen/commands/`

Then in your TUI, this skill loads automatically when you ask about bug bounty, recon, or vulnerability hunting.