...
## Production SPA Standards

### Layout
- Use CSS Grid for top-level shell (`grid-template-columns: sidebar-width 1fr`), not flexbox
- Collapsible sidebars: toggle a class on the root element that changes the grid column to `0px` with `transition`
- Set `html, body { overflow: hidden }` on dashboard/app layouts — prevents scroll escape to the page behind content panels
- Add `overscroll-behavior: contain` on any scrollable child (chat body, lists) — prevents scroll chaining to parent

### State management with Jotai
- Jotai atoms for all **shared** state (anything two+ components read or write)
- `useState` only for **local UI state** that doesn't cross component boundaries (e.g. dropdown open, modal open)
- Never use `useRef` to store values that could trigger re-renders — use atoms
- Never pass atom setters down more than one level — components should import atoms directly

### No hardcoded strings in components
- All static copy (labels, titles, subtitles, emails, URLs) lives in a single `constants/` file
- Components import from constants — never inline strings like `"Contact us"` or `"contact@company.com"` in JSX
- Mode/status display labels: one mapping object with all variants (short, full, etc.) — never duplicate in multiple components
- Adding a new mode/status → one change in constants, all components auto-update

### CSS variables — never hardcode colors
- All brand colors defined once in `:root` in the main CSS file
- Never use hex values or `rgb()` directly in component CSS or TSX — always `var(--color-name)`
- Never use inline `style={{ color: '#...' }}` in TSX — always a CSS class
- When a color is used in only one section (e.g. admin page accent), define a scoped variable at top of that CSS file with `:root { --section-color: #...; }` so it's still one place to change

### Dynamic props over hardcoded content
- Components like Topbar should receive `title`, `subtitle` as props — never hardcode display strings inside
- Detect "mode" from props (e.g. `!!onChatClose` to detect chat mode) rather than passing explicit boolean flags when possible
- Navigation handlers must close any open panels/modals before switching views — never leave UI in inconsistent state

### Brand/static assets
- SVG logos and icons: serve from `/public/` (Vite copies to `dist/` as-is, no build step)
- Committed to git — Azure/CI picks up on deploy automatically
- Use `<img src="/logo.svg" />` — never apply CSS `filter` or `color` on brand SVGs, they render exactly as designed
- Separate assets for different backgrounds: e.g. `logo-neg.svg` (white, for dark bg), `logo-pos.svg` (dark, for light bg)
- Favicon: use SVG with tight `viewBox` cropped to the icon shapes — prevents tiny invisible icon in browser tab

### Navigation safety
- Any handler that navigates (changes view) must also close chat/modal/panel state
- Clicking a list item while a panel is open: close panel + set view + select item — all three in one handler
- Never rely on component unmounting to clean up global atom state — reset atoms explicitly in navigation handlers