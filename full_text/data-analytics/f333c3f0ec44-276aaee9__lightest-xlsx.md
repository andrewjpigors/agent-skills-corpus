---
name: lightest-xlsx
skill_id: lightest_xlsx_skill
description: 当用户要创建或校验 Excel（.xlsx/.xlsm/.csv）、做透视汇总、公式 QA、openpyxl/pandas 数据处理，且交付物须含 .xlsx 时使用。**触发**：Excel、.xlsx、透视表、openpyxl、pandas、电子表格公式、CSV 转 xlsx。**加载**：`skills_fetch(skill_ids=["lightest_xlsx_skill"])`。**勿用**：matplotlib 出图嵌报告 → `lightest_data_viz_skill`；快速对话趋势图 → `chart_visualization_skill`。
---

# LighTest Excel（内置 Skill）

**官方内置**：`.lightest/skills/lightest-xlsx/`；`skill_id: lightest_xlsx_skill`。交付物**必须含 `.xlsx`**；勿附带用户未要的 readme 等旁路文件。

## 技能分工

| 技能 | 何时 |
|------|------|
| **本技能** | 创建/校验 xlsx、公式、透视、openpyxl/pandas |
| `lightest_data_viz_skill` | CSV 科研图、Typst/报告插图 |
| `chart_visualization_skill` | 对话内快速趋势图 |

## 工作流（强制）

| 阶段 | 动作 |
|------|------|
| 0. 澄清 | 目标表结构、口径、是否要透视/公式重算 |
| 1. 数据 | 外部数据须可追溯（`Source Name` / `Source URL` 列或 Sources sheet） |
| 2. 生成 | Python openpyxl/pandas；参与计算的单元格用**数值格式** |
| 3. 透视 | pandas `pivot_table` → 写回（见 `pivot-table.md`） |
| 4. **校验** | `scripts/xlsx_validate.py` 退出码 `0`；失败改代码重生，勿手改 zip/xml |
| 5. 交付 | 仅用户要的 xlsx |

**LighTest 无** 自包含 .NET Excel 二进制 CLI；校验与透视走本目录 Python 脚本（见 `references/python-validation.md`）。

## 示例：CSV 透视 + 公式 QA

用户：「用附件 CSV 按区域做透视，交付 xlsx，公式要能点开就算。」

1. pandas 读 CSV → `pivot_table` 写 Summary（`pivot-table.md`）
2. openpyxl 写明细与公式；引用区域为数值格式，避免文本导致 `#VALUE!`
3. `python3 .lightest/skills/lightest-xlsx/scripts/xlsx_validate.py out.xlsx`
4. 若 `#REF!` 或孤立公式模式 → 修引用再跑 validate

## 任务路由（简表）

| 用户要 | 先 read |
|--------|---------|
| 校验 / inspect | `references/python-validation.md` |
| 透视 | `pivot-table.md` |
| 交互分析全流程 | 下文 `<Analyze rule>` |
| 画图嵌报告 | `lightest_data_viz_skill` |
...
**✅ Validation & Pivot（LighTest，无自包含 .NET Excel CLI）:**
- **库**: openpyxl + pandas；详见 `references/python-validation.md`
- **脚本**: `scripts/xlsx_inspect.py`、`scripts/xlsx_validate.py`
- **透视**: pandas `pivot_table` → 见 `pivot-table.md`

**🔧 Execution Environment:**
- 写 Excel：Python / 终端脚本
- 校验：终端跑上述脚本；公式要真实重算时可选 LibreOffice `soffice --headless`
...
LighTest **不包含** 约 77MB 自包含 `.NET` Excel CLI。校验与透视改用 **Python 库 + 本目录脚本**。

| 目标 | 命令 |
|------|------|
| 结构 inspect | `python3 .lightest/skills/lightest-xlsx/scripts/xlsx_inspect.py FILE --pretty` |
| 交付 validate | `python3 .lightest/skills/lightest-xlsx/scripts/xlsx_validate.py FILE` |
| 透视 pivot | pandas，见 `pivot-table.md` |
| 细节说明 | 读 `references/python-validation.md` |

**原 recheck 要抓的问题**（openpyxl + 可选 LibreOffice 重算）：

- 公式错误 token：`#VALUE!` `#DIV/0!` `#REF!` `#NAME?` `#NULL!` `#NUM!` `#N/A`
- 公式结果为 0 的可疑单元格（常为引用错误）
- 隐式数组公式（LibreOffice 能算、Excel 不能）：如 `MATCH(TRUE(), range>0, 0)` → 改 `SUMPRODUCT` 或辅助列

**原 reference-check**：越界引用、表头被算进聚合、SUM 只覆盖 1–2 格、同列公式模式孤立 → openpyxl 遍历公式 + 人工 read 关键列

**交付硬规则**：`xlsx_validate.py` 退出码 `0`；失败则改代码重新生成，不要手改 zip/xml  patch。
...
For each sheet in workbook:
    ...
    4. CHECK  → xlsx_validate.py（必要时 LibreOffice 重算后再 validate）→ 修到通过
    5. NEXT   → Only proceed to next sheet after current sheet has 0 errors

After ALL sheets pass:
    6. VALIDATE → xlsx_validate.py → 退出码 0
    7. DELIVER  → Only deliver files that passed ALL validations
```

### Per-Sheet Check
```bash
python3 .lightest/skills/lightest-xlsx/scripts/xlsx_validate.py output.xlsx
python3 .lightest/skills/lightest-xlsx/scripts/xlsx_inspect.py output.xlsx --pretty
```
...
The validation step detects formula errors (#VALUE!, #DIV/0!, #REF!, #NAME?, #N/A, etc.) via cached values and forbidden-function scan. You MUST follow these rules strictly:

1. **ZERO TOLERANCE for errors**: If validation reports ANY errors, you MUST fix them before delivery.
...
2. **DO NOT assume errors will "auto-resolve"**:
   ...
   - ✅ CORRECT: Fix ALL errors until `xlsx_validate.py` exits 0

3. **Errors detected = Errors to fix** — only proceed when validate passes.

5. **Delivery gate**: Files that fail `xlsx_validate.py` CANNOT be delivered.
...
1. **READ** `.lightest/skills/lightest-xlsx/pivot-table.md` FIRST
2. Use **pandas `pivot_table`** + openpyxl 写 Summary sheet（静态汇总表，非 Excel 原生透视对象）
3. 完成后 `xlsx_validate.py`

**Quick Reference**:
```bash
python3 .lightest/skills/lightest-xlsx/scripts/xlsx_inspect.py data.xlsx --pretty
# 然后按 pivot-table.md 用 pandas 写 Summary + 可选图表
python3 .lightest/skills/lightest-xlsx/scripts/xlsx_validate.py output.xlsx
```

**⛔ 对用户说明**：若必须要 Excel 可拖拽的原生 PivotTable，需在 Excel 桌面端手动插入；LighTest 默认交付 **汇总表 + 图表**。

**Workflow**：先 openpyxl 完成数据 sheet → pandas 写 Summary → validate → 交付（勿再破坏性改写）
...
python3 .lightest/skills/lightest-xlsx/scripts/xlsx_validate.py output.xlsx
```
Non-zero exit = charts broken or other issues → MUST FIX.
