...
version: 1.0.0-ask
description: |
  Ship workflow (interactive variant): based on the original `/ship`, but with
  two mandatory AskUserQuestion checkpoints up front — base branch selection
  (main / develop / other) and base integration strategy (merge / rebase /
  skip). After those two questions, the rest of the flow (tests, diff review,
  VERSION / CHANGELOG bump, push, PR creation) runs automatically.

  Prefer this `ship-ask` over the standard `/ship` when:
  - The user says "ship-ask", "ship to develop", "ship with rebase",
    or otherwise signals intent to choose base branch / integration strategy
  - The repo has multiple plausible base branches (e.g. both `main` and
    `develop` exist) and the right one isn't obvious
  - The team's git workflow is rebase-based (linear history) — known or
    unconfirmed but worth checking before proceeding

  Use the regular `/ship` when full auto-progression is acceptable.
allowed-tools:
  - Bash
  - Read
  - Write
...
## Language localization (read first, applies to the whole skill)

Always respond in the language the user is speaking — including
**every `AskUserQuestion` `question`, `header`, `label`, and `description`
field**. The English text in this skill is a template, not a literal
output. Translate it to the user's language at call time.
...
Keep variable names, function names, file paths, CLI flags, commit
messages / trailers, and branch names verbatim regardless of language.
...
```bash
# --- ship-ask self-contained gstack shim (auto-installs only when gstack is absent) ---
# ship-ask reuses gstack's helper-CLI interface. When the full gstack toolkit is NOT
# installed, drop in minimal, local-only equivalents so the skill runs standalone.
# Real gstack, if present, is always preferred and left untouched. See bin/README.md.
_GSB="$HOME/.claude/skills/gstack/bin"
if [ ! -x "$_GSB/gstack-config" ]; then
  mkdir -p "$_GSB" "$HOME/.gstack"
  cat > "$HOME/.claude/skills/gstack/.ship-ask-shim" <<'MARK'
Minimal gstack shims auto-installed by the ship-ask skill so it runs without the
full gstack toolkit. Safe to delete: rm -rf ~/.claude/skills/gstack
If you install the real gstack, remove this directory first.
MARK
  cat > "$_GSB/gstack-config" <<'SH'
#!/usr/bin/env bash
_cf="$HOME/.gstack/config"; mkdir -p "$HOME/.gstack"; touch "$_cf"
case "$1" in
  get) v=$(grep -E "^$2=" "$_cf" 2>/dev/null | tail -1); [ -n "$v" ] || exit 1; printf '%s\n' "${v#*=}"; exit 0 ;;
  set) tmp=$(grep -vE "^$2=" "$_cf" 2>/dev/null); printf '%s\n' "$tmp" | grep -q . && printf '%s\n' "$tmp" > "$_cf" || : > "$_cf"; printf '%s=%s\n' "$2" "$3" >> "$_cf" ;;
esac
exit 0
SH
  cat > "$_GSB/gstack-slug" <<'SH'
#!/usr/bin/env bash
root=$(git rev-parse --show-toplevel 2>/dev/null)
printf 'SLUG=%s\n' "$(basename "${root:-$PWD}" | tr ' /' '__')"
SH
  cat > "$_GSB/gstack-repo-mode" <<'SH'
#!/usr/bin/env bash
printf 'REPO_MODE=solo\n'
SH
  cat > "$_GSB/gstack-diff-scope" <<'SH'
#!/usr/bin/env bash
base="${1:-HEAD}"; files=$(git diff --name-only "$base"...HEAD 2>/dev/null)
[ -z "$files" ] && files=$(git diff --name-only 2>/dev/null)
fe=false
printf '%s\n' "$files" | grep -qiE '\.(tsx?|jsx?|vue|svelte|css|scss|sass|less|html?|astro)$' && fe=true
printf 'SCOPE_FRONTEND=%s\n' "$fe"
SH
  cat > "$_GSB/gstack-learnings-log" <<'SH'
#!/usr/bin/env bash
root=$(git rev-parse --show-toplevel 2>/dev/null)
d="$HOME/.gstack/projects/$(basename "${root:-$PWD}" | tr ' /' '__')"; mkdir -p "$d"
[ -n "$1" ] && printf '%s\n' "$1" >> "$d/learnings.jsonl"; exit 0
SH
  cat > "$_GSB/gstack-learnings-search" <<'SH'
#!/usr/bin/env bash
root=$(git rev-parse --show-toplevel 2>/dev/null)
f="$HOME/.gstack/projects/$(basename "${root:-$PWD}" | tr ' /' '__')/learnings.jsonl"
n=3; while [ $# -gt 0 ]; do case "$1" in --limit) n="${2:-3}"; shift ;; esac; shift; done
[ -f "$f" ] && tail -n "$n" "$f"; exit 0
SH
  for _s in gstack-update-check gstack-timeline-log gstack-telemetry-log gstack-question-log gstack-question-preference gstack-review-log gstack-review-read gstack-specialist-stats gstack-team-init; do
    printf '#!/usr/bin/env bash\nexit 0\n' > "$_GSB/$_s"
  done
  chmod +x "$_GSB"/gstack-* 2>/dev/null || true
fi
# --- end ship-ask gstack shim ---
...
### ★ ship-ask extension: confirm base branch via AskUserQuestion

Do **not** use the auto-detected value as-is. Confirm with the user first.
This is ship-ask-specific behavior. Rationale: teams use varying conventions
(`main` / `develop` / `staging` / `release/*`), and auto-detection cannot
always pick the right one.

Build a candidate list (deduped, only existing branches):

```bash
# Fetch all remote branches and pick out common base names
git fetch --all --prune --quiet
git branch -r --format='%(refname:short)' | sed 's|origin/||' \
  | grep -E '^(main|master|develop|dev|staging|release/.*)$' | sort -u
```

Place the **auto-detected base** at the top of the list and present via
AskUserQuestion:

```
header: base branch
question: Which branch should the PR target as base?
options:
  - label: "<detected base> (Recommended)"
    description: "Default chosen by auto-detect"
  - label: "main"
  - label: "develop"
  - label: "Other (manual entry)"
    description: "Use Other to specify staging, release/*, etc."
```

If only two or fewer real candidates exist, narrow the question accordingly.
For values entered via `Other`, validate existence with
`git rev-parse --verify origin/<input>` before accepting; abort with an error
if the branch does not exist.

**Lock the chosen value as `<base>` for the rest of the workflow** — every
...
# Ship-Ask: Interactive Ship Workflow

You are running the `ship-ask` workflow. This is the `/ship` workflow with two
mandatory **AskUserQuestion** checkpoints up front (base branch + integration
strategy), and otherwise automated. Apart from those two questions, do NOT ask
for additional confirmations — run straight through and output the PR URL at
the end.

**Mandatory AskUserQuestion checkpoints (ship-ask only):**
1. Step 0 — base branch selection
2. Step 3 — base integration strategy (merge / rebase / skip)

**Otherwise stop only for:**
- On the base branch (abort)
- Merge/rebase conflicts that can't be auto-resolved (stop, show conflicts)
...
## Step 3: Integrate the base branch (BEFORE tests)

Fetch the base, then bring it into the feature branch so tests run against the
integrated state.

### ★ ship-ask extension: confirm integration strategy via AskUserQuestion

The original `/ship` is hardcoded to merge. `ship-ask` supports rebase users
too, so confirm with the user. **If the branch is unpushed
(`git rev-parse @{u} 2>/dev/null` fails), rebase is low-risk**. Rebasing an
already-pushed branch requires a force-push, so adjust which option carries
the `Recommended` tag based on push state.

Check push state first:

```bash
git fetch origin <base> --quiet
PUSHED=$(git rev-parse --abbrev-ref @{u} 2>/dev/null && echo yes || echo no)
```

Then present:

```
header: integrate base
question: How should <base> be integrated into the feature branch?
options:
  - label: "merge (Recommended)"
    description: "git merge origin/<base> --no-edit. Safe, no force-push needed"
  - label: "rebase"
    description: "git rebase origin/<base>. Linear history, but force-push required if already pushed"
  - label: "skip"
    description: "Skip integration (tests will run against an older base — risk)"
```

If the branch is already pushed and significantly behind base, **keep
`Recommended` on merge** — picking rebase here without realizing it can
clobber teammates' work via force-push. If the branch is unpushed, both merge
and rebase are safe; let team preference decide.

### Strategy execution

**If `merge` was chosen** (same behavior as original `/ship`):
...
- On conflict, run `git rebase --abort` to roll back cleanly, then report the
  state to the user and ask for manual resolution. (Unlike merge, an aborted
  rebase mid-state is fragile, so abort early.)
- After a successful rebase, if the branch was already pushed,
  `git push --force-with-lease` will be required. Do **not** push here — that
  happens in the push step (Step 17).
- Record a flag indicating "force-with-lease is required" so Step 17 can
  switch from a normal `git push` to `--force-with-lease`.

**If `skip` was chosen**:

Do nothing. Continue to Step 4. **Add a warning in the PR body** noting that
the latest base was not integrated (append it to the PR template in Step 19).

### Shared conflict handling

**For both merge and rebase**: if conflicts are simple (VERSION, schema.rb,
CHANGELOG ordering), attempt auto-resolution. If conflicts are complex or
ambiguous, **STOP** and show them to the user.
...
### ★ ship-ask extension: force-push after rebase

If Step 3 chose **rebase** and the branch was already pushed, use
`--force-with-lease` instead of a plain push:

```bash
git push --force-with-lease -u origin <branch-name>
```

Unlike `--force`, `--force-with-lease` aborts if the remote moved unexpectedly
(e.g. someone else pushed to the branch). This is the minimum compromise that
still respects `/ship`'s "Never force push" principle.

**Procedure**:

1. Check the flag set in Step 3 (whether rebase was selected).
2. If rebase + already-pushed → push with `--force-with-lease`.
3. Otherwise (merge / skip / unpushed) → normal `git push -u`.
4. On lease violation, STOP and tell the user: "The remote has moved. Run
   `git pull --rebase` manually, then re-run ship-ask."
...
- **Force push policy (ship-ask):** Use plain `git push` by default.
  Exception: if Step 3 selected **rebase** and the branch was already pushed,
  `git push --force-with-lease` is allowed. Bare `--force` (without lease) is
  never allowed.
