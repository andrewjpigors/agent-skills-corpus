...
- **Integrations** from `## Available integrations` — the Brazilian
  case-research connector (JusBrasil / Escavador / PJe, currently a
  placeholder — see `## Available integrations`) and Solve Intelligence each
  affect whether the triage can cite to case law, prior rulings, or prior art.
...
>
> When non-BR/non-US jurisdictions are genuinely in scope: "This analysis uses the BR (LPI) claim-charting framework by default. A product manufactured in China and sold in the EU needs CNIPA and EP analysis, not a BR or US claim chart. I can flag the issues a BR/US analysis surfaces, but the infringement and validity calls require [jurisdiction]-specific review."

### Workflow

- Accused product / process / method — described in technical detail.
- Identified patent(s) at issue (INPI registration number by default; USPTO number only for a US-designated asset).
- Claim chart for each independent claim: element-by-element mapping to the
  accused product (mechanics are jurisdiction-agnostic — see `litigation-legal:claim-chart`).
...
- **Invalidity defenses to consider (LPI arts. 8, 11-15, 18, 24, 46-57)** —
  ...
  10 — discoveries, scientific theories, mathematical methods, purely
  aesthetic creations, computer programs as such, business methods, and
  natural biological processes; art. 18 — contrary to morals/public order,
  substances from nuclear transformation, and living beings except
  ...
  tracks are the closest analogues; flag each, do not opine.
- **Unenforceability-adjacent defenses** — bad-faith prosecution flag (no
  ...
  licensee estoppel flag. Each is attorney-only.
- **Damages posture (LPI arts. 208-210)** — Brazilian patent damages use a
  **"greatest of" formula**, not a lost-profits-vs-reasonable-royalty
  election: the greatest of (a) the benefits the injured party would have
  earned but for the infringement, (b) the benefits actually earned by the
  infringer, or (c) the remuneration the infringer would have paid for a
  license (art. 210, incisos I-III). **There is no US-style treble-damages
  ...
  US-designated patent or US-territorial conduct is genuinely in scope, the
  US framework (lost profits vs. reasonable royalty under Georgia-Pacific,
  marking under §287, pre-suit notice, and willfulness under §284/*Halo*)
  applies instead — flag which regime governs before quantifying.
