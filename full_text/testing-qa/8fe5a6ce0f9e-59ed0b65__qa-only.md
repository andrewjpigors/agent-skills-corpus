...
echo '{"skill":"qa-only","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.mzstack/analytics/skill-usage.jsonl 2>/dev/null || true
...
| Mode | full | `--quick`, `--regression .mzstack/qa-reports/baseline.json` |
| Output dir | `.mzstack/qa-reports/` | `Output to /tmp/qa` |
...
1. **Project-scoped test plans:** Check `~/.mzstack/projects/` for recent `*-test-plan-*.md` files for this repo
   ```bash
   eval "$(~/.claude/skills/mzstack/bin/mzstack-slug 2>/dev/null)"
   ls -t ~/.mzstack/projects/$SLUG/*-test-plan-*.md 2>/dev/null | head -1
   ```
...
**Local:** `.mzstack/qa-reports/qa-report-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:** Write test outcome artifact for cross-session context:
```bash
eval "$(~/.claude/skills/mzstack/bin/mzstack-slug 2>/dev/null)" && mkdir -p ~/.mzstack/projects/$SLUG
```
Write to `~/.mzstack/projects/{slug}/{user}-{branch}-test-outcome-{datetime}.md`
