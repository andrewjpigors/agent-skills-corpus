...
Drive Sirin's AI-powered browser testing from external Codex sessions. Unlike Puppeteer/Playwright (scripted), Sirin tests are **goal-driven** — you describe what should happen, the LLM inside Sirin figures out how to click/type/verify.
...
2. **Sirin is registered as MCP server** in the caller's Codex Desktop config (or equivalent):
   ```json
   {
     "mcpServers": {
...
# auto_fix=true makes Sirin spawn a Codex session in the
...
**Two-tier diagnostic protocol.** When you (Tier 1, external Codex session)
...
|---|---|---|
| text + claude_cli | 2 | `Codex -p` is consistent; extras waste time |
...
| Codex Sonnet / Opus | High | Negative examples + scope usually sufficient |
| GPT-4o | Medium-high | Similar to Codex |
...
`auto_fix=true` spawns a Codex session only for `ui_bug` / `api_bug`.
...
❌ **Don't enable `auto_fix=true` on experimental tests** — Will waste Codex tokens trying to "fix" things that aren't bugs.
...
```
User: "Run the Agora Market login test and if it fails, let Codex fix it"
...
6. User sees: "Test failed — UI bug detected. Sirin auto-spawned a
   Codex session in the frontend repo to investigate."
