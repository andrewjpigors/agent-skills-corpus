...
- If blocked by another task, move the issue to `blocked` with `blockedByIssueIds`. For user input, create an interaction and use `in_review`; prose alone is not a blocker.
...
Headers: X-Paperclip-Run-Id: $PAPERCLIP_RUN_ID
{
  ...
  "completionReport": {
    ...
    "acceptanceCriteria": [{ "criterion": "Requested outcome", "evidence": "Concrete proof" }],
    "verification": ["Command or check that passed"],
    "deliverables": [{ "kind": "change", "title": "Implemented change", "ref": "workspace-relative path or URL" }],
    "remainingWork": "none"
  }
}
```
...
**Step 9 — Delegate if needed.** Create the minimum necessary subtasks with `POST /api/companies/{companyId}/issues`. Every delegated child must explicitly set exactly one `assigneeAgentId` or `assigneeUserId`, plus `parentId`, `goalId`, a concrete deliverable, and acceptance criteria. Never create an unassigned, speculative, monitoring, audit-of-audit, or recovery-of-recovery child. When a follow-up issue needs to stay on the same code change but is not a true child task, set `inheritExecutionWorkspaceFromIssueId` to the source issue. Set `billingCode` for cross-team work.
