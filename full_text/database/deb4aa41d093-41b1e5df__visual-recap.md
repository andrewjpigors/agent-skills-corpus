...
- For headless verification, run
  ...
  error.
- **Sharing a rendered HTML artifact (email/Slack/offline).** Do NOT use
  `plan local preview --out <file>.html` for this. That command is a raw SOURCE
  dump, not a render: it writes the literal block tags (`<RichText …>`,
  `<Diagram …>`, `<Table …>`) inside plain boxes, so a recipient sees code, not
  the recap. The blocks only render inside the hosted Plan UI fed by the bridge,
  and the bridge URL is machine-local (not shareable). So when the user asks for
  a shareable or offline HTML, hand-author a standalone `.html` that renders the
  content yourself: RichText → headings + paragraphs (convert the markdown),
  `Table` → real `<table>`, `Diagram` → its `data.html` wrapped in a scoped
  container (`#id .foo {…}`) with `data.css` applied and the `--wf-*` tokens
  (`--wf-border`, `--wf-surface`, `--wf-accent`, `--wf-accent-soft`, `--wf-muted`,
  `--wf-text`) defined in `:root`, `Endpoint` → a method-badge card with the
  request/response examples in `<pre>`, `DataModel`/`FileTree` → simple styled
  lists. Keep the MDX folder as the source of truth and regenerate the HTML from
  it. ALWAYS verify the result in a real browser before sharing — serve it over
  `http://localhost` and screenshot it (the browser-automation `navigate` tool
  may force `https://` and break `file://` URLs, so localhost is more reliable),
