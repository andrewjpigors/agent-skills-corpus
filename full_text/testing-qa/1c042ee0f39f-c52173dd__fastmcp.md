...
```bash
python ~/.myai/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.myai/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
- configure the server in `~/.myai/config.yaml` using the `native-mcp` skill, or
...
The server-building part may be correct while the MyAIOne config is not. Load the `native-mcp` skill and configure the server in `~/.myai/config.yaml`, then restart MyAIOne.
