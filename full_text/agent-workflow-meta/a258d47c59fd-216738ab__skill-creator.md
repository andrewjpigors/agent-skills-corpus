...
- Create a few test prompts and run an Ontheia agent with access to the skill on them
...
1. What should this skill enable Ontheia to do?
...
└── references/
    ...
    └── azure.md
```
Ontheia reads only the relevant reference file.
...
4. **Launch the viewer** with both qualitative outputs and quantitative data:
   ...
   **Ontheia (headless):** There is no display in the container — always use `--static <output_path>` to write a standalone HTML file instead of starting a server. Feedback will be downloaded as a `feedback.json` file when the user clicks "Submit All Reviews". After download, copy `feedback.json` into the workspace directory for the next iteration to pick up.
...
The description field in SKILL.md frontmatter is the primary mechanism that determines whether Ontheia invokes a skill. After creating or improving a skill, offer to optimize the description for better triggering accuracy.
...
Understanding the triggering mechanism helps design better eval queries. Skills appear in Ontheia's `available_skills` list with their name + description, and Ontheia decides whether to consult a skill based on that description. The important thing to know is that Ontheia only consults skills for tasks it can't easily handle on its own — simple, one-step queries like "read this PDF" may not trigger a skill even if the description matches perfectly, because Ontheia can handle them directly with basic tools. Complex, multi-step, or specialized queries reliably trigger skills when the description matches.

This means your eval queries should be substantive enough that Ontheia would actually benefit from consulting a skill. Simple queries like "read file X" are poor test cases — they won't trigger skills regardless of description quality.
...
- Run an Ontheia agent with access to the skill on test prompts
