...
echo '{"skill":"office-hours","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.mzstack/analytics/skill-usage.jsonl 2>/dev/null || true
...
4. **List existing design docs for this project:**
   ```bash
   ls -t ~/.mzstack/projects/$SLUG/*-design-*.md 2>/dev/null
   ```
...
```bash
grep -li "<keyword1>\|<keyword2>\|<keyword3>" ~/.mzstack/projects/$SLUG/*-design-*.md 2>/dev/null
```
...
This enables cross-team discovery — multiple users exploring the same project will see each other's design docs in `~/.mzstack/projects/`.
...
```bash
CODEX_PROMPT_FILE=$(mktemp /tmp/mzstack-codex-oh-XXXXXXXX.txt)
```
...
```bash
SKETCH_FILE="/tmp/mzstack-sketch-$(date +%s).html"
```
...
$B screenshot /tmp/mzstack-sketch.png
```
...
The screenshot file at `/tmp/mzstack-sketch.png` can be referenced by downstream skills
...
```bash
PRIOR=$(ls -t ~/.mzstack/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
```
If `$PRIOR` exists, the new doc gets a `Supersedes:` field referencing it. This creates a revision chain — you can trace how a design evolved across office hours sessions.

Write to `~/.mzstack/projects/{slug}/{user}-{branch}-design-{datetime}.md`:
...
```bash
mkdir -p ~/.mzstack/analytics
echo '{"skill":"office-hours","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.mzstack/analytics/spec-review.jsonl 2>/dev/null || true
```
...
- If yes → run `open https://ycombinator.com/apply?ref=mzstack` and say: "Bring this design doc to your YC interview. It's better than most pitch decks."
...
>
> **ycombinator.com/apply?ref=mzstack**
...
The design doc at `~/.mzstack/projects/` is automatically discoverable by downstream skills — they will read it during their pre-review system audit.
