...
**Characters as shapes (glyph outlines).** A character can be converted into its font outline as a real shape, positioned where it renders:

```csharp
var word = new VText(-100, 0, "Go", 120);
var circle = new VCircle(60, 50, 60);
var anim = new Animator();
// Recommended: the (VText, index, to, dur) overload keeps the whole word visible and
// blanks the character to a space exactly when its morph begins — so it reads as the
// letter itself transforming, starting from the letter's own position.
anim.AddToAnimations(new TransformAnimation(word, 0, circle, 2.0)); // 'G' morphs into a circle
anim.Animate();

// Lower-level building blocks:
var gShape = word[0];               // lift glyph 'G' AND blank it immediately (eager)
var oShape = word.ToCharShape(1);   // non-mutating: outline of 'o', text unchanged
var sel    = word.LiftChars(0, 2);  // lift a selection into one VGroup
```

| Member | Returns | Mutates text? | Notes |
|--------|---------|---------------|-------|
| `ToCharShape(int i)` | Shape? | no | Outline of char i. Single-contour → closed VPolyline; glyphs with holes (O, A, B) → VGroup of contours |
| `LiftChar(int i)` | Shape? | yes (→ space) | ToCharShape + replaces char with a space |
| `this[int i]` (indexer) | Shape? | yes (→ space) | Ergonomic alias for LiftChar; `text[0]` |
| `LiftChars(int start, int count)` | VGroup? | yes (→ spaces) | Lift a run of characters as one group |

Returns `null` for whitespace, an out-of-range index, or when no glyph provider is set (the desktop app supplies font outlines; the library itself is font-free).
...
| TransformAnimation | `(Shape from, Shape to, double duration)` | Morphs one shape into another (e.g. line → circle); source shown first, destination revealed at the end |
| TransformAnimation | `(VText text, int charIndex, Shape to, double duration)` | Morphs one character of a VText into a shape. The word stays visible; the character blanks to a space exactly when its morph starts, so it reads as the letter itself transforming |
