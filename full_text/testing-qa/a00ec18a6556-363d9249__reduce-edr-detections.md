...
description: Use when reducing VirusTotal/EDR detection rates on compiled binaries — A/B testing + PE structural analysis for ML classifier signals, plus a bisection methodology for locating named YARA-style signatures
...
## Localizing a Persistent Named Signature (Bisection Method)

Use this when **one specific named vendor signature** keeps firing on 100% of your samples after Phases 1–4 have shifted the ML/heuristic cluster. Vendor-specific named labels (e.g., `Trojan/Win.Sliver.R774471`, `WinGo/SomeFamily.A`) that are stable across distinct file hashes are YARA-style byte/string rules — Phase 3 statistical changes don't move them and never will. You have to find the literal feature the rule keys on and remove it at the source.

This is the YARA-fix track of [Core Principle 1](#core-principles) (Triage detection type before choosing remediation). The Phases 1–5 flow handles the ML side; this section handles the YARA side.

### When to use
- A single named signature is at 100/100% across your population while other engines fluctuate
- The label is reproducible across distinct SHA256s and per-build polymorphism
- Phase 3 hypotheses (entropy, sections, import pruning, signing diversification) don't move it
- You'd rather characterize the rule once than chase it through five more experiments

### Procedure

1. **Pick a clean-signal sample.** Prefer one whose only detection is the named signature, or that signature plus 1–2 generic engines. Less noise means less ambiguity. Note the SHA256.
2. **Tail-zero sweep.** Generate variants where progressively larger tail fractions of the file are zeroed (1/16, 1/8, 1/4, 1/2, 3/4, 7/8). Upload all to VT. The smallest cut where the named signature **stops** firing bounds the signal region from below; the largest cut where it **still** fires bounds it from above.
3. **Bisect inside the region.** Split the bounded region into 4 quarters and zero each independently. Whichever quarter(s) make the signature drop is where the signal lives. Repeat — split that quarter into halves, then thirds — until you're at sub-100KB resolution.
4. **Catalog the localized content.** Extract ASCII strings (`strings -n 12`) and byte patterns from the surviving region. Look for tokens that are:
   - **Tool-specific** (package paths from your project / its dependencies, distinctive function names, embedded resource markers) vs generic Go/C runtime
   - **Stable across builds** (not affected by your per-build polymorphism)
   - **High cardinality** (a string with thousands of instances is a better YARA target than a string with three)
5. **String-family substitution.** For each candidate family, globally replace **every** occurrence in the file with same-length junk and upload. The family whose elimination flips the signature is the YARA hook. (Note: families can be additive — sometimes only the union of two-or-three families flips it.)
6. **Structural variant probe.** Once you have a candidate string, replace its **prefix only**, then **middle only**, then **suffix only** — each with same-length filler bytes — to identify which parts are load-bearing. This distinguishes:
   - Literal substring matches (`/core` must be exactly there)
   - Regex/shape matches (`internal/<word>/<word>/internal/` — any words work, but the structure is fixed)
   - Token-set matches (`engine`, `compiler`, `objx` all need to be present somewhere)
7. **Apply the fix at the source and verify on fresh builds.** Post-hoc byte substitution can miss interactions with per-build polymorphism. Once you've identified the YARA hook, change the code that generates the offending string, then build a fresh N=20 batch and confirm the named signature stays at 0/20.
...
- **VT API budget.** Free tier ≈ 4 req/min, 250/day. A complete bisection from a 16MB file down to a 256KB localized region typically takes 5 rounds × 4–7 uploads = 25–40 uploads, ~30–45 minutes elapsed (the per-round VT analysis wait dominates).
- **One named signature at a time.** If multiple engines fire, focus on the most vendor-specific named one (e.g., ESET family labels > generic `MalwareX-gen`). Generic heuristic labels often disappear on their own when the named signature is killed because they were shadowing the same content.
- **Watch for shadowed signals.** After you kill the named signature, a previously-invisible engine may jump from 0% to 80%. That's not a regression — it's a separate rule that was being masked. Treat it as the next target.

### Worked example

ESET `WinGo/WasmForge.A` (publicly documented in wasmforge ≥v1.0.4):

- Tail-zero placed the signal in the .rdata section (file bytes ~4.2MB–8.4MB)
- Quarter-bisection of .rdata showed signal across multiple quarters — initial hypothesis was a count/score rule
- String-family substitution isolated `internal/azure/core` (2,997 instances) as load-bearing; replacing it with same-length junk flipped the verdict to clean. Other dense families (`engine/compiler` ×1,517, `/internal/engine` ×1,714, `objx` ×554) did **not** flip the verdict when removed individually
- Structural variants pinned the rule to the `internal/` PREFIX, NOT the `/core` suffix, NOT the middle word. `xxxxxxxx/azure/core` → CLEAN; `internal/azure/yyyy` → still HIT; `internal/zzzzz/core` → still HIT
- Root cause: the hardcoded `internal/<word>/core` format for the renamed wazero fork's module path concatenated with wazero's own `/internal/engine/...` subdirectory tree to produce a doubled `internal/.../internal/...` shape in every type-name string baked into reflection metadata. The doubled `internal/` is unusual in legitimate Go binaries.
- Fix: per-build random pick from a 13-shape pool, none of which start with `internal/`. Result: 25/25 baseline → **0/20** ESET hits across the next 20-sample batch, with no other engine compensating

The total investigation budget was ~40 VT uploads, ~3 hours elapsed (most of it waiting on VT analysis between rounds).
...
- **Bisection for stuck named signatures** — when a single vendor-specific YARA-style label sits at 100% across all samples and Phase 3 changes don't move it, localize the rule's trigger bytes by progressive byte-zeroing + same-length string substitution (see *Localizing a Persistent Named Signature*). Verified: identified the structural pattern behind ESET `WinGo/WasmForge.A` (a doubled `internal/.../internal/` shape from concatenating the module-path rename prefix with wazero's own internal subtree) — fix dropped 25/25 hits to 0/20.
