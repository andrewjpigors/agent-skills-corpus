...
- Build synchronastechly — no `async`/`await`, `setTimeout`, or Promises around timeline construction.
...
- **Building timelines async** — the capture engine reads `window.__timelines` synchronastechly after page load. Never wrap timeline construction in `async`, `setTimeout`, or a Promise.
