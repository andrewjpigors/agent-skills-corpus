...
テスト実行には2つのモードがある。Phase 0 で `execution_mode` を選択する。

### MCP Direct Mode（推奨、デフォルト）

各テスターエージェント（Task サブエージェント）がプラグイン定義の **専用 Playwright MCP インスタンス** を直接操作する。
計画と実行を一括で行うため、AI が状況に応じてテスト操作を適応的に変更できる。

- 各エージェントに独立ブラウザインスタンスを割り当て（`--isolated` + evaluate で認証復元）
- 最大5+1エージェントが並列でブラウザ操作
- 詳細は `references/mcp_direct_guide.md` 参照

### CLI Mode（フォールバック）

従来方式。エージェントがテスト計画のみ生成し、メインエージェントが Node.js スクリプトにコンパイルして CLI で並列実行する。
MCP Direct Mode が使用できない環境で使用。

## アーキテクチャ

### MCP Direct Mode（デフォルト）
...
│  → URL、実行モード、コンテキスト、エージェント選択、予算        │
│                                                                │
│  Phase 1: Recon (Main Agent, pw-recon)                         │
│  → BFS 巡回 → Interactive Discovery → Workflow Map 構築        │
│                                                                │
│  Phase 1b: Codebase Analysis (Task, optional)                  │
│  → コード/仕様分析 → ルート、バリデーション抽出                  │
│                                                                │
│  Phase 2+3a: Workflow Direct (Task × 1, pw-workflow)           │
│  ┌──────────┐                                                  │
│  │workflow  │ ← 計画 + 実行を一括                              │
│  │(opus)    │ → created_data 生成                              │
│  └────┬─────┘                                                  │
│       ▼                                                        │
│  Phase 2+3b: Swarm Direct (Task × 5, 並列)                     │
│  ┌──────────┬──────────┬──────────┬──────────┬──────────┐      │
│  │explorer  │naive-    │chaos-    │security- │spec-     │      │
│  │(sonnet)  │user      │input     │hunter    │aware     │      │
│  │pw-       │(sonnet)  │(sonnet)  │(opus)    │(opus)    │      │
│  │agent-1   │pw-       │pw-       │pw-       │pw-       │      │
...
│   └── ... (各エージェント)
├── shared/
│   ├── issue_registry.md        ← CLI Mode: 全エージェント共有
│   ├── issue_registry_{agent}.md ← MCP Direct: エージェント別（Phase 4 で統合）
│   ├── created_data.json        ← Phase 3a / 2+3a で生成（JSON 正本）
│   └── created_data.md          ← Phase 3a で生成（ビュー用、CLI Mode のみ）
└── screenshots/
    ├── recon-P001.png
    └── ...
```
...
3. AskUserQuestion で以下を確認:
   - **実行モード**: `mcp-direct`（推奨、デフォルト）/ `cli`（フォールバック）
...
### Phase 1: Recon（メインエージェント、pw-recon）

**目的**: ターゲットアプリの全ページ構造を収集し、インタラクティブ要素をカタログ化する。

**重要**: この Phase はメインエージェントが直接 Playwright MCP を使って実行する。Task に委譲しない。

**ツールプレフィックス**（実行モードにより異なる）:
| 実行モード | プレフィックス | 例 |
|-----------|-------------|-----|
| MCP Direct | `mcp__plugin_claude-monkey-test_pw-recon__` | `mcp__plugin_claude-monkey-test_pw-recon__browser_navigate` |
| CLI | `mcp__playwright__` | `mcp__playwright__browser_navigate` |

以下のコード例では `{pw}` を実行モードに応じたプレフィックスに読み替えること。
...
2. **初期ページ**:
   ```
   {pw}browser_navigate(url=target_url)
   {pw}browser_snapshot()
   ```
...
`execution_mode` に応じて以下のいずれかのフローを実行する。

---

### [MCP Direct Mode] Phase 2+3a: Workflow Direct（Task × 1、pw-workflow）

**目的**: tester-workflow エージェントが計画と実行を一括で行い、テストデータを生成する。

**背景**: tester-workflow が作成するテストデータ（`created_data.json`）を後続5エージェントが参照するため、先行実行が必要。
...
Task(
  ...
  prompt="agents/swarm/tester-workflow.md の指示に従い、テストプランを生成し、MCP Direct Mode で直接実行してください。

## Playwright MCP 操作

あなたには専用の Playwright MCP インスタンスが割り当てられています。
以下のプレフィックスでツールを呼び出してください:

プレフィックス: `mcp__plugin_claude-monkey-test_pw-workflow__`

例:
- browser_navigate: mcp__plugin_claude-monkey-test_pw-workflow__browser_navigate(url=...)
- browser_snapshot: mcp__plugin_claude-monkey-test_pw-workflow__browser_snapshot()
- browser_click: mcp__plugin_claude-monkey-test_pw-workflow__browser_click(ref=...)

## 認証状態の復元

認証が必要な場合（auth.required === true）、テスト開始前に以下を実行して認証状態を復元してください:

1. ターゲット URL に navigate する
2. 以下の JavaScript で Cookie/localStorage を復元する:

```javascript
{prefix}browser_evaluate({
  expression: `
    const authState = {AUTH_STATE_JSON};
    for (const cookie of authState.cookies || []) {
      document.cookie = cookie.name + '=' + cookie.value + '; path=' + (cookie.path || '/') + '; domain=' + (cookie.domain || '');
    }
    for (const [key, value] of Object.entries(authState.origins?.[0]?.localStorage || [])) {
...
3. ページをリロード（再度 navigate）して認証状態を適用する

※ `{AUTH_STATE_JSON}` はオーケストレーターが `.work/monkey-test/auth_storage_state.json` の内容を埋め込む

## 入力データ
Recon データ: [01_recon_data.md の内容]
Spec Context: [01b_spec_context.md の内容（あれば）]
Auth State: [auth_storage_state.json の内容（auth.required の場合）]
アクション予算: 45

## 出力
1. テストプラン: .work/monkey-test/02_plans/tester-workflow.md
2. 実行ログ: .work/monkey-test/03_execution/tester-workflow.md
3. 作成データ: .work/monkey-test/shared/created_data.json
4. Issue Registry: .work/monkey-test/shared/issue_registry_tester-workflow.md

[references/test_plan_schema.md]
[references/action_catalog.md]
[references/mcp_direct_guide.md]",
  model="opus"
)
```

**Done 条件**: `03_execution/tester-workflow.md` が生成され、`created_data.json` が存在する。

---

### [MCP Direct Mode] Phase 2+3b: Swarm Direct（Task × 5、並列）

**目的**: 残り5エージェントが計画と実行を一括で並列に行う。

**手順**:

1. 5エージェントを **並列** で Task 起動（1つのメッセージで5つの Task 呼び出し）:

```
Task(
  subagent_type="general-purpose",
  name="monkey-tester-explorer",
  prompt="agents/swarm/tester-explorer.md の指示に従い、テストプランを生成し、MCP Direct Mode で直接実行してください。

## Playwright MCP 操作
プレフィックス: `mcp__plugin_claude-monkey-test_pw-agent-1__`

## 認証状態の復元
（Phase 2+3a と同じ手順。auth.required の場合、テスト開始前に Auth State JSON で Cookie/localStorage を復元すること）

## 入力データ
Recon データ: [01_recon_data.md（更新済み）]
Spec Context: [01b_spec_context.md（あれば）]
Created Data: [shared/created_data.json]
Issue Registry: [shared/issue_registry_tester-workflow.md（先行結果参照用）]
Auth State: [auth_storage_state.json の内容（auth.required の場合）]
アクション予算: 30

## 出力
1. テストプラン: .work/monkey-test/02_plans/tester-explorer.md
2. 実行ログ: .work/monkey-test/03_execution/tester-explorer.md
3. Issue Registry: .work/monkey-test/shared/issue_registry_{自分のエージェント名}.md
...
# 同様に tester-naive-user (pw-agent-2), tester-chaos-input (pw-agent-3),
# tester-security-hunter (pw-agent-4), tester-spec-aware (pw-agent-5) を並列起動
```

2. **エージェントとインスタンスの対応**:

| エージェント | Playwright インスタンス | モデル |
|------------|---------------------|--------|
| tester-explorer | pw-agent-1 | sonnet |
| tester-naive-user | pw-agent-2 | sonnet |
| tester-chaos-input | pw-agent-3 | sonnet |
| tester-security-hunter | pw-agent-4 | opus |
| tester-spec-aware | pw-agent-5 | opus |

3. **全エージェント共通**の prompt に以下を含める:
   - エージェント定義ファイル（agents/swarm/*.md）の全文
   - `01_recon_data.md`（Phase 2+3a で更新済み）
   - `01b_spec_context.md`（存在する場合）
   - `auth_storage_state.json`（auth.required の場合。認証復元手順は Phase 2+3a 参照）
   - `shared/created_data.json`
   - `shared/issue_registry.md`
   - `references/test_plan_schema.md`, `references/action_catalog.md`, `references/mcp_direct_guide.md`

**Done 条件**: 全5エージェントの実行ログが `03_execution/` に生成されている。

**Phase 4 へ進む**（CLI Mode の Phase 2a〜3b をスキップ）。

---

### [CLI Mode] Phase 2a: Workflow Planning（Task × 1）
...
### [CLI Mode] Phase 3a: Workflow Execution（メインエージェント、Playwright MCP）
...
### [CLI Mode] Phase 2b: Planning Swarm（Task × 5、並列）
...
### [CLI Mode] Phase 3b: CLI 並列実行（メインエージェント + Bash）
...
### Playwright MCP（Phase 1, 3）

**MCP Direct Mode**: 各エージェントが割り当てられたプレフィックスの Playwright MCP ツールを使用。
**CLI Mode**: メインエージェントが `mcp__playwright__*` を使用（Phase 1, 3a）。

#### 利用可能ツール（Phase 1, 3 のみ）
...
| `references/mcp_direct_guide.md` | MCP Direct Mode リファレンス（インスタンス割り当て、命名規則） |
