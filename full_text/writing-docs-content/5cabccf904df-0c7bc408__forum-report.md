---
name: forum-report
description: Compose a pre-designed analytical report against a scraped Discourse corpus — pain-points audit, community health, decision brief, and (for software-platform forums) release-quality audit or customization-ceiling audit. Produces a multi-section Markdown artifact at `<data-dir>/reports/<type>/NN-<date>-<slug>.md`, built from stats probes with optional per-topic graph drill-downs. Use when the user wants a **report** (thematic synthesis, multi-section deliverable) rather than a one-shot answer. Triggers: "forum audit", "pain-points report", "community health", "analyze the forum", "summarize the forum", "report on X", "top themes", "top pain points", "decision brief", "executive summary of the forum". (Also: "release-quality audit", "customization audit" for software-platform corpora.) For single-question asks (route mode + knobs for one question), use `/query-advisor` instead.
---
...
If a user starts with a report-shape phrase but really wants one question answered, confirm via `AskUserQuestion` before committing.

## The flow

1. **Parse invocation + resolve data dir** — same rules as `/query-advisor`: CLI arg ≻ `DISCOURSE_DATA_DIR` ≻ `AskUserQuestion`.
...
3. **Pick report type** via `AskUserQuestion` — from the catalog below, plus "Custom" for an ad-hoc shape.
...
7. **Gate on drill-down cost** — for L2/L3, enumerate the top-N candidates the triggers surfaced and ask via `AskUserQuestion` before spending on graph queries.
8. **Delegate execution** to a Sonnet subagent (inherits `/query-advisor`'s two-tier delegation pattern) for anything above L1.
...
If the user picks a conditional type against a forum where it doesn't apply, stop and surface the mismatch via `AskUserQuestion` — offer the closest universal type + the "Custom" escape.
...
1. **Topics per version tag** — pattern depends on corpus version-tag convention. `QUERY-GUIDE.md` §4.4 lists them. If tags are year-based (`2024.06`, `2025.06`), filter `LIKE '20%'`; if semver (`v1.2.3`), filter `LIKE 'v%' OR LIKE '_.%.%'`; etc. Also watch for Unicode-quirk version separators (e.g., `U+2024` one-dot-leader in some Discourse instances — see `/query-advisor` §"SQL invariants").
   ```sql
   SELECT tag_name, COUNT(*) AS topics
   FROM topic_tags WHERE tag_name <corpus-specific version filter>
   GROUP BY tag_name ORDER BY 1 DESC
   ```
...
3. **Delegate** (L2+) to a Sonnet subagent with a self-contained prompt: the exact per-topic commands, persistence path, narrative outline, return contract. Inherits `/query-advisor`'s two-tier delegation pattern — same skeleton prompt shape.
...
- **Main conversation (Opus)** — pick report type, depth, scope, persistence path, gate drill-down cost. `AskUserQuestion` only works here.
- **Sonnet subagent** — run skeleton probes + drill-downs, compose narrative, persist, return condensed echo.
...
- **Two-tier delegation** — Opus routes, Sonnet executes.
...
- **SQL invariants** — `tag_name` not `tag`, `U+2024` in version tags, `role='system'` ≠ general bot filter, etc. `/query-advisor` §"SQL invariants".
