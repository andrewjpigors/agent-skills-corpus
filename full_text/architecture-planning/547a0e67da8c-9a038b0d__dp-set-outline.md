...
| 七、本章写作蓝图 | dp-chapter-draft | 草稿预审 | 大纲要点、揭示计划、概念/角色要点（含角色风格档案摘要）、前情衔接（含前序摘要全文与 `[TBD]` 前缀）、伏笔指令、风格指令、调优指令、成人场景指令、张力定位、优先级、铁律提醒、语域转换提醒、时间线执行要求（非顺叙时） |
...
5. **前序章节摘要符号链接清理**：
   - 若修订导致某章被删除或重编号，遍历所有后续章节文件夹（含已写和未写），移除指向已删除/已重命名 `docs/dreampowers/timeline/summary-*.md` 的符号链接，避免悬空
   - 重命名场景下，同步创建指向新文件名的符号链接
