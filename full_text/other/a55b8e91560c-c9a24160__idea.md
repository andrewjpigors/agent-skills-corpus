...
- Keep ordinary subtask completions concise. When the idea stage actually finishes a meaningful deliverable such as a selected idea package, a rejected-ideas summary, or a route-shaping ideation checkpoint, upgrade to a richer `artifact.record(payload={'kind': 'milestone'}, reply_mode='threaded', ...)` report.
...
When the stage reaches a route-shaping outcome, notify the user through `artifact.record(...)` deliberately:
