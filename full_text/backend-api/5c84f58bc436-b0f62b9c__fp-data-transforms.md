...
Why functional is better here: The predicate (isActive) is separated from the iteration logic. can reuse, test, and compose predicates independently.
...
Why functional is better here: Each step in the chain has a single responsibility. can read the transformation as a series of steps: "filter active, filter premium, get names, sort." Adding or removing a step is trivial.
...
Why functional is better here: The pick utility is reusable across your codebase. Type safety ensures can only pick keys that exist.
