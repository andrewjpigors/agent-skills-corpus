...
All three live in `$CLAWBOT_HOME/skills/devops/watchers/scripts/` once the skill is installed. Each reads `WATCHER_STATE_DIR` (defaults to `$CLAWBOT_HOME/watcher-state/`) for its state file, keyed by the `--name` argument.
...
```bash
python $CLAWBOT_HOME/skills/devops/watchers/scripts/watch_rss.py \
  --name hn --url https://news.ycombinator.com/rss --max 5
```

Watch a GitHub repo (set `GITHUB_TOKEN` in `~/.clawbot/.env` to avoid the 60 req/hr anonymous rate limit):

```bash
python $CLAWBOT_HOME/skills/devops/watchers/scripts/watch_github.py \
  --name clawbot-issues --repo Aayushsoam/clawbot-agent --scope issues
```

Poll an arbitrary JSON API:

```bash
python $CLAWBOT_HOME/skills/devops/watchers/scripts/watch_http_json.py \
...
Every watcher writes `$CLAWBOT_HOME/watcher-state/<name>.json`. Inspect:

```bash
cat $CLAWBOT_HOME/watcher-state/hn.json
```

Force a replay (next run treated as first poll):

```bash
rm $CLAWBOT_HOME/watcher-state/hn.json
```
