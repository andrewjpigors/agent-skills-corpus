...
description: Use when building, modifying, or debugging Adobe Photoshop UXP plugins or Lightroom Classic plugins — manifest.json/Info.lua setup, panel and dialog UI with Spectrum sp-* widgets, batchPlay action descriptors, executeAsModal, imaging.getPixels pixel access, UXP file I/O and settings persistence (storage.localFileSystem, session tokens, secureStorage), .psjs scripts, LrView data binding, LrDevelopController develop-slider observation, export/publish services, LrCatalog/LrPhoto metadata, develop presets, memory management in long-lived LrC processes, hybrid C++ native bridges, and UXP HTML/CSS/Canvas limits
license: MIT
metadata:
  version: "1.2.0"
---
...
API baseline: Photoshop API changelog 27.4 (Feb 2026) / UXP 9.2.0 / Manifest v5 — Adobe versions the plugin API by changelog entry, not app release, so this stays current for later 27.x builds until the API changelog advances. (PS 27.8 began replatforming UXP plugin rendering — no documented API changes.) LrC SDK 15.3 (latest published) / Lua 5.1.5. Last verified: 2026-07.

## When to Use

Use when:
- Writing or editing a UXP panel/command plugin (`manifest.json`, `entrypoints.setup`, Spectrum `sp-*` widgets) or a one-off `.psjs` script
...
- Reading/writing files or persisting settings and secrets from UXP (`storage.localFileSystem`, tokens, `secureStorage`)
...
| Document DOM | Full: 36 Document props + 29 methods, 28 Layer props + 55 methods (38 filter + 17 other) | Catalog-centric: `LrCatalog`, `LrPhoto` |
...
| File I/O | `uxp.storage.localFileSystem` — permission-gated pickers, plugin folders, tokens | Full Lua `io` library |
| Plugin types | Panel, Command, `.psjs` script | Export, Publish, Metadata, Filter, Web Gallery, Library menu |
...
- Lossless alternative (v7.0+, PS-only): `new ImageBlob(buffer, { width, height, components, componentSize, pixelFormat, colorSpace, hasAlpha })` -> `img.src = URL.createObjectURL(blob)` — skips the JPEG round-trip; accepts a `PhotoshopImageData` directly as options.
...
```
Unlocks: `box-shadow`, `transform-origin`, `scaleX`, `scaleY`, `translate`. Also accepts a granular array (`["boxShadow", "transformFunctions", "transformProperties"]`). Without the flag these properties are silently ignored — no error, no warning — **except** that `"enableSWCSupport": true` implicitly enables CSSNextSupport.

### UXP: localStorage and sessionStorage
Both are available as global storage APIs — contrary to some older references listing them as unavailable. However, they are **not** available inside WebView when loading local content. Never store secrets in them — use `storage.secureStorage` (encrypted; `getItem` returns a `Uint8Array`; treat it as a re-obtainable cache, not durable storage).

### UXP: DOM file APIs take Entries; batchPlay takes session tokens
`document.saveAs.*()` and `app.open()` take UXP storage **File entries** — never path strings. batchPlay descriptors take neither: convert with `fs.createSessionToken(entry)` and pass it as `_path`. For ask-once access across restarts, store a `createPersistentToken(entry)` and re-prompt when redemption fails.
...
### LrC: Observing "active photo changed"
`addAdjustmentChangeObserver` covers slider changes only. In a floating dialog, pass `selectionChangeObserver` (and `sourceChangeObserver`) to `LrDialogs.presentFloatingDialog` — LrC calls it when the selected photos/videos change; then read `catalog:getTargetPhoto()`. Outside a floating dialog there is no event — poll `catalog:getTargetPhoto()` in a `LrTasks.startAsyncTask` loop with coalescing.
...
| `sp-action-group` | `compact`, `justified`, `quiet` — SWC-originated, needs `enableSWCSupport` |
| `sp-action-menu` | `placement`, `quiet`, `change` — menu with `sp-menu-item` children; SWC-originated, needs `enableSWCSupport` |
| `sp-body` | Typography body text |
| `sp-button` | `variant` (cta default/primary/secondary/warning/overBackground), `quiet` (not with cta), `disabled`, `click` |
| `sp-button-group` | Container for button sets — SWC-originated, needs `enableSWCSupport` |
...
| `sp-icon` | `name`, `size` (xxs/xs/s/m/l/xl/xxl) — **36 built-in icons** |
| `sp-label` | Typography label text |
| `sp-link` | `href`, `quiet` |
| `sp-menu` | Wraps `sp-menu-item` and `sp-menu-divider` |
| `sp-menu-item` | `selected`, `disabled` |
| `sp-progressbar` | `max`, `value`, `value-label`, `show-value`, `size` — label via `<sp-label slot="label">` |
| `sp-radio` | `checked`, `disabled` — listen for `change` on the group |
| `sp-radio-group` | `column`, `change` (read `evt.target.value`) |
| `sp-slider` | `min`, `max`, `value`, `disabled`, `variant="filled"`, `show-value`, `input`/`change` — no `step` |
| `sp-textfield` | `placeholder`, `quiet`, `type`, `disabled`, `input`/`change` |
| `sp-textarea` | `placeholder`, `quiet`, `disabled`, `input`/`change` |

**SWC (Spectrum Web Components):** npm-wrapped packages for advanced Spectrum UI. Requires `enableSWCSupport: true` in manifest (which implicitly enables CSSNextSupport). Locked to v0.37.0 in UXP 8.0. Provides components not available as `sp-*` (picker, number-field, toast, tabs, accordion, table, etc.).

### CSS support summary

**Supported:** flexbox (all properties), box model (margin, padding, border, width/height, overflow), typography (font-family, font-size, font-weight, line-height, text-align, color, text-decoration, white-space, word-wrap), backgrounds (background-color, background-image incl. `linear-gradient`/`radial-gradient` since v2.0 — v8.1 aligned gradient start/end syntax with the web), opacity, `calc()`, CSS custom properties (`--var`), pseudo-classes (`:hover`, `:nth-child`, `:first-child`, …) and `::before`/`::after` (v3.0+).

**With CSSNextSupport flag:** `box-shadow`, `transform-origin`, `scaleX`, `scaleY`, `translate`.

**NOT supported:** Grid, `transition`, `animation`/`@keyframes`, `float`, `text-transform`, `font` shorthand, `position: sticky`.

**Theme CSS variables** (auto-update on theme change):
`--uxp-host-background-color`, `--uxp-host-text-color`, `--uxp-host-text-color-secondary`, `--uxp-host-border-color`, `--uxp-host-link-text-color`, `--uxp-host-link-hover-text-color`, `--uxp-host-label-text-color`, `--uxp-host-widget-hover-background-color`, `--uxp-host-widget-hover-text-color`, `--uxp-host-widget-hover-border-color`. Font sizes: `--uxp-host-font-size`, `--uxp-host-font-size-smaller`, `--uxp-host-font-size-larger`. Also supports `@media (prefers-color-scheme: dark|light)`. Spectrum `sp-*` widgets auto-theme without extra CSS.
...
entrypoints.setup({
  ...
  panels: {
    myPanel: {
      ...
      destroy() { /* cleanup */ },
      menuItems: [
        { id: "reload", label: "Reload", enabled: true, checked: false }
        // submenu: [...] for nesting; ids must be unique per panel
      ],
      invokeMenu(menuId) {           // flyout clicks arrive here, not per-item
        if (menuId === "reload") refresh();
      }
    }
  },
  commands: {
...
- `enableMenuRecording: true` — makes the plugin's *menu commands* recordable as Action steps ("Plugin Menu Command"); the separate `recordAction` API needs no flag

### File I/O and persistence
```javascript
const { storage } = require('uxp');
const fs = storage.localFileSystem;
const file = await fs.getFileForOpening({ types: storage.fileTypes.images }); // read-only
const out  = await fs.getFileForSaving("out.png", { types: ["png"] });        // null if cancelled
const data = await fs.getDataFolder();                                        // persistent plugin storage
await out.write(buffer, { format: storage.formats.binary });

// batchPlay needs a session token, not an entry or path:
const token = fs.createSessionToken(file);
await action.batchPlay([{ _obj: "open", target: { _path: token, _kind: "local" } }], {});
```
Where to persist: prefs → `localStorage` or a `plugin-data:/` file; secrets → `storage.secureStorage` (encrypted; `getItem` returns `Uint8Array`; can be lost — treat as cache); remembered file access → `createPersistentToken`. Full API in `ps-uxp-sdk.md` (File I/O & Storage).
...
### Info.lua manifest fields (full 23-field table with types/SDK versions in lrc-sdk.md §1)

- **Required:** `LrSdkVersion`, `LrToolkitIdentifier` (reverse-domain ID), `LrPluginName` (2.0+)
- **Menus:** `LrLibraryMenuItems` (Library > Plug-in Extras), `LrExportMenuItems` (File > Plug-in Extras), `LrHelpMenuItems`
- **Providers:** `LrExportServiceProvider` (export/publish), `LrExportFilterProvider`, `LrMetadataProvider`, `LrMetadataTagsetFactory`
- **Lifecycle:** `LrInitPlugin`, `LrForceInitPlugin` (4.0 — init at startup), `LrEnablePlugin`/`LrDisablePlugin` (3.0), `LrShutdownPlugin` (3.0), `LrShutdownApp` (4.0 — LrDialogs NOT available)
- **Plugin Manager:** `LrPluginInfoProvider`, `LrPluginInfoUrl`; **Misc:** `LrSdkMinimumVersion`, `URLHandler` (4.0 — script file, string), `LrAlsoUseBuiltInTranslations`, `LrLimitNumberOfTempRenditions` (Boolean throttle — plugin must delete each temp rendition), `VERSION` `{ major, minor, revision (numbers), build (string), display (string?) }`
...
### Develop parameters & LrDevelopController — full catalogs in lrc-sdk.md §7

`lrc-sdk.md` is the single source of truth for: every develop parameter by panel (adjustPanel, tonePanel, mixerPanel, colorGradingPanel, detailPanel, effectsPanel, lensCorrectionsPanel, calibratePanel, lensBlurPanel, plus `straightenAngle`), local-adjustment parameters by process version (v2: 8 · v3/v4: 18 · v5: 25 · v6: 27), plugin develop presets, and all **95 LrDevelopController functions** with exact signatures (get/set + tracking, tools, masks SDK 11+, spots SDK 14.1+, AI/enhance SDK 14.5+, point color SDK 13.2+, lens blur SDK 13.3+, color grading, process version, remove panel, navigation/reset).

Most-used calls (parameter names are case-sensitive):
```lua
LrDevelopController.getValue(param)          -- current value
LrDevelopController.setValue(param, value)   -- set; wrap drags with startTracking(param)/stopTracking()
LrDevelopController.getRange(param)          -- min, max
LrDevelopController.addAdjustmentChangeObserver(context, {}, callback)
LrDevelopController.revealPanel("adjustPanel")  -- panel ID or param name
...
| `z-index` for overlays in UXP | Supported, but nothing can overlay a text-editing widget. Prefer DOM/flex order for overlays. |
...
| Passing a path string to `saveAs`/`open`/batchPlay in UXP | DOM methods take storage File entries; batchPlay `_path` takes `fs.createSessionToken(entry)`. |
| Storing secrets in UXP `localStorage` | Use `storage.secureStorage` — but treat it as a re-obtainable cache; `getItem` returns a `Uint8Array`. |

## Deeper References (in this skill directory)

- **`lrc-sdk.md`** — Full Lightroom Classic SDK reference: all plugin types (export, publish, filter, metadata), `Info.lua` manifest, packaging (`.lrplugin`/`.lrdevplugin`), `LrView` widgets and data binding, `LrDevelopController` (95 functions) + develop parameter catalogs, plugin develop presets, LrCatalog method inventory, programmatic export (`LrExportSession` + destination keys), publish-service callback catalog, full module catalog (45+ modules), LrC memory-leak prevention details, external communication (LrSocket, LrHttp, Controller SDK), and the processor CLI spec for LrC-to-Rust bridging.
- **`ps-uxp-sdk.md`** — Full Photoshop UXP reference: Manifest v5, Document/Layer DOM (36+29 Document, 28+55 Layer properties/methods, 38 filter methods), Selection class, Imaging API full signatures + ImageBlob display, File I/O & storage (pickers, plugin folders, session/persistent tokens, secureStorage), batchPlay and action system (5 reference forms, action recording), executeAsModal details, `.psjs` scripts, `photoshop.constants`, UDT workflow, XMP metadata, programmatic path creation, localization, Spectrum UXP + SWC component catalogs, HTML/CSS support and limitations, Canvas API limits, event system, text/typography API, color management, external communication (WebView, hybrid C++ bridge, network, shell), and the known-issues catalog.

**When to load deep references:**
- `ps-uxp-sdk.md` — when using batchPlay descriptors, Document/Layer DOM methods, Selection class, filter methods, file I/O and tokens, SWC components, text/typography APIs, or debugging known issues
- `lrc-sdk.md` — when building export/publish services (callbacks, programmatic export), working with catalog queries (`findPhotos`, custom metadata, keywords, collections), applying develop presets, building UIs with `LrDialogs.presentFloatingDialog`, using LrSocket/Controller SDK, or needing the full module API reference
