...
- Keep ordinary subtask completions concise. When a main experiment actually finishes or reaches a stage-significant checkpoint, upgrade to a richer `ds_artifact_record payload={'kind': 'milestone'}, reply_mode='threaded', ...)` report rather than another short progress line.
...
- If the active communication surface is external messaging and external messaging milestone media is enabled in config, a completed main experiment may attach one summary PNG to that richer milestone update.
- That PNG should be a summary report chart, not a raw debug plot and not a draft paper figure.
- Do not auto-send every training curve, per-step plot, or intermediate slice image.
- Preferred summary-chart palettes are Morandi-like and restrained:
  - `sage-clay`: `#E7E1D6`, `#B7A99A`, `#7F8F84` for the default external messaging summary look
...
- If the chart may later be reused in the paper, export a vector copy (`pdf` or `svg`) alongside the summary `png`.
...
5. Run a bounded smoke test or pilot before the real long run, then launch the real run with durable logging and monitor it through `ds_bash_exec`.
...
- After each `ds_artifact_record_main_experiment(...)`, route from the measured result:
...
- `ds_bash_exec` session ids, progress markers, and exported logs from the actual run
...
- if `Result/metric.md` was used during the run, treat it as optional scratch memory only and reconcile it against the final submitted metrics before `ds_artifact_record_main_experiment(...)`

Before you begin a substantial run, send a concise threaded `ds_artifact_record payload={'kind': 'progress'}, ...)` update naming:
...
- after a real main run finishes, record it with `ds_artifact_record_main_experiment(...)` before moving to analysis or writing
...
- after `ds_artifact_record_main_experiment(...)`, if external messaging milestone media is enabled and the metrics are stable enough to summarize honestly, prefer one concise summary PNG over multiple attachments
...
- prefer `ds_bash_exec` instead of ephemeral shell invocations
...
- once the smoke test passes, launch the real run with `ds_bash_exec mode='detach', ...)` and normally leave `timeout_seconds` unset for that long run
- monitor through durable logs rather than only live terminal output
- `ds_bash_exec mode='read', id=...)` returns the full rendered log when it is 2000 lines or fewer; for longer logs it returns the first 500 lines plus the last 1500 lines and a hint to inspect omitted sections with `start` and `tail`
- if the middle of a long saved log matters, inspect that omitted region with `ds_bash_exec mode='read', id=..., start=..., tail=...)`
- use `ds_bash_exec mode='list')` and `ds_bash_exec mode='read', id=..., tail_limit=..., order='desc')` to monitor or revisit managed commands while focusing on the newest evidence first
- after the first read, prefer `ds_bash_exec mode='read', id=..., after_seq=last_seen_seq, tail_limit=..., order='asc')` so later checks only fetch new evidence
- if you need to recover ids or sanity-check the active session ordering, use `ds_bash_exec mode='history')`
- launch important runs with a structured `comment` such as `{stage, goal, action, expected_signal, next_check}`
- use `silent_seconds`, `progress_age_seconds`, `signal_age_seconds`, and `watchdog_overdue` from `ds_bash_exec mode='list'|'read', ...)` as your default watchdog signals
...
- if needed, use an explicit bounded wait such as `ds_bash_exec command='sleep 60', mode='await', timeout_seconds=70)` or `ds_bash_exec mode='await', id=..., timeout_seconds=...)` between checks
- canonical sleep choice:
  - if you only need wall-clock waiting between checks, use `ds_bash_exec command='sleep N', mode='await', timeout_seconds=N+buffer, ...)`
  - keep a real buffer on that sleep timeout; do not set `timeout_seconds` exactly equal to `N`
  - if you are waiting on an already running managed session, prefer `ds_bash_exec mode='await', id=..., timeout_seconds=...)` instead of starting a new sleep command
- after every completed sleep / await cycle, inspect logs first; only send `ds_artifact_record payload={'kind': 'progress'}, ...)` when the user-visible state, frontier, blocker status, or ETA materially changed
...
- if the run is clearly invalid, wedged, or superseded, stop it with `ds_bash_exec mode='kill', id=..., wait=true, timeout_seconds=...)`; if it must die immediately, add `force=true`, record the reason, fix the issue, and relaunch cleanly
- do not report completion until logs and output files both confirm completion

Always preserve the managed `ds_bash_exec` log and export it into the experiment artifact directory when the run artifact is written.
...
- when possible include `eta` in seconds and `next_reply_at` or `next_check_at` so web/removed terminal UI can show the next expected update
...
Every meaningful main run must be recorded through `ds_artifact_record_main_experiment(...)`.
...
`ds_artifact_record_main_experiment(...)` should include at least:
...
After `ds_artifact_record_main_experiment(...)` succeeds, do not assume the same branch should absorb the next round by default.
...
If the extra work should happen on an older durable branch rather than the current head, first switch the runtime back there with `a Codex-controlled branch activation action(...)`, then launch the analysis campaign from that activated workspace.

When `ds_artifact_record_main_experiment(...)` succeeds, send a richer threaded `ds_artifact_record payload={'kind': 'milestone'}, ...)` update rather than a generic one-line progress ping.
...
Do not treat a main run as durably complete until `ds_artifact_record_main_experiment(...)` succeeds.
...
- begin every experiment pass with `ds_memory_search(scope='quest', limit=5)`
- then run at least one experiment-relevant `ds_memory_search(...)` before a new run, retry, or material execution change
...
When calling `ds_memory_write(...)`, pass `tags` as an array like `["stage:experiment", "type:failure-pattern", "topic:<mechanism>"]`, not as one comma-joined string.
...
- every experiment `ds_memory_write(...)` must state whether the outcome was `success`, `partial`, or `failure`
- every experiment `ds_memory_write(...)` should also include the current `idea_id`, `branch`, and `run_id` so later retrieval does not mix different experiment lines
...
- `ds_artifact_record_main_experiment(...)` at main-run completion
...
- use `ds_artifact_record_main_experiment(...)` for each meaningful completed main experiment
- use `run` for analysis slice records when `ds_artifact_record_analysis_slice(...)` writes them
- use `report` for:
...
Use `backend automatic checkpointing(...)` when code evolution is meaningful and should be preserved in Git.
After a meaningful experiment checkpoint or completion, emit `ds_artifact_record payload={'kind': 'progress'} | 'milestone', ...)` so the user sees the concrete result and next step.
