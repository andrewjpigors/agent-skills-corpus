...
- save provider credentials into `~/.myai/.env`
...
```bash
SCRIPT="$(find ~/.myai/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
3. Buy it and save it into `~/.myai/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to `~/.myai/.env`
