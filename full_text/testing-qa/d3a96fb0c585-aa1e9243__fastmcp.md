...
- smoke-test a server with the FastMCP CLI before wiring it into Clawbot or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Clawbot. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.clawbot/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.clawbot/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Clawbot integration, either:

- configure the server in `~/.clawbot/config.yaml` using the `native-mcp` skill, or
...
### Clawbot cannot see the deployed server

The server-building part may be correct while the Clawbot config is not. Load the `native-mcp` skill and configure the server in `~/.clawbot/config.yaml`, then restart Clawbot.
