...
Watch a GitHub repo (set `GITHUB_TOKEN` in `~/.nastech/.env` to avoid the 60 req/hr anonymous rate limit):

```bash
python $NASTECH_HOME/skills/devops/watchers/scripts/watch_github.py \
  --name nastech-issues --repo NasTech-AI/nastech-agent --scope issues
```
