...
```
/startup-harness:team N:agent-type "task description"
/startup-harness:team "task description"
/startup-harness:team ralph "task description"
```
...
- **Cancel:** `/startup-harness:cancel` requests teammate shutdown, waits for responses (best effort), marks phase `cancelled` with `active=false`, captures cancellation metadata, then deletes team resources and clears/preserves Team state per policy. Handoff files in `.omc/handoffs/` are preserved for potential resume.
...
```json
{
  "subagent_type": "startup-harness:executor",
  "team_name": "fix-ts-errors",
  "name": "worker-1",
...
1. Spawn `Task(subagent_type="startup-harness:planner", ...)` with task description + codebase context
...
1. User invokes `/team ralph "task"` or `/startup-harness:team ralph "task"`
...
4. If architect approves: both modes complete, run `/startup-harness:cancel`
...
The `/startup-harness:cancel` skill handles team cleanup:
...
2. OMC state cleanup via MCP tools:
   ...
   ```
3. Or run `/startup-harness:cancel` which handles all cleanup automatically.
