...
For detailed guidance, refer to the bundled `scientific-schematics` and `imagegen` skill documentation.
...
- Journal manuscripts → Use `venue-templates` if installed; otherwise use this skill's report formatting or a journal-provided template
- Conference papers → Use `venue-templates` if installed; otherwise use `ml-paper-writing` or `systems-paper-writing` templates when applicable
...
|---------------|---------------------|
| Journal manuscripts | Use `venue-templates` if installed, otherwise journal-provided templates or this skill's report formatting |
| Conference papers | Use `venue-templates` if installed, otherwise `ml-paper-writing` or `systems-paper-writing` templates |
...
**Before writing for a specific venue, consult `venue-templates` only if it is installed.**
...
When available, `venue-templates` provides:
