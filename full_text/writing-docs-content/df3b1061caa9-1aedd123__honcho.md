---
name: honcho
description: Configure and use Honcho memory with Monika -- cross-session user modeling, multi-profile peer isolation, observation config, and dialectic reasoning. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation and recall settings.
version: 1.0.0
author: Monika
license: MIT
metadata:
  monika:
    ...
    related_skills: [monika-agent]
prerequisites:
  pip: [honcho-ai]
---

# Honcho Memory for Monika
...
```bash
monika honcho status    # shows resolved config, connection test, peer info
```
...
Honcho models conversations as interactions between **peers**. Monika creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Monika instance. Each profile gets its own AI peer so agents develop independent views.
...
Manual override: `monika honcho map my-project-name`
...
```bash
monika profile create coder --clone
# creates host block hermes.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `monika.coder` host block in `honcho.json`
...
```bash
monika honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Config file: `$MONIKA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `monika honcho setup`. Ensure `memory.provider: honcho` is in `~/.monika/config.yaml`.

### Memory not persisting across sessions
Check `monika honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `monika profile create <name> --clone`. For existing profiles: `monika honcho sync`.
...
| Command | Description |
|---------|-------------|
| `monika honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `monika honcho status` | Show resolved config, connection test, peer info for active profile |
| `monika honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `monika honcho disable` | Disable Honcho for the active profile |
| `monika honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `monika honcho peers` | Show peer identities across all profiles |
| `monika honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `monika honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `monika honcho sessions` | List known directory-to-session-name mappings |
| `monika honcho map <name>` | Map current working directory to a Honcho session name |
| `monika honcho identity` | Seed AI peer identity or show both peer representations |
| `monika honcho sync` | Create host blocks for all Hermes profiles that don't have one yet |
| `monika honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Monika + Honcho |
| `monika memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `monika memory status` | Show active memory provider and config |
| `monika memory off` | Disable external memory provider |
