...
- [web-pwn.md](web-pwn.md) - Web-facing memory corruption: V8 type confusion (addrof/fakeobj primitives), JIT bug exploitation, OOB array length corruption, V8 sandbox escape (WASM jump table, EPT abuse), JavaScriptCore NaN-boxing, WASM linear memory OOB, nginx/Apache module UAF, PHP ZMM heap bugs, HTTP parsing integer overflow, shared memory IPC race, d8 debug workflow
- [iot-pwn.md](iot-pwn.md) - IoT/embedded exploitation: firmware extraction (binwalk/sasquatch/jefferson), QEMU user-mode and full-system emulation, MIPS/ARM/ARM64 ROP chains, CGI command injection patterns, hardcoded credential extraction, UART/JTAG console access, U-Boot bootloader exploitation, cross-architecture exploit templates
...
## Exploit Reasoning Framework

**Always follow this 4-step process before writing any exploit code.**
...
strings ./binary | grep -iE '(flag|win|shell|system|exec|password)'
ldd ./binary          # linked libc version

# Run it first — understand normal behavior before exploiting:
./binary              # or: nc host port
# Note: what input does it accept? what does it print? does it crash?

# If source provided — read it fully before doing anything else
# Key questions:
# - What buffers exist and what are their sizes?
# - What input functions are used? (gets/fgets/read/scanf)
# - Are there any special byte values that trigger different behavior?
# - Is there a win() function or /bin/sh string already in the binary?
# - What happens with \x00, negative numbers, very long input?
```

### Step 2: List All Possible Attack Paths

Before choosing an approach, enumerate ALL viable paths:

```
For each vulnerability found, ask:
1. Can I win without any leak? (ret2win, magic value, direct GOT overwrite)
2. Can I bypass a check with a special byte/value? (\x00 terminator, signed int, off-by-one)
3. Can I get a shell with one stage? (no-PIE + no-canary = direct ROP to system)
4. Do I actually need a leak? (only if PIE enabled AND no ret2win AND no fixed gadgets)
5. Is there an unintended path? (logic bug, format string, use-after-free in simpler form)
```

### Step 3: Prioritize — Simplest Viable Path First

**Always attempt in this order — stop at the first that works:**

| Priority | Path | Requires |
|----------|------|----------|
| 1 | ret2win (call existing win function) | win() in binary, no leak needed |
| 2 | Special byte bypass (\x00, negative, overflow) | Understanding input parsing |
| 3 | Direct GOT overwrite | No PIE + Partial RELRO |
| 4 | One-stage ROP (no leak) | /bin/sh in binary + fixed gadgets |
| 5 | Two-stage ret2libc (with leak) | Need to leak libc base first |
| 6 | Heap exploitation | Leak + heap feng shui |
| 7 | Kernel / sandbox escape | Complex multi-stage chain |

**Critical checks before assuming you need a leak:**
- Is PIE actually enabled? (`checksec` — if No, all addresses are fixed)
- Is there a `/bin/sh` string already in the binary? (`strings binary | grep /bin/sh`)
- Is there a `system()` call already in the PLT? (`objdump -d binary | grep system`)
- Does the binary print any addresses itself? (read output carefully before leaking)
- Can a special input value bypass the check entirely without exploitation?

### Step 4: Escalate Complexity Only When Simpler Paths Fail

```
If Path 1-4 fail → diagnose WHY before escalating:
- ret2win fails?  → check stack alignment (add extra `ret` gadget)
- Byte bypass fails? → recheck parsing: is it strlen, strcpy, or read()?
- GOT overwrite fails? → verify RELRO status (Full RELRO = GOT is read-only)
- ROP crashes? → check bad chars in addresses, stack alignment, argument setup

Only move to the next complexity level after confirming the simpler path
is genuinely blocked, not just because the first attempt had a bug.
```

**Example: \x00 bypass vs leak**
```
Scenario: strcmp(input, secret) where secret starts with \x00

Wrong approach: attempt to leak secret via format string or side-channel
Correct approach: send \x00 as first byte → strcmp returns 0 immediately → bypass

Reasoning: \x00 terminates C strings → strcmp sees empty string == empty string
This requires zero leaks, zero ROP, zero heap spray.
```
