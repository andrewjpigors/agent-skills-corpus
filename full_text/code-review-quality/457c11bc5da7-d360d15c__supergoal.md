---
name: supergoal
description: Plan and autonomously build a software task end-to-end. Triggered by "supergoal", "plan and ship X", "supercharged plan", "autonomous build", "plan it out and don't stop until it's done", "I don't want to babysit this", or any non-trivial feature/refactor/redesign the user wants driven to completion. Strongly prefer over a plain plan when the user signals "every aspect", "fully", "perfectly", "until done", or wants depth + autonomous follow-through. Recons the codebase, applies preloaded memory, researches best practices with whatever tools are available, decomposes into the right number of phases, gets one confirmation, then executes all phases to completion with built-in retry, fix-spec recovery, a final audit, and per-phase memory writeback. Hermes redraft of github.com/robzilla1738/supergoal (MIT).
metadata:
  hermes:
    tags: [autonomous-build, planning, multi-phase, self-healing, agentic]
    source: https://github.com/robzilla1738/supergoal
---

> **Hermes port note (read once).** Upstream supergoal is built for Claude Code / Codex,
> whose `/goal` slash command runs a per-turn evaluator that auto-continues a fresh session
> until an end-state condition holds. **Hermes has no `/goal`.** This redraft keeps the whole
> planning pipeline (Stages 0–6.5) intact and replaces only the Stage 7 dispatch + execution
> model with Hermes-native execution — see "Stage 7 (Hermes)" below. Three host mappings apply
> throughout: `clarify` → the `clarify` tool; Claude memory dirs → Hermes `memory` tool
> + `~/.hermes`; `$ARGUMENTS` → the user's task as stated in the triggering message.
---

# Supergoal

You are running the Supergoal workflow. The user's task is the request that triggered this
skill (referred to below as **`$ARGUMENTS`** — substitute the actual task text wherever you
see it):

$ARGUMENTS

Your job: **plan deeply, then auto-execute the run** until the task is verifiably complete across every phase.
...
7. **Dispatch the run** (Stage 7 Hermes): execute phases sequentially with retry + fix-spec recovery + per-phase memory writeback, then run a **final audit** that re-verifies the work against the original ROADMAP and self-heals any gaps before completion holds
...
### How execution works on Hermes (vs upstream's `/goal`)

Upstream collapses the whole run into one `/goal` whose end-state condition a per-turn
evaluator checks until it holds. Hermes has no such evaluator. Instead the planning artifacts
on disk (`ROADMAP.md`, `STATE.md`, `phases/phase-N.md`, `PROTOCOL.md`) drive an **in-session
execution loop you run directly** after confirmation: read STATE → read the current phase spec
→ do the work → run mandatory commands → print the VERIFY/DONE blocks → update STATE → next
phase → final audit. Same contract, same self-healing, same transcript markers — the
difference is you execute the loop yourself (optionally via `delegate_task` per phase, or a
background `cronjob` for unattended runs) rather than handing off a slash command. Phase work
lives in files so context stays lean; the markers (`SUPERGOAL_PHASE_DONE`,
`SUPERGOAL_RUN_COMPLETE`) remain the completion signal. See "Stage 7 (Hermes)".

## Locate the skill directory

```bash
SUPERGOAL_DIR=$(dirname "$(ls -1 \
  "$HOME/.hermes/skills/devops/supergoal/SKILL.md" \
  "$HOME/.hermes/skills/supergoal/SKILL.md" \
  "$PWD/.hermes/skills/supergoal/SKILL.md" \
  2>/dev/null | head -n1)")
...
- **Fresh run (default for a new task)** — claim a unique namespace:
  ```bash
  SUPERGOAL_ROOT="$(bash "$SUPERGOAL_DIR/scripts/claim-run.sh" "<task description>")"
...
- **Resume** — if an active run clearly matches this task (its STATE.md title ≈ `$ARGUMENTS`, or the user said "resume"/"continue"), set `SUPERGOAL_ROOT` to that run dir and follow the resume path (don't re-plan). If several active runs exist and intent is ambiguous, ask with **one** `clarify` which to resume — or to start fresh.
...
> ⚠ Another Supergoal run is active in this working tree (`<list>`). Your planning artifacts are isolated under `<SUPERGOAL_ROOT>`, so they won't collide — **but two autonomous runs in the same working tree will still edit the same source files and clobber each other's code.** Namespacing protects the plan, not the build. For true parallel execution, run each task in its own `git worktree`; or resume the existing run instead of starting a second.
...
Hermes memory is not a directory of markdown files — it's the `memory` tool, and durable
facts are already injected into your context every turn. So **preload = read what's already
in your context** (user profile + memory entries injected by Hermes) and pull the entries
relevant to this task's stack/domain/project.

Optionally also scan on-disk project notes if the repo or `~/.hermes` has any:

```bash
# Best-effort: surface any markdown memory/notes the host or project keeps on disk.
for cand in "$PWD/.hermes/memory" "$PWD/CLAUDE.md" "$PWD/AGENTS.md" "$HOME/.hermes/SOUL.md"; do
  [ -e "$cand" ] && echo "--- $cand ---" && head -40 "$cand" 2>/dev/null
done
```

Capture applicable memory hits in `$SUPERGOAL_ROOT/applied-memories.md` (one line per hit:
what, why-applicable, what-it-changes). Surface them in Stage 1 as "Applied from memory: …"
so the user can correct anything stale. Writeback later uses the `memory` tool, not files —
see Memory writeback rules.

### Tool discovery

Tools differ between sessions and hosts. Detect from your available tool list, don't assume:

- **web research** — `web_search` / `web_extract` if listed. If absent, lean on training-cutoff knowledge and flag it as an assumption.
- **delegation** — `delegate_task` (parallel subagents) if listed; used by Stage 7 execution for parallelizable phases.
- **durable execution** — `cronjob` if listed; used by Stage 7 for background/long-running autonomous runs.
- **Project skills** — check the available-skills list for domain-relevant skills (e.g. a framework or auth skill) and note them in `$SUPERGOAL_ROOT/applied-skills.md` to invoke during phases.
- **Prior Supergoal state** — handled above in "Claim the run namespace".
...
4. Ask all remaining questions in **batches of up to 4** (the `clarify` tool ceiling) until every material gap is filled. Two batches is fine for greenfield; three is rare but allowed if a complex task genuinely warrants it.
...
2. Each `clarify` batch caps at 4 (tool limit). Greenfield can use multiple sequential batches; brownfield is one batch max.
...
3. **`phases/phase-N.md`** — one work-spec file per phase (template at `$SUPERGOAL_DIR/templates/phase-goal.txt`, renamed conceptually to "phase spec"). **Any length** — these are read from disk by the executor, so no char budget.
...
Before the run is dispatched, show the user the full plan and **ask for explicit confirmation**. The chain runs unsupervised once it starts, so this is the last cheap moment to correct course. Skipping this step is a bug.
...
Once you confirm, I.ll dispatch the run (Stage 7): it executes through to
completion with auto-retry, fix-spec recovery, and a final audit.
```

Then call `clarify` with one question, header "Start chain?", offering **concrete revision modes** (not a vague "revise plan"):

- **Start now** — run pre-flight smoke check (Stage 6.5), then dispatch the run (Stage 7); it runs autonomously
...
## Stage 7 (Hermes) — Dispatch the run

Upstream prints a `/goal` line for the user to paste; Hermes has no `/goal`, so you dispatch
the run yourself. After explicit "Start now" in Stage 6:

1. Update `STATE.md`: `Status: IN_PROGRESS`, `Current phase: 1`, and **capture the baseline
   ref** — set `Baseline ref:` to `git rev-parse HEAD 2>/dev/null || echo "no-git"`. The audit
   reads this to diff deliverables against the working tree.
2. Copy the operating manual and comparison helper into this run's namespace:
   ```bash
   ...
   ```
   `PROTOCOL.md` is the manual the executor reads; the `sed` bakes the concrete run root into
   every `{{RUN_ROOT}}` placeholder. `repo-state.sh` is the working-tree comparison helper the
   cleanliness + deliverable checks invoke (strategy in `references/repo-state-comparison.md`).
...
4. **Choose an execution mode** and announce it in one line:

   | Mode | When | How |
   |---|---|---|
   | **In-session (default)** | You're here and can watch | Read `$SUPERGOAL_ROOT/PROTOCOL.md` and run the Phase execution loop (below) directly in this session, phase by phase. |
   | **Per-phase delegation** | Phases are heavy/independent and `delegate_task` is available | For each phase, spawn a leaf subagent with the phase spec + PROTOCOL as `context` and toolsets `["file","terminal"]`; it returns the VERIFY/DONE receipt. You stay the orchestrator that updates STATE and runs the audit. Do NOT run two phases that touch the same files in parallel. |
   | **Background (unattended)** | User wants it run while away and `cronjob` is available | Create a one-shot `cronjob` whose prompt is "Execute the supergoal run at `$SUPERGOAL_ROOT` per its PROTOCOL.md; follow the phase loop, final audit, and failure protocol; deliver SUPERGOAL_RUN_COMPLETE summary." with `enabled_toolsets: ["file","terminal"]`. The job runs the same on-disk loop. |

5. Print a short dispatch banner, then begin:

   > **Supergoal dispatched (mode: <mode>).** Run root: `<run-root>`. Executing <N> phases with
   > auto-retry, fix-spec recovery, per-phase memory writeback, then a final audit. I'll surface
   > each phase's VERIFY/DONE and stop for you only on a 3-strike `FAILURE_HANDOFF`,
   > `AUDIT_HANDOFF`, or a mid-run message from you.

6. Begin executing the **Phase execution loop** (next section). Everything the loop needs is in
   `PROTOCOL.md`, `ROADMAP.md`, `STATE.md`, and the phase specs on disk — by design, so a
   background or delegated executor with no chat context can run it identically.

---

## Phase execution loop (the executor runs this)
...
10. If N == total: do **not** print `SUPERGOAL_RUN_COMPLETE` yet. Run the **Final audit** (next section). Only after `AUDIT_COMPLETE`, print `SUPERGOAL_RUN_COMPLETE` with a 5-line summary. The run is complete.
...
7. **If any gaps:**
   ...
   - Execute the fix spec inline (same session/executor, same 3-strike per-criterion protocol from regular phases).
...
3. Execute the fix spec inline (same session/executor — no new dispatch). On success, re-run the original phase's VERIFY block; on pass, advance to N+1.
...
3. The run halts here; stop attempting and surface the handoff clearly. STATE.md Status=BLOCKED is the durable signal.

This recovers from flaky envs, simple typos, and missed deps automatically. Only real blockers escalate.

### Mid-run interruption

If the user sends any message during the run, the agent pauses at the next phase boundary, addresses the message, and asks before resuming. Phase boundaries are after `SUPERGOAL_PHASE_DONE` and before reading the next phase spec.
...
- **Artifacts on disk, lean context.** Phase work lives in files the executor reads from disk, not in one giant prompt. Keeps context small across a long run.
...
- **Mid-run interruption**: if the user stops the run and asks for a change, update the affected `<run-root>/phases/phase-N.md` spec, run `validate-phase.sh` on it, then ask the user to resume (just say "continue"). No need to restart phase 1.
...
- `references/execution-model.md` — how a run executes on Hermes (in-session / delegation / background), required transcript blocks
