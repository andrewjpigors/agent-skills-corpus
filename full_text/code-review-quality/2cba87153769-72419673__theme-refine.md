...
description: >-
  ...
  (1) pixel-perfect visual refinement after theme-clone, and
...
**A) Pixel-perfect refinement** — after `/theme-clone` ships a 1:1 site clone, tighten visual fidelity to match the source.
...
Note: `base-theme/` in the greps below means the theme working directory — substitute `"$THEME_DIR"` (using `"$THEME_DIR"` in these greps is fine).

```bash
# 1. Splide anywhere (forbidden — breaks Fluid's DOM lifecycle).
#    Not covered by theme_audit.py — it's a JS/CSS asset rule.
grep -rn -i "splide" "$THEME_DIR" --include="*.liquid" --include="*.css" --include="*.js"

# 2. Body overflow that kills sticky nav.
#    Not covered by theme_audit.py — it's a cross-file CSS rule.
grep -rn "overflow-x:\s*hidden\|overflow:\s*hidden\|overflow-y:\s*hidden" "$THEME_DIR"/assets/ "$THEME_DIR"/layouts/ | grep -i "body\|html"
...
grep -rn "show-cart\|fluid-cart-count\|show-language-country-dropdown\|saveLocaleBtn\|country-selector\|language-selector" "$THEME_DIR"/components/ "$THEME_DIR"/sections/main_navbar/
```
...
**`components/pagination/index.liquid` — replace wholesale** with the v4 version. It's 30 lines, modern, and the styles are in `global_styles.css`. Copy from the base theme (cloned from https://github.com/Fluid-WeCommerce/base-theme) `components/pagination/index.liquid`.
...
Exit code 0 = clean. Exit code 1 = fix every violation before moving on. The full rule list lives in [theme-clone § Schema & Liquid audit](../theme-clone/SKILL.md#schema--liquid-audit-theme_auditpy).
...
fluid_api(f"/api/application_themes/{THEME_ID}/resources", "PUT",
    {"key": "page/<slug>/index.liquid", "content": content})
# A new template ID is created. Find it via fluid_api(f"/api/application_themes/{THEME_ID}", "GET").
```
...
resp = fluid_api(f"/api/application_themes/{THEME_ID}/resources", "GET")
# Filter: .application_theme_resources[] where key startswith "sections/foo/"
# resource_id will be the template ID (integer)

# Destroy it
fluid_api(f"/api/application_theme_templates/{TEMPLATE_ID}", "DELETE")
...
**Replace with** inline markup (copy from the base theme — cloned from https://github.com/Fluid-WeCommerce/base-theme — `sections/main_navbar/index.liquid`, the `{%- when 'cart_button' -%}` branch inside the `feature_buttons` capture). Carries the full 14-setting cart button including `id="show-cart"` + `id="fluid-cart-count"` hooks Fluid's cart JS depends on.
...
**Replace with** the v4 component (~30 lines). Copy the base theme's (cloned from https://github.com/Fluid-WeCommerce/base-theme) `components/pagination/index.liquid` wholesale, plus the matching styles in `global_styles.css` (pill buttons, circular shape, modern gap-based layout).
...
Copy the full Liquid + CSS + JS from the base theme (cloned from https://github.com/Fluid-WeCommerce/base-theme) `sections/main_navbar/index.liquid`. Key settings exposed: `overflow_mode` (`hamburger` vs `more_dropdown`), `nav_alignment` (flex-start / center / flex-end), `logo_min_width` (reserve clickable area when empty).
