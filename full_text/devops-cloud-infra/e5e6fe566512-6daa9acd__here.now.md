...
- Skill helper paths:
  - `${CLAWBOT_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${CLAWBOT_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${CLAWBOT_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client clawbot
```
...
```bash
PUBLISH="${CLAWBOT_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client clawbot
```
...
```bash
DRIVE="${CLAWBOT_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `clawbot`)    |
