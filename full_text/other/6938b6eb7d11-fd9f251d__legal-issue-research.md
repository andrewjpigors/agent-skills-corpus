...
> 2. **创建配置文件**：在 skill 目录创建 `config.json`
> 3. **填入凭证**：
>    ```json
>    {
>        "API_BASE_URL": "你的API端点",
>        "API_KEY": "你的API_KEY",
>        "API_SECRET": "你的API_SECRET（可选）",
>        "AUTH_TYPE": "Bearer"
>    }
>    ```
> 4. **使用**：配置完成后，skill 自动检测并调用，无需任何操作
>
> **失败处理**：API 未配置或调用失败不影响主流程，skill 继续使用公开渠道检索
...
| 2026年4月 | 增加第二部分，威科先行数据库使用模块 | 为高级数据库服务使用提供接口 |
---
