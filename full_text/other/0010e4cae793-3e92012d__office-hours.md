...
- **Timeout:** "Outside Voice timed out after 5 minutes." Fall back to Primary subagent.
- **Empty response:** "Outside Voice unavailable." Fall back to Primary subagent.

On any Outside Voice error, fall back to the Primary subagent below.
...
> A) Revise this premise based on Outside Voice's input
...
- If the second opinion (Outside Voice or Primary subagent) proposed a prototype in Phase 3.5, consider using it as a starting point for the creative/lateral approach.

**RECOMMENDATION:** Choose [X] because [one-line reason mapped to the founder's stated goal].

Emit ONE ask_user that lists every alternative (A/B and optionally C) as numbered options, using the preamble's ask_user Format section. The ask_user call is a tool_use, not prose — write the question text and call the tool.
...
Show each variant to the user inline first (read the PNGs with view tool), then
...
> "Want outside design perspectives on the chosen approach? Outside Voice proposes a visual thesis, content plan, and interaction ideas. A Primary subagent proposes an alternative aesthetic direction."
>
...
1. **Outside Voice** (via Bash, `model_reasoning_effort="medium"`):
```bash
_REPO_ROOT=$(git rev-parse --show-toplevel) || { echo "ERROR: not in a git repo" >&2; exit 1; }
```

2. **Primary subagent** (via task tool):
"For this product approach, what design direction would you recommend? What aesthetic, typography, and interaction patterns fit? What would make this approach feel inevitable to the user? Be specific — font names, hex colors, spacing values."

Present Outside Voice output under `OUTSIDE VOICE SAYS (design sketch):` and subagent output under `INDEPENDENT SUBAGENT (design direction):`.
...
- **Defended premise with reasoning** against cross-model challenge (kept original premise when Outside Voice disagreed AND articulated specific reasoning for why — dismissal without reasoning does not count)
...
```bash
.github/skills/bin/gstack-developer-profile --log-session '{"date":"TIMESTAMP","mode":"MODE","project_slug":"SLUG","signal_count":N,"signals":SIGNALS_ARRAY,"design_doc":"DOC_PATH","assignment":"ASSIGNMENT_TEXT","resources_shown":[],"topics":TOPICS_ARRAY}' 2>/dev/null || true
```
...
> **STOP.** Before writing the design doc and running the tiered relationship handoff (Phases 5-6, after the conversation and alternatives are done), Read `.github/skills/office-hours/sections/design-and-handoff.md` and execute it
