---
name: lightest-docx
skill_id: lightest_docx_skill
description: 当用户要创建或修改磁盘上的 Word .docx（封面、图表、修订、OOXML 级排版、模板填占位）且文档未在 Word/WPS 中打开时使用；须 unzip 解析 XML 以保留版式。**触发**：.docx、Word 文档、创建 Word、改 Word 版式、track changes、模板填充（磁盘文件）。**加载**：`skills_fetch(skill_ids=["lightest_docx_skill"])` → 按任务读 SKILL.md 与 scripts。**勿用**：Word 已打开 live 编辑 → `lightest_word_live_skill`；课程论文/三线表国标 → `lightest_docx_cn_paper_skill`。
---

# LighTest Word .docx（内置 Skill）

**官方内置**：连接项目时同步到 `.lightest/skills/lightest-docx/`；扫描器读 `skill_id: lightest_docx_skill`。

## 技能分工

| 技能 | 何时 |
|------|------|
| **本技能** | 磁盘 `.docx`：OOXML 创建/编辑、版式保留、修订/批注、模板填占位 |
| `lightest_word_live_skill` | Word/WPS **已打开**，要 live WYSIWYG |
| `lightest_docx_cn_paper_skill` | 中文高校论文国标（三线表等） |
| `lightest_writing_skill` | 先要 prose 结构与文风，再灌 Word |

## 工作流（强制）

| 阶段 | 动作 | 路径 / 工具 |
|------|------|-------------|
| 0. 路由 | 只要纯文本 → `read`；要版式/模板 → 本技能；Word 已开 → word_live | 下表「任务路由」 |
| 1. 环境 | 首次 `./scripts/docx init` | `.lightest/skills/lightest-docx/` |
| 2a. **新建** | C# SDK：`Program.cs` + `./scripts/docx build` | 工作区 `.lightest/aichat-tmp/docx-work/` → `docx-output/` |
| 2b. **改已有** | 解压 OOXML + Python lxml（禁 python-docx/docx-js） | `references/EditingGuide.md` |
| 3. 校验 | `./scripts/docx validate FILE` 或 build 内置校验 | `validator/` |
| 4. 交付 | 输出到项目或用户指定路径 | **勿混** C# 创建与 Python 编辑同一文档 |

**纪律**：保留版式必须 unzip/XML；`read` 仅适用于纯文本场景。`.doc` 先 `libreoffice --headless --convert-to docx`。

## 示例：模板填占位（保留版式）

用户：已有 `report.docx`，替换封面与正文占位符，不动版式。

1. `skills_fetch` → 确认非 word_live、非国标论文 skill
2. 按 `EditingGuide.md`：克隆段落样式改文本，不整段重建
3. `./scripts/docx validate report.docx`（若走 C# 新建链则用 `docx build` 内置校验）
4. 交还同路径或 `docx-output/` 副本

**勿**：用 `read` 当版式来源；用 python-docx 整文档重建。

## 任务路由（简表）

| 用户要 | 做法 |
|--------|------|
| 新建报告/封面/原生图表 | C# + `assets/templates/Example.cs` / `CJKExample.cs` + `docx build` |
| 改批注/修订/track changes | Python + `references/EditingGuide.md` |
| 仅读文字（摘要/翻译） | `read` 即可 |
| 国标课程论文 Word | `lightest_docx_cn_paper_skill` |

---

**以下** Part 1–4 为 Goals、执行环境、质量标准与 OOXML 技术参考（正文未删减）。
...
| **User has Word/WPS open and wants live WYSIWYG edits** | **`lightest-word-live` skill + `word_document` tool** — not this skill |
...
├── assets/
│   └── templates/
│       ├── LightestDocx.csproj       → Project file template (for creating new docs)
...
```bash
cd .lightest/skills/lightest-docx/
./scripts/docx init
```
...
| Path | Purpose |
|------|---------|
| `.lightest/skills/lightest-docx/` | SKILL directory, where commands are executed |
| `.lightest/aichat-tmp/docx-work/` | Working directory, edit `Program.cs` here |
| `.lightest/aichat-tmp/docx-output/` | Output directory, final deliverables |
| project root | User upload / input files location |
...
string outputFile = args.Length > 0 ? args[0] : ".lightest/aichat-tmp/docx-output/output.docx";
...
```bash
cd .lightest/skills/lightest-docx/
./scripts/docx validate .lightest/aichat-tmp/docx-output/report.docx
```
