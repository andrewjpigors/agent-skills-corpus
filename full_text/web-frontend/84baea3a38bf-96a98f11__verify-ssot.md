...
grep -lrE "createDrizzleSelectChain|createDrizzleInsertChain|createDrizzleUpdateChain|createSequentialDrizzleStub|createDrizzleInsertWithReturningChain|createDrizzleUpdateWithReturningChain" \
  apps/backend/src --include="*.spec.ts" \
  | wc -l
# 기대: ≥ 10 (drizzle-stub-spec-migration 2026-05-13 완료 기준)
```

**PASS**:
- drizzle-stub.ts SSOT 파일 존재 + 4 export 시그니처 보존
- backend spec 의 로컬 fluent chain 재정의 0건
- SSOT consumer ≥ 10 spec

**FAIL**:
- 신규 backend spec 이 `function createSelectChain` / `function makeDbStub` 로컬 정의 (SSOT 우회)

**예외 인정** (allow-list):
- `// intentional: flat-tx — multi-where discriminator` 주석이 달린 패턴: 단일 `where` 함수가 여러 ORM 연산을 분기하는 구조 (NC close-tx, notification multi-call 등). `createDrizzle*Chain` 분리 시 discriminator 로직 소실 — 유지 정당.
- `buildDrizzleMock` 패턴: `createDrizzleSelectChain(rows)` 후 `chain.from`/`chain.where`를 최상위에 노출하는 경우 — `expect(mockDb.where).toHaveBeenCalledTimes(N)` assertion 호환 목적. SSOT 사용 중이므로 정당.
...
## Step 67: schemas enum Values 객체 — type import만 하고 value import 누락 차단 (2026-05-13)

> 도입: 2026-05-13 inbound-bulk-receive-integration 시니어 자기감사. `type ConditionStatus`만 import하고 `ConditionStatusValues` 값 객체를 import하지 않아 `'normal'`/`'abnormal'` 리터럴이 산재한 SSOT 위반.

**핵심**: TypeScript `type` import는 런타임 값을 가져오지 않는다. 타입 레벨 안전성 ≠ 값 SSOT. 스키마 enum 타입을 사용하는 파일은 반드시 대응하는 Values 객체도 import해야 한다.

**검증 명령**:

```bash
# (1) ConditionStatus 관련 string literal 직접 비교 탐지
# 'normal' / 'abnormal' 리터럴이 컴포넌트/훅 파일에 남아있으면 잠재적 SSOT 위반
grep -rn "'normal'\|'abnormal'" apps/frontend/components apps/frontend/hooks \
  --include="*.tsx" --include="*.ts" \
  | grep -v "__tests__\|\.test\.\|\.spec\.\|messages/"
# 기대: 0 hit (ConditionStatusValues.NORMAL/ABNORMAL 경유)

# (2) AccessoriesStatus 관련 string literal 직접 비교 탐지
grep -rn "'complete'\|'incomplete'" apps/frontend/components apps/frontend/hooks \
  --include="*.tsx" --include="*.ts" \
  | grep -v "__tests__\|\.test\.\|\.spec\.\|messages/"
# 기대: 0 hit (AccessoriesStatusValues.COMPLETE/INCOMPLETE 경유)

# (3) ConditionStatusValues, AccessoriesStatusValues value import 확인
# 위 (1)(2) hit 파일이 있다면 해당 파일의 import 확인
grep -rn "ConditionStatusValues\|AccessoriesStatusValues" apps/frontend/components apps/frontend/hooks \
  --include="*.tsx" --include="*.ts" | grep "import"
# 기대: 각 사용 파일에서 value import 존재
```

**PASS**: 컴포넌트/훅 파일에 `'normal'`/`'abnormal'`/`'complete'`/`'incomplete'` 리터럴 0건.

**FAIL**: `type ConditionStatus` import + `'normal'`/`'abnormal'` 직접 비교. `ConditionStatusValues` value import 없음.

**Why**: `InboundBulkReceiveModal.tsx` 에서 `type ConditionStatus`만 있고 값 비교에 `'normal'`/`'abnormal'` 리터럴 산재 (2026-05-13 발견). schema 값이 변경되면 컴파일 에러 없이 런타임 버그 발생.

**How to apply**:
```typescript
// ✅ CORRECT — 타입과 값 모두 import
import {
...
const hasAbnormal = status === ConditionStatusValues.ABNORMAL; // SSOT 경유

// ❌ WRONG — type import만 하고 값을 리터럴로 비교
import type { ConditionStatus } from '@equipment-management/schemas';
const hasAbnormal = status === 'abnormal'; // SSOT 위반
```
