...
import upstream classes into Clawbot itself.
...
```bash
mkdir -p ~/.clawbot/cache/darwinian-evolver && cd ~/.clawbot/cache/darwinian-evolver
...
```bash
cd ~/.clawbot/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.clawbot/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.clawbot/skills/research/darwinian-evolver
DE_DIR=~/.clawbot/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Clawbot core.
   Custom driver scripts under `~/.clawbot/skills/...` are user-side and fine.
