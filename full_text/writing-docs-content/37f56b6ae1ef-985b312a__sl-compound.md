...
## Validated writer contract

The streamlined `sl-run` closeout does not invoke this full orchestration workflow. Its fresh closeout agent may reuse only this skill's co-located `references/schema.yaml`, `references/yaml-schema.md`, `assets/resolution-template.md`, and `scripts/validate-frontmatter.py` as a compact writer contract after the kernel packet's reusable, evidence-backed, novel, and behavior-changing gates all pass. A failed gate produces `no-learning`; it must not draft a document first and delete it later.
...
```bash
/sl-compound                            # Document the most recent fix
/sl-compound [brief context]            # Provide additional context hint
/sl-compound mode:headless              # Non-interactive run for automations
/sl-compound mode:headless [context]    # Non-interactive run with context hint
```

## CONCEPTS.md bootstrap requests

If invoked specifically to create or bootstrap `CONCEPTS.md` from scratch rather than to document a solved problem, do not run the normal phases — `sl-compound` populates `CONCEPTS.md` only as a side effect of documenting a real learning (it seeds the *learning's area*, not the whole repo; see Phase 2.4). Repo-wide concept-map creation is `sl-compound-refresh`'s job. Redirect a standalone bootstrap request to `sl-compound-refresh` (which asks whether to build the concept map or run a refresh cycle), then exit.
...
If the line above resolved to a plain branch name (like `feat/my-branch`), include it in the `sl-sessions` invocation payload in Phase 1 so the orchestrator does not waste a turn deriving it. If it still contains a backtick command string or is empty, omit it and let `sl-sessions` derive it at runtime.
...
**In interactive mode**, present the user with two options before proceeding, using `AskUserQuestion` (call `ToolSearch` with `select:AskUserQuestion` first if its schema isn't loaded). Fall back to numbered options in chat only if it is unavailable or the call errors. Never silently skip the question.
...
```
Would you also like to search your session history
for relevant knowledge to help the Compound process? This adds
time and token usage.
```

If the user says yes, invoke `sl-sessions` in Phase 1 (see step 4). If no, skip it. Do not ask this in lightweight mode or headless mode.
...
- **Then** invoke the `sl-sessions` skill via the platform's skill-invocation primitive (see step 4 below) — only if the user opted in to session history. The skill call is synchronous from this orchestrator's main-context turn, but the already-dispatched background subagents continue running in parallel underneath, so the wall-clock benefit is preserved (`max(sl-sessions, slowest background subagent)`, not their sum). Issuing the skill call before the parallel block would serialize sl-sessions in front of the research subagents and regress wall-clock time.
...
#### 4. **Session History via `sl-sessions`** (synchronous skill call, after launching the parallel block — only if the user opted in)
   ...
   - Invoke the `sl-sessions` skill via the `Skill` tool. Pass the dispatch payload below as the skill argument string. `sl-sessions` runs in main context — it owns discovery, branch/keyword filtering, scan-window selection, the deep-dive cap, per-session extraction to a `mktemp` scratch dir, and dispatch of the synthesis-only `sl-session-historian` subagent. The compound orchestrator only needs to pass the topic and time window and read back the findings text.

   **Dispatch payload — keep tight.** A long, keyword-rich payload licenses sl-sessions to keep widening. Use this shape:
   ...
   Do not append additional context blocks, exclusion lists, or topic-keyword bullets — verbose payloads give sl-sessions license to keep widening the search and rapidly compound wall time. If keyword search is needed, sl-sessions owns that decision internally based on the topic.
   - Returns: structured digest of findings from prior sessions, or "no relevant prior sessions" if none found.
   - **sl-sessions is the final Phase 1 input, not a workflow stop.** When it returns, proceed directly to Phase 2 with its output as the last input — do not emit a summary and do not pause for the user. A "no relevant prior sessions" return is still a valid input; the documentation gets written without session context.

### Phase 2: Assembly & Write

<sequential_tasks>

**WAIT for all Phase 1 inputs to complete before proceeding** — the three parallel subagents and, when the user opted in, the synchronous `sl-sessions` skill call. sl-sessions is a Phase 1 input even though it is a skill rather than a subagent.
...
3. **Incorporate session history findings** (if available). When `sl-sessions` returned relevant prior-session context:
...
8. **Validate parser-safety of the written frontmatter** to catch silent-corruption issues the prose rules miss: malformed `---` delimiter lines, unquoted ` #` in scalar values (silent comment truncation), and unquoted `: ` in scalar values (silent mapping confusion). The bundled validator ships **inside the skill bundle**; `${CLAUDE_SKILL_DIR}` resolves to the skill directory, but the runtime Bash tool's CWD is the user's project, so a project-relative path (without the `${CLAUDE_SKILL_DIR}` prefix) would miss. Run it through an existence guard so it falls back to a manual check instead of silently skipping the protection when the script cannot be located:
...
`sl-compound-refresh` is **not** a default follow-up. Use it selectively when the new learning suggests an older learning or pattern doc may now be inaccurate.

It makes sense to invoke `sl-compound-refresh` when one or more of these are true:
...
It does **not** make sense to invoke `sl-compound-refresh` when:
...
- If there is **one obvious stale candidate**, invoke `sl-compound-refresh` with a narrow scope hint after the new learning is written
- If there are **multiple candidates in the same area**, ask the user whether to run a targeted refresh for that module, category, or pattern set
- If context is already tight or you are in lightweight mode, do not expand into a broad refresh automatically; instead recommend `sl-compound-refresh` as the next step with a scope hint
- **In headless mode**, never invoke `sl-compound-refresh` and never ask the user. Surface the recommended scope hint in the terminal report's "Refresh recommendation" line and let the caller decide

When invoking or recommending `sl-compound-refresh`, be explicit about the argument to pass. Prefer the narrowest useful scope:
...
- `/sl-compound-refresh plugin-versioning-requirements`
- `/sl-compound-refresh payments`
- `/sl-compound-refresh performance-issues`
- `/sl-compound-refresh critical-patterns`
...
Do not invoke `sl-compound-refresh` without an argument unless the user explicitly wants a broad sweep.
...
4. If not:
   ...
   b. Draft the smallest addition that communicates the three things. Match the file's existing style and density. The addition should describe the knowledge store itself, not the plugin — an agent without the plugin should still find value in it.
      ...
      ```
   c. In full interactive mode, explain to the user why this matters — agents working in this repo (including fresh sessions, other tools, or collaborators without the plugin) won't know to check `docs/solutions/` unless the instruction file surfaces it. Show the proposed change and where it would go, then use `AskUserQuestion` to get consent before making the edit (call `ToolSearch` with `select:AskUserQuestion` first if its schema isn't loaded). Fall back to presenting the proposal in chat only if it is unavailable or the call errors. Never silently skip the question. In lightweight mode, output a one-liner note and move on. In headless mode, apply the edit directly without prompting and surface it in the terminal report under "Instruction-file edit"
...
- **performance_issue** → `sl-performance-oracle`
- **security_issue** → `sl-security-sentinel`
- **database_issue** → `sl-data-integrity-guardian`
- Any code-heavy issue → always run `sl-code-simplicity-reviewer` for minimal, clear examples. Structural concerns in the diff are already covered when the same work goes through `/sl-code-review` (maintainability persona).
...
re-run /sl-compound in a fresh session.
```
...
In lightweight mode, the overlap check is skipped (no Related Docs Finder subagent). This means lightweight mode may create a doc that overlaps with an existing one. That is acceptable — `sl-compound-refresh` will catch it later. Only suggest `sl-compound-refresh` if there is an obvious narrow refresh target. Do not broaden into a large refresh sweep from a lightweight session.
...
Refresh recommendation: <none | scope hint for /sl-compound-refresh>
...
Specialized Agent Reviews (Auto-Triggered):
  ✓ sl-performance-oracle: Validated query optimization approach
  ✓ sl-code-simplicity-reviewer: Solution is appropriately minimal
...
**After displaying the interactive success output above, present the "What's next?" options using `AskUserQuestion`** (call `ToolSearch` with `select:AskUserQuestion` first if its schema isn't loaded). Fall back to numbered options in chat only if it is unavailable or the call errors. Never silently skip the question. Do not continue the workflow or end the turn without the user's selection. (Interactive mode only — headless skips this per the headless block above.)
...
<manual_override> Use /sl-compound [context] to document immediately without waiting for auto-detection. </manual_override> </auto_invoke>
...
### Code Quality & Review
- **sl-code-simplicity-reviewer**: Ensures solution code is minimal and clear
- **sl-pattern-recognition-specialist**: Identifies anti-patterns or repeating issues

### Specific Domain Experts
- **sl-performance-oracle**: Analyzes performance_issue category solutions
- **sl-security-sentinel**: Reviews security_issue solutions for vulnerabilities
- **sl-data-integrity-guardian**: Reviews database_issue migrations and queries

### Enhancement & Research
- **sl-best-practices-researcher**: Enriches solution with industry best practices
- **sl-framework-docs-researcher**: Links to framework/library documentation references

### When to Invoke
- **Auto-triggered** (optional): Agents can run post-documentation for enhancement
- **Manual trigger**: User can invoke agents after /sl-compound completes for deeper review

## Related Commands

- `/research [topic]` - Deep investigation (searches docs/solutions/ for patterns)
- `/sl-plan` - Planning workflow (references documented solutions)
