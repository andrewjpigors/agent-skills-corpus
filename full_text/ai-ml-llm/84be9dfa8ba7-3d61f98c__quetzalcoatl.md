...
| `/spec`                                    | 모호한 의도를 실행 가능한 명세로 바꾼다 (§8.1)                                                    |
| `/plan`                                    | 단계별 실행 계획을 만든다 (§8.2)                                                                  |
| `/design-review`                           | 제품/UX/시스템 설계를 평가한다 — 규모 적합성 포함 (§9.2)                                          |
| `/eng-review`                              | 아키텍처, 데이터 흐름, 테스트, 엣지 케이스를 검토한다 (§9.1)                                      |
...
| `/decision-log`                            | 결정 기록을 남긴다 (§6.3 · C-10)                                                                  |
| `/ship`                                    | 배포 전 체크리스트와 릴리즈 판단을 만든다 (§12.1)                                                 |
| `/retro`                                   | 회고, 배운 점, 다음 개선을 정리한다 (§12.2)                                                       |
...
모호한 의도를 실행 가능한 명세로 바꿀 때 쓴다 — 📋 **§22 부록 C-12 (SPEC)**. 규모(§1.2)는 기능처럼 명세한다 — 어느 범위까지 성립해야 하는지, 그 밖은 무엇이 명시된 한계인지.

### 8.2 Plan

승인된 방향(§3)과 명세(§8.1)를 실행 순서로 바꾼다. **방향이 아직 합의되지 않은 원시 아이디어에 `/plan`이 바로 들어오면, 먼저 §3(Intake → Office Hours)으로 돌아간다(§1.1).** 📋 **§22 부록 C-22 (Plan)**. 규칙:

1. 단계마다 산출물과 검증 방법(증거, §14)을 붙인다.
2. §1.6 게이트에 걸리는 단계는 계획에서 미리 `게이트`로 표시한다 — 자율 루프(§18.3)가 그 앞에서 멈춘다.
3. 의존을 명시한다 — 미해결 결정 위에 쌓지 않는다(§1.7).
4. 계획의 작업은 대시보드 작업 보드(§7.1)로 옮겨 추적한다.

---

## 9. Design & Engineering Review 프로토콜

### 9.1 Engineering Review
...
출력 형식 — 📋 **§22 부록 C-13 (Engineering Review)** · 아키텍처 문서 양식 — 📋 **§22 부록 C-25 (Architecture)**.
...
제품/UX/시스템 설계를 사용자 관점에서 평가한다 — 📋 **§22 부록 C-23 (Design Review)**. 반드시 본다:

- 첫 사용 여정의 마찰 · 정보 구조 · 상태 커버리지(빈/로딩/오류/성공) · 일관성(용어·패턴) · 접근성 · 10-star(§3 Phase 1-6)와의 거리.
- **규모 적합성(scale envelope)**: 설계는 **명시된 규모 범위(§1.2·§8.1)의 양끝에서** 성립해야 한다. 규모가 **자릿수로** 바뀌면 **같은 UI를 그대로 늘리는 것을 기본값으로 삼지 않는다** — 상호작용 패러다임 전환을 검토하고, 기존 패러다임을 유지하려면 그 근거를 남긴다. 예: 항목 수십 개는 평면 목록 편집으로 충분하지만, 수백~수천 개면 보통 계층·구역(필터) 선택·검색·일괄 작업이 필요해진다. 장래 규모를 지금 다 만들라는 뜻이 아니다(§15 — 과설계 금지). **어느 규모까지 성립하는 설계인지 선언하고, 그 밖은 명시된 한계로 기록**한다.

평가는 취향이 아니라 사용자 가치와 근거로 한다(§1.4). 고칠 것은 반드시/나중에로 나눈다(§15).
...
사이클이 끝나면(배포 직후, 또는 중단·보류로 닫을 때) 회고를 남긴다 — 📋 **§22 부록 C-24 (Retro)**. 성과 나열이 아니라 **다음 사이클을 바꿀 교훈**을 뽑는다.

- 잘된 점은 원인까지, 아쉬운 점은 **교정 행동**까지 적는다. 교정은 가능하면 규율이 아니라 강제 수단으로 옮긴다(§18.7 — 예: 반복 실수를 CI 검사로).
- 실패·미수행도 기록한다(§15 — 불확실성을 숨기지 않는다).
- deferred 목록을 갱신해 다음 사이클의 입력으로 넘긴다.

---

## 13. 기본 응답 형식

사용자의 요청이 들어오면 **먼저 크기를 정하고, 그 크기의 형식만 쓴다**(작은 일에 큰 프로세스를 강제하지 않는다 — §15). **크기는 형식의 크기일 뿐이다 — §1.1(의미 먼저)과 §1.6(게이트)는 어떤 크기에서도 생략되지 않는다.** 애매하면 가벼운 쪽으로 시작하고, 진행 중 커지면 승격한다.

| 크기     | 신호                                                          | 형식                                                                                                                                  |
| -------- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| 간단     | 질문·단답·소수정 — 가역적, 새 결정 없음                       | §13.1                                                                                                                                 |
| 중간     | 실작업이지만 방향이 자명하고 가역적 — 다단계여도 큰 결정 없음 | **Intake-lite**: 목표·가정·**의미/가치 리스크**를 1~3줄 명시 후 바로 실행. Office Hours·문서 트리는 생략, 게이트(§1.6)·증거(§14) 유지 |
| 프로젝트 | 방향 결정 필요, 비가역, 이해관계자·다세션                     | §13.2 + 전체 워크플로우(§3)                                                                                                           |
...
| 규모 범위(Scale Envelope)        | 설계가 성립해야 하는 명시된 규모 구간(사용자·항목·동시성의 현재→장래). 자릿수가 바뀌면 상호작용 패러다임 전환을 검토한다(§9.2).                                             |
...
### 부록 C. 템플릿 모음 → `TEMPLATES.md` (v3.0.0 분리)

> 본문(§0~§24)은 **규칙(Core Contract)**, 그 규칙이 쓰는 **양식(forms)** 전문은 함께 배포되는 **[`TEMPLATES.md`](TEMPLATES.md)**에 있다. 각 절의 "📋 §22 부록 C-N"은 그 파일의 C-N을 가리킨다.
>
> - **에이전트(파일) 환경**: 템플릿이 필요한 시점에 이 스킬 폴더의 `TEMPLATES.md`에서 **해당 C-N만** 읽어 쓴다. 정직(§15): 본문만 로드되는 것은 로더의 동작이고, **"필요한 C-N만 읽는다"는 강제가 아니라 규율**이다.
> - **도구 없는 챗 환경**: 본문+템플릿 합본인 페이스트 번들 `dist/Quetzalcoatl-FULL.md`를 붙여넣는다 — 번들에서는 템플릿 전문이 이 문서 끝에 포함된다.
> - **템플릿 파일도 번들도 없을 때(강등)**: 아래 색인의 이름과 "쓰는 절"이 최소 명세다 — 해당 규칙 절의 요구대로 같은 구조의 마크다운을 즉석 생성해 쓴다(§18.6 강등과 같은 원리).
...
| C-11 | Project Dashboard + 핸드오프   | §7 · §21.1    |
| C-12 | SPEC                           | §8.1          |
| C-13 | Engineering Review             | §9.1          |
| C-14 | Risk Register                  | §10           |
| C-15 | Test Plan                      | §11           |
| C-16 | Ship Checklist                 | §12.1         |
| C-17 | 응답 형식(프로젝트성·교차검증) | §13.2 · §13.3 |
| C-18 | 첫 응답 미니 템플릿            | §17           |
| C-19 | 실행 허용목록                  | §18.2         |
| C-20 | Resume Check                   | §19.3         |
| C-21 | Project Context Briefing       | §24           |
| C-22 | Plan                           | §8.2          |
| C-23 | Design Review                  | §9.2          |
| C-24 | Retro                          | §12.2         |
| C-25 | Architecture                   | §6.3 · §9.1   |
