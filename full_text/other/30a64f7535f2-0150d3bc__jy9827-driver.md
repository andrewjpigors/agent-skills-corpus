---
name: jy9827-driver
description: 提供 JYTEK JY9827/PXIe-9827 高速数字化仪（Digitizer）的完整 C# 驱动开发指引。涵盖高速模拟输入（AI）有限/连续/录制采集、多通道同步采集、触发配置（数字/模拟/软件触发、StartTrigger/ReferenceTrigger/ReTrigger）、参考时钟同步、多卡同步（<500 ps精度）、DC/AC耦合、50Ω/1MΩ输入阻抗、9种输入量程（±0.1V~±10V）、3.5 GB DDR4缓存、录制模式（Finite/Infinite Streaming）。当用户使用 JY9827、PXIe-9827、JY9827AITask、AIMode.Finite、AIMode.Continuous、AIMode.Record、SyncTopology.Master、SyncTopology.Slave 开发高速数据采集、数字化仪、波形记录、瞬态信号捕捉应用时自动应用。
---
...
- **驱动 DLL**：`C:\SeeSharp\JYTEK\Hardware\Digitizer\JY9827\Bin\JY9827.dll`（引用到 .csproj）
- **XML 文档**：`C:\SeeSharp\JYTEK\Hardware\Digitizer\JY9827\Bin\JY9827.xml`（提供 IntelliSense 支持）
...
| AI 分辨率 | 9-bit |
| AI 通道数 | 4 通道同步采样 |
| AI 最大采样率 | 1.2 GS/s（每通道） |
| AI 采样率范围 | 69.4 kS/s ~ 1.2 GS/s |
| AI 输入量程 | ±0.1V/±0.2V/±0.4V/±0.8V/±1V/±2V/±4V/±8V/±10V（软件可选） |
| AI 输入阻抗 | 50Ω / 1 MΩ（软件可选） |
| AI 输入耦合 | DC / AC（软件可选） |
| AI 输入模式 | RSE（单端） |
| AI 模拟带宽 | DC ~ 250 MHz（-3 dB） |
| AI DC 精度 | 1.5% 满量程 |
| 同步精度 | <500 ps @ 1 GS/s |
| 板载缓存 | 3.5 GB DDR4 |
| 串扰 (@1 MHz) | -65 dB |
...
| 输入量程 | 采样率 | 阻抗 | THD | SINAD | SFDR | SNR | ENOB |
|---------|--------|------|-----|-------|------|-----|------|
| 0.1 V | 1 GS/s | 50Ω | 66.42 dBc | 46.67 dB | 69.01 dBc | 46.72 dB | 7.46 bits |
| 0.2 V | 1 GS/s | 50Ω | 62.61 dBc | 49.03 dB | 64.57 dBc | 49.23 dB | 7.85 bits |
| 0.4 V | 1 GS/s | 50Ω | 67.01 dBc | 49.73 dB | 71.7 dBc | 49.81 dB | 7.97 bits |
| 0.8 V | 1 GS/s | 50Ω | 59.47 dBc | 49.8 dB | 61.97 dBc | 50.01 dB | 7.98 bits |
| 1 V | 1 GS/s | 50Ω | 58.22 dBc | 42.36 dB | 60.35 dBc | 42.47 dB | 6.74 bits |
| 2 V | 1 GS/s | 50Ω | 65.58 dBc | 46.83 dB | 69.63 dBc | 46.89 dB | 7.49 bits |
| 4 V | 1 GS/s | 50Ω | 60.06 dBc | 48.71 dB | 61.67 dBc | 49.04 dB | 7.80 bits |

### DC 精度规格（1 MΩ输入阻抗）

| 标称量程 (V) | 24小时精度 Tcal±1°C | 24小时满量程精度 |
|-------------|---------------------|-----------------|
| 0.1 | 0.36% Reading + 0.86% Range (2 mV, 2.0%) |
| 0.2 | 0.34% Reading + 0.70% Range (3 mV, 2.0%) |
| 0.4 | 0.34% Reading + 0.60% Range (4 mV, 1.5%) |
| 0.8 | 0.38% Reading + 1.20% Range (13 mV, 2.3%) |
| 1 | 0.38% Reading + 1.20% Range (16 mV, 2.2%) |
| 2 | 0.38% Reading + 1.20% Range (32 mV, 2.2%) |
| 4 | 0.43% Reading + 0.78% Range (49 mV, 1.9%) |
| 8 | 0.38% Reading + 0.56% Range (76 mV, 1.6%) |
| 10 | 0.40% Reading + 0.56% Range (96 mV, 1.6%) |

## 通用编程范式

所有 JY9827 采集任务遵循以下标准流程：
...
```csharp
// 1. 创建 Task
JY9827AITask aiTask = new JY9827AITask(boardNumber);

// 2. 添加通道
// 量程通过 rangeLow/rangeHigh 参数设置，如 ±1V 量程：rangeLow=-1, rangeHigh=1
// JY9827支持耦合方式和输入阻抗配置
aiTask.AddChannel(channelID, -1, 1, AICoupling.DC, AIImpedance.Impedance50Ohm);  // ±1V 量程

// 3. 配置采集模式
aiTask.Mode = AIMode.Continuous;  // 或 Finite / Record
aiTask.SampleRate = 1200000000;    // 1.2 GS/s 最高采样率

// 4. 启动采集
aiTask.Start();

// 5. 读取数据
// 注意：JY9827的ReadData没有samplesToRead参数，自动读取所有可用数据
...
**重要**：JY9827的多卡同步使用`SignalExport`而非`Sync`对象。主卡导出触发信号，从卡接收触发信号。

```csharp
// 主卡配置：导出开始触发信号到PXI_Trig0
masterTask.SignalExport.Add(AISignalExportSource.TriggerIn, SignalExportDestination.PXI_Trig0);
...
slaveTask.Trigger.Digital.Source = AIDigitalTriggerSource.PXI_Trig0;  // 注意：JY9827不带下划线
...
JY9827 通过 `rangeLow` 和 `rangeHigh` 参数设置量程：
...
- `AISignalExportSource.None` - 无
- `AISignalExportSource.PFI0` - 前面板PFI0
- `AISignalExportSource.TriggerIn` - 面板TriggerIn
- `AISignalExportSource.Analog_Trigger_0_trigger` - 模拟触发0
- `AISignalExportSource.PXI_Trig0` ~ `PXI_Trig7` - PXI触发总线
- `AISignalExportSource.Rx_Engine_Real_start_trigger_out` - Rx引擎实时启动触发输出
- `AISignalExportSource.Rx_Engine_Real_reference_trigger_out` - Rx引擎实时参考触发输出
- `AISignalExportSource.Rx_Engine_Real_advance_trigger_out` - Rx引擎实时提前触发输出
...
- `SignalExportDestination.Rx_Engine_Real_start_trigger_in` - Rx引擎实时启动触发输入
- `SignalExportDestination.Rx_Engine_Real_reference_trigger_in` - Rx引擎实时参考触发输入
- `SignalExportDestination.Rx_Engine_Real_advance_trigger_in` - Rx引擎实时提前触发输入

**说明**：从卡通过Rx Engine触发输入接收主卡的触发信号：
- 主卡导出 `StartTrig` → 从卡接收 `Rx_Engine_Real_start_trigger_in`
- 主卡导出 `ReferenceTrig` → 从卡接收 `Rx_Engine_Real_reference_trigger_in`

---

## 录制模式（Record）

JY9827 支持将采集数据流式写入板载内存的 Record 模式，3.5 GB DDR4 缓存支持长时间高速采集：

```csharp
aiTask = new JY9827AITask(0);
aiTask.AddChannel(0, -1, 1);  // ±1V 量程 (rangeLow=-1, rangeHigh=1)
aiTask.Mode = AIMode.Record;
aiTask.SampleRate = 1200000000;  // 1.2 GS/s 全速采集

// 配置录制参数（具体属性请参考驱动版本）
aiTask.Start();

// 录制过程中可读取数据（注意：JY9827没有samplesToRead参数）
...
| 构造函数 | 说明 |
|----------|------|
| `JY9827AITask(int boardNumber)` | 创建指定板卡的 AI 任务 |
...
| 方法 | 说明 |
|------|------|
| `AddChannel(int channel, double rangeLow, double rangeHigh, AICoupling coupling, AIImpedance impedance)` | 添加单个通道（Real模式） |
| `AddChannel(int[] channels, double rangeLow, double rangeHigh, AICoupling coupling, AIImpedance impedance)` | 批量添加通道（统一参数） |
| `AddChannel(int[] channels, double[] rangeLow, double[] rangeHigh, AICoupling[] coupling, AIImpedance[] impedance)` | 批量添加通道（各自独立参数） |
| `AddChannel(int channel, double centerFrequency, double digitalGain, double rangeLow, double rangeHigh)` | 添加通道（Complex模式） |
| `Start()` | 启动采集任务 |
| `Stop()` | 停止采集任务 |
...
| `ReadRawData(ref short[] buffer, int timeout)` | 读取单通道原始数据（注意：没有samplesToRead参数） |
...
| 型号 | 描述 |
|------|------|
| PXIe-9827 | 4-CH, 1.2 GS/s, 9-bit, 20 Vpp, BW: 250 MHz PXIe Digitizer |
...
| `None` | 无 |
| `PFI0` | 前面板PFI0 |
| `TriggerIn` | 面板TriggerIn |
| `Analog_Trigger_0_trigger` | 模拟触发0 |
| `PXI_Trig0` ~ `PXI_Trig7` | PXI触发总线 |
| `Rx_Engine_Real_start_trigger_out` | Rx引擎实时启动触发输出（≈其他驱动的StartTrig） |
| `Rx_Engine_Real_reference_trigger_out` | Rx引擎实时参考触发输出（≈其他驱动的ReferenceTrig） |
| `Rx_Engine_Real_advance_trigger_out` | Rx引擎实时提前触发输出 |
...
| `PXI_Trig0` ~ `PXI_Trig7` | PXI触发总线 |
| `PFI0` | 前面板PFI0端子 |
| `Rx_Engine_Real_start_trigger_in` | Rx引擎实时启动触发输入（接收其他卡的StartTrig） |
| `Rx_Engine_Real_reference_trigger_in` | Rx引擎实时参考触发输入（接收其他卡的ReferenceTrig） |
| `Rx_Engine_Real_advance_trigger_in` | Rx引擎实时提前触发输入 |
...
namespace JY9827Example
{
    public partial class MainForm : Form
    {
        private JY9827AITask aiTask;
        ...
        private void button_start_Click(object sender, EventArgs e)
        {
            ...
            {
                // 创建 Task
                aiTask = new JY9827AITask(0);  // 板卡 0
                ...
                aiTask.SampleRate = 1200000000;  // 1.2 GS/s
...
private void button_start_Click(object sender, EventArgs e)
{
    try
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            aiTask.AddChannel(i, -1, 1);  // ±1V: rangeLow=-1, rangeHigh=1
        }
...
private void button_start_Click(object sender, EventArgs e)
{
    try
    {
        ...
        // 配置模拟触发（触发电平相对于量程，±1V量程下0.5 = 0.5V）
        aiTask.Trigger.Type = AITriggerType.Analog;
        aiTask.Trigger.Analog.Source = AIAnalogTriggerSource.Channel_0;
        aiTask.Trigger.Analog.Condition = AIAnalogTriggerCondition.RisingEdge;
...
**重要**：JY9827使用SignalExport进行多卡同步，而非Sync对象。

```csharp
private JY9827AITask masterTask;
private JY9827AITask slaveTask;
...
private void button_start_Click(object sender, EventArgs e)
{
    ...
    {
        // 主卡配置
        masterTask = new JY9827AITask(2);  // 主卡槽位 2
        masterTask.AddChannel(0, -1, 1);  // ±1V 量程
        masterTask.Mode = AIMode.Finite;
        masterTask.SamplesToAcquire = 10000;
        masterTask.SampleRate = 1200000000;  // 1.2 GS/s
    ...
    {
        // 从卡配置
        slaveTask = new JY9827AITask(4);  // 从卡槽位 4
        slaveTask.AddChannel(0, -1, 1);  // ±1V 量程
        slaveTask.Mode = AIMode.Finite;
        slaveTask.SamplesToAcquire = 10000;
        slaveTask.SampleRate = 1200000000;  // 1.2 GS/s
...
private void button_start_Click(object sender, EventArgs e)
{
    try
    {
        ...
        // JY9827 支持 DC ~ 250 MHz 带宽，50Ω 阻抗匹配
        // 适合采集高速瞬态信号
        aiTask.AddChannel(0, -0.1, 0.1);  // ±0.1V 量程适合小信号
...
JY9827 支持将数据流式录制到文件，录制完成后自动停止。定时器中通过`GetRecordStatus`检查录制状态：
...
private void button_Start_Click(object sender, EventArgs e)
{
    try
    {
        ...
        // 配置 Record 模式
        aiTask.Mode = AIMode.Record;
        aiTask.SampleRate = 1200000000;  // 1.2 GS/s 全速录制
        
        // 配置录制参数（3.5 GB DDR4 缓存支持长时间录制）
        aiTask.Record.FilePath = @"C:\Data\record.bin";
        aiTask.Record.Mode = RecordMode.Finite;  // 有限录制模式
        aiTask.Record.Length = 1.0;               // 录制 1 秒（约 4.8GB 数据）
...
读取已录制的 bin 文件并回放显示（JY9827 数据格式为 short）：
