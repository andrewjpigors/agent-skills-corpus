...
description: Orchestrates a council of personality "seats" for Pi. Use when the user runs /council, asks to convene a council, hold a council meeting, inspect council info, or have the council work on a task. Handles the convene, info, meeting, and work verbs by reading .council/, spawning seat workers when available, driving the shared scratchpad, and having the chair synthesize.
---

# Council Orchestrator

A **council** is a set of named **seats** (personalities) plus a **chair** that
routes them and synthesizes their output. This skill runs the four council
verbs. You are the orchestrator. Prefer an isolated worker per seat using the
host's worker facility carrying that seat's persona, and let the chair route and
synthesize. In Pi sessions without a worker/subagent tool, you may use the
current agent as a clearly labeled inline fallback after telling the user that
seat turns are not isolated.
...
- **Pi:** the user invokes the packaged prompt template as `/council ...`,
  invokes `/skill:council-orchestrator`, or asks conversationally, e.g.
  `council convene software-team`, `council info`,
  `council meeting "<task>"`, or `council work "<task>"`. Resolve
  `COUNCIL_PLUGIN_ROOT` as the installed Pi package root that contains this
  `skills/council-orchestrator/SKILL.md` file plus the sibling `templates/` and
  `personalities/` directories.
- **Worker facility:** when a `subagent` tool (or another Pi worker-spawn tool)
  is available, use it for seat workers one seat at a time. Pass the seat persona
  as the worker's system prompt or task preamble, and do not set a model override
  unless the user explicitly requests it.
- **Inline fallback:** if no worker facility is loaded, say so before `meeting`
  or `work`, then run each seat turn inline in the current session. For each
  inline seat turn, reread that seat's persona, label the scratchpad heading as
  normal, and include a one-line note that the turn was executed inline rather
  than in an isolated subagent. Do not claim context isolation in fallback mode.
...
2. **Guard existing council.** If `$ROOT/.council/council.yaml` already exists, warn
   ...
   hand-edits live) and confirm before proceeding. Use a structured confirmation
   prompt if the host exposes one, with options **Cancel — keep the current
   council** first and **Recreate — overwrite roster and seats** second (plain
   conversation if no structured prompt is available). Do not clobber without a
   yes.
...
4. **Round loop:**
   ...
   b. After the round, give the user a tight summary of what each seat said —
      ...
      by design. Then **ask for their input** with a structured question prompt
      if the host exposes one, otherwise ask in plain conversation and wait.
      ...
      (If no structured question tool is available, ask in plain conversation
...
Spawn each seat with Pi's available worker tool when possible:

- **Preferred:** use a `subagent` tool if one is loaded. Invoke single-agent mode
  (`agent`/`task` or the equivalent exposed by the loaded tool) one seat at a
  time and wait for the result before appending the scratchpad. If the tool
  supports a `cwd`, set it to the worktree path for `work` turns; otherwise name
  the absolute worktree path in the prompt and require all file operations under
  it.
- **Other Pi worker tools:** if a different worker-spawn tool is available, use
  the same prompt and the closest read-only/worker execution mode it exposes.
- **Inline fallback:** if no worker tool is available, execute the seat turn in
  the current session by strictly adopting the seat persona for that one section,
  then immediately return to the orchestrator role. Prefix the appended response
  with `_Execution: inline fallback (not an isolated subagent)._`.

Meeting and work are sequential, so run one seat at a time and wait.
