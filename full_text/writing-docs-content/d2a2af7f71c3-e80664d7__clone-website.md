---
name: clone-website
description: Reverse-engineer and clone one or more websites in one shot — crawls the starting URL's internal child pages, extracts assets, CSS, and content page-by-page and section-by-section, and proactively dispatches parallel builder agents in worktrees as it goes. Use this whenever the user wants to clone, replicate, rebuild, reverse-engineer, or copy any website. Also triggers on phrases like "make a copy of this site", "rebuild this page", "pixel-perfect clone". Provide one or more target URLs as arguments.
...
By default, this is a **multi-page clone**. Treat each starting URL as the root of a same-origin site graph. Discover the page itself plus its internal child pages, then rebuild the route structure in Next.js so links like docs, pricing, register, product details, and other internal destinations work inside the clone.

## Scope Defaults

The target is whatever page `$ARGUMENTS` resolves to. Unless the user specifies otherwise, use these defaults:

- **Fidelity level:** Pixel-perfect — exact match in colors, spacing, typography, animations
- **Page coverage:** Clone the starting page and all same-origin internal child pages you can discover from navigation, CTAs, cards, footer links, sitemap sections, and in-content links
- **In scope:** Visual layout and styling, component structure and interactions, responsive design, mock data for demo purposes
- **Out of scope:** Real backend / database, authentication, real-time features, SEO optimization, accessibility audit, user-specific/private pages, checkout/account flows that require server state
- **Customization:** None — pure emulation

Apply these crawl boundaries unless the user overrides them:

- Follow only `http`/`https` links on the same origin or an explicitly approved sibling subdomain
- Skip external links, `mailto:`, `tel:`, hash-only anchors, file downloads, logout links, and obvious session-specific endpoints
- Normalize URLs before deduping: drop hash fragments, sort query params, and collapse duplicate trailing-slash variants
- Treat faceted search, sort/filter permutations, and infinite pagination as unbounded spaces; ask the user whether to include them before cloning hundreds of near-duplicate pages
...
5. Before building anything, discover the internal route graph for each target and write it down. Produce `docs/research/<hostname>/SITE_MAP.md` (or `docs/research/SITE_MAP.md` for a single site) with every normalized internal URL, how it was discovered, its route depth, and its page family (marketing, docs, blog, pricing, product, product detail, auth, etc.).
6. If the route graph is obviously huge or unbounded (for example, hundreds of product detail pages, faceted search URLs, calendar archives, or paginated listings), stop and ask the user whether they want the full crawl, a capped representative subset, or only key route families.
...
For multi-page sites, foundation also means shared layouts, shell components, route helpers, and content schemas. Build the pieces that every page depends on before dispatching page-specific builders.
...
**Global UI patterns** — Identify any site-wide CSS or JS: custom scrollbar hiding, scroll-snap on the page container, global keyframe animations, backdrop filters, gradients used as overlays, **smooth scroll libraries** (Lenis, Locomotive Scroll — check for `.lenis`, `.locomotive-scroll`, or custom scroll container classes), and any motion/runtime libraries (Framer Motion, GSAP, Lottie, Swiper, Embla, canvas/WebGL renderers). Add these to `globals.css`, note any libraries that need to be installed, and record where exact runtime reproduction is required instead of CSS-only approximation.
...
- For scroll-driven motion, record whether the effect is discrete or scrubbed, which properties are animated, and whether the timing is controlled by CSS, JS, or a third-party library.
...
- For internal navigation links: record whether they navigate to another page, open a new tab, or reveal in-page content; add newly discovered same-origin destinations to the site map
...
**Responsive sweep:** Test at these viewport widths via browser MCP:
- Desktop XL: 1440px
- Desktop: 1280px
- Laptop: 1024px
...
- Small mobile: 375px
- Narrow mobile: 360px
- At each width, note which sections change layout (column → stack, sidebar disappears, navigation collapses, media crops differently, typography scales, etc.) and the exact breakpoint range where the change occurs.
...
Before diving deep into one page's sections, map the whole cloneable surface area:

- Enumerate every visible same-origin link from the header, footer, sidebars, cards, CTAs, sitemap sections, and in-content rich text
- Visit each unique internal destination and repeat the link discovery pass until no new in-scope pages remain
- Record canonical URL, page title, route depth, parent/source page, page family, and notes about auth/protected/dead-end states
- Detect route templates: pages that share one shell but differ only by content data (for example docs article pages, product detail pages, blog posts)
- Save the final route graph to `docs/research/<hostname>/SITE_MAP.md`
- For large families, create a representative sample list and a data extraction plan rather than blindly hand-building each page one at a time

### Page Topology
For each discovered page, map out every distinct section from top to bottom. Give each a working name. Document:
- Their visual order
...
Save this as `docs/research/<hostname>/<route-slug>/PAGE_TOPOLOGY.md` when multiple pages are involved, or `docs/research/PAGE_TOPOLOGY.md` for a single-page clone — it becomes your assembly blueprint.
...
3. **Create shared layout primitives** — route shell, header, footer, navigation, breadcrumbs, and any shared page chrome used across multiple routes
4. **Create TypeScript interfaces** in `src/types/` for the content structures and route families you've observed
5. **Plan the Next.js route tree** — map discovered URLs onto `src/app/` routes, route groups, dynamic segments, and shared layouts before dispatching page builders
...
7. **Download global assets** — write and run a Node.js script (`scripts/download-assets.mjs`) that downloads all images, videos, and other binary assets from the page to `public/`. Preserve meaningful directory structure.
...
This is the core loop. Work page family by page family, then section by section. For each discovered route, you do THREE things: **extract**, **write the spec file**, then **dispatch builders**.

Start with shared templates before leaf pages:

- Build the global app shell and navigation once
- Build route-family templates next (for example docs index, docs article, pricing, blog index, blog post, product listing, product detail)
- Finally build page-specific exceptions that do not fit an existing template

If ten pages share one template, do not dispatch ten unrelated builders. Dispatch one builder for the shared template and one data-extraction task for the per-page content.

### Step 1: Extract

For each page section, use browser MCP to extract everything:
...
For animation-heavy sections, also capture:
- duration
- delay
- easing / cubic-bezier
- transform origin
- keyframe names or motion variants
- whether motion is one-shot, looped, staggered, or scroll-scrubbed
- whether the source uses CSS, WAAPI, requestAnimationFrame, canvas, video, or a motion library
...
For each section (or sub-component, if you're breaking it up), create a spec file in `docs/research/components/` or a page-scoped equivalent. This is NOT optional — every builder must have a corresponding spec file.

**File path:** `docs/research/<hostname>/<route-slug>/components/<component-name>.spec.md` for multi-page clones, or `docs/research/components/<component-name>.spec.md` for a single-page clone
...
- **Motion fidelity note:** <exact enough with CSS only | requires Framer Motion | requires GSAP timeline | requires canvas/video fallback>
...
## Responsive Behavior
- **Desktop XL (1440px):** <layout description>
- **Desktop (1280px):** <what changes>
- **Laptop (1024px):** <what changes>
- **Tablet landscape (820px):** <what changes>
- **Tablet portrait (768px):** <what changes>
- **Large mobile (430px):** <what changes>
- **Mobile (390px):** <what changes>
- **Small mobile (375px):** <what changes>
- **Narrow mobile (360px):** <what changes>
- **Breakpoints:** <exact breakpoint ranges and what changes in each range>
```
...
After all sections are built and merged, wire everything together in the Next.js route tree:

- Create the required `src/app/` pages, layouts, route groups, and dynamic segments for every discovered internal route
- Import all shared shell and page section components
- Implement each page-level layout from its topology doc (scroll containers, column structures, sticky positioning, z-index layering)
- Connect real content to component props or route data files
...
After assembly, do NOT declare the clone complete. Take side-by-side comparison screenshots for the homepage and representative child pages:

1. Open the original site and your clone side-by-side (or take screenshots at the same viewport widths)
2. Compare section by section, top to bottom, at `1440px`, `1280px`, `1024px`, `820px`, `768px`, `430px`, `390px`, `375px`, and `360px`
3. For route families, spot-check at least one representative child page per family at the same viewport matrix
...
6. Verify smooth scroll feels right, header transitions work, tab switching works, animations play at the correct speed/easing, and no layout shifts or crop regressions appear across the viewport matrix
7. If the source uses a motion library or canvas/video-driven effect, verify the implementation approach still matches the perceived behavior instead of silently replacing it with a generic fade/slide
...
- [ ] Responsive behavior is documented for `1440`, `1280`, `1024`, `820`, `768`, `430`, `390`, `375`, and `360`
- [ ] Motion details are documented with timing, easing, trigger, and implementation approach
...
- **Don't skip responsive extraction.** If you only inspect at desktop width, the clone will break at tablet and mobile. Test at `1440`, `1280`, `1024`, `820`, `768`, `430`, `390`, `375`, and `360` during extraction.
- **Don't replace complex motion with generic fades.** If the source uses Framer Motion, GSAP, Lottie, video, canvas, or scrubbed scroll animation, document that explicitly and reproduce the perceived behavior as closely as possible.
