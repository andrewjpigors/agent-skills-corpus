...
**Options:**
1. **Update CODEX.md only** - Download latest CODEX.md without re-running full setup
2. **Run full setup again** - Go through the complete setup wizard
3. **Cancel** - Exit without changes

**If user chooses "Update CODEX.md only":**
- Detect if local (.codex/CODEX.md) or global (~/.codex/CODEX.md) config exists
...
curl -fsSL "https://raw.githubusercontent.com/sigridjineth/oh-my-codex/main/docs/CODEX.md" -o "$TEMP_OMC"

if [ ! -s "$TEMP_OMC" ]; then
  echo "ERROR: Failed to download CODEX.md. Aborting."
  rm -f "$TEMP_OMC"
  return 1
fi
...
if [ ! -f "$TARGET_PATH" ]; then
  ...
  if grep -q '<!-- OMC:START -->' "$TARGET_PATH"; then
    ...
    OLD_CONTENT=$(cat "$TARGET_PATH")
    {
      ...
      echo '<!-- OMC:END -->'
      echo ""
      echo "<!-- User customizations (migrated from previous CODEX.md) -->"
      printf '%s\n' "$OLD_CONTENT"
    } > "${TARGET_PATH}.tmp"
    mv "${TARGET_PATH}.tmp" "$TARGET_PATH"
    echo "Migrated existing CODEX.md (added OMC markers, preserved old content)"
  fi
  rm -f "$TEMP_OMC"
fi
...
https://raw.githubusercontent.com/sigridjineth/oh-my-codex/main/docs/CODEX.md
...
```bash
# Define target path
TARGET_PATH="$HOME/.codex/CODEX.md"
...
LATEST_VERSION=$(npm view oh-my-codex version 2>/dev/null)
...
- Use `/oh-my-codex:help` for guidance
- Use `/oh-my-codex:doctor` for diagnostics
...
**Options:**
1. **Built-in Tasks (default)** - Use Codex's native TaskCreate/TodoWrite. Tasks are session-only.
...
Agent teams are an experimental Codex feature that lets you spawn N coordinated agents working on a shared task list with inter-agent messaging. **Teams are disabled by default** and require enabling via `settings.json`.

Reference: https://code.codex.com/docs/en/agent-teams

Use the AskUserQuestion tool to prompt:

**Question:** "Would you like to enable agent teams? Teams let you spawn coordinated agents (e.g., `/team 3:executor 'fix all errors'`). This is an experimental Codex feature."
...
**CRITICAL**: Agent teams require `CODEX_EXPERIMENTAL_AGENT_TEAMS` to be set in `~/.codex/settings.json`. This must be done carefully to preserve existing user settings.
...
Then use the Read tool to read `~/.codex/settings.json` (if it exists). Use the Edit tool to merge the teams configuration while preserving ALL existing settings.
...
if [ -f "$SETTINGS_FILE" ]; then
  ...
  TEMP_FILE=$(mktemp)
  jq '.env = (.env // {} | . + {"CODEX_EXPERIMENTAL_AGENT_TEAMS": "1"})' "$SETTINGS_FILE" > "$TEMP_FILE" && mv "$TEMP_FILE" "$SETTINGS_FILE"
  echo "Added CODEX_EXPERIMENTAL_AGENT_TEAMS to existing settings.json"
else
...
Store the team configuration in `~/.codex/.omc-config.json`:
...
**Note:** Teammates do not have a separate model default. Each teammate is a full Codex session that inherits your configured model. Subagents spawned by teammates can use any model tier.
...
if jq -e '.env.CODEX_EXPERIMENTAL_AGENT_TEAMS' "$SETTINGS_FILE" > /dev/null 2>&1; then
...
Skip this step. Agent teams will remain disabled. User can enable later by adding to `~/.codex/settings.json`:
```json
{
  "env": {
    "CODEX_EXPERIMENTAL_AGENT_TEAMS": "1"
...
Or by running `/oh-my-codex:omc-setup --force` and choosing to enable teams.
...
- /oh-my-codex:team 3:executor "fix all TypeScript errors"
- /oh-my-codex:team 5:build-fixer "fix build errors in src/"
Teams use Codex native tools (TeamCreate/SendMessage/TaskCreate).

MCP SERVERS:
Run /oh-my-codex:mcp-setup to add tools like web search, GitHub, etc.
...
That's it! Just use Codex normally.
```
...
Spawn coordinated agents with shared task lists and real-time messaging:
- /oh-my-codex:team 3:executor "fix all TypeScript errors"
- Uses Codex native tools (TeamCreate/SendMessage/TaskCreate)
...
```bash
gh api -X PUT /user/starred/sigridjineth/oh-my-codex 2>/dev/null && echo "Thanks for starring! ⭐" || true
```
...
echo "  https://github.com/sigridjineth/oh-my-codex"
echo ""
```
...
# Get current OMC version from CODEX.md
OMC_VERSION=""
if [ -f ".codex/CODEX.md" ]; then
  OMC_VERSION=$(grep -m1 "^# oh-my-codex" .codex/CODEX.md 2>/dev/null | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
elif [ -f "$HOME/.codex/CODEX.md" ]; then
  OMC_VERSION=$(grep -m1 "^# oh-my-codex" "$HOME/.codex/CODEX.md" 2>/dev/null | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
fi
...
echo "Note: Future updates will only refresh CODEX.md, not the full setup wizard."
```
...
**Automatic**: Just run `/oh-my-codex:omc-setup` - it will detect you've already configured and offer a quick "Update CODEX.md only" option that skips the full wizard.
...
When user runs `/oh-my-codex:omc-setup --help` or just `--help`, display:
...
USAGE:
  ...
  /oh-my-codex:omc-setup --local   Configure local project (.codex/CODEX.md)
  /oh-my-codex:omc-setup --global  Configure global settings (~/.codex/CODEX.md)
...
EXAMPLES:
  /oh-my-codex:omc-setup           # First time setup (or update CODEX.md if configured)
...
For more info: https://github.com/sigridjineth/oh-my-codex
```
