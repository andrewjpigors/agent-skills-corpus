...
echo '{"skill":"retro","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.mzstack/analytics/skill-usage.jsonl 2>/dev/null || true
...
cat ~/.mzstack/greptile-history.md 2>/dev/null || true
...
# 12. mzstack skill usage telemetry (if available)
cat ~/.mzstack/analytics/skill-usage.jsonl 2>/dev/null || true
...
**Greptile signal (if history exists):** Read `~/.mzstack/greptile-history.md` (fetched in Step 1, command 8). Filter entries within the retro time window by date. Count entries by type: `fix`, `fp`, `already-fixed`. Compute signal ratio: `(fix + already-fixed) / (fix + already-fixed + fp)`. If no entries exist in the window or the file doesn't exist, skip the Greptile metric row. Skip unparseable lines silently.
...
**Skill Usage (if analytics exist):** Read `~/.mzstack/analytics/skill-usage.jsonl` if it exists. Filter entries within the retro time window by `ts` field. Separate skill activations (no `event` field) from hook fires (`event: "hook_fire"`). Aggregate by skill name. Present as:
...
**Eureka Moments (if logged):** Read `~/.mzstack/analytics/eureka.jsonl` if it exists. Filter entries within the retro time window by `ts` field. For each eureka moment, show the skill that flagged it, the branch, and a one-line summary of the insight. Present as:
...
**Note:** Only include the `greptile` field if `~/.mzstack/greptile-history.md` exists and has entries within the time window. Only include the `backlog` field if `TODOS.md` exists. Only include the `test_health` field if test files were found (command 10 returns > 0). If any has no data, omit the field entirely.
...
cat ~/.mzstack/projects/$SLUG/*-reviews.jsonl 2>/dev/null | grep '"skill":"ship"' | grep '"plan_items_total"' || echo "NO_PLAN_DATA"
```
...
```bash
DISCOVER_BIN=""
[ -x ~/.claude/skills/mzstack/bin/mzstack-global-discover ] && DISCOVER_BIN=~/.claude/skills/mzstack/bin/mzstack-global-discover
[ -z "$DISCOVER_BIN" ] && [ -x .claude/skills/mzstack/bin/mzstack-global-discover ] && DISCOVER_BIN=.claude/skills/mzstack/bin/mzstack-global-discover
[ -z "$DISCOVER_BIN" ] && which mzstack-global-discover >/dev/null 2>&1 && DISCOVER_BIN=$(which mzstack-global-discover)
[ -z "$DISCOVER_BIN" ] && [ -f bin/mzstack-global-discover.ts ] && DISCOVER_BIN="bun run bin/mzstack-global-discover.ts"
echo "DISCOVER_BIN: $DISCOVER_BIN"
```

If no binary is found, tell the user: "Discovery script not found. Run `bun run build` in the mzstack directory to compile it." and stop.

Run the discovery:
```bash
$DISCOVER_BIN --since "<window>" --format json 2>/tmp/mzstack-discover-stderr
```

Read the stderr output from `/tmp/mzstack-discover-stderr` for diagnostic info. Parse the JSON output from stdout.
...
- Top Work: 3 bullet points summarizing the user's major themes, inferred from
  ...
  not "feat: mzstack-global-discover" + "feat: /retro global template".
...
```bash
ls -t ~/.mzstack/retros/global-*.json 2>/dev/null | head -5
```
...
```bash
mkdir -p ~/.mzstack/retros
```

Determine the next sequence number for today:
```bash
today=$(date +%Y-%m-%d)
existing=$(ls ~/.mzstack/retros/global-${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
```

Use the Write tool to save JSON to `~/.mzstack/retros/global-${today}-${next}.json`:
...
```json
{
  ...
  "projects": [
    {
      "name": "mzstack",
      "remote": "https://github.com/YOUR_ORG/mzstack",
      "commits": 47,
      "insertions": 3200,
      "deletions": 800,
  ...
  "tweetable": "Week of Mar 14: 5 projects, 182 commits, 15.3k LOC | CC: 48, Codex: 8, Gemini: 3 | Focus: mzstack (58%) | Streak: 52d"
}
```
...
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `~/.mzstack/retros/` (not `.context/retros/`). Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".
