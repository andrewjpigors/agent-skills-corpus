...
Brainstorming helps answer **WHAT** to build through collaborative dialogue. It precedes `/sl-plan`, which answers **HOW** to build it.

The durable output of this workflow is a **requirements document**. In other workflows this might be called a lightweight PRD or feature brief. In super looper, keep the workflow name `brainstorm`, but make the written artifact strong enough that planning does not need to invent product behavior, scope boundaries, or success criteria.
...
4. **Default to the blocking question tool** - Use `AskUserQuestion` (call `ToolSearch` with `select:AskUserQuestion` first if its schema isn't loaded). It includes a free-text fallback, so well-chosen options scaffold the answer without confining it. This default holds for opening and elicitation questions too, not only narrowing. Fall back to numbered options in chat only if it is unavailable (including `ToolSearch` returning no match for it) or the call errors — not because a schema load is required. Never silently skip the question.
...
- **Extraction tier** — the grounding scout: retrieval and quoting work. The cheapest capable model (`model: "haiku"`). "Capable" is part of the spec — escalate to the generation tier when the repo is large or the stack obscure.
- **Generation tier** — the claim verifier: evidence-driven mechanical verification. The mid-tier model (`model: "sonnet"`).
...
**Degradation rule.** When the platform's subagent primitive does not support per-agent model selection, dispatch the scout and verifier on the inherited model and keep their read budgets and output caps — cost control then comes from structure, not tiering. When the platform has no subagent primitive at all, do the topic scan inline at Phase 1.1 — still writing the grounding dossier to the scratch path, because downstream consumers (the Phase 2.6 verifier, the sl-plan handoff) receive that path — and verify claims inline before the Phase 3 write, with the same budgets.
...
4. **Pipeline override.** When invoked from LFG or any `disable-model-invocation` context, force `OUTPUT_FORMAT=md` regardless of steps 1-3. Downstream consumers (`sl-plan`, `sl-work`) parse markdown reliably; HTML in pipeline runs is unnecessary friction.
...
The `output:` preference does NOT auto-propagate to `sl-plan` on handoff — sl-plan re-resolves its own `plan_output` config independently. Asymmetric output (`requirements.html` + `plan.md`) is acceptable; users who want HTML for both set both keys in `.super-looper/config.local.yaml`.
...
*Topic Scan (grounding scout)* — Create a scratch dir at `/tmp/super-looper/sl-brainstorm/<run-id>/` (short unique slug), then dispatch one extraction-tier sub-agent via `Agent`/`Task` — in the background where supported — and proceed to Phase 1.2/1.3 **without waiting**: the scout runs during the user's think-time on the opening questions. Scout prompt:
...
- **Tools available + user asked**: Dispatch `sl-slack-researcher` with a brief summary of the brainstorm topic alongside Phase 1.1 work. Incorporate findings into constraint and context awareness.
...
**Approach granularity: mechanism / product shape, not architecture.** Approach descriptions name mechanism-level distinctions ("pause as a rule property" vs "pause as an event filter" vs "pause as a separate entity") and product-relevant trade-offs (plan-tier coupling, complexity surface, migration difficulty). They do NOT name implementation specifics — column names, table names, file paths, service classes, JSON shapes, exact method names. Those are sl-plan's job. Bringing architecture forward at brainstorm time forces the user to make architectural decisions on sl-brainstorm's intentionally-shallow research, and the synthesis at Phase 2.5 then has to filter out the leak.
...
**Skip this step entirely if `CONCEPTS.md` does not exist at repo root** — creation is owned by sl-compound and sl-compound-refresh.
...
Read `references/handoff.md` now — before presenting any options. The option set and its visibility conditions, the rendering-mode rule, the per-selection dispatch instructions (including what gets passed to `sl-plan`), and the closing summary formats all live there — none of them appear in this main body. An improvised menu silently breaks pipeline routing: options surface in states where they must be hidden, and downstream skills receive the wrong payload.
