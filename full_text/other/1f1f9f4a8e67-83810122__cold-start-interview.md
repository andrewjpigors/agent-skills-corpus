...
description: >
  ...
  `config/local/codex-for-legal/ai-governance-legal/CLAUDE.md` from
...
1. Check `config/local/codex-for-legal/ai-governance-legal/CLAUDE.md` — if populated and no `--redo`, confirm before overwriting.
...
5. Migration: if a populated CLAUDE.md (no `[PLACEHOLDER]` markers) exists at `config/local/cache/codex-for-legal/ai-governance-legal/*/CLAUDE.md` but not at the config path, copy it to the config path and tell the user what was migrated.
6. Write `config/local/codex-for-legal/ai-governance-legal/CLAUDE.md` (create parent directories as needed). Show summary. Offer first task.

## Flags

- `--redo` — re-run the full interview and overwrite `config/local/codex-for-legal/ai-governance-legal/CLAUDE.md`.
- `--check-integrations` — re-scan available MCP connectors and refresh the `## Available integrations` table in `config/local/codex-for-legal/ai-governance-legal/CLAUDE.md` without re-running the full interview. Use after setting up a new connector (Slack (connector optional; not enabled in v1), document storage, scheduled-tasks).
...
If a CLAUDE.md exists at the old cache path `config/local/cache/codex-for-legal/ai-governance-legal/*/CLAUDE.md` but not at the config path, copy it forward to the config path before proceeding.
...
> "This plugin maintains: your practice profile (governance tiers, red lines, policy commitments), a use-case registry, impact assessments, and vendor AI reviews — all in `config/local/codex-for-legal/ai-governance-legal/`. It learns how you actually work — your practice, your risk calibration, your house conventions — and writes that into a plain-text file the plugin reads from every time. Everything you answer can be changed later."
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `$ai-governance-legal:cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `config/local/codex-for-legal/ai-governance-legal/CLAUDE.md` with a `<!-- SETUP PAUSED AT: [section name] — run $ai-governance-legal:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
> This plugin can work with: document storage (Google Drive (connector optional; not enabled in v1), SharePoint, Box), scheduled-tasks, Slack (connector optional; not enabled in v1). Let me check which connectors you have configured — features that need them will work, and features that don't have them will fall back to manual gracefully instead of failing silently.
...
For connectors that show as not connected, tell the user how to connect. Example phrasing: "Google Drive (connector optional; not enabled in v1) isn't connected. In Codex: add the Drive MCP to your config or via `/mcp`. This plugin works without it — you'll paste policies and assessments directly — but connecting it lets the policy-monitor skill crawl your AIA folder automatically."
...
- **Embedded AI in tools you've already approved:** Slack (connector optional; not enabled in v1) AI summaries, Microsoft Copilot, Salesforce Einstein, Gmail smart compose, Zoom AI Companion, CRM lead scoring, email drafting assistants. Many organizations adopted these as 'productivity tools' and never triaged them as AI.
...
> what's real for you. (This feeds /gap-check and $ai-governance-legal:policy-monitor — the gap analysis diffs new regulations against your stated scope, and policy-monitor only watches regimes you've marked in scope.)
...
> what you can. (The AI policy feeds $ai-governance-legal:policy-monitor drift detection; the prior impact assessment becomes the $ai-governance-legal:aia-generation template; the vendor agreements become the starting playbook for /review-vendor-ai.)"
>
...
- **Where do you save completed AIAs, triage results, and vendor AI reviews?** A folder
  path or shared drive location. (This feeds $ai-governance-legal:policy-monitor — the skill crawls this folder to detect when your practice has drifted ahead of your written AI policy.)
...
5. **Close with a note on changeability.** End with something like:
   ...
   > "Done. Your configuration is at `config/local/codex-for-legal/ai-governance-legal/CLAUDE.md` — it's a plain text file you can read and edit directly. Anything you answered can be changed:
   >
...
6. **Before your first triage**: connect a research tool. Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Cowork: Settings → Connectors. In Codex: authorize when a skill prompts you.
