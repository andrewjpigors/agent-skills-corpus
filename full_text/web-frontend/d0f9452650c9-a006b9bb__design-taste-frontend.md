...
* **SERIF DISCIPLINE (VERY DISCOURAGED AS DEFAULT):**
  ...
  * **PROJECT OVERRIDE — ICB brush-smear accent (documented brand signature):** This project (Indian Coffee Beans) defines a single, consistent accent treatment — the coffee brush-smear `<Accent>` primitive (an alpha-mask stroke recoloured to the accent token, see `src/components/primitives/accent.tsx`). Because it is ONE locked treatment applied identically everywhere, it is an intentional design-system signature, NOT the banned "random serif word" tell. **Per-section-title accent use is allowed and expected** on this project — the ≤1-per-page rationing does NOT apply here. Two rules still hold: (a) it must always be the `<Accent>` primitive, never ad-hoc `text-accent italic font-serif` spans; (b) at most one accented phrase per individual title.
