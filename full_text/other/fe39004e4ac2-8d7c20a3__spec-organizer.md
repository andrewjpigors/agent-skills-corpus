...
若需求涉及工作台、dashboard、viewer、review tool、setup flow 或任何容易堆疊資訊的介面，規格必須額外把 task model、state model、資訊分類、content audit 與揭露策略寫成獨立章節；不接受只有功能清單。
...
- 任何工作台型或流程型 UI，都要先做 content audit，把資訊分成 `must-see-now`、`next-step-only`、`error-only`、`on-demand-reference`、`keep-off-first-viewport`。
...
- 每個延後揭露區塊都要寫出 `hidden_now_because`、`reveal_trigger` 與 `container`，不能只寫「之後再顯示」。
...
- 對每個 deferred block 再補：
  - hidden_now_because
  - reveal_trigger
  - container
- 若首屏已經超過 3 個主要群組，或頁面出現 4 個以上大卡區塊，優先改寫成 tabs / wizard / step navigation，而不是繼續加卡片。
...
- 檢查 content audit 是否存在，且 deferred blocks 是否真的有 `hidden_now_because`、`reveal_trigger`、`container`。
...
- 若 content audit、deferred block 理由、step-flow 觸發判準任一塊缺失，視為 UI spec 不可落地缺漏。
- 修正後形成內部 `Spec v3`。
...
- 技術版若涉及工作台或流程型 UI，還必須補 1 組 content audit 結果（`must-see-now` / `next-step-only` / `error-only` / `on-demand-reference` / `keep-off-first-viewport`），以及每個 deferred block 的隱藏理由與 reveal trigger。
