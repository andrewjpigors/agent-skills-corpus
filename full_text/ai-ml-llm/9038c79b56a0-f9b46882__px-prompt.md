...
| Claude Code | `claude-code.md` | No hard limit (repo CLAUDE.md) |

Each project has up to **3 output files** — one per platform, same filenames regardless of mode.
...
- `claude-code.md` — project CLAUDE.md for Claude Code CLI/IDE. Condensed from system-prompt with repo-specific sections (identity, commands, stack, conventions). User copies to repo root as `CLAUDE.md`.
...
**Platform inference:**
- Default: Claude Projects + Perplexity Spaces + Claude Code
- Perplexity-only if description contains: "research only", "analysis only", "investigation"
- Claude Code excluded if project has no repo (e.g., research-only projects)
...
**Triggered when:** standalone project has `system-prompt.md` but missing `space-instructions-perplexity.md` or `claude-code.md`.
...
### 3b. Generate Claude Code CLAUDE.md

**Target:** `claude-code.md` | **Budget:** no hard limit (but keep concise — Claude Code reads this every session)

Generate a project CLAUDE.md that the user copies to their repo root. This is NOT a system prompt — it's a project configuration file for Claude Code CLI/IDE.

**Derive from `system-prompt.md` + `prompt-config.yaml`:**
...
## Overview
<1-2 sentence project description from system-prompt.md Role section>

## Identity
- **Type**: <Work | Personal>
- **Git profile**: <inferred from project org>
- **Email**: <from git config or prompt-config>

## Tech Stack
<Languages and frameworks from prompt-config.yaml domains and system-prompt.md>

## Commands
```bash
dev:    <inferred from stack or "N/A">
test:   <inferred from stack or "N/A">
lint:   <inferred from stack or "N/A">
build:  <inferred from stack or "N/A">
format: <inferred from stack or "N/A">
```

## Code Style
<3-5 conventions derived from domain expertise in system-prompt.md>

## Domain Context
<Key domain knowledge condensed from system-prompt.md Domain Expertise section>
<Reference engagement context, compliance targets, architecture principles as applicable>

## Verification
<How to verify work is correct — derived from system-prompt.md Common Tasks and Quality Controls>
...
**Generation rules:**
- Keep it actionable — Claude Code needs commands and constraints, not prose
- Domain expertise is condensed to key terms and constraints, not full reference material
- Commands section uses actual package manager / build tool commands for the stack
- If the repo has existing CI workflows, reference them in Verification
- Identity section uses the correct git identity for the org (work vs personal)
- No quality controls or reasoning sections — those belong in system-prompt.md, not CLAUDE.md

Write to `prompts/projects/<project-name>/claude-code.md`.
...
- `claude-code.md` — no hard limit, but flag if over 3,000 chars (sign of bloat)
...
| space-instructions-perplexity.md         | X      | 4,500  | OK     |
| claude-code.md                           | X      | ~3,000 | OK     |
...
**For Claude Code:**
1. Read `claude-code.md`
2. Copy content to clipboard: `cat <file> | pbcopy`
3. Print: "Copied to clipboard. Paste as `CLAUDE.md` in the repo root."
4. If the repo is known (from prompt-config.yaml): offer to push directly via `gh api`
...
[3/3] Claude Code
  → Copied claude-code.md to clipboard (1,842 chars)
  → Paste as CLAUDE.md in repo root, or push directly
  Press Enter when done...

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
DEPLOYED to 3 platforms.
Knowledge files uploaded to Claude Projects and Perplexity Spaces.
...
- **Claude Code**: project CLAUDE.md — actionable config (identity, stack, commands, conventions, domain context). Not a system prompt — no quality controls or reasoning sections.

### File naming
- Claude Projects output: `system-prompt.md` (all modes)
- Perplexity output: `space-instructions-perplexity.md`
- Claude Code output: `claude-code.md` (user copies to repo root as `CLAUDE.md`)
