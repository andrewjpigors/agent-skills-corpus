...
- **Decorative** — principle stated inline, citation is a "see also" pointer or bare attribution. Delete is fine; clean up citations in the same refresh pass.
...
After all actions are executed and the report is generated, stop before any git
or remote mutation in the TraceWeaver packaged alpha. Skip this phase if no
files were modified (all Keep, or all writes failed).

Do not stage files, create commits, create branches, push, or open PRs.

Report follow-up commit content without publishing:

1. List the files that this refresh changed.
2. List any unrelated dirty files separately, if visible.
3. Draft a proposed commit message that summarizes what was refreshed (for
   example, "update 3 stale learnings, consolidate 2 overlapping docs, delete 1
   obsolete doc") and follows the repo's existing style when it is already in
   context.
4. State that publication remains held until TraceWeaver review, traceability,
   verification, and the controlled publication route explicitly approve it.
...
6. **Report follow-up commit content without publishing when the check produces edits.** If step 4 or step 5 resulted in an edit to an instruction file, include that file in the held Phase 5 file list and draft a small follow-up message such as `docs: add docs/solutions discoverability to AGENTS.md`, `docs: add CONCEPTS.md discoverability to AGENTS.md`, or a combined message when both edits landed. Leave the instruction-file edits unstaged alongside the other uncommitted refresh changes and state that publication remains held.
