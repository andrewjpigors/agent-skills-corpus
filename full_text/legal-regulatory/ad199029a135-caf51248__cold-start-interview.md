...
Employment law in Brazil is federal at its core (CLT), but far from uniform in practice — the right answer for a base territorial with an aggressive sindicato and a restrictive CCT is not the right answer for a base with a weak or absent collective-bargaining history. This interview maps your footprint — every município/estado and country with employees, and the CCTs that govern them — and builds an escalation table that knows which rules apply where.
...
> If you need to find an attorney: in Brazil, contact the OAB (Ordem dos Advogados do Brasil) — the seccional (state chapter) offers a lawyer referral service, and many have a Comissão de Assistência Judiciária. Outside Brazil: your jurisdiction's equivalent professional regulator (SRA/Bar Standards Board in England & Wales, Law Society in Scotland/NI/Ireland/Canada/Australia, state bar in the US). For individuals without means, Defensoria Pública covers labor matters in many states.
...
- Every estado/município with at least one employee. All of them. Note the base territorial (município/sindicato) each falls under — that's what determines which CCT applies, not the state alone.
- Every country outside Brazil (expatriados, expansão internacional).
- Remote-first (home office, CLT art. 75-A a 75-E) or office-based/híbrido? (Remote-first means the footprint keeps expanding without anyone telling you — an employee working from a new município can trigger a new base territorial/CCT.)
- Which base territorial has the most employees, and which CCT is the most restrictive? That's your default jurisdiction when the question doesn't specify.
...
- What's in the standard carta-proposta? Non-compete clauses in Brazil are, structurally, a different animal from the US patchwork: there's no state-by-state on/off switch — a non-compete is enforceable nationwide only if it's time-limited, geographically/functionally limited, and paired with compensação financeira durante a vigência da restrição (Súmula 6 TST / entendimento consolidado dos tribunais trabalhistas) `[model knowledge — verify]`. Flag any offer letter with a non-compete and no compensation clause as a likely-unenforceable restriction, not a jurisdiction question.
...
This is the core output. For each base territorial/CCT and country in the footprint:

| Base/CCT | Special rules | Auto-escalate |
|---|---|---|
| CCT sindicato SP (comerciários) | Assistência sindical obrigatória na rescisão. Piso salarial diferenciado. Cláusula de estabilidade pré-aposentadoria. | Qualquer rescisão sem justa causa. Qualquer cláusula de não-concorrência. |
| CCT sindicato RJ (categoria X) | Adicional de insalubridade acima do mínimo legal. Data-base em [mês]. | Rescisões durante período de estabilidade convencional. |
| [etc.] | | |

Don't invent rules for bases they didn't name. If they have one employee in a município/CCT with no history, note `[base X: 1 empregado, sem histórico — pesquisar CCT vigente no primeiro caso]`.
...
- "Here's your jurisdiction table. The [base/CCT with the most restrictive clauses] row is the one to double-check."
