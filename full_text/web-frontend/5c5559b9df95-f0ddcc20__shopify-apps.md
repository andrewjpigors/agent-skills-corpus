...
[access_scopes]
scopes = "readproducts,writeproducts,read_orders"
...
const shopify = shopifyApp({
 ...
 apiSecretKey: process.env.SHOPIFYAPISECRET!,
 scopes: process.env.SCOPES?.split(","),
 appUrl: process.env.SHOPIFYAPPURL!,
 authPathPrefix: "/auth",
...
...(process.env.SHOPCUSTOMDOMAIN
? { customShopDomains: [process.env.SHOPCUSTOMDOMAIN] }
: {}),
});
...
export async function loader({ request }: LoaderFunctionArgs) {
 ...
 return json({ apiKey: process.env.SHOPIFYAPIKEY! });
}
...
export async function action({ request }: ActionFunctionArgs) {
 ...
 returnUrl: https://${session.shop}/admin/apps/${process.env.SHOPIFYAPIKEY},
...
must implement handlers even if you don't store data.
