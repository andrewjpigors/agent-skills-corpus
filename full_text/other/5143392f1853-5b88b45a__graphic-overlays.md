...
- GSAP (NOT bundled — GreenSock's license does not permit redistribution). Install it yourself with `npm install gsap` (then stage `node_modules/gsap/dist/gsap.min.js` in Step 9) or load it from the official CDN.
...
# GSAP is NOT bundled with this skill (GreenSock license — no redistribution).
# Install it via npm and stage it (or reference the official CDN in index.html instead):
npm install gsap --prefix "$WORK_DIR" --no-save
cp -n "$WORK_DIR/node_modules/gsap/dist/gsap.min.js" "$WORK_DIR/public/vendor/"
