...
description: >
  ROS2 web integration patterns for REST APIs, WebSocket bridges, dashboards, browser robot
  interfaces, and media streaming. Use when building FastAPI or Flask bridges with rclpy,
  using rosbridge or roslibjs, exposing ROS2 services or actions through HTTP, streaming
  camera feeds via MJPEG, WebRTC, or compressed WebSocket, handling authentication, CORS, rate
  limits, async web frameworks, executor threads, or topic allowlists.
---
