...
- `config/local/codex-for-legal/legal-builder-hub/CLAUDE.md` → practice profile and installed skills list (provides context
...
- The skill's associated `CLAUDE.md` (template in the plugin directory, user config at `config/local/codex-for-legal/<plugin>/CLAUDE.md`) — if available, what practice profile the skill reads and depends on
...
3. **Config-override instructions** — text telling Claude to modify the user's
   ...
   configs, or `config/local/...` outside the skill's own
   directory.
4. **Out-of-scope reads** — instructions to read paths outside the skill's own
   directory and `config/local/codex-for-legal/<plugin>/`. Flag
   specifically reads from: `~/.ssh/`, `~/.aws/`, `~/.config/gh/`, password
   managers, browser profiles, Mail, Messages, Slack (connector optional; not enabled in v1) files, or any path that
