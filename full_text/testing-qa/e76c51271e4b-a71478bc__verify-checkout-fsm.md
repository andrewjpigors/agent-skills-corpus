...
### Step 54: `bulkXxx` → `*Internal` 위임 패턴 — `preloadedCheckout` 2N read 방지 (2026-05-13 bulk-receive-2n-read)

**배경**: `bulkApprove`/`bulkReject`/`bulkCancel`/`bulkReceive` 는 각 id마다 `findCheckoutEntity(id)` 로
미리 checkout 을 조회한 뒤, 해당 entity 를 `*Internal` 메서드의 마지막 선택 인자로 전달한다.
이 패턴 없이 `*Internal` 내부에서 재조회하면 N건 처리 시 2N DB read 가 발생한다.

- `bulkApprove` → `approveInternal(id, dto, req, preloadedCheckout)`
- `bulkReject` → `rejectInternal(id, dto, req, preloadedCheckout)`
- `bulkCancel` → `cancelInternal(id, version, req, preloadedCheckout)`
- `bulkReceive` → `submitConditionCheckInternal(id, dto, checkerId, req, preloadedCheckout)`

**PASS**: `preloadedCheckout` 등장 ≥ 8건 + `preloadedCheckout ?? ` (nullish fallback) ≥ 4건 + `*Internal` 4종 모두 존재.
**FAIL**: preloadedCheckout 없음 또는 Internal 메서드가 누락되어 2N read 회귀.

```bash
# (1) preloadedCheckout 총 등장 건수 (정의 4 + bulkXxx 전달 4 = ≥ 8)
grep -c "preloadedCheckout" apps/backend/src/modules/checkouts/checkouts.service.ts
# 기대: ≥ 8

# (2) nullish fallback — Internal 내부에서 findCheckoutEntity 조건부 호출
grep -n "preloadedCheckout ??" apps/backend/src/modules/checkouts/checkouts.service.ts
# 기대: ≥ 4건 (4개 Internal 메서드)

# (3) 4개 Internal 메서드 존재
grep -n "approveInternal\|rejectInternal\|cancelInternal\|submitConditionCheckInternal" \
  apps/backend/src/modules/checkouts/checkouts.service.ts | grep "async "
# 기대: 4건 이상

# (4) 회귀 spec: bulkCancel/bulkReceive spec에서 preloadedCheckout 전달 증명
grep -n "mock.calls\[0\]\[3\]\|mock.calls\[0\]\[4\]\|mock.calls\[1\]\[3\]\|mock.calls\[1\]\[4\]" \
  apps/backend/src/modules/checkouts/__tests__/bulk-cancel.dto.spec.ts \
  apps/backend/src/modules/checkouts/__tests__/bulk-receive.spec.ts
# 기대: ≥ 4건 (각 spec에서 spy.mock.calls 인자 검증)
```
...
| 54 | bulkXxx → *Internal preloadedCheckout 위임 (2N read 방지) | PASS/FAIL | preloadedCheckout ≥ 8건 + nullish fallback ≥ 4건 + Internal 4종 존재 |
