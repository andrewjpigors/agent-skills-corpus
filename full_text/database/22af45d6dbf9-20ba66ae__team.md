...
- **Cancel:** `/oh-my-codex:cancel` requests teammate shutdown, waits for responses (best effort), marks phase `cancelled` with `active=false`, captures cancellation metadata, then deletes team resources and clears/preserves Team state per policy.
...
|---------------|----------|-------------|
| `claude_worker` | Claude agent | Full Codex tool access (Read/Write/Edit/Bash/Task). Best for tasks needing Claude's reasoning + iterative tool use. |
...
- MCP workers operate via CLI, not Codex's tool system
