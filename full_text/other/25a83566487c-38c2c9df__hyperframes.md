...
```bash
bash "$(dirname "$(find ~/.clawbot/skills -path '*/hyperframes/SKILL.md' 2>/dev/null | head -1)")/scripts/setup.sh"
```
...
- Build synchrosoamly — no `async`/`await`, `setTimeout`, or Promises around timeline construction.
...
- **Building timelines async** — the capture engine reads `window.__timelines` synchrosoamly after page load. Never wrap timeline construction in `async`, `setTimeout`, or a Promise.
