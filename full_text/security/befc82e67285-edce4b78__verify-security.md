...
| File | Purpose |
|------|---------|
| `apps/frontend/lib/security/csp.ts` | CSP 지시문 조립 SSOT (`buildCspHeader`) — ADR-0015 |
| `apps/frontend/proxy.ts` | per-request nonce 발급 + CSP 헤더 부착 (조립은 csp.ts 위임) |
...
### Step 1: CSP SSOT (lib/security/csp.ts + proxy.ts)

**PASS:** CSP 지시문 조립은 `lib/security/csp.ts`의 `buildCspHeader`가 유일(SSOT), `proxy.ts`는 nonce 발급·헤더 부착만. `API_ENDPOINTS.SECURITY.CSP_REPORT` SSOT 경로 사용, prod에서 `'unsafe-eval'` 없음, `connect-src` dev에서만 `ws: wss:` 허용. **style 계열은 ADR-0015 계층 정책**: `style-src`(fallback)/`style-src-attr`는 nonce 없이 `'unsafe-inline'`, `style-src-elem`은 prod nonce 보호/dev unsafe-inline (인바리언트: `lib/security/__tests__/csp.test.ts`).
**FAIL:** proxy.ts 내 지시문 문자열 조립 재등장, 하드코딩된 CSP report 경로, prod에서 `unsafe-eval`, dev/prod 분기 없는 `connect-src`, **style-src(fallback)/style-src-attr/dev style-src-elem에 `'nonce-` 등장**(SSR style 속성·HMR 파손), prod style-src-elem에 nonce 부재.
...
### Step 22 — alert.rules.yml 전수 runbook 완전성 검증 (2026-05-13 추가)

**근거**: `system-wide-alert-runbook-backfill` sprint — HighCPUUsage 등 11개 infrastructure/container/application alert에 runbook 미보유 운영 maturity 갭 발견. alertmanager Slack 템플릿이 `{{ if .Annotations.runbook }}` 가드 패턴으로 이미 준비되어 있었으나 기존 alert들이 backfill 없이 방치됨. 신규 alert 추가 시 동일 회귀 차단용 영구 invariant.

**검증 명령** (python3 기반 — ugrep 환경 호환):

```python
# python3 - <<'EOF'
import re, yaml

with open('infra/monitoring/prometheus/alert.rules.yml') as f:
    content = f.read()

# (1) alert 수 vs runbook/runbook_url 수 비교
total = len(re.findall(r'- alert:', content))
runbooks = len(re.findall(r'runbook: \|', content))
runbook_urls = len(re.findall(r'runbook_url:', content))
print(f"alerts={total}  runbooks={runbooks}  runbook_urls={runbook_urls}")
assert total == runbooks == runbook_urls, "FAIL: runbook 누락 alert 존재"
...
# (3) anchor docs 정합
anchors = set(re.findall(r'runbook_url:.*#([a-z]+)', content))
with open('docs/operations/prometheus-alert-rules.md') as f:
    docs = f.read()
sections_lower = {s.lower().replace(' ', '') for s in re.findall(r'^### (.+)$', docs, re.MULTILINE)}
missing = anchors - sections_lower
print(f"missing_anchors={missing or 'NONE'}")
assert not missing, f"FAIL: docs 섹션 누락 — {missing}"
print("ALL PASS")
# EOF
```

**PASS 기준:**
- (1) total == runbooks == runbook_urls (현재 기준 15)
- (2) YAML_OK 출력
- (3) anchor 미포함 runbook_url 0건
- (4) docs 미매칭 anchor 0건

**FAIL 시 수정 지시**:
- (1) FAIL: `total > runbooks` — 신규 alert에 `runbook: |` + 5단계 이상 즉시 행동 절차 추가 필수. `docs/operations/prometheus-alert-rules.md` §Workflow.2 양식 참조
- (1) FAIL: `total > runbook_urls` — 신규 alert에 `runbook_url: 'docs/operations/prometheus-alert-rules.md#alertnamelowercase'` 추가
- (3) FAIL: anchor 누락 — runbook_url 값을 `'docs/operations/prometheus-alert-rules.md#alertnamelowercase'` 형식으로 수정
- (4) FAIL: docs 섹션 누락 — `docs/operations/prometheus-alert-rules.md` `## Runbook` 섹션 하단에 `### AlertName` 절차 추가

**관련 sprint**: `system-wide-alert-runbook-backfill` (2026-05-13 backfill closure).

### Step 23 — CSP body parser content-type 회귀 차단 (2026-05-13 추가)

**근거**: `leaf-section-rtl-spec-backfill` sprint에서 TC-3 (legacy report-uri + Reporting API payload 수락) 실 Playwright 실행 시 500 반환 발견. 근본 원인: Express `json()` 미들웨어 기본값이 `application/json`만 파싱 — `application/csp-report` / `application/reports+json` content-type body가 `undefined`로 도달하여 controller에서 예외 발생. `apps/backend/src/main.ts` `json({ type: [...] })` 배열 확장으로 수정. 해당 설정 제거 시 TC-3 무음 500 회귀 + CSP 위반 보고 파이프라인 단절 — 회귀 차단 필수.

**검증 명령**:

```bash
# (1) application/csp-report 포함 확인
grep -c "application/csp-report" apps/backend/src/main.ts  # ≥ 1

# (2) application/reports+json 포함 확인
grep -c "application/reports+json" apps/backend/src/main.ts  # ≥ 1

# (3) json() 미들웨어 type 배열 내 위치 확인
grep -A8 "json({" apps/backend/src/main.ts | grep -c "type:"  # ≥ 1
```

**PASS:** 3 검증 모두 통과.
**FAIL:** type 배열에서 CSP content-type 제거 → TC-3 Playwright 500 무음 회귀 + CSP report 파이프라인 단절.

**위반 시 수정 지시**:
```typescript
// apps/backend/src/main.ts — json() 미들웨어에 type 배열 추가
app.use(
  json({
    limit: bodyLimit,
...
**관련 sprint**: `leaf-section-rtl-spec-backfill` (2026-05-13 — TC-3 실 wire 검증 후 발견).
