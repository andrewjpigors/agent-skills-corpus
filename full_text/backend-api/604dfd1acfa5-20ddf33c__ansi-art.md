...
name: ansi-art
description: >
  ANSI art creation skill for LLMs using the window.ansi programmatic API. Covers CGA palette
  color theory, CP437 character selection, shadow/highlight techniques, half-block pixel art,
  shading and dithering, reference-to-ANSI conversion workflows, composition at low resolution,
  and efficient API usage patterns. Use this skill whenever the user asks to create ANSI art,
  draw something in the ANSI editor, convert an image to ANSI, or asks about ANSI art techniques.
  Also trigger when the user mentions ASCII art, text art, terminal art, CP437, half-block drawing,
  CGA palette art, or wants to use the window.ansi API. Even if they just say "draw this" or
  "make art" in the context of the ANSI editor, use this skill.
---
