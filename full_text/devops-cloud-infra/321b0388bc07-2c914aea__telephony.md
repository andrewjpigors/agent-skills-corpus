...
- save provider credentials into `~/.monika/.env`
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Monika memory
...
- pairs well with Monika `text_to_speech` plus a public file host or tunnel
...
```bash
SCRIPT="$(find ~/.monika/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
monika skills search telephony
monika skills install official/productivity/telephony
```
...
Then save credentials into Monika:
...
3. Buy it and save it into `~/.monika/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
This is the main path for reusing Monika's existing `text_to_speech` support.

Use this when:
- you want the call to use Monika's configured TTS voice rather than Twilio `<Say>`
...
1. Generate the audio with Monika `text_to_speech`.
...
7. Summarize the outcome without persisting third-party numbers to Monika memory.
...
- Do not store arbitrary third-party phone numbers in Monika memory.
...
3. persist that number to `~/.monika/.env`
