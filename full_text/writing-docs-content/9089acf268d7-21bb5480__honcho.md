...
Config file: `$MYAI_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `myai honcho setup`. Ensure `memory.provider: honcho` is in `~/.myai/config.yaml`.
