...
## Step 1.5: Pre-Fetch Shared Context (MANDATORY)

**⛔ CRITICAL: Gather ALL shared context ONCE before dispatching reviewers.** Do NOT let individual reviewers WebFetch or git-diff independently — 7 agents fetching the same data wastes minutes per reviewer.

```text
shared_context = {}

1. Read the full diff ONCE:
   shared_context.diff = Execute: git diff [base_sha] [head_sha]

2. Detect project language from implementation_files:
   shared_context.language = "typescript" (if *.ts files) | "go" (if *.go files)

3. WebFetch standards ONCE (based on detected language):
   if language == "typescript":
     shared_context.standards = WebFetch typescript.md standards
   if language == "go":
     shared_context.standards = WebFetch go.md standards

4. Read standards-coverage-table.md ONCE:
   shared_context.coverage_table = Read standards-coverage-table.md

NOTE: This context is injected into EVERY reviewer prompt below via
[shared_context.diff], [shared_context.standards], [shared_context.coverage_table].
Reviewers MUST NOT WebFetch or run git diff themselves.
```
...
**Model: All reviewers use `model: "sonnet"`.** Seven specialized Sonnet reviewers with different lenses provide comprehensive coverage. The redundancy of 7 parallel agents compensates for individual model differences, while reducing wall-clock time from ~40 min (Opus) to ~8-12 min.
...
FOR EACH slice IN review_state.slicing.slices:
  ...
  3. Dispatch all 7 reviewers with slice-scoped context:
     ...
     - model: "sonnet" (same as standard dispatch — all reviewers use Sonnet)
...
The following dispatch is used when `review_state.slicing.enabled == false`.

**⛔ CRITICAL: Inject `[shared_context.diff]` into each reviewer prompt under "## Git Diff". Reviewers MUST NOT run `git diff` themselves. The diff was pre-fetched in Step 1.5.**
...
Task:
  ...
  prompt: |
    ...
    **Head SHA:** [head_sha]
    ...
    ## Git Diff (pre-fetched — do NOT run git diff yourself)
    [shared_context.diff]

    ## Pre-Analysis Context
    [IF preanalysis_state.context["marsai:code-reviewer"] exists: INSERT it]
    ...
    **Standards have been pre-fetched by the orchestrator. Do NOT WebFetch — use the context below.**
    
    ### Pre-Fetched Standards
    [shared_context.standards]
    
    ### Standards Coverage Table
    [shared_context.coverage_table]
    
    **Check the changed code against ALL applicable sections.**
...
Task:
  ...
  prompt: |
    ...
    **Head SHA:** [head_sha]
    ...
    ## Pre-Analysis Context
    [IF preanalysis_state.context["marsai:business-logic-reviewer"] exists: INSERT it]
...
Task:
  ...
  prompt: |
    ...
    **Head SHA:** [head_sha]
    ...
    ## Pre-Analysis Context
    [IF preanalysis_state.context["marsai:security-reviewer"] exists: INSERT it]
    ...
    **Check ALL applicable security sections:**
...
Task:
  ...
  prompt: |
    ...
    **Head SHA:** [head_sha]
    ...
    ## Pre-Analysis Context
    [IF preanalysis_state.context["marsai:test-reviewer"] exists: INSERT it]
...
Task:
  ...
  prompt: |
    ...
    **Languages:** [Go|TypeScript|both - detect from files]
    ...
    ## Pre-Analysis Context
    [IF preanalysis_state.context["marsai:nil-safety-reviewer"] exists: INSERT it]
...
Task:
  ...
  prompt: |
    ...
    **Head SHA:** [head_sha]
    ...
    ## Pre-Analysis Context
    [IF preanalysis_state.context["marsai:consequences-reviewer"] exists: INSERT it]
...
Task:
  ...
  prompt: |
    ...
    **Head SHA:** [head_sha]
    ...
    ## Pre-Analysis Context
    [IF preanalysis_state.context["marsai:dead-code-reviewer"] exists: INSERT it]
