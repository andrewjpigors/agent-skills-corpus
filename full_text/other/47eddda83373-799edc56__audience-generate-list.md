...
The shared **group** / **traverse** strategies are **not available** in this leaf: name them honestly when asked, never build or run them.

This is a thin delta over `audience-generate`: the unique work is getting the owned list to something the operator can build from — a **chainable entity-ID set** (resolve-only via the inline resolution procedure, expand via the inline expand-roster procedure), the **signals that define the list** (lookalike via the inline profiling procedure), or the **scored-and-ranked list** (overlay via the inline overlay-score procedure). The parent's lane (build, not export; entity IDs only), language, and landing plumbing are composed with verbatim. The parent's full discover→score→**compose** spine still does **not** run here — a list isn't a brief, and no play composes a new set: lookalike *profiles the resolved seed* (the profiler's read of who's already on the list) and stops at the signal pool; **overlay** is the one play that runs inline discovery + scoring — but over a brief that defines the *scoring layer*, never the population, and it scores the resolved list rather than composing a set.
...
- **Runs inline:**
  - **resolution** *(resolve-only play, and the lookalike seed-resolve, `context/resolution.md`)* — run inline with the supplied identifiers (inline) or a `csv_resource_uri`, plus the `entity_type` and a `workflow_id` → a `workflow://` entity-IDs URI and the counts (`input_identifier_count`, `resolved_count`, `below_floor_count`). The resolve is a `format=csv` export the script reduces to an entity-ID set, so the leaf handles only the URI and the counts — never a resolved contact record.
  - **expand** *(expand play, `context/expand-roster.md`)* — run inline with the supplied identifiers (inline) or a `csv_resource_uri` **plus the `lookup_columns` mapping the leaf elicited** (which columns to match on; required on the CSV path), the `entity_type`, an overridable `quality_floor` (default `0`), and a `workflow_id` → a `workflow://` **roster** URI (columns `entity_id, match_confidence, match_criteria_count`), a small ID-only `sample`, and the `coverage` counts (`input_identifiers`, `resolved`, `below_floor`, `quality_floor`). The procedure owns the wide `entity_resolve` 1:many pass (a `format=csv` export) and the dedupe/floor script, keeping the result IDs-only — so the leaf handles only the roster URI, the sample, and the counts, never a resolved contact record. The leaf chooses the columns and (when a transform is needed) builds the artifact; the procedure resolves against exactly the `lookup_columns` it's handed.
  - **profiling** *(lookalike play, mode B, `context/profiling.md`)* — run inline on the seed's `entity_ids_uri` (from the resolution procedure), the `entity_type`, and a `workflow_id` → a structured two-half read of the seed: the **durable identity by lift** (`discovered`) and the **top intents by reach within the seed** (the `breakdown` intent panel, tagged by size). It runs in **mode B (`entity_set`)** — an entity set, no signal stack — exactly as `audience-analyze-list` does; it groups the **full** seed (never a sample — sparse intents need the whole set) and returns aggregates only. This leaf renders them as the tunable signal pool and never sees a record.
  - **inline discovery + scoring** *(overlay play)* — the parent's discover-and-score procedure, run here to turn the overlay **brief** (the layer to score by — "in-market for X", "engaged with Y") into a resolved, scored **signal pool**. Discovery runs **inline** on the main thread (one angle per pass, `context/discovery.md`); scoring runs **inline** (`context/scoring.md`) by `trait_hash`, the brief as the grounding frame. The pool defines what to score the resolved list *against* — never who's on the list.
  - **overlay** *(overlay play, `context/overlay-score.md`)* — run inline with the resolved list (an `entity_ids_uri` from the resolution procedure), the scored **signal pool** (expressions over `trait_hash`es), an optional per-signal `weights` map (the operator's; default every signal `1`), the `entity_type`, and a `workflow_id` → a `workflow://` **roster** URI (columns `entity_id, overlay_score, signals_matched, rank`), a small ID-only `sample`, and `coverage` (`source_size`, `scored`, `scored_zero`). The procedure **only scores and ranks** — it returns the whole list, never cut; slicing to the intersection (keep `signals_matched ≥ N`) is this leaf's post-step. Weights flow operator → the procedure; never elicited by it.
- **Hands off to:** `audience-activate` (export the matched set — Meta, Google, Reddit, or TikTok) and `audience-analyze` (read who the matched set is) — offered at the landing, on the user's say-so. For the **lookalike** play the deliverable is the tuned signal pool, not a matched set: hand it to **`audience-generate-search`** to compose into an audience (the working set pre-seeded — discovery skipped for what's already there, a landing mode picked there), or to **`audience-analyze`** to read it.
...
- **A list with a READ intent** ("who are these people", "what do they have in common", "profile my customer list"). That's `audience-analyze-list`, not this leaf — both run the same resolve, but analyze reads aggregates and this leaf builds toward export. Route there.
...
- **A widen/maximize-reach intent on a list** ("expand my list to every match", "the widest reach from my MAIDs and phones", "every entity these identifiers hit"). That's the **expand play** — settle the list, then run the expand-roster procedure inline (`context/expand-roster.md`, floor `0`).
...
Name the **identifier types present** — email, phone, name, address. **Addresses need explicit handling** — call them out, because they resolve on a different call shape (the resolution procedure names their types explicitly; a default email-only resolve 500s on addresses). This input-shape handling mirrors `audience-analyze-list`'s step 1 exactly; the only difference is the downstream intent (build/export here vs. read there).

**For a CSV, settle which columns to match on** — the resolution procedure resolves against exactly the columns you map (`lookup_columns`) and ignores the rest, so "match on these two of the twenty" needs no file surgery: just map the two. Two cases *do* need a prepared artifact, which **the leaf builds in the sandbox before the resolve** (the resolution procedure never reshapes a file):
...
When neither applies, pass the user's `csv_resource_uri` straight through with the chosen `lookup_columns`. When one does, write the cleaned CSV in the sandbox, upload it (`generate_upload_url`), and carry that artifact's URI plus the `lookup_columns` for it into the resolution step — the column choice and any reshape are the leaf's, never the resolution procedure's. End the turn confirming the list, its shape, and the match columns before resolving.

### 2 — Pick the play

The user's words usually answer this — route silently when so (*"customer match"* → resolve-only, *"the widest reach"* → expand, *"more like them"* → lookalike); otherwise land the choice as the decision — native clickable options, with the tradeoff plain:
...
### 3 — Resolve to entity IDs (matching plays — the play's procedure)

For resolve-only and expand: on the user's go, run the play's resolve with the identifiers (inline) or the `csv_resource_uri` (the user's, or the sandbox artifact from step 1) **plus the chosen `lookup_columns`**, the `entity_type`, and a `workflow_id` — resolve-only runs the resolution procedure inline (`context/resolution.md`), expand runs the expand-roster procedure inline (`context/expand-roster.md`). Either way the resolve stays IDs-only — the leaf handles only the URI(s), the sample, and the counts, never a resolved contact record.

- **resolve-only → the resolution procedure inline** (`context/resolution.md`) — returns the `entity_ids_uri` and the counts (`input_identifier_count`, `resolved_count`, `below_floor_count`). Narrate plainly ("5,000 identifiers in, 4,200 people matched").
- **expand → the expand-roster procedure inline** (`context/expand-roster.md`; pass `quality_floor: 0` unless the user set one, and the `lookup_columns` for the CSV) — runs the wide `entity_resolve` 1:many pass and returns the **roster URI**, an ID-only `sample`, and `coverage` (`input_identifiers`, `resolved`, `below_floor`, `quality_floor`). Narrate plainly ("5,000 identifiers in, 6,300 entities matched wide at floor 0"). Because the match is 1:many, **`resolved` can exceed the identifiers in** — say so; it's a union, not a per-row rate. When the match columns include addresses, that union includes the co-residents at them.
...
- **expand** — columns `entity_id, match_confidence, match_criteria_count` (the expand procedure's roster, verbatim); no rank, no groups. The confidence + corroboration columns are the audit trail for *why each entity is in the set*, and must survive downstream.
...
Narrate the read plainly ("Your 4,200 customers over-index hardest on X and Y; their most common in-market intents are Z…"). Render the result as the **tunable signal pool** — each signal with its lift or reach, size, freshness, and which half it came from — and write the signal-pool record per the record contract (`context/record.md`); the keep/drop **tuning** is the decision the render lands. The user **tunes**: keep the portable signals, drop the self-referential ones (owning the product, the brand's own app) and the generic high-reach intents that describe everyone. This is the decision beat — the turn ends at the tuned pool.
...
On the user's go, resolve the list to the **matched set** by running the resolution procedure inline (`context/resolution.md`) — the same resolve as resolve-only (overlay scores who's genuinely on the list, the tight match), with the identifiers/`csv_resource_uri` and chosen `lookup_columns`, the `entity_type`, and a `workflow_id`. Confirm coverage honestly (step 4). The resolved `entity_ids_uri` is the population to score. **A roster input skips this step** — its `entity_ids_uri` is already the population; go straight to O2.

### O2 — Elicit the brief for the scoring pool, discover, and score it (overlay)

Overlay needs a **brief** — but it defines *what to score by*, never who's on the list. Ask: *"What should I score your list by — in-market for what, engaged with what, interested in what?"* Read it as angles and, one angle per beat, run discovery **inline** (narrow scope, the population's entity type — `person` for a list; a crossed *business* roster re-entering discovers business-side signals, `entity_type: "business"`) for the candidate signals, then the **inline scoring procedure** to score them (by `trait_hash`, the brief as the grounding frame) — the parent's discover→score procedure, composed with verbatim. Show the scored pool — name · what it means · ~size · relevance · freshness · score — and write the pool record per the record contract (`context/record.md`). End the turn on the pick: which signals make up the pool, and — if the operator wants — a **per-signal weight** (default every signal `1`, so the default score is a plain count of signals expressed). Nothing is scored until the user approves the pool.

### O3 — Score and rank (the overlay procedure), then choose rank-or-cut (overlay)

On the user's go, run the overlay-score procedure inline (`context/overlay-score.md`) with the resolved `entity_ids_uri`, the approved pool (expressions by `trait_hash`), the `weights` map (the operator's, or omitted for uniform), the `entity_type`, and the `workflow_id`. Narrate plainly ("Scoring your 18,400 matched customers against the 6-signal pool…"). It returns the **roster** ranked by `overlay_score`, an ID-only `sample`, and `coverage` (`source_size`, `scored`, `scored_zero`). The procedure scores the **whole** list — nobody is cut. Land the closing decision as native clickable options:
...
Show the ranked set — the top rows by `overlay_score` + `signals_matched`, `scored_zero` called out — and write the roster record per the record contract (`context/record.md`). Be honest: the `scored_zero` people express none of the pool — a real result, not a drop; with uniform weights `overlay_score` is just a count, not a precision verdict.

### O4 — Emit the roster and hand off (overlay)

Emit the scored set in the **shared roster record** frame — columns `entity_id, overlay_score, signals_matched, rank`. When the intersection cut was applied, the emitted roster is the kept rows and **the record says so**: a `# set:` header line reads `intersection (signals_matched ≥ N) — 4,100 of 18,400 kept`, and a `# full_set_uri:` line retains the overlay procedure's full ranked `roster_uri` — so a downstream consumer is never handed the slice as if it were the whole list, and the full ranking stays reachable:
...
- **The list is the anchor — start from what it is, not who they want.** Elicit the list and its shape first; no play composes a new set from a description. resolve-only and expand run no discovery or scoring at all; lookalike profiles the resolved seed; **overlay** runs the parent's discover→score procedures inline, but only to build a *signal pool to score the resolved list by* — the list stays the population.
- **Stay IDs-only through the resolve.** resolve-only, lookalike's seed-resolve, and overlay's list-resolve run the resolution procedure inline (`context/resolution.md`); expand runs its resolve inline via the expand-roster procedure (`context/expand-roster.md`) — a `format=csv` `entity_resolve` export the script reduces to an IDs-only roster. Either way the leaf handles only entity-ID URIs, samples, and counts — never a resolved contact record, and never `resolve_and_enrich_rows` (enrichment to PII is the activator's lane).
- **Resolve-only, expand, lookalike, and overlay ship; the shared group / traverse don't.** Group / traverse aren't available in this leaf — name them honestly when asked, never run them. expand runs the expand-roster procedure inline (`context/expand-roster.md`) and overlay runs the overlay-score procedure inline (`context/overlay-score.md`); lookalike runs the profiling procedure inline (`context/profiling.md`, mode B); overlay also runs the parent's discovery + scoring inline to build its pool; and resolve-only, lookalike's seed-resolve, and overlay's list-resolve all run the resolution procedure inline (`context/resolution.md`). Households are *not* a separate strategy: they fall out of expand on an address-bearing list, or via the export-addresses-then-expand round-trip; only a one-shot household-from-a-non-address-list (which would need enrichment, the activator's lane) is unbuilt.
...
- **The record contract loads on demand.** This leaf skips the parent's spine, so it inherits no record-contract cue — and a list build is often a session's first record write. The first time a play writes a record — a matched/expanded roster, a lookalike pool, or an overlay pool/roster — read the record contract first from the bundle: `cat "<Bundle root>/context/record.md"` (the path published at session start). It isn't injected at session start; read it once, then honor it for every record this session.
...
- **US-only, adults-only, person audiences only.** (A crossed *business* roster re-entering for overlay is the people-anchored B2B exception — its entity type rides through the inline scoring.)
...
- **(lookalike) The profiling read caps or halts on a very large seed.** The profiling procedure groups the full seed; if the server caps the aggregation or a step halts, relay its `note` plainly (which step, what was returned) and stop — never silently profile a truncated slice as if it were the whole seed, and never pad the pool with guessed signals.
