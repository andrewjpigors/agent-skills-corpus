...
class ComputerUseAgent:
 ...
 def init(self, client: Anthropic, model: str = "claude-sonnet-4-20250514"):
 self.client = client
 self.model = model
 ...
 system_prompt = """You are a computer use agent. can see the screen
 ...
 screenshotb64 = self.capturescreenshot()
...
class SandboxedAgent:
 ...
 def init(self, config: SandboxConfig):
 self.config = config
 ...
 --cap-drop ALL \
 --memory {self.config.maxmemorymb}m \
 --cpus 2 \
 --read-only \
 --tmpfs /tmp \
 {network_rules} \
 ...
 subprocess.Popen([
 "sh", "-c",
 f"sleep {self.config.maxruntimeseconds} && docker kill {self.container_id}"
 ])
 ...
 json={"task": task},
 timeout=self.config.maxruntimeseconds
)
...
class AnthropicComputerUse:
 ...
 name="computer",
 displaywidthpx=self.screen_size[0],
 displayheightpx=self.screen_size[1],
),
 BetaToolBash20241022(
 type="bash_20241022",
 name="bash",
 ...
 if name == "computer":
 return self.handlecomputer_action(input)
 elif name == "bash":
 return self.handlebash(input)
 elif name == "strreplaceeditor":
 return self.handleeditor(input)
 else:
 return {"error": f"Unknown tool: {name}"}

 def handlecomputer_action(self, input: dict) -> dict:
 ...
 xdotoolkey = keymap.get(key.lower(), key)
 ...
 def handlebash(self, input: dict) -> dict:
 ...
 def handleeditor(self, input: dict) -> dict:
 ...
 oldstr = input.get("oldstr")
 newstr = input.get("newstr")
 try:
 with open(path, "r") as f:
 content = f.read()
 ...
 def runtask(self, task: str, maxsteps: int = 50) -> dict:
...
class BrowserUseAgent:
 ...
 async def getpagesnapshot(self) -> dict:
 """
 ...
 await self.page.waitforload_state("networkidle", timeout=5000)
 return {"success": True}
 ...
 async def runwithllm(self, task: str, llmclient, maxsteps: int = 20):
 """
 ...
 snapshot = await self.getpagesnapshot()
 ...
 @ Parse and execute
 import json
 actiondict = json.loads(assistanttext)
...
"ChatGPT agent also pauses and asks for confirmation before taking
sensitive steps e.g. completing a purchase."
...
class ConfirmationGate:
 ...
 "entercreditcard": ActionSeverity.CRITICAL,
 ...
 autoconfirmlow: bool = True,
 autoconfirmmedium: bool = False
):
 self.confirmcallback = confirmcallback or self.defaultconfirm
 self.autoconfirmlow = autoconfirmlow
 self.autoconfirmmedium = autoconfirmmedium
 self.action_log = []

 def defaultconfirm(self, action: SensitiveAction) -> bool:
 ...
 def classifyaction(self, actiontype: str, context: dict) -> ActionSeverity:
 """Classify action severity, considering context."""
 baseseverity = self.ACTIONSEVERITY.get(action_type, ActionSeverity.MEDIUM)
 ...
 """
 details = details or {}
 severity = self.classifyaction(actiontype, details)
 ...
 if severity == ActionSeverity.LOW and self.autoconfirmlow:
 return True, "auto-approved (low severity)"

 @ Maybe auto-approve medium
 if severity == ActionSeverity.MEDIUM and self.autoconfirmmedium:
...
class ConfirmedComputerUseAgent:
 ...
 def init(self, baseagent, confirmationgate: ConfirmationGate):
 ...
 context = {
 "involvescredentials": self.looks_sensitive(action.get("text", "")),
 "involvesmoney": self.mentions_money(action),
 }
 ...
 def lookssensitive(self, text: str) -> bool:
 ...
 def mentionsmoney(self, action: dict) -> bool:
...
gate = ConfirmationGate(
 autoconfirmlow=True,
 autoconfirmmedium=False # Confirm clicks, typing
)
...
class ActionLogEntry:
 ...
 "parameters": self.sanitizeparams(self.parameters),
 "success": self.success,
 "error": self.error,
 "screenshotbefore": self.screenshotbefore,
 "screenshotafter": self.screenshotafter,
 "modelreasoning": self.modelreasoning,
 "durationms": self.durationms
 }

 def sanitizeparams(self, params: dict) -> dict:
 """Remove sensitive data from params."""
 sanitized = {}
 sensitivekeys = ['password', 'secret', 'token', 'key', 'creditcard']
...
class ActionLogger:
 ...
 def init(self, logdir: str = "./agentlogs"):
 self.logdir = logdir
 self.screenshotdir = os.path.join(logdir, "screenshots")
 os.makedirs(self.screenshotdir, existok=True)
 ...
 screenshot_paths = {}
 timestampstr = datetime.now().strftime("%Y%m%d%H%M%S_%f")

 if screenshot_before:
 path = os.path.join(
 self.screenshot_dir,
 f"{self.currentsession.sessionid}{timestampstr}_before.png"
)
 ...
 path = os.path.join(
 self.screenshot_dir,
 f"{self.currentsession.sessionid}{timestampstr}_after.png"
)
 with open(path, "wb") as f:
 ...
 success=success,
 error=error,
 screenshotbefore=screenshotpaths.get("before"),
 screenshotafter=screenshotpaths.get("after"),
 ...
 self.appendto_log(entry)

 def appendto_log(self, entry: ActionLogEntry):
 """Append entry to JSONL log file."""
 log_file = os.path.join(
 self.log_dir,
 f"session{self.currentsession.session_id}.jsonl"
)
 ...
 self.currentsession.endtime = datetime.now()
 self.current_session.success = success
 self.currentsession.finalresult = result

 @ Write session summary
 summary_file = os.path.join(
 self.log_dir,
 f"session{self.currentsession.sessionid}summary.json"
)

 summary = {
 "sessionid": self.currentsession.session_id,
 "task": self.current_session.task,
 "starttime": self.currentsession.start_time.isoformat(),
 "endtime": self.currentsession.end_time.isoformat(),
 "duration_seconds": (
 self.currentsession.endtime -
 self.currentsession.starttime
).total_seconds(),
 "totalactions": len(self.currentsession.actions),
 ...
 def getsessionreplay(self, session_id: str) -> list[dict]:
 """Get all actions from a session for replay/debugging."""
 logfile = os.path.join(self.logdir, f"session{sessionid}.jsonl")
...
class LoggedComputerUseAgent:
 ...
 def init(self, base_agent, logger: ActionLogger):
 self.agent = base_agent
 self.logger = logger

 def run_task(self, task: str) -> dict:
 """Run task with full logging."""
 sessionid = self.logger.startsession(task)

 try:
 result = self.runwith_logging(task)
 ...
 def runwith_logging(self, task: str) -> dict:
...
Message: Consider adding maxcostper_task to prevent expensive runaway tasks.
