...
- **Agent-ready** — each sub-plan contains enough structured detail (objective, context, approach, success/failure criteria) for /beads to create self-contained bead descriptions that serve as execution contracts via `br show --json`
...
**Step 1.3b — Removal Impact Analysis:**

For each task that removes, replaces, or hides existing output/behavior:
1. Enumerate what information or capability the current behavior provides to the operator/user
2. For each item, specify where that information will be available after the change
3. Flag items with no replacement as "information loss" — these must be addressed before the task is complete

```
Example: "Remove static spawn output lines 241-262" →
  - Session ID: now in watch header ✓
  - Runtime URL: LOST — must inject as startup event
  - Density warning: LOST — must inject as startup event
  - Pending agents: LOST — must inject as startup event
```

This prevents plans from silently swallowing useful signals during refactors, UI rewrites, or output format changes. The analysis doesn't need its own sub-plan — it belongs in the Approach section of the task that performs the removal.
...
**Step 1.4e — Field Source Verification:**

When a sub-plan says "construct X from Y" or "populate X using Y's fields," grep Y's type definition and confirm every field X requires is present on Y. If fields are missing, specify the actual source for each field or add a prerequisite task. Do NOT hand-wave with "from SpawnResult fields" — list each field and its verified source:

```
Example: "Construct WatchSessionInfo from SpawnResult"
  SessionId     ← SpawnResult.SessionId   ✓ (verified via grep)
  ControlPort   ← SpawnResult.???         ✗ NOT FOUND — actual source: options.Port
  MaxAgents     ← SpawnResult.???         ✗ NOT FOUND — actual source: scaleConfig.MaxAgents
  CreatedAt     ← SpawnResult.???         ✗ NOT FOUND — actual source: DateTime.UtcNow
```

This applies to any cross-type data flow in the plan: mapper inputs, DTO projections, event payloads, constructor arguments. If the plan claims data flows from A to B, verify A actually has the data.
...
- **Resource Lifetimes:** {Shared stateful resources — databases, connections, caches. Which DI scope owns them? Which tasks share them? When a task introduces a new consumer of an existing shared resource (e.g., a watch loop using a DI-scoped CoordinationDb), specify whether it should share the existing instance or create its own. Note concurrency model for shared instances.}
...
The boundary: **plans describe WHAT to build with enough implementation guidance to prevent the executing agent from re-deriving design decisions. Beads package plan content into self-contained execution contracts and add execution mechanics: file modification lists, commit messages, test commands, and session structure.** Pseudocode, contract shapes, and failure criteria are plan-level concerns because they encode design intent. Git workflow, file lists, and test commands are bead-level concerns because they encode execution mechanics.

**Downstream consumption:** Beads are consumed by hive's BeadsAdapter via `br show --json`. The adapter passes `bead.Description` directly to executing agents as their objective. This means the br record is the execution contract, not just a tracking stub — `/beads` must push the full structured description (Objective through Verification) into each br record. Plans should structure sub-plans with enough detail that `/beads` can produce self-contained bead descriptions without re-reading the design.
...
- [ ] For tasks sharing stateful resources (databases, connections): is the instance ownership specified? Is concurrent access addressed?
- [ ] For tasks that construct types from other types: are all field sources verified against actual type definitions?
...
- [ ] Each sub-plan is self-contained enough for /beads to produce a bead description that works as a standalone execution contract? (An agent receiving only the bead via `br show --json` must understand what to build without reading other sub-plans.)
- [ ] No sub-plan relies on implicit context from other sub-plans? (e.g., "Apply the same pattern as T02" — the pattern must be stated inline.)
...
For BRIEF mode, produce a single `overview.md`. Content comes from the same phases (with Phase 3 skipped) — this template shows the expected output format. Note: BRIEF tasks are intentionally sparse. /beads will supplement with context from the design and codebase, but the "What" field must still state clear intent — an agent receiving only the bead description via `br show --json` must understand what to build.
...
**Hollow Sub-Plans** — Sub-plans that say "implement the save endpoint" without pseudocode, contract shapes, or pattern references. If the design produced this detail, the sub-plan should carry it through. The executing agent shouldn't have to re-derive algorithmic intent from prose descriptions. The design did the thinking — the plan preserves it at the task level. Hollow sub-plans are especially damaging because /beads transforms them into bead descriptions pushed to br — hive's BeadsAdapter passes `bead.Description` directly to executing agents as their sole objective. A hollow sub-plan produces a hollow execution contract.
...
*Skill Version: 4.4 — [Version History](VERSIONS.md)*
