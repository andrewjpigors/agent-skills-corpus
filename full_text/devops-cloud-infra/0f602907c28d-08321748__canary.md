...
description: |
  ...
  "watch production", "verify deploy". (Nexus)
allowed-tools:
  - Bash
  - Read
  - Write
...
mkdir -p .nexus/canary-reports
mkdir -p .nexus/canary-reports/baselines
mkdir -p .nexus/canary-reports/screenshots
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".nexus/canary-reports/baselines/<page-name>.png"
$B console --errors
$B perf
$B text
```
...
Save the baseline manifest to `.nexus/canary-reports/baseline.json`:
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".nexus/canary-reports/screenshots/pre-<page-name>.png"
$B console --errors
$B perf
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".nexus/canary-reports/screenshots/<page-name>-<check-number>.png"
$B console --errors
$B perf
```
...
Screenshots:   .nexus/canary-reports/screenshots/
