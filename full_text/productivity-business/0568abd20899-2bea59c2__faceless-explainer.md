...
> **Concept-teaching pitfall — do NOT just transcribe the source text onto slides.** When the input is a tutorial, knowledge explainer, or any concept that has *structure underneath* (syntax trees, memory layouts, call stacks, state machines, type hierarchies, scope chains, execution traces, evaluation order, …), the lazy failure mode is to render the text as fancy headings + bullet cards with code snippets. That is a slide deck, not a video. **The structural diagram IS the visual.** For each concept the source explains, ask: *what does this look like in memory / at runtime / when the rule fires?* — and animate that. A user who already reads the source text doesn't need a video that paraphrases it; they need a video that **shows them the thing the text can only describe**. If a scene's visual is "code block + caption" with no animation that reveals a structure, it is wrong; rewrite that scene's `creative_brief` to draw the structure, even if it means hand-authoring SVG (a syntax tree, a memory cell, a binding table, a stack frame) instead of using shipped components. See `hyperframes-creative` → "Structural visual" for the full set of diagnostic questions.
