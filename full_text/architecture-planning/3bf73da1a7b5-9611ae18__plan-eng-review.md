...
Read the `/office-hours` skill file at `~/.openclaw/skills/gstack/office-hours/SKILL.md` using the Read tool.
...
```bash
_CROSS_PROJ=$(~/.openclaw/skills/gstack/bin/gstack-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  ~/.openclaw/skills/gstack/bin/gstack-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
  ~/.openclaw/skills/gstack/bin/gstack-learnings-search --limit 10 2>/dev/null || true
fi
```
...
If A: run `~/.openclaw/skills/gstack/bin/gstack-config set cross_project_learnings true`
If B: run `~/.openclaw/skills/gstack/bin/gstack-config set cross_project_learnings false`
...
**Persist the result:**
```bash
~/.openclaw/skills/gstack/bin/gstack-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
