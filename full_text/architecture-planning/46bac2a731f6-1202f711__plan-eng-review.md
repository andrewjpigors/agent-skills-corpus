...
echo '{"skill":"plan-eng-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.mzstack/analytics/skill-usage.jsonl 2>/dev/null || true
...
Write to `~/.mzstack/projects/{slug}/{user}-{branch}-eng-review-test-plan-{datetime}.md`:
...
```bash
~/.claude/skills/mzstack/bin/mzstack-review-log '{"skill":"plan-eng-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"MODE","commit":"COMMIT"}'
```
