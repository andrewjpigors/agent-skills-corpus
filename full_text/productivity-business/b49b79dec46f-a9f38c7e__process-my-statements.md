...
Take `redirect_url` from response, present to user as markdown link with `#qaio_toolkit=googlesheets` appended (so Qaio render connect card). Wait for approval before continuing.
