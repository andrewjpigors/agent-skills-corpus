...
- smoke-test a server with the FastMCP CLI before wiring it into Monika or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Monika. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.monika/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.monika/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
- configure the server in `~/.monika/config.yaml` using the `native-mcp` skill, or
...
The server-building part may be correct while the Monika config is not. Load the `native-mcp` skill and configure the server in `~/.monika/config.yaml`, then restart Hermes.
