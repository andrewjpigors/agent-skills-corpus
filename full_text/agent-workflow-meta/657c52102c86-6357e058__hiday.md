...
│  - Check current state in .hiday/state/orchestrator.json        │
│  - Review pending tasks in .hiday/queue/pending.json            │
...
│  - Update queue files (.hiday/queue/*.json)                     │
├─────────────────────────────────────────────────────────────────┤
...
1. **Max Iterations Reached** - Safety limit (default: 1000)
   - Set via `HIDAY_MAX_ITERATIONS` environment variable
   - Set to very high number (999999) for near-infinite operation

2. **Explicit Completion Promise** (optional) - Only if `HIDAY_COMPLETION_PROMISE` is set:
   - Must output the EXACT text to stop
   - Example: `HIDAY_COMPLETION_PROMISE="SHIPPED TO 1000 CUSTOMERS"`
...
3. Current state in `.hiday/` (what's done, what's pending)
...
3. **Generate new tasks** - Add found improvements to `.hiday/queue/pending.json`
4. **Continue the loop** - Go back to REASON phase

### Explicit Completion (Rare)

**Only output completion if HIDAY_COMPLETION_PROMISE is set and condition is met:**
...
**CRITICAL:** Hiday uses a distributed task queue system for the live dashboard. You MUST:

1. **NEVER use the TodoWrite tool** - It's invisible to the dashboard
2. **ALWAYS use queue JSON files** for task tracking:
   - `.hiday/queue/pending.json` - Tasks not yet started
   - `.hiday/queue/in-progress.json` - Tasks currently being worked on
   - `.hiday/queue/completed.json` - Successfully finished tasks
   - `.hiday/queue/failed.json` - Tasks that failed
...
QUEUE=$(cat .hiday/queue/pending.json)

# Add new task using jq or Write tool
cat > .hiday/queue/pending.json << 'EOF'
...
**CRITICAL:** Long-running autonomous sessions WILL hit context limits. Instead of letting Claude's compaction degrade context quality (summaries of summaries), Hiday uses **ledger-based state preservation**.
...
Every agent maintains a ledger at `.hiday/memory/ledgers/LEDGER-{agent-id}.md`:

```markdown
# Hiday Context Ledger
...
Write .hiday/memory/ledgers/LEDGER-{agent-id}.md with current state

# 2. Update orchestrator with checkpoint
Update .hiday/state/orchestrator.json lastCheckpoint timestamp

# 3. If context is heavy, signal wrapper script
Create .hiday/signals/CONTEXT_CLEAR_REQUESTED
...
**Location:** `.hiday/memory/handoffs/{from-agent}-to-{to-agent}-{timestamp}.md`
...
After each major task completion, extract learnings to `.hiday/memory/learnings/`:
...
- prompt: |
    ...
    ## Relevant Learnings
    [Include applicable learnings from .hiday/memory/learnings/]
    ...
    ## When Complete
    1. Update your ledger at .hiday/memory/ledgers/LEDGER-{your-id}.md
...
def search_memory(query: str) -> List[str]:
    ...
    for rule in glob('.hiday/rules/*.md'):
        if matches(rule, query):
            results.append(f"RULE: {rule}")

    # 2. Search learnings
    for learning in glob('.hiday/memory/learnings/*.md'):
        if matches(learning, query):
            results.append(f"LEARNING: {learning}")

    # 3. Search recent handoffs
    for handoff in sorted(glob('.hiday/memory/handoffs/*.md'), reverse=True)[:10]:
...
1. Load orchestrator state from `.hiday/state/orchestrator.json`
2. Load relevant agent ledger from `.hiday/memory/ledgers/`
3. Check for pending handoffs in `.hiday/memory/handoffs/`
...
When Hiday is invoked WITHOUT a PRD, it operates in **Codebase Analysis Mode**:
...
### Step 3: Generate PRD
Create a comprehensive PRD at `.hiday/generated-prd.md`:
...
The prompt includes `SDLC_PHASES_ENABLED: [...]` listing which phases to execute. Execute each enabled phase in order. Log results to `.hiday/logs/sdlc-{phase}-{timestamp}.md`.
...
5. Log all failures to `.hiday/logs/api-test-failures.md`

**Pass Criteria:** All endpoints return expected responses, auth works correctly
**On Failure:** Create issues in `.hiday/queue/pending.json` for each failing endpoint
...
7. Log findings to `.hiday/logs/competitive-analysis.md`
...
5. Log all findings to `.hiday/logs/uat-findings.md`
...
| Field | Value |
|-------|-------|
| **Trigger** | "Hiday" or "Hiday with PRD at [path]" |
...
ORCHESTRATOR executes as primary Claude Code session
    ...
    │   claude -p "$(cat .hiday/prompts/agent-role.md)" --dangerously-skip-permissions
...
tmux new-session -d -s hiday-pool
for i in {1..5}; do
  tmux new-window -t hiday-pool -n "agent-$i" \
    "claude --dangerously-skip-permissions -p '$(cat .hiday/prompts/agent-$i.md)'"
done
...
└── scripts/
    ├── bootstrap.sh             # Initialize .hiday structure
...
mkdir -p "$HIDAY_ROOT"/{state/{agents,checkpoints,locks},queue,messages/{inbox,outbox,broadcast},logs/{agents,decisions,archive},config,prompts,artifacts/{releases,reports,metrics,backups},scripts}

# Initialize queue files
for f in pending in-progress completed failed dead-letter; do
  echo '{"tasks":[]}' > "$HIDAY_ROOT/queue/$f.json"
done

# Initialize orchestrator state
cat > "$HIDAY_ROOT/state/orchestrator.json" << 'EOF'
{
  "version": "2.1.0",
  "startupId": "",
  "phase": "bootstrap",
...
if [[ "$OSTYPE" == "darwin"* ]]; then
  sed -i '' "s/\"startupId\": \"\"/\"startupId\": \"$STARTUP_ID\"/" "$HIDAY_ROOT/state/orchestrator.json"
else
  sed -i "s/\"startupId\": \"\"/\"startupId\": \"$STARTUP_ID\"/" "$HIDAY_ROOT/state/orchestrator.json"
fi

echo "Bootstrap complete: $HIDAY_ROOT initialized"
```

## State Schema

### `.hiday/state/orchestrator.json`
```json
{
  "version": "2.1.0",
  "startupId": "uuid",
...
### Agent State Schema (`.hiday/state/agents/[id].json`)
```json
{
  "id": "eng-backend-01",
  "role": "eng-backend",
...
**Circuit Breaker Config (`.hiday/config/circuit-breakers.yaml`):**
```yaml
defaults:
  failureThreshold: 5
  cooldownSeconds: 300
...
- instructions: |
    1. Read task requirements from .hiday/queue/in-progress.json
...
```bash
claude --dangerously-skip-permissions \
  -p "$(cat .hiday/prompts/eng-backend.md)" \
  --output-format json \
  > .hiday/messages/outbox/eng-backend-01/result.json
```
...
tmux new-window -t reviewers -n code "claude -p '$(cat .hiday/prompts/code-reviewer.md)' --dangerously-skip-permissions"
tmux new-window -t reviewers -n business "claude -p '$(cat .hiday/prompts/business-reviewer.md)' --dangerously-skip-permissions"
tmux new-window -t reviewers -n security "claude -p '$(cat .hiday/prompts/security-reviewer.md)' --dangerously-skip-permissions"
...
QUEUE_FILE=".hiday/queue/pending.json"
LOCK_FILE=".hiday/state/locks/queue.lock"
...
def claim_task(agent_id, agent_capabilities):
    with file_lock(".hiday/state/locks/queue.lock", timeout=10):
        pending = read_json(".hiday/queue/pending.json")
...
def complete_task(task_id, result, success=True):
    with file_lock(".hiday/state/locks/queue.lock"):
...
1. Read `.hiday/queue/dead-letter.json`
2. For each task:
...
4. Log decision in `.hiday/logs/decisions/dlq-review-{date}.md`
```
...
### Phase 0: Bootstrap
1. Create `.hiday/` directory structure
...
5. Generate final report in .hiday/artifacts/reports/final-review.md
...
grep -rn "TODO(review)\|FIXME(nitpick)" src/ > .hiday/artifacts/reports/tech-debt.txt

# Count by severity
echo "Low: $(grep -c 'Severity: Low' .hiday/artifacts/reports/tech-debt.txt)"
echo "Cosmetic: $(grep -c 'Severity: Cosmetic' .hiday/artifacts/reports/tech-debt.txt)"
```
...
def acquire_file_lock(file_path, agent_id, timeout=300):
    lock_file = f".hiday/state/locks/files/{hash(file_path)}.lock"
...
3. **Evaluate**: Orchestrator compares:
   ...
   - Alignment with PRD
   - Simplicity
4. **Decide**: Orchestrator makes final call, documents in HIDAY-LOG.md
...
Questions (logged in HIDAY-LOG.md):
...
class AgentWatchdog:
    ...
    def update_heartbeat(self):
        ...
        state_file = f'.hiday/state/agents/{self.agent_id}.json'
...
Each agent maintains own state in `.hiday/state/agents/[id].json`

### Orchestrator Recovery
1. On startup, check `.hiday/state/orchestrator.json`
...
echo '{"command":"pause","reason":"manual pause","timestamp":"'$(date -Iseconds)'"}' \
  > .hiday/messages/broadcast/system-pause.json
...
# Resume system
rm .hiday/messages/broadcast/system-pause.json
...
```bash
#!/bin/bash
# .hiday/scripts/shutdown.sh

echo "Initiating graceful shutdown..."

# 1. Stop accepting new tasks
touch .hiday/state/locks/shutdown.lock

# 2. Wait for in-progress tasks (max 30 min)
TIMEOUT=1800
ELAPSED=0
while [ -s .hiday/queue/in-progress.json ] && [ $ELAPSED -lt $TIMEOUT ]; do
  echo "Waiting for $(jq '.tasks | length' .hiday/queue/in-progress.json) tasks..."
  sleep 30
  ELAPSED=$((ELAPSED + 30))
done

# 3. Checkpoint everything
cp -r .hiday/state .hiday/artifacts/backups/shutdown-$(date +%Y%m%d-%H%M%S)

# 4. Update orchestrator state
jq '.phase = "shutdown" | .systemHealth = "offline"' \
  .hiday/state/orchestrator.json > tmp && mv tmp .hiday/state/orchestrator.json
...
```bash
#!/bin/bash
# .hiday/scripts/backup-state.sh
# Run hourly via orchestrator or cron

BACKUP_DIR=".hiday/artifacts/backups"
...
cp .hiday/state/orchestrator.json "$BACKUP_PATH/"
cp -r .hiday/state/agents "$BACKUP_PATH/"
cp -r .hiday/queue "$BACKUP_PATH/"
cp .hiday/logs/HIDAY-LOG.md "$BACKUP_PATH/"
...
# Rotate main log
if [ -f "$LOG_DIR/HIDAY-LOG.md" ]; then
  mv "$LOG_DIR/HIDAY-LOG.md" "$ARCHIVE_DIR/HIDAY-LOG-$DATE.md"
  echo "# Hiday Log - $(date +%Y-%m-%d)" > "$LOG_DIR/HIDAY-LOG.md"
fi
...
send_alert() {
  ...
  # Log locally
  echo "[$(date -Iseconds)] [$SEVERITY] $MESSAGE" >> .hiday/logs/alerts.log
  
  # Send to Slack if configured
  if [ -n "$SLACK_WEBHOOK_URL" ]; then
    curl -s -X POST "$SLACK_WEBHOOK_URL" \
      -H 'Content-type: application/json' \
      -d "{\"text\":\"[$SEVERITY] Hiday: $MESSAGE\"}"
  fi
}
```
...
**"Hiday"** or **"Hiday with PRD at [path]"**

### Startup Sequence
```
╔══════════════════════════════════════════════════════════════════╗
║                    HIDAY v2.0 ACTIVATED                       ║
...
Generated at `.hiday/artifacts/reports/dashboard.md`:
```
# Hiday Dashboard
...
- **NEVER** delete .hiday/state/ directory while running
...
- **ALWAYS** log decisions with evidence in HIDAY-LOG.md
