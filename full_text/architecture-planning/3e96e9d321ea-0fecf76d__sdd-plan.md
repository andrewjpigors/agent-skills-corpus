...
7. **Ensure directories exist**:
   ...
   - `.opencode/skills/` - Reusable skill documents
...
- Do not read task files in .opencode or .specs directories, your job is orchestrate agents that will do the work, not do it by yourself!
...
3. **Provide the installed `.opencode/sdd` path so agents can resolve paths like `.opencode/sdd/scripts/create-scratchpad.sh`**
...
- Skill file path (e.g., `.opencode/skills/<skill-name>/SKILL.md`)
