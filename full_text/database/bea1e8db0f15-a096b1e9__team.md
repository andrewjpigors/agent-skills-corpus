---
name: team
description: N coordinated agents on shared task list using Grok Build native teams
aliases: []
level: 4
---
...
Spawn N coordinated agents working on a shared task list using Grok Build's native team tools. Replaces the legacy `/swarm` skill (SQLite-based) with built-in team management, inter-agent messaging, and task dependencies -- no external dependencies required.
...
- **agent-type** - OMG agent to spawn for the `team-exec` stage (e.g., executor, debugger, designer, codex, gemini). Optional; defaults to stage-aware routing. Use `codex` to spawn Codex CLI workers or `gemini` for Gemini CLI workers (requires respective CLIs installed). See Stage Agent Routing below.
...
**Storage layout (managed by Grok Build):**
```
~/.grok/
  teams/fix-ts-errors/
...
- **Cancel:** `/oh-my-grok:cancel` requests teammate shutdown, waits for responses (best effort), marks phase `cancelled` with `active=false`, captures cancellation metadata, then deletes team resources and clears/preserves Team state per policy. Handoff files in `.omc/handoffs/` are preserved for potential resume.
...
{
  "team_name": "fix-ts-errors",
  "team_file_path": "~/.grok/teams/fix-ts-errors/config.json",
...
4. **Delete team** -- Call `TeamDelete` to clean up:
   ...
   ```
5. **Clean OMG state** -- Remove `.omc/state/team-state.json`
...
```bash
node "${GROK_PLUGIN_ROOT}/scripts/cleanup-orphans.mjs" --team-name fix-ts-errors
```
...
|---------------|----------|-------------|
| `claude_worker` | Claude agent | Full Grok Build tool access (Read/Write/Edit/Bash/Task). Best for tasks needing Claude's reasoning + iterative tool use. |
...
- CLI workers operate via tmux, not Grok Build's tool system
...
1. Spawn `Task(subagent_type="oh-my-grok:planner", ...)` with task description + codebase context
...
This approach complements the existing `SendMessage`-based communication by providing a pull-based mechanism for MCP workers that cannot use Grok Build's team messaging tools.
...
1. User invokes `/team ralph "task"` or `/oh-my-grok:team ralph "task"`
...
4. If architect approves: both modes complete, run `/oh-my-grok:cancel`
...
1. Check `~/.grok/teams/` for teams matching the task slug
...
|--------|--------------|----------------------|
| **Storage** | JSON files in `~/.grok/teams/` and `~/.grok/tasks/` | SQLite in `.omc/state/swarm.db` |
...
| **Heartbeat** | Automatic idle notifications from Grok Build | Manual heartbeat table + polling |
...
**When to use Team over Swarm:** Always prefer `/team` for new work. It uses Grok Build's built-in infrastructure, requires no external dependencies, supports inter-agent communication, and has task dependency management.

## Cancellation

The `/oh-my-grok:cancel` skill handles team cleanup:
...
If teammates are unresponsive, `TeamDelete` may fail. In that case, the cancel skill should wait briefly and retry, or inform the user to manually clean up `~/.grok/teams/{team_name}/` and `~/.grok/tasks/{team_name}/`.
...
> **Note:** Team members do not have a hardcoded model default. Each teammate is a separate Grok Build session that inherits the user's configured model. Since teammates can spawn their own subagents, the session model acts as the orchestration layer while subagents can use any model tier.

## State Cleanup

On successful completion:

1. `TeamDelete` handles all Grok Build state:
   - Removes `~/.grok/teams/{team_name}/` (config)
   - Removes `~/.grok/tasks/{team_name}/` (all task files + lock)
2. OMG state cleanup via MCP tools:
   ...
   ```
3. Or run `/oh-my-grok:cancel` which handles all cleanup automatically.
...
1. **Worktree creation**: Before spawning a worker, call `createWorkerWorktree(teamName, workerName, repoRoot)` to create an isolated worktree at `.omc/worktrees/{team}/{worker}` with branch `omg-team/{teamName}/{workerName}`.
