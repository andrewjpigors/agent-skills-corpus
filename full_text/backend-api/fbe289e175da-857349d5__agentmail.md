---
name: agentmail
description: Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. monika-agent@agentmail.to).
version: 1.0.0
metadata:
  monika:
...
Add to `~/.monika/config.yaml` (paste your actual key — MCP env vars are not expanded from .env):
```yaml
mcp_servers:
  agentmail:
    command: "npx"
...
1. Create a dedicated inbox:
   - Use `create_inbox` with a username (e.g. `monika-agent`)
   - The agent gets address: `monika-agent@agentmail.to`
2. Send an email:
...
```
monika --toolsets mcp -q "Create an AgentMail inbox called test-agent and tell me its email address"
```
