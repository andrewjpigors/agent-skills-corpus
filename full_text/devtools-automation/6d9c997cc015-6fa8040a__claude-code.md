---
name: claude-code
description: "Delegate coding tasks (features, refactors, PR reviews) to the Claude Code CLI from bash; use when offloading a bounded or multi-turn coding job to claude."
license: MIT
...
Delegate coding tasks to [Claude Code](https://code.claude.com/docs/en/cli-reference) (Anthropic's autonomous coding agent CLI) by invoking the `claude` CLI from bash. Claude Code v2.x can read files, write code, run shell commands, spawn subagents, and manage git workflows autonomously.

All examples below are plain shell commands — run them with your bash tool (set the working directory with `cd` or run inside the project dir).
...
Interactive mode gives you a full conversational REPL where you can send follow-up prompts, use slash commands, and watch Claude work in real time. **Requires tmux orchestration** (drive it with `tmux send-keys` / `tmux capture-pane` from bash).
...
# Wait for startup, then send your task (after ~3-5s for the welcome screen)
...
```bash
cd /project && claude -p 'List all functions in src/' --output-format json \
...
cd /project && claude -p 'Continue and add connection pooling' \
  --resume "$(python3 -c 'import json,sys; print(json.load(open("/tmp/session.json"))["session_id"])')" --max-turns 5
...
Run multiple independent Claude tasks simultaneously (each in its own tmux session, or just background each `claude -p` with `&`):
...
1. **Interactive mode REQUIRES tmux** — Claude Code is a full TUI app. Driving it through tmux gives you `capture-pane` for monitoring and `send-keys` for input, which is essential for orchestration from a non-interactive bash tool. (For one-shot tasks, prefer `-p` print mode and skip tmux entirely.)
...
2. **Use tmux for multi-turn interactive work** — the only reliable way to orchestrate the TUI from bash
3. **Always run in the right directory** — `cd` into the target project so Claude stays focused
