...
bugs are invisible until you log in. This repo has no session-loader; instead
pass the **same auth headers to every MCP `run_tool` call** (httpx, katana,
ffuf, nuclei, dalfox, the SQLi / SSTI / upload PoC verifiers).

```bash
# Cookie auth — reuse the same -H across every tool:
run_tool("httpx", "-H 'Cookie: session=eyJabc...' -u https://T")
# Bearer auth:
run_tool("nuclei", "-H 'Authorization: Bearer eyJhbGciOi...' -u https://T")
```

**For IDOR / BOLA hunts**, replay the same request under two identities and diff:

```bash
run_tool("curl", "-s -H 'Cookie: session=<user-a>' https://T/api/resource/123")
run_tool("curl", "-s -H 'Cookie: session=<user-b>' https://T/api/resource/123")
# Diff status/body per identity → endpoints that behaved differently are IDOR candidates.
```

**Safety**: the MCP audit log redacts credential-shaped strings (cookies, bearer
tokens, API keys) from `run_script` content before writing — see `audit.py`.
...
Keep target auth headers (cookie / bearer / `X-API-Key`) in a gitignored notes
file and pass them to each `run_tool` call via `-H`; never paste them into prose.
...
# Parameter discovery
ffuf -w /usr/share/seclists/Discovery/Web-Content/burp-parameter-names.txt -u "https://target.com/api/endpoint?FUZZ=test" -ac -mc 200

# Hidden POST parameters
ffuf -w /usr/share/seclists/Discovery/Web-Content/burp-parameter-names.txt -X POST -d "FUZZ=test" -u "https://target.com/api/endpoint" -ac
