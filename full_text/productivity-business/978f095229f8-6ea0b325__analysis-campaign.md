...
- Keep ordinary subtask completions concise. When an analysis campaign or a stage-significant campaign checkpoint is complete, upgrade to a richer `artifact.record(payload={'kind': 'milestone'}, reply_mode='threaded', ...)` report.
...
After the charter and launch decision are durably recorded, send one threaded `artifact.record(payload={'kind': 'milestone'}, ...)` update naming:
...
- after the first meaningful signal and then at real checkpoints (e.g., completion, blocker, recovery, or a materially changed evidence frontier), send `artifact.record(payload={'kind': 'progress'}, ...)` so the user sees the newest real state
- after each completed sleep / await monitoring cycle for an active slice, inspect state first; only send another `artifact.record(payload={'kind': 'progress'}, ...)` update if the user-visible state materially changed
...
Before a long slice starts, emit a `progress` artifact or `artifact.record(payload={'kind': 'progress'}, ...)` update so the quest shows that the slice is active.
...
Send a threaded `artifact.record(payload={'kind': 'milestone'}, ...)` update at that point with the new boundary or risk.
...
When the aggregated campaign report is complete, send a richer threaded `artifact.record(payload={'kind': 'milestone'}, ...)` update.
