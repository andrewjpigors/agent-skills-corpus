...
description: >
  ...
  MaxwellBoltzmannDistribution, thermalize_momenta, exact_temperature, MDLogger, timestep, friction, temperature_K,
...
**ASE 3.29 以降の推奨対応（`thermalize_momenta`）**: ASE 3.29 で `MaxwellBoltzmannDistribution` は非推奨（deprecated）となり、後継の `thermalize_momenta` への移行が推奨されています。引数はほぼ同一で、そのまま置き換えられます。`MaxwellBoltzmannDistribution` も 3.29 では `DeprecationWarning` 付きで動作し続けますが、3.29 未満の ASE には `thermalize_momenta` が存在しないため、Matlantis 環境の ASE バージョンに応じて選択してください（新規コードは `thermalize_momenta` を推奨）。

```python
# ASE 3.29 以降
from ase.md.velocitydistribution import thermalize_momenta, Stationary, ZeroRotation

thermalize_momenta(atoms, temperature_K=300)          # MaxwellBoltzmannDistribution(atoms, temperature_K=300) と等価
Stationary(atoms)
# 再現性が必要な場合: thermalize_momenta(atoms, temperature_K=300, rng=rng)
```

引数の対応は以下の通りです（それ以外の `atoms` / `temperature_K` / `rng` は同じ）。

| `MaxwellBoltzmannDistribution` | `thermalize_momenta`（3.29+） |
| :--- | :--- |
| `force_temp=True`（運動エネルギーを厳密に 3/2 N k T へ再スケール） | `exact_temperature=True` |
...
estimator_fn = pfp_estimator_fn(model_version="v9.0.0", calc_mode="R2SCAN")
...
| `DeprecationWarning: MaxwellBoltzmannDistribution` | ASE 3.29 で `MaxwellBoltzmannDistribution` が非推奨化された | 警告のみで動作は継続。ASE 3.29 以降では `thermalize_momenta`（`force_temp` は `exact_temperature` に対応）への移行を検討（準備段階の該当ノート参照） |
