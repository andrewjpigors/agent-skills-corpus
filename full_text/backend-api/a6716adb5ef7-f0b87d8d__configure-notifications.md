...
Set up omcp notification integrations so you're alerted when sessions end, need input, or complete background tasks.
...
Set up Telegram notifications so omcp can message you when sessions end, need input, or complete background tasks.

### How This Skill Works

This is an interactive, natural-language configuration skill. Walk the user through setup by asking questions with AskUserQuestion. Write the result to `.omcp/notifications.json`.

### Step 1: Detect Existing Configuration

```bash
CONFIG_FILE=".omcp/notifications.json"
...
2. Send /newbot
3. Choose a name (e.g., "My omcp Notifier")
...
RESPONSE=$(curl -s -w "\n%{http_code}" \
  ...
  -d "text=omcp test notification - Telegram is configured!")
...
Config saved to: .omcp/notifications.json

You can also set these via environment variables:
  OMCP_TELEGRAM_BOT_TOKEN=123456789:ABCdefGHI...
  OMCP_TELEGRAM_CHAT_ID=123456789
...
```bash
export OMCP_TELEGRAM_BOT_TOKEN="123456789:ABCdefGHIjklMNOpqrsTUVwxyz"
export OMCP_TELEGRAM_CHAT_ID="123456789"
```

Env vars are auto-detected by the notification system without needing `.omcp/notifications.json`.

---

## Discord Setup

Set up Discord notifications so omcp can ping you when sessions end, need input, or complete background tasks.
...
**Options:**
1. **omcp (default)** - Display as "omcp"
...
Build the notifications object with the collected values and merge into `.omcp/notifications.json` using jq:
...
curl -s -o /dev/null -w "%{http_code}" \
  ...
  -d "{\"content\": \"${MENTION:+$MENTION\\n}omcp test notification - Discord is configured!\"}" \
  "$WEBHOOK_URL"
```
...
You can also set these via environment variables:
  OMCP_DISCORD_WEBHOOK_URL=https://discord.com/api/webhooks/...
  OMCP_DISCORD_MENTION=<@1465264645320474637>
...
**Webhook method:**
```bash
export OMCP_DISCORD_WEBHOOK_URL="https://discord.com/api/webhooks/..."
export OMCP_DISCORD_MENTION="<@1465264645320474637>"  # optional
```

**Bot API method:**
```bash
export OMCP_DISCORD_NOTIFIER_BOT_TOKEN="your-bot-token"
export OMCP_DISCORD_NOTIFIER_CHANNEL="your-channel-id"
export OMCP_DISCORD_MENTION="<@1465264645320474637>"  # optional
```
...
Set up Slack notifications so omcp can message you when sessions end, need input, or complete background tasks.
...
3. Name your app (e.g., "omcp Notifier") and select your workspace
...
curl -s -o /dev/null -w "%{http_code}" \
  -H "Content-Type: application/json" \
  -d "{\"text\": \"${MENTION_PREFIX}omcp test notification - Slack is configured!\"}" \
  "$WEBHOOK_URL"
```
...
You can also set these via environment variables:
  OMCP_SLACK_WEBHOOK_URL=https://hooks.slack.com/services/...
  OMCP_SLACK_MENTION=<@U1234567890>
...
```bash
export OMCP_SLACK_WEBHOOK_URL="https://hooks.slack.com/services/T00/B00/xxx"
export OMCP_SLACK_MENTION="<@U1234567890>"  # optional
```
...
All notification platforms require explicit activation per session via
environment variables. Export the relevant `OMCP_*` flag (the plugin does not
ship a standalone `omc`/`omcp` launcher binary — set the env var directly, or
let `/omcp:configure-notifications` set it for the session):

- `OMCP_TELEGRAM=1` — Activates Telegram notifications
- `OMCP_DISCORD=1` — Activates Discord notifications
- `OMCP_SLACK=1` — Activates Slack notifications
- `OMCP_WEBHOOK=1` — Activates webhook notifications
- `OMCP_OPENCLAW=1` — Activates OpenClaw gateway integration
...
**Examples:**
- `OMCP_TELEGRAM=1 OMCP_DISCORD=1` — Telegram + Discord active
- `OMCP_TELEGRAM=1 OMCP_SLACK=1 OMCP_WEBHOOK=1` — Telegram + Slack + Webhook active
- `OMCP_TELEGRAM=1 OMCP_OPENCLAW=1` — Telegram + OpenClaw active
- (none set) — No notifications sent (all platforms require explicit activation)

---

## Hook Event Templates

Customize notification messages per event and per platform using `.omcp/notifications.hook.json`.
...
Check if `.omcp/notifications.hook.json` exists. If it does, show the current configuration. If not, explain what it does.
...
Config file: .omcp/notifications.hook.json
```
...
RAW FIELDS:
  ...
  {{activeMode}}     - Active omcp mode name
...
Read or create `.omcp/notifications.hook.json` and merge the new settings:
...
If `.omcp/notifications.openclaw.json` exists, detect and offer migration:

**Step 1: Detect Legacy Config**
```bash
LEGACY_CONFIG=".omcp/notifications.openclaw.json"
...
If migrate:
- Read `.omcp/notifications.openclaw.json`
- Transform to custom integration format
- Save to `.omcp/notifications.json`
- Backup legacy to `.omcp/notifications.openclaw.json.bak`
...
curl -X POST \
  ...
  -d '{"event":"test","instruction":"omcp test notification","timestamp":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' \
  "$WEBHOOK_URL"
```
...
Merge into `.omcp/notifications.json`:
