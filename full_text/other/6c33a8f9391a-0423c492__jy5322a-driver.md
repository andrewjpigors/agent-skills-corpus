---
name: jy5322a-driver
description: 提供 JYTEK JY5322A 系列（PXIe/PCIe/USB-5322A）多功能数据采集卡（DAQ）的完整 C# 驱动开发指引。涵盖模拟输入（AI）单点/有限/连续/录制采集、数字输入/输出（DI/DO）单点/有限/连续模式、计数器输入/输出（CI/CO）边沿计数/频率/周期/脉冲/编码器测量/脉冲生成、触发配置（数字/模拟/软件触发）、时钟配置（内部/外部时钟）、多卡同步、录制模式（Finite/Infinite Streaming）。当用户使用 JY5322A、PXIe-5322A、PCIe-5322A、USB-5322A、JY5322AAITask、JY5322ADITask、JY5322ADOTask、JY5322ACITask、JY5322ACOTask、AIMode.Single、AIMode.Finite、AIMode.Continuous、AIMode.Record 开发数据采集、信号采集、计数器测量、PWM输出、自动化测试应用时自动应用。
---
...
- **驱动 DLL**：`C:\SeeSharp\JYTEK\Hardware\DAQ\JY5322A\Bin\JY5322A.dll`（引用到 .csproj）
- **XML 文档**：`C:\SeeSharp\JYTEK\Hardware\DAQ\JY5322A\Bin\JY5322A.xml`（提供 IntelliSense 支持）
...
```csharp
var task = new JY5322AAITask(0);         // 1. 创建（按槽位号）
...
```csharp
var diTask = new JY5322ADITask(0);
diTask.AddChannel(0);                    // 添加第 0 线（0-15）
diTask.Start();
bool readValue;
...
```csharp
var doTask = new JY5322ADOTask(0);
doTask.AddChannel(0);                    // 添加第 0 线（0-15）
doTask.Start();
...
**构造**：`new JY5322ACITask(int slotNumber, int channel)` — channel 仅 0
...
```csharp
coTask = new JY5322ACOTask(0, 0);        // channel 仅 0
...
多张 JY5322A 板卡之间实现同步采集，主卡导出采样时钟和触发信号，从卡接收：

```csharp
// ===== 主卡配置（Slot 0）=====
var masterTask = new JY5322AAITask(0);
...
// ===== 从卡配置（Slot 1）=====
var slaveTask = new JY5322AAITask(1);
...
```csharp
// ===== AI 配置（主设备）=====
var aiTask = new JY5322AAITask(0);
aiTask.AddChannel(0, -10, 10);
...
```csharp
using JY5322A;
// DLL 路径：C:\SeeSharp\JYTEK\Hardware\DAQ\JY5322A\Bin\JY5322A.dll
```

---

## JY5322AAITask — 模拟输入任务

### 构造函数

```csharp
new JY5322AAITask(int slotNumber)      // 按槽位号创建（推荐）
```
...
## JY5322ADITask — 数字输入任务

### 构造函数

```csharp
new JY5322ADITask(int slotNumber)
```

### 方法

```csharp
void AddChannel(int lineID)                         // 添加线（0~15）
...
## JY5322ADOTask — 数字输出任务

### 构造函数

```csharp
new JY5322ADOTask(int slotNumber)
```
...
## JY5322ACITask — 计数器输入任务

### 构造函数

```csharp
new JY5322ACITask(int slotNumber, int channel)      // channel: 仅 0
```
...
## JY5322ACOTask — 计数器输出任务

### 构造函数

```csharp
new JY5322ACOTask(int slotNumber, int channel)      // channel: 仅 0
```
...
> 所有示例均来自 `JY5322A_V1.0.0_Examples/` 范例工程，已提取核心逻辑并加注中文注释。
...
JY5322AAITask aiTask = new JY5322AAITask(boardNum);

// 2. 设置模式
aiTask.Mode = AIMode.Single;

// 3. 添加通道：通道 0，量程 ±10V
Console.WriteLine("请输入通道号（0-15）：");
...
```csharp
private JY5322ACITask ciTask;

private void button_start_Click(object sender, EventArgs e)
{
    // 计数器通道 0（JY5322A 仅支持通道 0）
    ciTask = new JY5322ACITask(0, 0);
    ciTask.Mode = CIMode.Single;
    ciTask.Type = CIType.EdgeCounting;
...
```csharp
ciTask = new JY5322ACITask(0, 0);        // JY5322A 仅支持通道 0
ciTask.Mode = CIMode.Single;
...
```csharp
private JY5322ACOTask coTask;

private void button_start_Click(object sender, EventArgs e)
{
    coTask = new JY5322ACOTask(0, 0);    // JY5322A 仅支持通道 0
    coTask.Mode = COMode.Single;
    coTask.IdleState = COIdleState.Low;
    coTask.InitialDelay = 0;
...
读取已录制的 bin 文件并回放显示（JY5322A 数据格式为 short）：
