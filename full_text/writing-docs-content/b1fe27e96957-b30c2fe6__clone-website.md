...
# Clone Website - Homepage-Only Mode

**Primary Goal:** Clone ONLY the homepage at the provided URL. Do not crawl, follow links, or clone subpages unless explicitly requested. Ignore navigation menus, footers linking elsewhere, and any multi-page structure.

**URL Handling:**
- Target exactly the provided homepage URL (e.g., https://cash.app/ or https://cash.app/ + trailing slash if needed).
- Set base path to `/` and treat all relative assets/links as homepage-only.
- In reconnaissance, do not use site-wide crawlers or sitemaps.

**Enhanced Scroll & Animation Capture:**
- Perform a full interaction sweep: scroll from top to bottom multiple times at varying speeds (slow for parallax/GSAP/Framer, normal for standard).
- Use browser dev tools / computed styles to capture:
  - All `IntersectionObserver`, scroll-triggered classes, data attributes (e.g., `data-scroll`, `data-animate`).
  - CSS animations, transitions, keyframes (including custom properties like `--scroll-y`, transform/scale/opacity on scroll).
  - JavaScript event listeners for `scroll`, `wheel`, `resize`.
  - Canvas/WebGL if present (e.g., Cash App's kinetic/scroll interactions).
- Take timed screenshots or record a short video/scroll session for reference.
- Extract GSAP, Framer Motion, Lenis, Locomotive Scroll, or custom scroll libraries via network tab or script inspection.
- In component specs: Document exact scroll positions where effects trigger, before/after states, easing curves, durations, and mobile vs desktop differences.

**Pipeline Adjustments:**
- Limit reconnaissance to the single page.
- In component specs (`docs/research/components/`), emphasize scroll behaviors, sticky elements, parallax layers, and progressive reveal animations.
- During build: Prioritize React hooks like `useScroll`, `useTransform` (Framer Motion), or `useEffect` + `requestAnimationFrame` for smooth replication.
- For Cash App-like sites: Pay special attention to bold typography animations, canvas scroll interactions, and color-shift-on-scroll effects.