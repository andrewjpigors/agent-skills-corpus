...
- Give it a name like "monika-agent"
- Select scopes:
...
- Give it a title like "monika-agent-<machine-name>"
...
elif [ -f ~/.monika/.env ] && grep -q "^GITHUB_TOKEN=" ~/.monika/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.monika/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
