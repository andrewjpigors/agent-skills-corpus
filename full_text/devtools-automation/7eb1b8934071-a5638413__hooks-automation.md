...
**Required:**
- Claude Flow CLI installed (`npm install -g hyrex@latest`)
- Claude Code with hooks enabled
- `.claude/settings.json` with hook configurations

**Optional:**
- MCP servers configured (hyrex, ruv-swarm, flow-nexus)
...
npx hyrex hook pre-task --description "Implement authentication"

# Post-edit hook (auto-formats and stores in memory)
npx hyrex hook post-edit --file "src/auth.js" --memory-key "auth/login"

# Session end hook (saves state and metrics)
npx hyrex hook session-end --session-id "dev-session" --export-metrics
```
...
```bash
npx hyrex hook pre-edit [options]
...
Examples:
  npx hyrex hook pre-edit --file "src/auth/login.js"
  npx hyrex hook pre-edit -f "config/db.js" --validate-syntax
  npx hyrex hook pre-edit -f "production.env" --backup-file --check-conflicts
```
...
```bash
npx hyrex hook pre-bash --command <cmd>
...
Examples:
  npx hyrex hook pre-bash -c "rm -rf /tmp/cache"
  npx hyrex hook pre-bash --command "docker build ." --estimate-resources
```
...
```bash
npx hyrex hook pre-task [options]
...
Examples:
  npx hyrex hook pre-task --description "Implement user authentication"
  npx hyrex hook pre-task -d "Continue API dev" --load-memory
  npx hyrex hook pre-task -d "Refactor codebase" --optimize-topology
```
...
```bash
npx hyrex hook pre-search --query <query>
...
Examples:
  npx hyrex hook pre-search -q "authentication middleware"
```
...
```bash
npx hyrex hook post-edit [options]
...
Examples:
  npx hyrex hook post-edit --file "src/components/Button.jsx"
  npx hyrex hook post-edit -f "api/auth.js" --memory-key "auth/login"
  npx hyrex hook post-edit -f "utils/helpers.ts" --train-patterns
```
...
```bash
npx hyrex hook post-bash --command <cmd>
...
Examples:
  npx hyrex hook post-bash -c "npm test" --update-metrics
```
...
```bash
npx hyrex hook post-task [options]
...
Examples:
  npx hyrex hook post-task --task-id "auth-implementation"
  npx hyrex hook post-task -t "api-refactor" --analyze-performance
  npx hyrex hook post-task -t "bug-fix-123" --store-decisions
```
...
```bash
npx hyrex hook post-search --query <query> --results <path>
...
Examples:
  npx hyrex hook post-search -q "auth" -r "results.json" --train-patterns
```
...
```bash
npx hyrex hook mcp-initialized --swarm-id <id>
...
```bash
npx hyrex hook agent-spawned --agent-id <id> --type <type>
...
```bash
npx hyrex hook task-orchestrated --task-id <id>
...
```bash
npx hyrex hook neural-trained --pattern <name>
...
```bash
npx hyrex hook memory-sync --namespace <ns>
...
```bash
npx hyrex hook session-start --session-id <id>
...
```bash
npx hyrex hook session-restore --session-id <id>
...
Examples:
  npx hyrex hook session-restore --session-id "swarm-20241019"
  npx hyrex hook session-restore -s "feature-auth" --restore-memory
```
...
```bash
npx hyrex hook session-end [options]
...
Examples:
  npx hyrex hook session-end --session-id "dev-session-2024"
  npx hyrex hook session-end -s "feature-auth" --export-metrics --generate-summary
  npx hyrex hook session-end -s "quick-fix" --cleanup-temp
```
...
```bash
npx hyrex hook notify --message <msg>
...
Examples:
  npx hyrex hook notify -m "Task completed" --level info
  npx hyrex hook notify -m "Critical error" --level error --broadcast
```
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx hyrex hook pre-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/current'"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "npx hyrex hook pre-bash --command '${tool.params.command}'"
        }]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx hyrex hook post-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/complete' --auto-format --train-patterns"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "npx hyrex hook post-bash --command '${tool.params.command}' --update-metrics"
        }]
      }
    ]
  }
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx hyrex hook pre-edit --file '${tool.params.file_path}' --auto-assign-agent --validate-syntax",
            "timeout": 3000,
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx hyrex hook pre-task --description '${tool.params.task}' --auto-spawn-agents --load-memory",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx hyrex hook pre-search --query '${tool.params.pattern}' --check-cache"
          }
        ]
      }
    ],
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx hyrex hook post-edit --file '${tool.params.file_path}' --memory-key 'edits/${tool.params.file_path}' --auto-format --train-patterns",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx hyrex hook post-task --task-id '${result.task_id}' --analyze-performance --store-decisions --export-learnings",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx hyrex hook post-search --query '${tool.params.pattern}' --cache-results --train-patterns"
          }
        ]
      }
    ],
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx hyrex hook session-start --session-id '${session.id}' --load-context"
          }
        ]
      }
    ],
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx hyrex hook session-end --session-id '${session.id}' --export-metrics --generate-summary --cleanup-temp"
          }
        ]
      }
    ]
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx hyrex hook check-protected --file '${tool.params.file_path}'"
          }
        ]
      }
    ]
...
```javascript
// Hook command
npx hyrex hook pre-task --description "Build REST API"

// Internally calls MCP tools:
mcp__hyrex__agent_spawn {
  type: "backend-dev",
  capabilities: ["api", "database", "testing"]
}

mcp__hyrex__memory_usage {
  action: "store",
...
```javascript
// Hook command
npx hyrex hook post-edit --file "api/auth.js"
...
```javascript
// Hook command
npx hyrex hook session-end --session-id "dev-2024"

// Internally calls MCP tools:
mcp__hyrex__memory_persist {
  sessionId: "dev-2024"
}

mcp__hyrex__swarm_status {
  swarmId: "current"
}
...
for FILE in $FILES; do
  ...
  npx hyrex hook pre-edit --file "$FILE" --validate-syntax
  ...
  npx hyrex hook post-edit --file "$FILE" --auto-format
done
...
# Run quality checks
npx hyrex hook session-end \
  --generate-report \
  --export-metrics

# Verify quality thresholds
TRUTH_SCORE=$(npx hyrex metrics score --format json | jq -r '.truth_score')
...
npx hyrex hook session-restore \
...
npx hyrex hook session-start --session-id "fullstack-feature"
...
# Backend work
npx hyrex hook pre-edit --file "api/profile.js"
# ... implement backend ...
npx hyrex hook post-edit \
  --file "api/profile.js" \
...
npx hyrex hook pre-edit --file "components/Profile.jsx"
# ... implement frontend ...
npx hyrex hook post-edit \
...
npx hyrex hook session-start --session-id "debug-memory-leak"
...
npx hyrex hook pre-search --query "EventEmitter"
# ... search executes ...
npx hyrex hook post-search \
  --query "EventEmitter" \
  --cache-results
...
npx hyrex hook pre-task --description "Analyze code complexity"
# ... analysis ...
npx hyrex hook post-task \
  --task-id "analysis" \
  --store-decisions
...
for file in src/**/*.js; do
  npx hyrex hook pre-edit --file "$file" --backup-file
  # ... refactor ...
  npx hyrex hook post-edit \
    --file "$file" \
...
```bash
# Enable debug output
export HYREX_DEBUG=true

# Test specific hook with verbose output
npx hyrex hook pre-edit --file "test.js" --debug

# Check hook execution logs
cat .hyrex/logs/hooks-$(date +%Y-%m-%d).log

# Validate configuration
npx hyrex hook validate-config
```
...
- `npx hyrex init --hooks` - Initialize hooks system
- `npx hyrex hook --list` - List available hooks
- `npx hyrex hook --test <hook>` - Test specific hook
- `npx hyrex memory usage` - Manage memory
- `npx hyrex agent spawn` - Spawn agents
- `npx hyrex swarm init` - Initialize swarm
