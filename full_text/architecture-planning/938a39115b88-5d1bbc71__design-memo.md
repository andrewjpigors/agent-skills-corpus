...
- A fixed right-side section rail that acts as a persistent, floating table
  of contents. Each entry shows a single-key shortcut lozenge (left-aligned
  within the row, so the key is the first thing the eye lands on) and the
  section label, so a keyboard user can always see the full sequence of
  topics and the key that jumps to each one. The current section's entry is
  visually prominent and transitions smoothly as the active section changes;
  inactive entries remain fully readable, not just dots.
- A sticky or fixed chapter indicator with the current section number and
  label, positioned so it does not collide with the rail.
- Active-section transitions on both the rail entry and the slide content,
  such as subtle opacity, lift, or scale changes.
- Keep all interaction implemented with small vanilla JavaScript.
- Hide progress UI in print and simplify it on small screens. In print, stamp
  each section's Crockford shortcut character as a lozenge in the bottom-right
  of its page so a printed copy stays keyboard-addressable.
...
- Add `aria-keyshortcuts` to each rail entry, set to the Crockford Base32
...
9. **Appendix — Original prompt** (always last). See "Appendix: Original Prompt".
...
Always end the memo with an appendix section that records the prompt or
instructions that initiated the design memo. Reviewers use it to see what
steering the agent received without reading the chat transcript.

- Place this section last, after sources, confidence, and caveats.
- Use an eyebrow such as "Appendix" and a heading like "Original prompt".
- Quote the user's instruction verbatim inside `<pre class="prompt-original">`
  (or another preformatted block) so whitespace and line breaks are preserved.
  Escape `&`, `<`, and `>` so the text renders literally.
- If the design emerged from a multi-turn exchange, include the substantive
  steering messages in chronological order — not every conversational aside,
  but enough that a reviewer can reconstruct the brief. Label each block with
  its turn (e.g., "Initial request", "Follow-up: scope clarification").
- If the prompt referenced files, attachments, or pasted artifacts that the
  agent treated as part of the brief, list them in a short sub-note.
- Do not paraphrase, summarize, or "improve" the wording. Fidelity matters
  more than polish here.
- Give the section a stable id of `appendix-original-prompt` so it can be
  deep-linked.

## Deep Linking and Section Anchors

Every slide must be addressable by URL so reviewers can share a link to a
specific section.

- Assign a stable `id` attribute to every `<main> > <section>`. Prefer an
  ...
  characters.
- As the active section changes — whether from scrolling, the section rail,
  keyboard shortcuts, or programmatic navigation — update the URL hash to
  `#<section-id>` using `history.replaceState(null, "", "#" + id)`. Use
  `replaceState`, not `pushState`, so the browser Back button continues to
  navigate between pages rather than between slides.
- Only update the hash when the active section index actually changes; do not
  thrash `history.replaceState` on every scroll tick.
- On initial page load, if `window.location.hash` matches an existing section
  ...
  the active-section state afterwards.
- Decode the hash with `decodeURIComponent` before lookup, and guard against
  ids that do not exist (do not throw).
- Hashes are user-visible. Keep slugs human-readable: prefer
  `#decision-model` over `#section-3`.
