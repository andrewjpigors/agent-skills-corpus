...
- Parse the inline subagent response as JSON. The success signal is valid JSON with `nodes` and `edges`.
- If the subagent result includes token usage metadata, add it to that chunk's `input_tokens` and `output_tokens`; otherwise leave token counts at 0.
- If a subagent failed or returned invalid JSON, print a warning and skip that chunk - do not abort.

If more than half the chunks failed or returned invalid JSON, stop and tell the user to re-run with smaller chunks.

Accumulate all valid inline results in memory, then write the merged semantic result once:
...
# Replace CHUNK_RESULTS_JSON with the JSON array of valid inline subagent results
# collected by the host Codex session.
chunks = CHUNK_RESULTS_JSON
all_nodes = []
all_edges = []
all_hyperedges = []
total_in = 0
