...
本技能根据调用上下文有三种模式，读取范围不同：

| 调用模式 | 触发时机 | 允许读取 | 禁止读取 |
|---------|---------|---------|---------|
| **场景模式** | 由 `dp-chapter-draft` 阶段 C 触发，指导单个场景写作 | 当前章节文件夹内的 `spec.md`（§7 张力定位、大纲要点、风格指令） | 章节文件夹之外的任何文件，含 `tracking/tension-tracker.md`、其他章节的 `release/`、`outlines/` |
| **宏观模式** | 由 `dp-set-outline` 大纲阶段触发，规划全篇张力走势 | `docs/dreampowers/outlines/outline-*.md`、`docs/dreampowers/tracking/thread-*.md`、`docs/dreampowers/tracking/tension-tracker.md` | 章节草稿（场景模式专用） |
| **审计模式** | 用户显式要求（"每5-10章张力审计"、"检查张力弧线"） | `docs/dreampowers/release/chapter-*.md` 全部、`docs/dreampowers/tracking/tension-tracker.md`、`docs/dreampowers/tracking/thread-*.md` | 不限 |

**张力追踪表**存储于 `docs/dreampowers/tracking/tension-tracker.md`，仅在宏观模式和审计模式下读写。场景模式不访问该文件。

**张力追踪表 bootstrap**：宏观模式首次运行时若文件不存在，创建 `docs/dreampowers/tracking/tension-tracker.md` 并写入表头：
...
后续宏观模式追加卷/章级张力评分行，审计模式重新读取全表评估。

**默认模式判定**：若无显式说明调用上下文，按调用方推断——由 `dp-chapter-draft` 调用视为场景模式；由 `dp-set-outline` 调用视为宏观模式；用户直接调用视为审计模式。
