...
```
/pepcode:team N:agent-type "task description"
/pepcode:team "task description"
/pepcode:team ralph "task description"
```

### Parameters

- **N** - Number of teammate agents (1-20). Optional; defaults to auto-sizing based on task decomposition.
- **agent-type** - PEP agent to spawn for the `team-exec` stage (e.g., executor, build-fixer, designer). Optional; defaults to stage-aware routing (see Stage Agent Routing below).
...
2. **MCP providers complement Claude agents.** Route UI/large-context work to Gemini (`ask_gemini`) when available. MCP workers are one-shot and don't participate in team communication.
...
- **team-plan**
  ...
  - Agents: `explore` scans codebase, `planner` creates task graph, optionally `analyst`/`architect` for complex tasks. May use Gemini for large-context planning.
...
- **Cancel:** `/pepcode:cancel` requests teammate shutdown, waits for responses (best effort), marks phase `cancelled` with `active=false`, captures cancellation metadata, then deletes team resources and clears/preserves Team state per policy.
...
- Extract **agent-type**, validate it maps to a known PEP subagent
...
Write PEP state using the `state_write` MCP tool for proper session-scoped persistence:
...
```json
{
  "subagent_type": "pepcode:executor",
  "team_name": "fix-ts-errors",
  "name": "worker-1",
...
On every stage transition, update PEP state:
...
4. **Delete team** -- Call `TeamDelete` to clean up:
   ...
   ```
5. **Clean PEP state** -- Remove `.pep/state/team-state.json`
...
The team skill supports **hybrid execution** combining Claude agent teammates with external MCP workers (Gemini CLI). Both types can make code changes -- they differ in capabilities and cost.
...
Gemini CLI runs in full-auto mode with filesystem access. It is an **autonomous executor**, not just an analyst:

1. Lead writes task instructions to a `prompt_file`
2. Lead calls `ask_gemini` with `working_directory` set to the project root
...
| Code review / security audit | Claude teammate | Requires iterative analysis + tool access |
| Architecture analysis / planning | Claude teammate | Needs deep reasoning + codebase tool access |
| Refactoring (well-scoped) | Claude teammate | Requires iterative verification + testing |
...
Task decomposition:
#1 [claude_worker] Security review of current auth code -> output to .pep/research/auth-security.md
#2 [claude_worker] Refactor auth/login.ts and auth/session.ts (uses #1 findings)
...
#5 [claude_worker] Final code review of all changes
```

The lead runs #1 (security analysis), then #2 and #3 in parallel (Claude refactors backend, Gemini redesigns frontend), then #4 (Claude teammate handles test iteration), then #5 (final review).

### MCP Pre-flight Analysis (Optional)

For large ambiguous tasks spanning many files, run Gemini analysis before team creation:

1. Call `ToolSearch("mcp")` to discover deferred MCP tools (required before first use)
2. Call `ask_gemini` (planner role) with task description + large codebase context
3. Use the analysis to produce better task decomposition
4. Create team and tasks with enriched context

If ToolSearch finds no MCP tools, proceed with Claude agents alone.

This is especially useful when the task scope spans many files and benefits from Gemini's 1M token context window before committing to a specific decomposition.
...
1. User invokes `/team ralph "task"` or `/pepcode:team ralph "task"`
...
4. If architect approves: both modes complete, run `/pepcode:cancel`
...
|--------|--------------|----------------------|
| **Storage** | JSON files in `~/.claude/teams/` and `~/.claude/tasks/` | SQLite in `.pep/state/swarm.db` |
...
The `/pepcode:cancel` skill handles team cleanup:
...
Optional settings via `.pep-config.json`:
...
2. PEP state cleanup via MCP tools:
   ```
   ...
   ```
3. Or run `/pepcode:cancel` which handles all cleanup automatically.
...
1. **Worktree creation**: Before spawning a worker, call `createWorkerWorktree(teamName, workerName, repoRoot)` to create an isolated worktree at `.pep/worktrees/{team}/{worker}` with branch `pep-team/{teamName}/{workerName}`.
...
11. **MCP workers are one-shot, not persistent** -- Gemini CLI has full filesystem access and CAN make code changes. However, it runs as an autonomous one-shot job -- it cannot use TaskList/TaskUpdate/SendMessage. The lead must manage its lifecycle: write prompt_file, call MCP, read output_file, mark task complete. It doesn't participate in team communication like Claude teammates do.
