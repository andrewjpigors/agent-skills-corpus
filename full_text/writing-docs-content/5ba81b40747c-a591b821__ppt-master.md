...
>
> - This installed skill uses a local virtual environment at `${SKILL_DIR}/.venv`.
> - When `${SKILL_DIR}/.venv/bin/python` exists, use it for every Python command in this skill.
> - In other words, replace `python3 ${SKILL_DIR}/scripts/...` with `${SKILL_DIR}/.venv/bin/python ${SKILL_DIR}/scripts/...` during execution.
> - Fall back to `python3` only if the local virtual environment does not exist.
