---
name: jy5113-driver
description: 提供 JYTEK JY5113 系列多功能数据采集卡（DAQ）的完整 C# 驱动开发指引。涵盖模拟输入（AI）单点/有限/连续/录制采集、模拟输出（AO）单点/有限/连续输出、数字输入/输出（DI/DO）按 line 操作的单点/有限/连续模式、计数器输入/输出（CI/CO）边沿计数/频率/周期/脉冲/编码器测量/脉冲生成、触发配置（数字/模拟/软件触发）、时钟配置（内部/外部时钟）、多卡同步（PXI_Trig0~7）、录制模式（Finite/Infinite Streaming）。当用户使用 PCIe-5113、PXIe-5113、PCIe-5113s、PXIe-5113s、JY5113AITask、JY5113AOTask、JY5113DITask、JY5113DOTask、JY5113CITask、JY5113COTask、AIMode、AOMode.ContinuousWrapping、AIBandWidth._39KHz/_15KHz/_80KHz、AITerminal.RSE/Differential 开发数据采集、信号发生、数字 IO 控制、计数/定时测量、多卡同步、流盘录波等自动化测试应用时自动应用。
---
...
- **驱动 DLL**：`C:\SeeSharp\JYTEK\Hardware\DAQ\JY5113\Bin\JY5113.dll`（引用到 .csproj）
- **XML 文档**：`C:\SeeSharp\JYTEK\Hardware\DAQ\JY5113\Bin\JY5113.xml`（IntelliSense 支持）
...
- **范例工程**：`D:\JYTEK_Work\Examples\C#\DAQ\JY5113.Examples`
...
| AI 分辨率 | 16-bit |
| AI 通道数 | 64 单端 / 32 差分 |
| AI 最大采样率 | 1 MS/s（单通道）；多通道 1 MS/s/n（n = 1..64） |
| AI 输入量程 | ±10V / ±5V / ±2.5V |
| AI 精度（时基） | 340 ppm |
| AI 输入阻抗 | >1 MΩ \|\| 330 pF |
| AI CMRR | 85 dB |
| AI 通道间串扰 | −74 dB |
| AI 过压保护 | ±25 V |
| AI FIFO / 板载缓冲 | 128 M Samples |
| AI 抗混叠带宽档位 | `AIBandWidth._39KHz` / `_15KHz` / `_80KHz` |
| AO 分辨率 | 16-bit |
| AO 通道数 | 4 通道单端 |
| AO 最大更新率 | 单通道 2 MS/s；2 通道 2 MS/s；4 通道 1 MS/s |
| AO 输出量程 | ±10V |
| AO FIFO / 板载缓冲 | 32 M Samples |
| DIO 通道 | 32 线（其中 16 线硬件定时，另 16 线静态） |
...
> `AIBandWidth` 枚举值**带下划线前缀**：`_39KHz / _15KHz / _80KHz`。
...
| `SampleRateParameterInvalid`                 | 采样率超过硬件上限           | 单通道 ≤ 1 MS/s，多通道共享     |
