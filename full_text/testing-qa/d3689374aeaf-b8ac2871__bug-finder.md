---
name: bug-finder
description: Find bugs in codebases using parallel discovery and refinement subagents. Scans recent code changes, system health, test failures, and consistency issues. Trigger words - find bugs, bug finder, bug scan, what's broken, check for bugs, workflow, run as workflow.
---

# Bug Finder

> **Workflow mode:** This skill can be run as a Claude Code Workflow for true parallel execution across all discovery and refinement tracks. Invoke with `/workflow` or the Workflow tool instead of `/bug-finder` for best performance on large codebases.
...
0. **Contact opt-out check.** Before proposing any fix that re-enables, migrates, or reactivates contact-facing scheduled behavior (LaunchAgents, crons, reminder-crons, recurring injects or messages tied to a specific contact), read that contact's recent conversation for disable/opt-out requests:
   ~/.claude/skills/sms-assistant/scripts/read-sms --chat <chat_id> --limit 60
   If an opt-out is found and not clearly superseded by a later re-enable request:
   - Fix the underlying code defect WITHOUT restoring the schedule
   - Add a "contact_conflict" field to the fix proposal quoting the opt-out message (date + quote)
   - Require explicit admin sign-off before any re-arm
   "Fix the code defect" and "re-arm the schedule that fires it" are separate decisions — a fix must never silently revive a schedule a contact just disabled.
     ...
     "proposals": [
       {
         ...
         "contact_conflict": "null, or the contact's opt-out (date + quoted message) if this fix would re-arm scheduled behavior they disabled — re-arm then requires explicit admin sign-off"
       }
     ]
   }
...
6. **Contact opt-out safety** — Does the fix re-enable, migrate, or reactivate contact-facing scheduled behavior (LaunchAgents, crons, reminder-crons, recurring messages) that a contact opted out of? Check the proposal's contact_conflict field and the underlying finding. A proposed fix that revives a feature a contact opted out of scores 0 on this criterion.

**Overall score** = average of the 6 criteria, rounded to nearest integer.
...
- If Contact opt-out safety = 0: REVISE — regardless of the other scores. The revised proposal must fix the code defect without re-arming the schedule, or carry a contact_conflict field gating the re-arm on explicit admin sign-off
...
## Batch Fix Execution (After Report)

When admin says "fix all" or "fix these bugs" after receiving a REF-NNN enumerated report:

1. **Enumerate and confirm**: List all REF-NNN IDs with file path + line context + fix description. Confirm count with admin before starting.

2. **Group by file**: Assign one agent per file-group. Never assign two agents to the same file (causes conflicts).

3. **Contact opt-out gate**: Before dispatching a fix agent for any proposal touching contact-tied scheduled behavior (LaunchAgents, crons, reminder-crons, recurring messages), confirm the proposal's `contact_conflict` field is absent or explicitly cleared by admin. If present and uncleared, dispatch only the code-defect portion — skip the re-arm portion and note the skip in the summary.

4. **Dispatch in parallel**: Send all agent Tool calls simultaneously (one message with multiple agent calls). Include the relevant REF descriptions in each agent's prompt so it knows exactly what to fix.

5. **Collect results**: Wait for all agents to complete. Note any failures or skips.

6. **Restart daemon if needed**: If dispatch system files (bus.py, sdk_backend.py, sdk_session.py, etc.) were changed, restart the daemon.

7. **SMS summary**: Send structured summary with severity tiers:
   - 🔴 Critical: [list]
   - 🟠 High: [list]
   - 🟡 Medium: [list]
   - 🟢 Low/Minor: [list]
...
**REF ID convention**: Zero-padded 3 digits (REF-001, REF-002...), stable within a session. Reference in commit messages: "fix: REF-001 XSS in sven-pages worker".
...
Contact opt-outs are discoverable via their chat history and MUST be honored across fix cycles:

```bash
~/.claude/skills/sms-assistant/scripts/read-sms --chat <chat_id> --limit 60
```

Canonical example: on 2026-06-30 a favorite-tier contact asked to disable their scheduled evening debrief and was told it was gone. On 2026-07-01 an audit fix migrated that debrief to a new reminder-cron mechanism, which re-armed the schedule and fired it that evening — contradicting the confirmed opt-out. Fixing a code defect and re-arming the schedule it fires are separate decisions. See the Phase 3 contact opt-out check, Phase 4 criterion 6, and the Batch Fix Execution contact opt-out gate.
