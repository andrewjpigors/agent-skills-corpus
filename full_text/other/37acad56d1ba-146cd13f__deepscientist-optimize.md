...
It does not replace the normal quest runtime. It tells you how to use the existing DeepScientist artifact, memory, ds_bash_exec, Git, and worktree mechanisms as an optimization system.
...
## Required native dsctl-driven workflow
...
1. `ds_artifact_record / analysis artifact lookup(...)`
2. `ds_memory_search(scope='quest', limit=5)`
3. `ds_memory_search(...)`
...
- `ds_artifact_record payload={'kind': 'decision', 'action': 'branch'|'continue'|'stop', ...})`
...
- implementation-level candidate attempt inside one line:
  - `ds_artifact_record payload={'kind': 'report', 'report_type': 'optimization_candidate', ...})`
- real measured main result:
  - `ds_artifact_record_main_experiment(...)`
- route change after the result:
  - `ds_artifact_record payload={'kind': 'decision', 'action': 'iterate'|'branch'|'continue'|'stop', ...})`
...
- Use `ds_bash_exec mode='detach', ...)` for long runs and monitor with `list/read/await`.
...
- After pause, resume, or runtime recovery, recover session state before spawning new runs.
...
- Use `ds_artifact_record payload={'kind': 'report', 'report_type': 'optimization_candidate', ...})` for implementation-level candidate attempts inside one durable line.
- Before deciding the next route, call `ds_artifact_record / analysis artifact lookup(...)` when available and use it as the primary optimization-state summary.
...
- run at least one `ds_memory_search(...)`
- read `ds_artifact_record / analysis artifact lookup(...)`
- update `OPTIMIZE_CHECKLIST.md`
...
Before broad new search, run at least one `ds_memory_search(...)` using:
...
- `ds_artifact_record payload={'kind': 'report', 'report_type': 'optimization_candidate', ...})` for within-line attempts
- `ds_artifact_record payload={'kind': 'decision', 'action': 'iterate'|'branch'|'continue'|'stop', ...})` for route changes
- `ds_artifact_record_main_experiment(...)` for real measured line results
...
- write at least one `ds_memory_write(...)` when the pass produced a reusable success pattern, repeated failure pattern, fusion lesson, or explicit non-retry rule
- update `OPTIMIZE_CHECKLIST.md`
...
- [ ] Read `ds_artifact_record / analysis artifact lookup(...)` or equivalent durable frontier summary
