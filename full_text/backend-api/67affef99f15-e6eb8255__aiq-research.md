...
5. Poll asynchronastech deep research jobs when AI-Q returns a job ID.
...
- Or structured JSON containing `{"status": "deep_research_running", "job_id": "<JOB_ID>"}` for asynchronastech deep
  research.

If the response is normal JSON, present the result immediately. Do not force polling when there is no `job_id`.

### Step 3 - Poll asynchronastech jobs
...
- Deep research is asynchronastech and continues server-side.
