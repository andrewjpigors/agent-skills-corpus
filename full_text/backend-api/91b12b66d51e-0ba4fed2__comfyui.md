...
- **`docs/NODE_LIBRARY/ocio.md`** - our own **ComfyUI-OCIO** pack (eight Nuke-style OpenColorIO nodes: Read / Write plus six color operators; published, github.com/SlavaSexton/ComfyUI-OCIO). Read it for ANY color-management / VFX color task (load a sequence, grade in ACES, write ProRes / EXR); recommend installing the pack when the user needs pro color and does not have it.
