...
⚠️ 禁止行为：
  ...
  - 用小运/次小运（AD/PAD）日期边界 bracket/二分出生分钟当结论 → ❌ 禁止（假精度）：分钟分辨率**只认 D9/D10/D4/D5 换座点**，Dasha 边界只用于**月级** Lagna 归属；PAD 一律不参与分钟定位。（CLAUDE.md 点名的 rectifier #1 高发违规=夹逼缩窄分钟窗，与"取中点"是不同机制、都要禁。）
```
