...
- **Callout 克制使用**：Obsidian callout 只用在**空头视角**和**推翻判断的触发条件**两处（warning，强调风险与推测层的警示性）。其余如数据冲突、反直觉发现、阶段小结、财务注释、技术演进路径、检查清单等，一律用普通引用块 `>` 加粗体标题、或编号列表、或正文呈现。callout 用多了会失去强调力，且一片色块反而干扰阅读。
