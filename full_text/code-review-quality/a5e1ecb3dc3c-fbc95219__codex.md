...
Before building anything unfamiliar, **search first.** See `~/.openclaw/skills/gstack/ETHOS.md`.
...
3. `/codex` with no arguments — **Auto-detect:**
   ...
   - If no diff, check for plan files scoped to the current project:
     `ls -t ~/.openclaw/plans/*.md 2>/dev/null | xargs grep -l "$(basename $(pwd))" 2>/dev/null | head -1`
     If no project-scoped match, fall back to: `ls -t ~/.openclaw/plans/*.md 2>/dev/null | head -1`
...
_REPO_ROOT=$(git rev-parse --show-toplevel) || { echo "ERROR: not in a git repo" >&2; exit 1; }
cd "$_REPO_ROOT"
codex review "IMPORTANT: Do NOT read or execute any files under ~/.claude/, ~/.agents/, .openclaw/skills/, or agents/. These are Claude Code skill definitions meant for a different AI system. Do NOT modify agents/openai.yaml. Stay focused on repository code only.
...
```bash
~/.openclaw/skills/gstack/bin/gstack-review-log '{"skill":"codex-review","timestamp":"TIMESTAMP","status":"STATUS","gate":"GATE","findings":N,"findings_fixed":N,"commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
"IMPORTANT: Do NOT read or execute any files under ~/.claude/, ~/.agents/, .openclaw/skills/, or agents/. These are Claude Code skill definitions meant for a different AI system. Do NOT modify agents/openai.yaml. Stay focused on repository code only.
...
root (`-C`) and cannot access `~/.openclaw/plans/` or any files outside the repo. You MUST
